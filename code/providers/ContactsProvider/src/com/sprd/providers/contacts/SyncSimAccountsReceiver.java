...
class FSA {
                    ...
                    case Event.ICC_LOADED:
                        if (isEventOn(Event.ICC_LOADED)) {
                            onEvent(Event.FDN_ENABLED,
                                   mTelephonyManager.getIccFdnEnabled(mSubId));
                        }
                        // NOTE: fall-through
    ...
    private int addAccount() {
        ...
        ContentResolver cr = mContext.getContentResolver();
        IIccPhoneBook ipb = IIccPhoneBook.Stub.asInterface(ServiceManager
        ...
        if (!isEventOn(Event.FDN_ENABLED)) {
            ...
            log("addAccount: not usim type and enabled FDN, return");
            return -1;
        }
        ...
        */
        ...
        if (action.startsWith(TelephonyIntents.ACTION_SIM_STATE_CHANGED)) {
            ...
            Log.e(TAG, "FDN_STATE_CHANGED: state: " + isFdnEnabled + " phoneId: " + phoneId
                    + " subId: " + subId);
            FSA.getInstance(context, phoneId, subId).onEvent(FSA.Event.FDN_ENABLED, isFdnEnabled);
