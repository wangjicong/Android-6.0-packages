...
public class ContactAggregator2 extends AbstractContactAggregator {
     ...
     */
     synchronized void aggregateContact(TransactionContext txContext, SQLiteDatabase db,
         ...
         /**
          * SPRD:Bug506357 this code of checking needAggregate would consume 3ms, its effect is
          * for optimization of combination, but has no use for batch delete and other batch
          * operations, so would move it to behind of batch delete's return
          * @{
          * orig:
        ...
        if (aggregationMode == RawContacts.AGGREGATION_MODE_DEFAULT) {
          ...
          */
        /**
         * SPRD:Bug497849 The sim contact should not be merged into phone contact.
         * @{
         */
        Set<Long> rIds = matchingCandidates.getRawContactIdSet();
        ...
        Cursor cursor = db.query(Tables.RAW_CONTACTS_JOIN_ACCOUNTS,
                                  new String[]{"raw_contacts._id"}, where, null, null, null, null);
        if (cursor != null) {
            try {
                while (cursor.moveToNext()) {
                    long rId = cursor.getLong(0);
                    if (rIds.contains(rId)) {
                        matchingCandidates.remove(rId);
                    }
                }
            } finally {
                cursor.close();
