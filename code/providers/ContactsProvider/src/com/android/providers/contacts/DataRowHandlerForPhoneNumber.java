...
public class DataRowHandlerForPhoneNumber extends DataRowHandlerForCommonDataKind {
    ...
    private ContentValues insertPhoneLookup(SQLiteDatabase db, long rawContactId,
            long dataId, String normalizedNumber) {
    ...
    private void updatePhoneLookup(SQLiteDatabase db, long rawContactId, long dataId,
        ...
        if (number != null) {
            ...
            if (!TextUtils.isEmpty(normalizedNumber)) {
                ContentValues phoneValues = insertPhoneLookup(db, rawContactId, dataId, normalizedNumber);
                ...
                /**
                 * SPRD:Bug479181 New feature, match Russian code.
                 * @{
                 */
                String countryCodeNumber;
                if (normalizedNumber.startsWith("8")) {
                    countryCodeNumber = normalizedNumber.replaceFirst("8", "+7");
                    insertPhoneLookup(db, rawContactId, dataId, countryCodeNumber);
                } else if (normalizedNumber.startsWith("+7")) {
                    countryCodeNumber = normalizedNumber.replaceFirst("\\+7", "8");
                    insertPhoneLookup(db, rawContactId, dataId, countryCodeNumber);
                }
                /**
                 * @}
    ...
    public void appendSearchableData(IndexBuilder builder) {
        ...
        if (normalizedNumber.startsWith("8")) {
            countryCodeNumber = normalizedNumber.replaceFirst("8", "+7");
            builder.appendToken(countryCodeNumber);
        } else if (normalizedNumber.startsWith("+7")) {
            countryCodeNumber = normalizedNumber.replaceFirst("\\+7", "8");
            builder.appendToken(countryCodeNumber);
        }
        ...
        /**
         * SPRD: bug429332, Cannot search by segment phone number in Contacts.
         * @{
         */
