...
public class ContactsUpgradeReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        try {
            ...
            /* SPRD: add for bug517374 @{ */
            } else if (TelephonyIntents.ACTION_APN_LOAD_COMPLETED.equals(intent.getAction())) {
                SharedPreferences prefs = context.getSharedPreferences(TAG_CALLLOG_UPGRADE,
                        Context.MODE_PRIVATE);
                boolean isCalllogUpgraded = prefs.getBoolean(IS_CALLLOG_UPGRADED, true);
                Log.i(TAG, "ACTION_APN_LOAD_COMPLETED isCalllogUpgraded = " + isCalllogUpgraded);
                if (!isCalllogUpgraded) {
                    ContactsDatabaseHelper helper = ContactsDatabaseHelper.getInstance(context);
                    helper.upgradeToVersion1003ForCalllog();
                }
            }
            /* @} */
        } catch (Throwable t) {
