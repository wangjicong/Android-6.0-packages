...
public class ContactsDatabaseHelper extends SQLiteOpenHelper {
    ...
    /* @} */
    /* SPRD: add for bug517374 @{ */
    static final String TAG_CALLLOG_UPGRADE = "CalllogUpgradeReceiver";
    static final String IS_CALLLOG_UPGRADED = "calllog_upgraded";
    /* @} */
     ...
     */
    /**
     * fail to use dialpad contact
     * orig:
     static final int DATABASE_VERSION = 1013;
     */
    static final int DATABASE_VERSION = 1015;
    /**
     *
     */
    ...
    public interface Joins {
         ...
         */
       /**
        * SPRD: bug114970, after joining contacts, group member count is error
        * Orig:
        *
    ...
    public interface Clauses {
       ...
       String USIM_ACCOUNT_ID =
                "(SELECT " + AccountsColumns._ID +
                " FROM " + Tables.ACCOUNTS +
                " WHERE " + AccountsColumns.ACCOUNT_TYPE + " = 'sprd.com.android.account.usim')";
        ...
        */
        ...
        /**
        * SPRD:Bug427937 Android L fail to open Contacts list in Private Calls.
        ...
        */
        final String RAW_CONTACT_IS_LOCAL = RawContactsColumns.CONCRETE_ACCOUNT_ID
                + "=" + NOT_SYNCABLE_ACCOUNT_IDS;
        				
        final String RAW_CONTACT_IS_USIM = RawContactsColumns.CONCRETE_ACCOUNT_ID
                + "=" + USIM_ACCOUNT_ID;
        ...
        public static final String RAW_SQL =
                ...
                " WHERE " + Data.RAW_CONTACT_ID + "=?" +
                        " AND (" + Data.DATA1 + " NOT NULL OR " +
                                ...
                                /**
                                 * SPRD:Bug474798 Update RawContact for AppBackup.
                                 *
                                 * Original Android code:
     ...
     * @{
     */
    private long mMimeTypeIdWebset;
    /**
     * @}
     */
    ...
    protected ContactsDatabaseHelper(
        ...
        * @{
        */
        initialUnrestrictedPkg(context);
        /**
        * @}
        */
    }
    ...
    private void initializeCache(SQLiteDatabase db) {
         ...
         * @{
         */
        mMimeTypeIdWebset = lookupMimeTypeId(Website.CONTENT_ITEM_TYPE, db);
        /**
         * @}
         */
    }
...
//        ");");
    ...
    private void createContactsViews(SQLiteDatabase db) {
        ...
        * @{
        */
        createRestrictedViews(db, contactsSelect, dataSelect, rawContactsSelect, rawEntitiesSelect, entitiesSelect);
        /**
        * @}
        */
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        */
        if (oldVersion < 1012) {
            upgradeViewsAndTriggers = true;
            // Adding is_restricted to raw_contacts table.
            if (!checkColumnExists(db,"raw_contacts","is_restricted")) {
                db.execSQL("ALTER TABLE raw_contacts"
                        + " ADD is_restricted INTEGER DEFAULT 0;");
            }
            oldVersion = 1012;
        }
        /**
        ...
        */
        ...
        /**
         * fail to use contact for default contact feature.
         */
        if (oldVersion < 1015) {
            upgradeViewsAndTriggers = true;
            oldVersion = 1015;
    ...
    /**
    * SPRD : add for bug 506336. @{
    */
    private void upgradeToVersion1013(SQLiteDatabase db) {
        boolean result = false;
        Cursor cursor = null;
        try {
            cursor = db.rawQuery("SELECT * FROM " + Tables.CALLS + " LIMIT 0",
                    null);
            result = cursor != null && cursor.getColumnIndex("users_id") != -1;
            if (!result) {
                db.execSQL("ALTER TABLE calls"
                        + " ADD users_id INTEGER DEFAULT 0;");
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
    ...
    public void upgradeToVersion1003(SQLiteDatabase db) {
        ...
        /* SPRD: add for bug517374 @{ */
        SharedPreferences prefs = mContext.getSharedPreferences(TAG_CALLLOG_UPGRADE,
                Context.MODE_PRIVATE);
        SharedPreferences.Editor editor = prefs.edit();
        editor.putBoolean(IS_CALLLOG_UPGRADED, false);
        editor.commit();
        /* @} */
     ...
     * SPRD: add for bug517374 @{
     */
    public void upgradeToVersion1003ForCalllog() {
        SQLiteDatabase db = getWritableDatabase();
        Cursor cursor = null;
        Log.i(TAG, "upgradeToVersion1003ForCalllog");
        try {
            if (mContext != null) {
                ...
                cursor = mContext.getContentResolver().query(SubscriptionManager.CONTENT_URI,
                ...
                if (cursor != null && cursor.moveToNext()) {
                    do {
                        ...
                        Log.i(TAG, "subId = " + subId + "  iccId = " + iccId);
                        if (!TextUtils.isEmpty(iccId) &&
                                subId != SubscriptionManager.INVALID_SUBSCRIPTION_ID) {
                            StringBuilder sb = new StringBuilder();
                            sb.append("UPDATE calls SET subscription_id=");
            ...
            SharedPreferences prefs = mContext.getSharedPreferences(TAG_CALLLOG_UPGRADE,
                    Context.MODE_PRIVATE);
            SharedPreferences.Editor editor = prefs.edit();
            editor.putBoolean(IS_CALLLOG_UPGRADED, true);
            editor.commit();
            if (cursor != null && !cursor.isClosed()) {
                cursor.close();
            }
    ...
    public void buildFallbackPhoneLookupAndContactQuery(SQLiteQueryBuilder qb, String number) {
         ...
         * @{
         */
        String contactsView = getContactsView(ContactsDatabaseHelper.canAccessRestrictedData(mContext));
        sb.append(" JOIN " + contactsView + " as contacts_view"
                + " ON (contacts_view._id = " + Tables.RAW_CONTACTS
                + "." + RawContacts.CONTACT_ID + ")" +
                " JOIN (SELECT " + PhoneLookupColumns.DATA_ID + "," +
                PhoneLookupColumns.NORMALIZED_NUMBER + " FROM "+ Tables.PHONE_LOOKUP + " "
    ...
    private void appendPhoneLookupTables(
        ...
        if (joinContacts) {
             ...
             */
            String contactsView = getContactsView(ContactsDatabaseHelper.canAccessRestrictedData(mContext));
            sb.append(" JOIN " + contactsView + " contacts_view"
                    + " ON (contacts_view._id = raw_contacts.contact_id)");
            /**
             * @}
             */
    ...
    */
     ...
     * @{
     */
    private void createRestrictedViews(SQLiteDatabase db, String contactsSelect, String dataSelect,
            String rawContactsSelect, String rawEntitiesSelect, String entitiesSelect) {
        ...
        db.execSQL("CREATE VIEW " + Views.CONTACTS_RESTRICTED + " AS " + contactsSelect + " WHERE "
                + " name_raw_contact." + RawContacts.IS_RESTRICTED + "=0");
        db.execSQL("CREATE VIEW " + Views.DATA_RESTRICTED + " AS " + dataSelect + " WHERE " + " raw_contacts." + RawContacts.IS_RESTRICTED
                + "=0");
        db.execSQL("CREATE VIEW " + Views.RAW_CONTACTS_RESTRICTED + " AS " + rawContactsSelect + " WHERE "
                + " raw_contacts." + RawContacts.IS_RESTRICTED + "=0");
        db.execSQL("CREATE VIEW " + Views.RAW_ENTITIES_RESTRICTED + " AS " + rawEntitiesSelect + " WHERE "
                + " raw_contacts." + RawContacts.IS_RESTRICTED + "=0");
        db.execSQL("CREATE VIEW " + Views.ENTITIES_RESTRICTED + " AS " + entitiesSelect + " WHERE "
                + " raw_contacts." + RawContacts.IS_RESTRICTED + "=0");
    }
    ...
    private void initialUnrestrictedPkg(Context context) {
        Resources resources = context.getResources();
        ...
        setPermitedProcessNames(mUnrestrictedPackages);
    }

    public String getDataView(boolean canAccessRestrictedData) {
        return (hasAccessToRestrictedData() && canAccessRestrictedData) ?
                Views.DATA : Views.DATA_RESTRICTED;
    }

    public String getRawContactsView(boolean canAccessRestrictedData) {
        return (hasAccessToRestrictedData() && canAccessRestrictedData) ?
                Views.RAW_CONTACTS : Views.RAW_CONTACTS_RESTRICTED;
    }

    public String getContactsView(boolean canAccessRestrictedData) {
        Log.i(TAG, "[getContactView] excludeRestrictedData = " + canAccessRestrictedData);
        return (hasAccessToRestrictedData() && canAccessRestrictedData) ?
                Views.CONTACTS : Views.CONTACTS_RESTRICTED;
    }

    public String getEntitiesView() {
        return getEntitiesView(true);
    }

    public String getEntitiesView(boolean canAccessRestrictedData) {
        return (hasAccessToRestrictedData() && canAccessRestrictedData) ?
                Views.ENTITIES : Views.ENTITIES_RESTRICTED;
    }

    public String getRawEntitiesView(boolean canAccessRestrictedData) {
        return (hasAccessToRestrictedData() && canAccessRestrictedData) ?
                Views.RAW_ENTITIES : Views.RAW_ENTITIES_RESTRICTED;
    }

    private static HashMap<Integer, Boolean> sCurrentProcesses;
    private static Set<String> sPermittedProcessNames;

    /**
     * Sets processes that can access restricted data.
     *
     * @param permitedProcessNames
     *            the name of processes.
     */
    public static synchronized void setPermitedProcessNames(String[] permitedProcessNames) {
        if (sPermittedProcessNames == null) {
            sPermittedProcessNames = new HashSet<String>();
        } else {
            sPermittedProcessNames.clear();
        }
        if (permitedProcessNames == null) {
            ...
            for (int i = 0; i < length; i++) {
                sPermittedProcessNames.add(permitedProcessNames[i]);
                Log.v(TAG, "setPermitedProcessNames() add [" + i + "]=" + permitedProcessNames[i]);
            }
        }
    }

    /**
     * Checks out whether the process can access restricted data or not.
     * @param context Context
     * @return true if the package is listed in the XML
     */
    public static synchronized boolean canAccessRestrictedData(Context context) {
        ...
        if (result == null) {
            ...
            sCurrentProcesses.clear();
            ActivityManager am = (ActivityManager) context.getSystemService(Context.ACTIVITY_SERVICE);
            List<RunningAppProcessInfo> list = am.getRunningAppProcesses();
            int size = list.size();
            for (int i = 0; i < size; i++) {
                RunningAppProcessInfo runInfo = list.get(i);
                boolean allow = sPermittedProcessNames.contains(runInfo.processName);
        ...
        if (!result) {
            Log.v(TAG, "permittedAccessRestrictedData(" + pid + ") return " + result);
        }
        return result;
    }
