...
public class MediaScannerService extends Service implements Runnable
{
    ...
    // SPRD: optimize the data scan
    private volatile ServiceHandler mServiceHandlerExt;
    private boolean mHasHandlerExt = false;
    private PowerManager.WakeLock mWakeLock;
    private String[] mExternalStoragePaths;
    //post runnable to main thread to fix bug 535399
    private Handler mMainLooper = new Handler(Looper.getMainLooper());
    private final Runnable mStopSelfTask = new Runnable() {
        public void run() {
            mIndex--;
            Log.d(TAG, "Runnable: mIndex=" + mIndex + ", mRecentId=" + mRecentId);
            if (mIndex == 0) {
                Log.d(TAG, "stopSelf");
                stopSelf(mRecentId);
            }
    ...
    public void onCreate()
    {
        ...
        /* SPRD: optimize the data scan @{ */
        File dbFile = this.getDatabasePath("internal.db");
        if (!dbFile.exists()) {
            Thread thrExt = new Thread(null, this, "MediaScannerServiceExt");
            mHasHandlerExt = true;
            thrExt.start();
        }
        /* @} */
    ...
    {
        //add startId to fix bug 535399
        mIndex++;
        mRecentId = startId;
        Log.d(TAG, "onStartCommand: mIndex=" + mIndex + ", mRecentId=" + mRecentId);
        ...
        String volume = ((Bundle) obj).getString("volume");
        if (mHasHandlerExt == true && MediaProvider.EXTERNAL_VOLUME.equals(volume)) {
            while (mServiceHandlerExt == null) {
                synchronized (this) {
                    try {
                        wait(100);
                    } catch (InterruptedException e) {
            ...
            Message msg = mServiceHandlerExt.obtainMessage();
            msg.arg1 = startId;
            msg.obj = obj;
            mServiceHandlerExt.sendMessage(msg);
    ...
    public void run()
    {
        ...
        /* SPRD: optimize the data scan @{ */
        if ("MediaScannerServiceExt".equals(Thread.currentThread().getName())) {
            Log.d(TAG, "MediaScannerServiceExt created");
            mServiceHandlerExt = new ServiceHandler();
        } else if ("MediaScannerService".equals(Thread.currentThread().getName())) {
    ...
    private final class ServiceHandler extends Handler
    {
        ...
        public void handleMessage(Message msg)
        {
            ...
            //move this to main thread to fix bug 535399
            //stopSelf(msg.arg1);
