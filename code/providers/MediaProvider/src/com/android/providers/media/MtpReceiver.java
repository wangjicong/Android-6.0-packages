...
public class MtpReceiver extends BroadcastReceiver {
    ...
    private void handleUsbState(Context context, Intent intent) {
        ...
        /* SPRD: We should prevent MtpService from starting without primary volume unmounted. @{ */
        boolean volumeReady = true;
        StorageManager storageManager = (StorageManager) context
                .getSystemService(Context.STORAGE_SERVICE);
        StorageVolume[] volumes = storageManager.getVolumeList();
        if (volumes.length == 0) {
            Log.e(TAG, "no storage volume found in StorageManager");
            volumeReady = false;
        } else {
            if (!Environment.MEDIA_MOUNTED.equals(storageManager
                    .getVolumeState(volumes[0].getPath()))) {
        ...
        if (volumeReady && connected && (mtpEnabled || ptpEnabled)) {
