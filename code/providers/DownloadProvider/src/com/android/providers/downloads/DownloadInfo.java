...
public class DownloadInfo {
    ...
    public void sendIntentIfRequested() {
        Log.i(Constants.TAG, "DownloadInfo sendIntentIfRequest mPackage--- " + mPackage);
        Log.i(Constants.TAG, "DownloadInfo sendIntentIfRequest  mClass--- " + mClass);
        if (mPackage == null) {
            return;
        }
         ...
         *@{
         */
        if (mIsPublicApi && !"com.android.browser".equals(mPackage)) {
        /*@}*/
    ...
    private boolean isReadyToDownload() {
        if(Constants.LOGVV) {
            Log.v(Constants.TAG, "DownloadInfo isReadyToDownload  mControl: " + mControl + ", mStatus: " + mStatus);
        }
        ...
        switch (mStatus) {
            ...
            case Downloads.Impl.STATUS_WAITING_TO_RETRY:
                ...
                if (Constants.LOGVV) {
                    Log.v(Constants.TAG, "DownloadInfo isReadyToDownload  restartTime(now): "
                         + restartTime(now) + " nowtime: "+now);
                }
    ...
    public boolean startDownloadIfReady(ExecutorService executor) {
        synchronized (this) {
            ...
            if(Constants.LOGVV) {
                Log.v(Constants.TAG, "DownloadInfo startDownloadIfReady isReady: " + isReady
                        + "  isActive: "+isActive);
            }
            if (isReady && !isActive) {
    ...
    public long nextActionMillis(long now) {
        ...
        */
        // if status is not WAITING_TO_RETRY,just don't schedule next action
        Log.i(Constants.TAG, "[" + mId + "] " + "nextActionMillis, mStatus = " + mStatus);
        if (mStatus != Downloads.Impl.STATUS_WAITING_TO_RETRY) {
            return Long.MAX_VALUE;
        }
        long when = restartTime(now);
        Log.i(Constants.TAG, "[" + mId + "] " + "nextActionMillis, when = " + when + ", now = " + now);
        if (when <= now) {
            // if when is less than now, it means should schedule next action immediately, give it a small time to schedule next action
            return 100;
        }
        return when - now;
    }
