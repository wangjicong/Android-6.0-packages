...
public class MmsSmsProvider extends ContentProvider {
    ...
    private static final int URI_TEMP_CONVERSATION                 = 22;
    // bug 495194 : add for search feature end
    ...
    private static final String SMS_MMS_QUERY = SMS_QUERY + " UNION "
            ...
            + MMS_QUERY2 + " UNION "+ MMS_QUERY3 +" ) GROUP BY _id";
    ...
    static {
        ...
        URI_MATCHER.addURI(AUTHORITY, "temp_conversation",URI_TEMP_CONVERSATION);
        // bug 495194 : add for search feature end
    ...
    class MyRunnable implements Runnable {
        public void run() {
            SearchSyncHelper.getInstance(getContext(), mOpenHelper).CheckTable();
            SearchSyncHelper.getInstance(getContext(), mOpenHelper).syncRecord(20000);
        }
    }
    // bug 495194 : add for search feature end
    ...
    public Cursor query(Uri uri, String[] projection,
        ...
        switch (match) {
            ...
            case URI_SEARCH_SUGGEST: {
                // bug 495194 : add for search feature begin
                System.out.println("enter URI_SEARCH_SUGGEST URI in query(...)");
                // SPRD: Modify for bug#283281
                // SEARCH_STRING[0] = uri.getQueryParameter("pattern") + '*' ;

                // find the words which match the pattern using the snippet
                // function. The
                // snippet function parameters mainly describe how to format the
                // result.
                ...
                try {
                    ...
                    if ("".equals(searchString)) {
                        ...
                        sql = " SELECT index_text as snippet FROM words WHERE index_text LIKE '%"
                                + searchString
                                + "%' escape '/' UNION "
                                + " SELECT recipient_name as snippet FROM temp_conversation WHERE recipient_name LIKE '%"
                                + searchString
                                + "%' escape '/' UNION "
                                + " SELECT recipient_address as snippet FROM temp_conversation WHERE recipient_address LIKE '%"
                                + searchString.replaceAll(" ", "") + "%' escape '/'"
                                + " ORDER BY snippet LIMIT 50;";
                        //add for bug 543676 end
                    }
            ...
            case URI_SEARCH: {
                 System.out.println("enter URI_SEARCH in query(...)");
                 if (       sortOrder != null
                         || selection != null
                         || selectionArgs != null
                         || projection != null) {
                 ...
                 try {
                     System.out.println("SMS_MMS_QUERY = [" + SMS_MMS_QUERY + "]");
                     cursor = db.rawQuery(SMS_MMS_QUERY, new String[] {
                             searchString, searchString, searchString, strMatch, searchString,
                             searchString, searchString, searchString, searchString, searchString,
                             searchString, searchString, strMatch, searchString, searchString
                     });
                 } catch (Exception ex) {
                     Log.e(LOG_TAG, "got exception: " + ex.toString());
                 }
           ...
           case URI_CREATE_TEMP_TABLE: {
               ...
               try {
                   SearchSyncHelper.getInstance(getContext(), mOpenHelper).syncRecord(1000);
               } catch (Exception e) {
    ...
    private void insertThread(String recipientIds, int numberOfRecipients) {
        ...
        getContext().getContentResolver().notifyChange(MmsSms.CONTENT_URI, null, true,
                UserHandle.USER_ALL);
        // bug 495194 : add for search feature begin
        SearchSyncHelper.getInstance(getContext(), mOpenHelper).insertRecord((int) result);
        // bug 495194 : add for search feature end
    }
    ...
    private int deleteConversation(Uri uri, String selection, String[] selectionArgs) {
        ...
        // bug 495194 : add for search feature begin
        SearchSyncHelper.getInstance(getContext(), mOpenHelper).deleteRecord((Integer.valueOf(threadId)).intValue());
        // bug 495194 : add for search feature end
