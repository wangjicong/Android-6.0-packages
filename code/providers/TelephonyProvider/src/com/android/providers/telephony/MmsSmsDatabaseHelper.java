...
public class MmsSmsDatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
       ...
       // onUpgrade(db, 61, DATABASE_VERSION);
        ...
        sendEmptyMesssagingSprd(getHandler(), FDN_CHECK_EVN ,0);
        // sprd: fix for telcel bug 557685 begin
...
//        }
    ...
    public static final int lOCALCONTACTS_UPDATA_FLAG = 1001;
    public static final int FDN_CLEAN_ALL_DATA        = 1004;
    public static final int FDN_CHECK_EVN             = 1005;
    ...
    private boolean getFdnEnable(final Context context, int phoneid) {
        return SubscriptionManager.from(context).getFdnEnable(phoneid);
    }
    ...
    private void checkEvn(){
        // check table ;
        boolean bEvn = true;
        if(checkTableExit(CONTACTS_SMS_TABLE) && checkTableExit(CONTACTS_FDN_TABLE)){
            bEvn = bEvn & true;
        }else{
            bEvn = bEvn & false;
            System.out.println("---MmsSmsDatabaseHelper---checkEvn()-------table no all exit");
        }
        if(bEvn){
            sendEmptyMesssagingSprd(getHandler(), FDN_CLEAN_ALL_DATA, 0);
            sendEmptyMesssagingSprd(getHandler(), INIT_ALL_UPDATA_FLAG, 100);
        }
    }

    private boolean checkTableExit(String tableName){
