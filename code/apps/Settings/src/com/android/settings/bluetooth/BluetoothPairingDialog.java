...
public final class BluetoothPairingDialog extends AlertActivity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        /* SPRD : add for that makesure that screeon when a pairing request come in  @{ */
    ...
    private void createUserEntryDialog() {
        ...
        p.mTitle = getString(R.string.bluetooth_pairing_request,
                convertToHtmlFmt(mCachedDeviceManager.getName(mDevice)));
    ...
    protected void onDestroy() {
        super.onDestroy();
        /* SPRD: add for release the wakelock @{ */
        if (mWakeLock != null && mWakeLock.isHeld()) {
            mWakeLock.release();
        }
        /* @} */
        //SPRD: modified to catch the exception when try to unregister the receiver.
        try {
            if (mFlag){
            ...
            Log.e(TAG,"get a exception when try to unregister the receiver");
        }
    }
    ...
    private String convertToHtmlFmt(String name) {
        String tmpName = Html.escapeHtml(name);
        return Html.fromHtml(tmpName).toString();
    }
}
