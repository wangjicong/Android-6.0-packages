...
public class WifiSettings extends RestrictedSettingsFragment
    ...
    // SPRD: wifi plugin
    private static final int MENU_ID_TRUSTED_AP = Menu.FIRST + 10;
    ...
    void addOptionsMenuItems(Menu menu) {
        ...
        /* SPRD: wifi plugin @{ */
        if (supportCMCC) {
            menu.add(Menu.NONE, MENU_ID_TRUSTED_AP, 0, R.string.trust_accesspoints_list)
        ...
        switch (item.getItemId()) {
            ...
            case MENU_ID_TRUSTED_AP:
                if (mWifiManager.isWifiEnabled()) {
                    mAddonStub.startTrustedApListActivty();
                }
                return true;
                /* @} */
        }
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen screen, Preference preference) {
        if (preference instanceof AccessPointPreference) {
            ...
            if (mSelectedAccessPoint.getSecurity() == AccessPoint.SECURITY_NONE &&
                ...
                if (selectedAccessPointHasPasspointMatch()) {
                    if (!savedNetworksExist) {
                        savedNetworksExist = true;
                        getActivity().invalidateOptionsMenu();
                    }
                    connect(mSelectedAccessPoint.getConfig());
                } else {//<-- Add for SPRD Passpoint R1 Feature
                    mDlgModify = false;
                    showDialog(mSelectedAccessPoint, true);
                }
            }
    ...
    public void onAccessPointsChanged() {
        ...
        switch (wifiState) {
            case WifiManager.WIFI_STATE_ENABLED:
                ...
                int index = 0;
                // SPRD: show cmcc ap lists when wifi plugin
                if (supportCMCC) {
                    hasAvailableAccessPoints = addAccessPointsForCMCCVersion(accessPoints);
                } else {
    ...
    protected void connect(final WifiConfiguration config) {
        ...
        //SPRD: if already in connecting/connected state, have the same meaning as manual disconnect
        if (supportCMCC && mAddonStub.isWifiConnectingOrConnected() == true) {
            mAddonStub.setManulConnectFlags(true);
        }
    ...
    //NOTE: Add for SPRD Passpoint R1 Feature -->
    private boolean selectedAccessPointHasPasspointMatch() {
        if (mSelectedAccessPoint == null) return false;

        String credID = mSelectedAccessPoint.getPasspointCredID();

        if (credID == null) return false;

        if(mWifiManager.getPasspointConfiguredCred(credID) == null) return false;

        return (mSelectedAccessPoint.hasPasspointMatch()
                    && mSelectedAccessPoint.generateSimplePasspointNetworkConfig());
    }
    //<-- Add for SPRD Passpoint R1 Feature


    /* SPRD: add for cmcc wifi feature */
    private boolean addAccessPointsForCMCCVersion(Collection<AccessPoint> accessPoints) {
        ...
        addAccessPointCategory(mDefinedApsCategory, R.string.trust_accesspoints_list);
        ...
        addAccessPointCategory(mOtherApsCategory, R.string.unkown_accesspoints_list);
            ...
            if (accessPoint.getLevel() != -1) {
                ...
                if (accessPoint.getTag() != null) {
                    ...
                    if (accessPoint.isSaved()) {
                        mDefinedApsCategory.addPreference(pref);
                    } else {
                        mOtherApsCategory.addPreference(pref);
                    }
                    continue;
                }
                AccessPointPreference preference = new AccessPointPreference(accessPoint,
        ...
        int mOtherApsCount = mOtherApsCategory.getPreferenceCount();
        if (mOtherApsCount == 0) {
            preferenceScreen.removePreference(mOtherApsCategory);
        }
        return hasAvailableAccessPoints;
    }
