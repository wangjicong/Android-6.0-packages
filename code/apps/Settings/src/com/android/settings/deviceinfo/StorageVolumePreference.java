...
public class StorageVolumePreference extends Preference {
    ...
    private int mColor;
    public long mTotal;//wangxing add 
    ...
    public StorageVolumePreference(Context context, VolumeInfo volume, int color) {
        ...
        if (volume.isMountedReadable()) {
            ...
            if(VolumeInfo.ID_PRIVATE_INTERNAL.equals(volume.getId())&&(OptConfig.SUN_MEMORY_INFO_32G||OptConfig.SUN_MEMORY_INFO_16G || OptConfig.SUN_MEMORY_INFO_8G)){
                ...
                long TotalSpace = totalBytes;
                TotalSpace = ROM_TOTAL*(1000*TotalSpace/ROM_REAL)/1000;
                usedBytes = TotalSpace*(1000*(usedBytes)/path.getTotalSpace())/1000;
                totalBytes = TotalSpace;
            }
            //Kalyy add for SUN_MEMORY_INFO_32G
             
            mTotal = totalBytes;//wangxing add 
