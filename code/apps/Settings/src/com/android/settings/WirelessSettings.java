...
public class WirelessSettings extends SettingsPreferenceFragment implements Indexable {
    ...
    private static final String KEY_NETWORK_PREFERENCE_SETTINGS = "network_preference_settings";
    ...
    private static boolean mSupportCUCC = SystemProperties.getBoolean("ro.wifi.support_cucc", false);
    ...
    public void onManageMobilePlanClick() {
        ...
        if (mTm.hasIccCard() && (ni != null)) {
            ...
            if (!TextUtils.isEmpty(url)) {
                ...
                operatorName = TeleUtils.updateOperator(operatorName, "spn");
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (!mSupportCUCC) {
            removePreference(KEY_NETWORK_PREFERENCE_SETTINGS);
        }
        ...
        if (isSecondaryUser || !cm.isTetheringSupported()
                || mUm.hasUserRestriction(UserManager.DISALLOW_CONFIG_TETHERING) || WCN_DISABLED) {
        ...
        /* SPRD: Add for Bug 505217,refresh the UI when airplane on/off. @{ */
        IntentFilter intentFilter = new IntentFilter();
        intentFilter.addAction(Intent.ACTION_AIRPLANE_MODE_CHANGED);
        if (getActivity() != null) {
            getActivity().registerReceiver(mAirplaneEnableReceiver, intentFilter);
        }
        /* @} */
    }
    ...
    public void onResume() {
        ...
        /* SPRD: add for fix Bug 492127  */
        /* to disenabled the mobile network settings when there is no sim card active @{ */
            if (getActivity() != null) {
                Preference cellular = findPreference(KEY_MOBILE_NETWORK_SETTINGS);
                if (cellular != null) {
                    cellular.setEnabled(hasAvailableSim() && !mTm.isAirplaneModeOn());
                }
            }
    }
    ...
    public void onDestroy() {
        super.onDestroy();
        if (getActivity() != null) {
            getActivity().unregisterReceiver(mAirplaneEnableReceiver);
        }
        /* @} */
    }

    private boolean hasAvailableSim() {
        SubscriptionManager subManager = SubscriptionManager
                .from(getActivity());
        int phoneCount = mTm.getPhoneCount();
        if (subManager.getActiveSubscriptionIdList().length == 0) {
            return false;
        }
        int count = 0;
        for (int i = 0; i < phoneCount; i++) {
            if (mTm.getSimState(i) == TelephonyManager.SIM_STATE_READY
                    && mTm.isSimStandby(i)) {
                log("[hasAvailableSim]isSimStandby = " + mTm.isSimStandby(i)
                        + ", SimState = " + mTm.getSimState(i) + ",phoneId = "
                        + i);
                count++;
            }
        }
    ...
    /* SPRD: Add for Bug 505217,refresh the UI when airplane on/off. @{ */
    private BroadcastReceiver mAirplaneEnableReceiver = new BroadcastReceiver() {
        @Override
