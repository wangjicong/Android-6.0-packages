...
public class ApnSettings extends SettingsPreferenceFragment implements
    ...
    private void fillList() {
        ...
        String orderBy = "_id";//SPRD:default apn
        ...
        if (cursor != null) {
            ...
            while (!cursor.isAfterLast()) {
                ...
                /* @} */
                    ...
                    pref.setType(type);//SPRD:default apn
            ...
            cursor.close();
            /* SPRD:default APN @{ */
            Log.d(TAG, "mvno,mno mvnoApnList.isEmpty()="+mvnoApnList.isEmpty() + 
				", mnoApnList.isEmpty()="+mnoApnList.isEmpty()+" mSelectedKey="+mSelectedKey); 
				
            if (!mvnoApnList.isEmpty()) {
				for(ApnPreference pref: mvnoApnList){
					String key = pref.getKey().toString();
					String apn = pref.getSummary().toString();
					String type =pref.getType();
					
					boolean isNeedSetChecked = ApnSettingsPluginsUtils.
                            getInstance(getActivity()).getIsNeedSetChecked(mccmnc, apn);
							
					Log.d(TAG, " add by sprd mvnoApnList: mSelectedKey="+mSelectedKey+"  key="+key+"  apn="+apn+" isNeedSetChecked="+isNeedSetChecked);				
            		if ((mSelectedKey != null) && mSelectedKey.equals(key)) {
                		pref.setChecked();
                	}/* SPRD: Bug 548128 no default apn is set checked @{ */
            		else if (mSelectedKey == null && !"dm".equals(type) && isNeedSetChecked) {
                		pref.setChecked();
						Log.d(TAG,"mvnoApnList set key="+key);
                    	setSelectedApnKey(key);
                	}								
				}
            }else if (!mnoApnList.isEmpty()) {
				for(ApnPreference pref: mnoApnList){
					String key = pref.getKey().toString();
					String apn = pref.getSummary().toString();
					String type =pref.getType();
					...
					Log.d(TAG, " add by sprd mnoApnList: mSelectedKey="+mSelectedKey+"  key="+key+"  apn="+apn+" isNeedSetChecked="+isNeedSetChecked);	
            		if ((mSelectedKey != null) && mSelectedKey.equals(key)) {
                		pref.setChecked();
                	}
                        /* SPRD: Bug 548128 no default apn is set checked @{ */
                		...
                		pref.setChecked();
						Log.d(TAG,"mnoApnList set key="+key);
                    	setSelectedApnKey(key);
                	}								
				}
            }				
