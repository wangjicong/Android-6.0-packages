...
public class ManageDefaultApps extends SettingsPreferenceFragment
    ...
    /* SPRD Bug 497404 when delete third party messages, Rotation DefaultSmsPreference, settings stop running @{ */
    ...
    public void onStop() {
        ...
        if (mDefaultBrowserPreference != null) {
            Dialog preDialog = mDefaultBrowserPreference.getDialog();
            if(preDialog != null && preDialog.isShowing()){
                preDialog.dismiss();
            }
        }
        if (mDefaultPhonePreference != null) {
            Dialog preDialog = mDefaultPhonePreference.getDialog();
            if(preDialog != null && preDialog.isShowing()){
                preDialog.dismiss();
            }
        }
        if (mDefaultSmsPreference != null) {
            Dialog preDialog = mDefaultSmsPreference.getDialog();
            if(preDialog != null && preDialog.isShowing()){
                preDialog.dismiss();
            }
        }
