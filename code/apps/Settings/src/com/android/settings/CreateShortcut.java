...
import com.android.settings.Settings.SmartControlsSettingsActivity;
...
public class CreateShortcut extends LauncherActivity {
    ...
    // SPRD: remove SmartControlsActivity for bug 536457
    private static final boolean SUPPORT_SMART_CONTROL = SettingsActivity.fileIsExists();
        ...
        for (int i = activities.size() - 1; i >= 0; i--) {
            ...
            if (WCN_DISABLED
                    && (info.activityInfo.name.endsWith(WifiSettingsActivity.class.getSimpleName())
                            || info.activityInfo.name.endsWith(HotspotSettingsActivity.class
                                    .getSimpleName())
                            || info.activityInfo.name.endsWith(BluetoothSettingsActivity.class
                                    .getSimpleName())
                            || info.activityInfo.name.endsWith(LocationSettingsActivity.class
                            .getSimpleName()))) {
                activities.remove(i);
            }
            if (info.activityInfo.name.endsWith(TetherSettingsActivity.class.getSimpleName())) {
                if (!TetherUtil.isTetheringSupported(this) || WCN_DISABLED) {
                    activities.remove(i);
                }
            }
            /* SPRD: remove DataUsageSummaryActivity for bug 475998 @{ */
            if (info.activityInfo.name.endsWith(DataUsageSummaryActivity.class.getSimpleName()) && SUPPORT_CMCC) {
                activities.remove(i);
            }
            /* @} */
            /* SPRD: remove SmartControl for bug 536457 @{ */
            if (info.activityInfo.name.endsWith(SmartControlsSettingsActivity.class.getSimpleName())
                    && !SUPPORT_SMART_CONTROL) {
                activities.remove(i);
            }
            /* @} */
        }
