...
public class DreamBackend {
    ...
    public List<DreamInfo> getDreamInfos() {
        ...
        /*
         * Add for Bug 515807 check if it has Sprd DeskClock and if it is set by
         * user@{
         */
        boolean hasSprdClock = false;
        boolean hasValidComponent = false;
        for (ResolveInfo resolveInfo : resolveInfos) {
            if (resolveInfo.serviceInfo == null)
                continue;
            ComponentName componentName = getDreamComponentName(resolveInfo);
            if (getActiveDream().equals(componentName)) {
                hasValidComponent = true;
                break;
            }

            if (getDefaultDream().equals(componentName)) {
                hasSprdClock = true;
                break;
            }
        }
        ...
        for (ResolveInfo resolveInfo : resolveInfos) {
            ...
            /* Add for Bug 515807 @{ */
            if (hasSprdClock || hasValidComponent) {
                dreamInfo.isActive = dreamInfo.componentName.equals(activeDream);
            } else {
                try {
                    dreamInfo.isActive = dreamInfo.componentName.toShortString().contains(
                            (activeDream.toShortString().split("/"))[1]);
                    if (dreamInfo.isActive) {
                        setActiveDream(dreamInfo.componentName);
                    }
                } catch (Exception e) {
                }
            }
