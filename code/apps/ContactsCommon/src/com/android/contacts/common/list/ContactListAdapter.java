...
public abstract class ContactListAdapter extends ContactEntryListAdapter {
     ...
     * @param position shows the position of the contact in the list.
     * @param cursor contains the contact info of the exact contact.
     * @return return true if the contact is from local db.
     *  @{
     */
    public boolean isLocalContact(int position, Cursor cursor) {
        if (cursor == null) {
            cursor = (Cursor) getItem(position);
        }
        if (cursor != null) {
            String lookupKey = cursor.getString(ContactQuery.CONTACT_LOOKUP_KEY);
            if (lookupKey.indexOf("display_name") != -1) {
                return false;
            } else {
                return true;
            }
