    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_CREATE_DOCUMENT) {
            if (resultCode == Activity.RESULT_OK && data != null && data.getData() != null) {
                ...
                /**
                 * SPRD:Bug502105 while export contacts to internel storage with vcf file,
                 * go into Settings and switch language, go back to export interface and save,
                 * contact crash.
                 *
                 * @{
                 */
                if (DEBUG) Log.d(LOG_TAG, "mservice == " + mService);
                if (mService == null) {
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(getApplicationContext(),
                                    R.string.vcfdata_change_toast,
                ...
                * @{
                */
                final ExportRequest request = new ExportRequest(mTargetFileName,mSelectedContacts);
                /**
                 * @}
                 */
