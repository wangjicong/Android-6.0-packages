        ...
        if (filter != null) {
            if (forPhone) {
                if (filter.filterType == ContactListFilter.FILTER_TYPE_ALL_ACCOUNTS) {
                	...
                	String text1 = context.getString(
                            R.string.listAllContactsInAccount, filter.accountName);
                    if(OptConfig.SUNVOV_CUSTOM_C7301_XLL_FWVGA){
				        text1 = text1.replace("SIM1","Master Sim");
				        text1 = text1.replace("SIM2","Slave Sim");
				    }        
                            
                    headerTextView.setText(text1);
                ...
                if (filter.filterType == ContactListFilter.FILTER_TYPE_ALL_ACCOUNTS) {
                   ...
                   /*
                    * SPRD:Bug447277 DUT shows Chinese character on headerView after select phone option to display contacts.
                    *
                    ...
                    */
                    if (PhoneAccountType.ACCOUNT_TYPE.equals(filter.accountType)) {
                        headerTextView.setText(context.getString(
                                R.string.listAllContactsInAccount,context.getString(R.string.label_phone)));
                    } else {
                    /*wangxing 20160628 modify for rename sim1 sim2 start @{ */					
                        //headerTextView.setText(context.getString(
                        //        R.string.listAllContactsInAccount, filter.accountName));
        ...
        if (filter != null) {
            if (filter.filterType == ContactListFilter.FILTER_TYPE_ALL_ACCOUNTS) {
                ...
                if (PhoneAccountType.ACCOUNT_TYPE.equals(filter.accountType)) {
                    ...
                    if(OptConfig.SUNVOV_CUSTOM_C7301_XLL_FWVGA){
				        text = text.replace("SIM1","Master Sim");
				        text = text.replace("SIM2","Slave Sim");
				    }
