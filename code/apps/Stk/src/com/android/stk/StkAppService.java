...
import static com.android.internal.telephony.cat.CatCmdMessage.SetupEventListConstants.DATA_AVAILABLE_EVENT;
import static com.android.internal.telephony.cat.CatCmdMessage.SetupEventListConstants.CHANNEL_STATUS_EVENT;
...
public class StkAppService extends Service implements Runnable {
    ...
    // SPRD: Be carefully the value must follow Spec.
    ...
    boolean isShowSetupMenuTitle() {
        ...
        if (count == 1) {
            //PersistableBundle pBundle = mCarrierConfigManager.getConfigForPhoneId(id);
     ...
     boolean isCUCCOperator() {
         CatLog.d(LOG_TAG, "isCUCCOperator()");
//         PersistableBundle pBundle = mCarrierConfigManager.getConfig();
//         if (pBundle != null && pBundle.getBoolean(CarrierConfigManager.KEY_FEATURE_STK_NAMEANDICON_CONFIG_BOOL)) {
    ...
    private final class ServiceHandler extends Handler {
        ...
        private void launchRefreshMsg(int slotId) {
            ...
            //if (pBundle != null && pBundle.getBoolean(CarrierConfigManager.KEY_FEATURE_STK_REFRESH_NOTOAST_BOOL)) {
    ...
    private void handleSessionEnd(int slotId) {
        ...
        if (StkMenuActivity.STATE_SECONDARY == mStkContext[slotId].mMenuState) {
            /* SPRD:Bug#490274: When receive session_end,return to mainmenu,if go into the stk not by user @{ */
            //if (needCloseMenu(slotId) && !mAccessByUser) {
            // if (StkPluginsHelper.getInstance(mContext).needCloseMenu() && !mAccessByUser) {
            /* SPRD: add for cucc two icon feature . @{ */
    ...
    private void handleCmd(CatCmdMessage cmdMsg, int slotId) {
        ...
        case GET_CHANNEL_STATUS:
            ...
            break;
        case LAUNCH_BROWSER:
            TextMessage BrowserMsg = mStkContext[slotId].mCurrentCmd.geTextMessage();
            if (BrowserMsg.text == null || BrowserMsg.text.length() == 0) {
                CatLog.d(this, "[stkapp] BrowserMsg is null");
                BrowserMsg.text = getResources().getString(R.string.action_launch_browser);
            }
            launchConfirmationDialog(BrowserMsg, slotId);
            break;
        case SET_UP_CALL:
            ...
            if (ni != null && ni.getType() == ConnectivityManager.TYPE_MOBILE_SUPL) {
                ...
                if (ni.getState() == NetworkInfo.State.CONNECTED) {
                    ...
                    if (mStkContext[slotId].mChannelEstablished) {
                        mStkContext[slotId].mChannelStatus.SetStatus(AppInterface.DEFAULT_CHANNELID, false,
    ...
    private void onCreateSocket(int slotId) {
        ...
        mStkContext[slotId].mChannelStatus.SetStatus(AppInterface.DEFAULT_CHANNELID, true);
        mStkContext[slotId].mLastChannelStatus.SetStatus(
        ...
        if (createSocket(slotId) == SOCKET_OK) {
            ...
            if (mStkContext[slotId].mOpenChannelData.bufferSize > DEFAULT_BUFFER_SIZE) {
                ...
                SendChannelResponse(mStkContext[slotId].mOpenChannelData,
                        ResultCode.PRFRMD_WITH_MODIFICATION, slotId);
            } else {
                SendChannelResponse(mStkContext[slotId].mOpenChannelData, ResultCode.OK, slotId);
            ...
            SendChannelResponse(mStkContext[slotId].mOpenChannelData,
                    ResultCode.BEYOND_TERMINAL_CAPABILITY, slotId);
    ...
    private void openChannel(int slotId) {
        ...
        if (!phoneIsIdle()) {
            ...
            SendChannelResponse(mStkContext[slotId].mOpenChannelData,
                    ResultCode.TERMINAL_CRNTLY_UNABLE_TO_PROCESS, slotId);
            return;
        }
    ...
    private String getDefaultBearerNetAccessName(int slotId) {
        return mStkContext[slotId].mOpenChannelData.getDefaultBearerNetAccessName(slotId);
    }
    ...
    private void processOpenChannel(int slotId) {
        ...
        if (mStkContext[slotId].mOpenChannelData.text == null) {
            ...
            TextMessage channelMsg = new TextMessage();
    ...
    private void launchCloseChannel(int slotId) {
        ...
        synchronized (mTcpUdpLock) {
            ...
            if (mStkContext[slotId].mOpenChannelData != null) {
                ...
                SendCloseChannelResponse(ResultCode.OK, slotId);
                ...
                SendCloseChannelResponse(ResultCode.BIP_ERROR, slotId);
            }
        }
    }
    ...
    private void launchReceiveData(int slotId) {
        ...
        if (mStkContext[slotId].mReceiveDataLen > 0 && mStkContext[slotId].mRecevieData != null) {
            ...
            /* @} */
            ReceiveDataResponse(dataStr, remainLen, ResultCode.OK, slotId);
        } else {
            CatLog.d(this, "launchReceiveData receive nothing");
            ReceiveDataResponse(null, 0, ResultCode.BEYOND_TERMINAL_CAPABILITY, slotId);
        }
        }
    }

    private void ReceiveDataResponse(String datastr, int len, ResultCode resCode, int slotId) {
    ...
    private void launchSendData(CommandDetails cmddet, int slotId) {
        ...
        if (type == OpenChannelData.TRANSPORT_TYPE_TCP) {
            ...
            if (mStkContext[slotId].mTcpSocket == null) {
                ...
                SendDataResponse(0, ResultCode.BEYOND_TERMINAL_CAPABILITY, slotId);
                return;
            }
            if (cmddet.commandQualifier == SEND_DATA_STORE) {
                ...
                SendDataResponse(channellen, ResultCode.OK, slotId);
    ...
    private void SendDataResponse(int len, ResultCode resCode, int slotId) {
    ...
    private void launchChannelStatus(int slotId) {
        ...
        SendChannelStatusResponse(ResultCode.OK, slotId);
    }

    private void SendChannelStatusResponse(ResultCode resCode, int slotId) {
    ...
    private void SendCloseChannelResponse(ResultCode resCode, int slotId) {
        ...
        public void run() {
            ...
            //while (true) {
                    try {
                        ...
                        closeSocket(slotId);
                        if (mStkContext[slotId].mChannelEstablished) {
                            mStkContext[slotId].mChannelStatus.SetStatus(AppInterface.DEFAULT_CHANNELID, false, CHANNEL_MODE_LINK_DROPPED);
                            sendEventChannelStatus(slotId);
                        }
...
//            while(true) {
    ...
    private void SendChannelResponse(OpenChannelData data, ResultCode resCode, int slotId) {
        ...
        if (resCode == ResultCode.BEYOND_TERMINAL_CAPABILITY) {
            deleteOpenChannelApn(slotId);
            if (mStkContext[slotId].mConnectType == ConnectivityManager.TYPE_MOBILE_SUPL) {
                endConnectivity(slotId);
            }
        ...
        case RES_ID_CONFIRM:
            ...
            case OPEN_CHANNEL:
                ...
                if (confirmed) {
                    ...
                    // Cancel
                    resMsg.setConfirmation(confirmed);
                    SendChannelResponse(mStkContext[slotId].mOpenChannelData, ResultCode.USER_NOT_ACCEPT, slotId);
                }
                return;
            /* @} */
            }
    ...
    public boolean needCloseMenu(int slotId) {
        ...
        if (pBundle != null) {
            needClose = pBundle.getBoolean(
                    CarrierConfigManager.KEY_FEATURE_STK_ENDSESSION_TOMAINMENU_BOOL);
        }
