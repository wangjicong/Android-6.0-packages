...
public class SearchEngineFragment extends PreferenceFragment {
    ...
    private void fillList() {
        ...
        for (int id = 0; id < EngineKey.length; id++) {
            ...
            /* @} */
            /*ljb 20170508 add for remove baidu */
            if(OptConfig.SUN_CUSTOM_C7367_MD_FWVGA){
               if (id == BAIDU_SEARCH_ID ) {
                   continue;
               }
            }
            /*ljb 20170508 end for remove baidu */
    ...
    public void onResume() {
        ...
        if (TextUtils.isEmpty(mSelectedEngineKey)) {
            // defaule
            mSelectedEngineKey = com.android.quicksearchbox.util.Util.ENGINES[getResources().getInteger(R.integer.sunvov_default_selected_engine_key)];//SUN:jicong.wang modify for default engine
        }
        ...
        if (mAllEngineMap != null) {
            for (HashMap.Entry<String, SearchEnginePreference> entry : mAllEngineMap
                ...
                if (chooseKey.equals(entry.getKey())) {
                    ...
                    if (TextUtils.isEmpty(mSelectedEngineKey)) {
                        // default
                        mSelectedEngineKey = com.android.quicksearchbox.util.Util.ENGINES[0];
                    }
                    //SPRD: 493103 set search engine in global settings to avoid crash
