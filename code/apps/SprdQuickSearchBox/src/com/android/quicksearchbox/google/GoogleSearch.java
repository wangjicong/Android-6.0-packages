...
public class GoogleSearch extends Activity {
    ...
    private Intent createLaunchUriIntentFromSearchIntent(Intent intent) {
        ...
        try {
            String searchUri = getSearchUrl(source, URLEncoder.encode(query, "UTF-8"));
...
//                    + "&q=" + URLEncoder.encode(query, "UTF-8");
     ...
     */
    public String getSearchUrl(String source,String query) {
        Log.d("logs1", "aaaa");
        String baseUrl = null;
        //SPRD: 493103 set search engine in global settings to avoid crash
        String saveEngine =Settings.Global.getString(this.getContentResolver(),
                com.android.quicksearchbox.util.Util.MYENGINE);
        /*SUN:jicong.wang add for default url*/
        if (TextUtils.isEmpty(saveEngine)) {
            // defaule
            saveEngine = com.android.quicksearchbox.util.Util.ENGINES[this.getResources().getInteger(R.integer.sunvov_default_selected_engine_key)];//SUN:jicong.wang modify for default engine
        }     
        /*SUN:jicong.wang add for default url*/
        if(com.android.quicksearchbox.util.Util.ENGINES[4].equals(saveEngine)){
            //bing
            baseUrl = this.getResources().getString(R.string.common_search_base_pattern,
                    getSearchDomain(4));
            query = "/search?q="+query;
        }else if(com.android.quicksearchbox.util.Util.ENGINES[3].equals(saveEngine)){
            //yahoo
            baseUrl = this.getResources().getString(R.string.common_search_base_pattern,
                    getSearchDomain(3));
            // SPRD: modify this for Bug210932 at 06/09/2013
            query = "/s?p="+query;
        }else if(com.android.quicksearchbox.util.Util.ENGINES[2].equals(saveEngine)){
            //139
            baseUrl = this.getResources().getString(R.string.common_search_base_pattern,
                    getSearchDomain(2));
            // SPRD: modify this for Bug210932 at 06/09/2013
            query = "/page/search?rs=monternet&q=" +query;
            return baseUrl + query;
        }else if(com.android.quicksearchbox.util.Util.ENGINES[1].equals(saveEngine)){
            //google
            baseUrl = this.getResources().getString(R.string.google_search_base_pattern,
                    getSearchDomain(1),GoogleSearch.getLanguage(Locale.getDefault()));
            query = "&q=" +query;
        }else {
            //baidu
            baseUrl = this.getResources().getString(R.string.common_search_base_pattern,
        ...
        return baseUrl + query + "&source=android-" + source;
    }

    public String getSearchDomain(int engine) {
