...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    //SPRD: add for SPRD_CIRCLE_SLIDE_SUPPORT.
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if(mIsEnableCircular) {
            mOverPageLeft[(isRtl ? OVER_LAST_INDEX : OVER_FIRST_INDEX)] = -mViewport.width();
            mOverPageLeft[(isRtl ? OVER_FIRST_INDEX : OVER_LAST_INDEX)] = (mViewport.width()) * childCount;
        }
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (pageCount > 0) {
            ...
            if (mIsEnableCircular) {
                drawCircularPageIfNeeded(canvas);
            }
        }
    }
    ...
    protected boolean isInOverScroll() {
        return ((mOverScrollX > mMaxScrollX || mOverScrollX < 0) && !mIsEnableCircular);
    }
    ...
    private void drawCircularPageIfNeeded(Canvas canvas) {
        ...
        if (isXBeforeFirstPage || isXAfterLastPage) {
            ...
            int offset = (isRtl ? - childCount : childCount) * (width);
