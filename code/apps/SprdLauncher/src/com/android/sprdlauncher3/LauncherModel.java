...
public class LauncherModel extends BroadcastReceiver
        ...
        Runnable r = new Runnable() {
            public void run() {
                ...
                synchronized(sBgLock) {
                    ...
                    boolean isFirstLoad = LauncherAppState.getLauncherProvider().isFirstLoad();
                    if (LauncherAppState.isDisableAllApps() && isFirstLoad) {
                        startSearchPageIndex = workspaceScreens.isEmpty()
                                ? 0 : (workspaceScreens.size());
                    }
                    while (iter.hasNext()) {
                        ...
                        if (!(LauncherAppState.isDisableAllApps() && isFirstLoad)) {
                            startSearchPageIndex = workspaceScreens.isEmpty() ? 0 : 1;
                        }
                    ...
                    if (isFirstLoad) {
                        LauncherAppState.getLauncherProvider().clearFirstLoadFlag();
                    }
                }
    ...
    void forceReload() {
        ...
        if (LogUtils.DEBUG_SL) {
            LogUtils.d(TAG, "forceReload: mLoaderTask =" + mLoaderTask + ", mAllAppsLoaded = "
    ...
    public void resetLoadedState(boolean resetAllAppsLoaded, boolean resetWorkspaceLoaded) {
        synchronized (mLock) {
            if (LogUtils.DEBUG_SL) {
                LogUtils.d(TAG, "resetLoadedState: mLoaderTask =" + mLoaderTask
                        + ", this = " + this);
            }
    ...
    public void startLoaderFromBackground() {
        if (LogUtils.DEBUG_SL) {
            LogUtils.d(TAG, "startLoaderFromBackground: mCallbacks = " + mCallbacks + ", this = " + this);
        }
    ...
    private boolean stopLoaderLocked() {
        ...
        if (LogUtils.DEBUG_SL) {
            LogUtils.d(TAG, "stopLoaderLocked: mLoaderTask =" + mLoaderTask + ", isLaunching = "
    ...
    public void stopLoader() {
        synchronized (mLock) {
            if (mLoaderTask != null) {
                if (LogUtils.DEBUG_SL) {
                    LogUtils.d(TAG, "stopLoader: mLoaderTask = " + mLoaderTask
    ...
    private class LoaderTask implements Runnable {
        ...
        private boolean loadAndBindWorkspace() {
            ...
            if (!mWorkspaceLoaded) {
                ...
                synchronized (LoaderTask.this) {
                    if (mStopped) {
                        LogUtils.d(TAG, "loadAndBindWorkspace returned by stop flag.");
        ...
        private void waitForIdle() {
            ...
            synchronized (LoaderTask.this) {
                ...
                if (LogUtils.DEBUG_SL) {
                    LogUtils.d(TAG, "waitForIdle start, workspaceWaitTime : " + workspaceWaitTime + "ms, Thread priority :"
        ...
        void runBindSynchronousPage(int synchronousBindPage) {
            if (LogUtils.DEBUG_SL) {
                LogUtils.d(TAG, "runBindSynchronousPage: mAllAppsLoaded = " + mAllAppsLoaded
        ...
        public void run() {
            ...
            synchronized (mLock) {
                if (LogUtils.DEBUG_SL) {
                    LogUtils.d(TAG, "Set load task running flag >>>>, mIsLaunching = " +
            ...
            keep_running: {
                ...
                if (mStopped) {
                    LogUtils.i(TAG, "LoadTask break in the middle, this = " + this);
                    break keep_running;
                }
            ...
            synchronized (mLock) {
                ...
                if (LogUtils.DEBUG_SL) {
                    LogUtils.d(TAG, "Reset load task running flag <<<<, this = " + this);
                }
        ...
        public void stopLocked() {
            ...
            if (LogUtils.DEBUG_SL) {
                LogUtils.d(TAG, "stopLocked completed, this = " + LoaderTask.this
        ...
        Callbacks tryGetCallbacks(Callbacks oldCallbacks) {
            synchronized (mLock) {
                if (mStopped) {
                    LogUtils.i(TAG, "tryGetCallbacks returned null by stop flag.");
                    return null;
                }
        ...
        private void verifyApplications() {
            ...
            synchronized (sBgLock) {
                for (AppInfo app : mBgAllAppsList.data) {
                    tmpInfos = getItemInfoForComponentName(app.componentName, app.user, true);
                    if (tmpInfos.isEmpty()) {
                            ...
                            case LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT:
                                ...
                                if (restored) {
                                    ...
                                    info = getShortcutInfo(manager, intent, user, context, c,
                                            iconIndex, titleIndex, itemType, mLabelCache, allowMissingTarget);
                                ...
                                if (info != null) {
                                    ...
                                    //init download flags when read Favorites from DB
                                    if(!Utilities.isSystemApp(context,info.getIntent())){
    ...
    private class PackageUpdatedTask implements Runnable {
        ...
        public void run() {
            ...
            if (LogUtils.DEBUG_SL) {
                LogUtils.d(TAG, "PackageUpdatedTask: added = " + added + ",modified = "
            ...
            if (!removedPackageNames.isEmpty() || !removedApps.isEmpty()) {
                ...
                if (mOp == OP_UNAVAILABLE) {
                    ...
                    for (AppInfo a : removedApps) {
                        ArrayList<ItemInfo> infos = getItemInfoForComponentName(a.componentName, mUser, false);
    ...
    public ShortcutInfo getShortcutInfo(PackageManager manager, Intent intent,
        ...
        return getShortcutInfo(manager, intent, user, context, null, -1, -1, -1, null, false);
    }
     ...
     */
    public ShortcutInfo getShortcutInfo(PackageManager manager, Intent intent,
            UserHandleCompat user, Context context, Cursor c, int iconIndex, int titleIndex, int itemType,
    ...
    private ArrayList<ItemInfo> getItemInfoForComponentName(final ComponentName cname,
            final UserHandleCompat user, final boolean isVerify) {
        ItemInfoFilter filter  = new ItemInfoFilter() {
            @Override
            public boolean filterItem(ItemInfo parent, ItemInfo info, ComponentName cn) {
                //Add for in case custom wrong type.
                if (info.user == null) {
                    return isVerify ? ((info.itemType != LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT ) && cn.equals(cname))
                            : cn.equals(cname);
                } else {
                    return isVerify ? ((info.itemType != LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT ) && (cn.equals(cname) && info.user.equals(user)))
                            : (cn.equals(cname) && info.user.equals(user));
                }
            }
        };
        return filterItemInfos(sBgItemsIdMap.values(), filter);
    ...
    ShortcutInfo infoFromShortcutIntent(Context context, Intent data) {
        ...
        //init download flags when shortcut created by Intent
        if(!Utilities.isSystemApp(context,info.getIntent())) {
            info.flags |= AppInfo.DOWNLOADED_FLAG;
        }
