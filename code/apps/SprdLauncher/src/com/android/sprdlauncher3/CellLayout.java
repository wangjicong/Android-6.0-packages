...
public class CellLayout extends ViewGroup {
    ...
    public void setHomeImageView(HomeImageView view) {
        mHomeImageView = view;
        addView(mHomeImageView);
    }
    ...
    private DefaultPageController getDefaultPageController() {
        if (mHomeImageView != null) {
            return mHomeImageView.getDefaultPageController();
        } else {
            return null;
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        LauncherAppState app = LauncherAppState.getInstance();
        DeviceProfile grid = app.getDynamicGrid().getDeviceProfile();
        DefaultPageController defaultPageController = getDefaultPageController();
        ...
        if (mFixedCellWidth < 0 || mFixedCellHeight < 0) {
            int cw = 0;
            if (defaultPageController != null) {
                cw = defaultPageController.getCalculateCellWidth(childWidthSize, mCountX, mHomeImageView);
            } else {
        ...
        // add height offset due to home image view
        int heightOffset  = defaultPageController != null ?
                defaultPageController.getHeightOffsetDueToHome(mHomeImageView) : 0;
        if (mFixedWidth > 0 && mFixedHeight > 0) {
            setMeasuredDimension(maxWidth, maxHeight + heightOffset);
        } else {
            setMeasuredDimension(widthSize, heightSize + heightOffset);
        }
    }

    @Override
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        // width offset due to home image view
        DefaultPageController defaultPageController = getDefaultPageController();
        int widthOffset = defaultPageController != null ?
                defaultPageController.getWidthOffsetDueToHome(mHomeImageView) : 0;
        int offset = getMeasuredWidth() - getPaddingLeft() - getPaddingRight() -
                (mCountX * mCellWidth + widthOffset);
        int left = getPaddingLeft() + (int) Math.ceil(offset / 2f);
        int top = getPaddingTop();
        int count = getChildCount();
        // height offset due to home image view
        int heightOffset = defaultPageController != null ?
                defaultPageController.getHeightOffsetDueToHome(mHomeImageView) : 0;
        for (int i = 0; i < count; i++) {
            View child = getChildAt(i);
            if(child instanceof HomeImageView) {
                LauncherAppState app = LauncherAppState.getInstance();
                DeviceProfile grid = app.getDynamicGrid().getDeviceProfile();
                int homeWidth = child.getMeasuredWidth();
                int homeHeight = child.getMeasuredHeight();
                boolean isLandscape = grid.isLandscape();
                if (DefaultPageController.H_LAYOUT_OF_LEFT && isLandscape) {
                    int homeImageViewTop = top + (b - t - homeHeight) / 2;
                    child.layout(left, homeImageViewTop, left + homeWidth, top + homeHeight);
                } else {
                    int homeImageViewLeft = left + (r - l - homeWidth)/2;
                    child.layout(homeImageViewLeft, top, homeImageViewLeft
                            + homeWidth, top + homeHeight);
                }
            } else {
                child.layout(left + widthOffset, top + heightOffset,
                    left + r - l,
                    top + b - t);
            }
        }
