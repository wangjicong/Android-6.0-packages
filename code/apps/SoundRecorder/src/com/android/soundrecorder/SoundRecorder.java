     ...
     private void doRecord(){
         ...
         if(mNowState == Recorder.IDLE_STATE) {
             ...
             if (!isRequestType){
                 ...
                 mRequestedType = recordSavePreferences.getString(RecordingFileList.SAVE_RECORD_TYPE,getResources().getString(R.string.default_record_type)/*AUDIO_AMR*/);//Kalyy add for default record type
             }
             /* SPRD: fix bug 521431 @{ */
             long startTime= System.currentTimeMillis();
             if ((startTime - mSampleStart) > 1000) {
...
//                 finish();
 ...
 /* @} */
    ...
    public void updateTimerView(boolean initial) {
        ...
        if (state == Recorder.PLAYING_STATE) {
            if (mRecorder.sampleLengthSec() != 0 && mRecorder.sampleLengthSec() != 0) {
                mStateProgressBar.setProgress((int)(100*time/mRecorder.sampleLengthSec()));
            } else {
                mStateProgressBar.setProgress(mStateProgressBar.getMax());
            }
        } else if (state == Recorder.RECORDING_STATE && mService != null) {
            updateTimeRemaining();
        }
