...
class DumbBitmapRegionDecoder implements SimpleBitmapRegionDecoder {
    ...
    private static InputStream regenerateInputStream(Context mContext, Uri mUri) throws FileNotFoundException {
        if (mUri != null) {
            return new BufferedInputStream(mContext.getContentResolver().openInputStream(mUri));
        }
        return null;
    }

    public static DumbBitmapRegionDecoder newInstance(InputStream is, Uri uri, Context mContext)
        ...
        while (noBitmap) {
            try {
                ...
                if (++num_tries >= BitmapCropTask.MAX_NUM_COMPRESSION) {
                    noBitmap = false;
                }
                System.gc();
                sampleSize *= 2;
                ...
                is = regenerateInputStream(mContext, uri);
                Log.w("BitmapRegionTileSource", "decodeStream oom");
            }
        }
        if (is != null) {
            Utils.closeSilently(is);
...
public class BitmapRegionTileSource implements TiledImageRenderer.TileSource {
    ...
    private static int sTargetSize = -1;
    ...
    public static abstract class BitmapSource {
        ...
        public boolean loadInBackground(InBitmapProvider bitmapProvider) {
            ...
            if (mDecoder == null) {
                ...
                // SPRD:ADD 499611 preview maybe null, so catch exception.
                catch (Exception e) {
                    mState = State.ERROR_LOADING;
                    Log.w(TAG, "Image cannot be rendered on a GL surface", e);
                }
                */
                /*@}*/
                return true;
    ...
    public static class UriBitmapSource extends BitmapSource {
        ...
        public SimpleBitmapRegionDecoder loadBitmapRegionDecoder() {
            try {
                ...
                if (regionDecoder == null) {
                    ...
                    //SPRD: bug 508632 OOM when select a image whose size is 9000*9000
                    regionDecoder = DumbBitmapRegionDecoder.newInstance(is,mUri,mContext);
                    //Utils.closeSilently(is);
                }
        ...
        public Bitmap loadPreviewBitmap(BitmapFactory.Options options) {
            ...
            // SPRD:ADD 499611 catch OOM error and tip to user.
                ...
                Log.w("BitmapRegionTileSource", "Failed to load URI " + mUri, error);
                return null;
            }
        }
        @Override
    ...
    public BitmapRegionTileSource(Context context, BitmapSource source, byte[] tempStorage) {
        ...
        if (mDecoder != null) {
            ...
            int targetsize = getTargetSize(context);
            Bitmap preview = decodePreview(source, targetsize);
            /* @} */
            if (preview != null &&
            ...
            // SPRD:ADD 499611 preview maybe null, so do not show its message.
            else {
                Log.w(TAG, String.format(
                        "Failed to create preview of apropriate size! "
                        + " in: %dx%d",
    ...
    private static int getTargetSize(Context context) {
        ...
        sTargetSize = width > height ? width : height;
