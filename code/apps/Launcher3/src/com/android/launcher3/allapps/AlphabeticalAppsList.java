...
public class AlphabeticalAppsList {
    ...
    public static class AdapterItem {
        ...
        public static AdapterItem asPredictedApp(int pos, SectionInfo section, String sectionName,
                                        int sectionAppIndex, ItemInfo appInfo, int appIndex) {
        ...
        public static AdapterItem asApp(int pos, SectionInfo section, String sectionName,
                                        int sectionAppIndex, ItemInfo appInfo, int appIndex) {
            AdapterItem item = new AdapterItem();
            item.viewType = PluginAlphabeticalApplist.getInstance().setItemViewType(appInfo);
            item.position = pos;
            item.sectionInfo = section;
    ...
    public void setApps2Folder(List<AppInfo> apps, List<ItemInfo> items) {
        PluginAlphabeticalApplist.getInstance().setApps2Folder(this, apps, items, mComponentToAppMap);

    }

    public void setApps2Folder(List<ItemInfo> items) {
         PluginAlphabeticalApplist.getInstance().setApps2Folder(this, items, mComponentToAppMap);
    }
    ...
    public void addApps(List<AppInfo> apps, List<ItemInfo> items) {
        PluginAlphabeticalApplist.getInstance().addApps(this, apps, items);
    }

    public void addApps(List<ItemInfo> items,int dummy) {
        PluginAlphabeticalApplist.getInstance().addApps(this, items, dummy);
    }
    /**
    ...
    public void updateApps(List<AppInfo> apps, List<ItemInfo> items) {
        PluginAlphabeticalApplist.getInstance().updateApps(this, apps, items, mComponentToAppMap);
    }

    public void updateApps(List<ItemInfo> items,int dummy) {
        PluginAlphabeticalApplist.getInstance().updateApps(this, items, dummy, mComponentToAppMap);
    }
    ...
    public void onAppsUpdated() {
        if (FolderPlugins.getInstance().isEnable()) {
            PluginAlphabeticalApplist.getInstance().onAppsUpdated(mApps, mComponentToAppMap, mAppNameComparator,
                 mLauncher, this);
            return;
        }
    ...
    public void updateAdapterItems() {
        if (FolderPlugins.getInstance().isEnable()) {
            PluginAlphabeticalApplist.getInstance().updateAdapterItems(
                this, mFilteredApps, mFastScrollerSections, mAdapterItems, mSections,
                mPredictedApps, mPredictedAppComponents, mComponentToAppMap, mLauncher,
                mNumPredictedAppsPerRow, mNumAppsPerRow, mNumAppRowsInAdapter,
                 mAdapter, mSearchResults, mApps);
            return;
        }
        ...
        if (mPredictedAppComponents != null && !mPredictedAppComponents.isEmpty() && !hasFilter()) {
            for (ComponentKey ck : mPredictedAppComponents) {
                ItemInfo info = mComponentToAppMap.get(ck);
                if (info != null) {
                    mPredictedApps.add((AppInfo)info);
                } else {
        ...
        for (ItemInfo info : getFiltersAppInfos()) {
            String sectionName = getAndUpdateCachedSectionName(info.title);
    ...
    public List<ItemInfo> getFiltersAppInfos() {
        if (FolderPlugins.getInstance().isEnable()) {
            return PluginAlphabeticalApplist.getInstance().getFiltersAppInfos(mSearchResults, mApps, mComponentToAppMap);
        }
        ...
        for (ComponentKey key : mSearchResults) {
            ItemInfo match = mComponentToAppMap.get(key);
            if (match != null) {
                result.add(match);
            }
        }
     ...
     */
    public String getAndUpdateCachedSectionName(CharSequence title) {
