...
public class AudioProfileProvider extends ContentProvider {
    ...
    private void createProfileTable(SQLiteDatabase db) {
        ...
        //Kalyy add for custom
        int defVibrate = AudioProfile.NOT_VIBRATE;
        int defVibrateMsg = AudioProfile.NOT_VIBRATE_MSG;
        /*SUN:jicong.wang add for call msg generalProfile vibrate {@ */
        defVibrate =  Integer.parseInt(mContext.getResources().getString(com.android.internal.R.string.def_generalAudioProfileVibrate_when_call));
        
        defVibrateMsg = Integer.parseInt(mContext.getResources().getString(com.android.internal.R.string.def_generalAudioProfileVibrate_when_message));
        /*SUN:jicong.wang add for call msg generalProfile vibrate @} */
        
        //qiuyaobo,20161024,begin  
        if(mContext.getResources().getBoolean(com.android.internal.R.bool.def_generalAudioProfile_sound_effects_enabled))
            defSoundEffects = AudioProfile.IS_SOUND_EFFECTS;
        else
            defSoundEffects = AudioProfile.NOT_SOUND_EFFECTS;  
        //qiuyaobo,20161024,end
 
        //qiuyaobo,20161026,begin  
        if(mContext.getResources().getBoolean(com.android.internal.R.bool.def_generalAudioProfile_haptic_feedback))
            defHapticFeedback = AudioProfile.IS_HAPTIC_FEEDBACK;
        else
            defHapticFeedback = AudioProfile.NOT_HAPTIC_FEEDBACK;    
        //qiuyaobo,20161026,end  
                
        if(OptConfig.SUN_C7359_C5S_FWVGA_FPT){
            defSoundEffects = AudioProfile.NOT_SOUND_EFFECTS;
            defVibrate = AudioProfile.IS_VIBRATE;
            defVibrateMsg = AudioProfile.IS_VIBRATE_MSG;
        }
        ...
        Uri defTwoUri = RingtoneManager.getActualDefaultRingtoneUri(mContext,RingtoneManager.TYPE_RINGTONE, 1);
        ...
        String defMesTwoUri = Settings.System.getString(resolver, Settings.System.DEFAULT_MESSAGE + 1);
        String defMesOneUri = Settings.System.getString(resolver, Settings.System.DEFAULT_MESSAGE + 0);
        String profileColumns = AudioProfileColumns.NAME + " TEXT, "
                ...
                + AudioProfileColumns.IS_VIBRATE + " INTEGER DEFAULT " + defVibrate/*AudioProfile.NOT_VIBRATE*/ + ", "
                + AudioProfileColumns.IS_VIBRATE_MSG + " INTEGER DEFAULT " + defVibrateMsg/*AudioProfile.NOT_VIBRATE_MSG*/ + ", "
    ...
    private void resetDefProfiles(SQLiteDatabase db) {
        ...
        Uri defRingtoneOneUri = AudioProfile.getDefaultUriForId(mContext, RingtoneManager.TYPE_RINGTONE, 0);
        Uri defRingtoneTwoUri = AudioProfile.getDefaultUriForId(mContext, RingtoneManager.TYPE_RINGTONE, 1);
        ...
        String defMessagetoneTwoUri = Settings.System.getString(resolver, Settings.System.DEFAULT_MESSAGE + 1);
        String defMessagetoneOneUri = Settings.System.getString(resolver, Settings.System.DEFAULT_MESSAGE + 0);
        ...
        for (int i = 0; i < mPhoneCount; i++) {
            if (mPhoneCount - 1 == i) {
                ...
                genernalValues += defRingtoneTwoUri + "', '" + defMessagetoneTwoUri;
            } else {
                genernalColumns += ringtoneStr[i] + "', '" + mMessagetoneStr[i] + "', '";
                genernalValues += defRingtoneOneUri + "', '" + defMessagetoneOneUri + "', '";
            }
        ...
        for (int i = 0; i < mPhoneCount; i++) {
            if (mPhoneCount - 1 == i) {
                ...
                outdoorValues += defRingtoneTwoUri + "', '" + defMessagetoneTwoUri;

            } else {
                outdoorColumns += ringtoneStr[i] + "', '" + mMessagetoneStr[i] + "', '";;
                outdoorValues += defRingtoneOneUri + "', '" + defMessagetoneOneUri + "', '";
            }
