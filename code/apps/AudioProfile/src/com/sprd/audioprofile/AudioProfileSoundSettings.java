...
public class AudioProfileSoundSettings extends PreferenceActivity {
    ...
    private static Uri[] mDefaultMessageUri;
    ...
    private void setRingTone(int phoneId) {
        ...
        if(OptConfig.SUNVOV_CUSTOM_C7301_XLL_FWVGA){
            String s = getString(R.string.sim_ringtone_title, (phoneId + 1), " ");
            s = s.replace("SIM1","Master Sim");
            s = s.replace("SIM2","Slave Sim");
            mRingtonePreference[phoneId].setTitle(s);
        }else{
        ...
        /*wangxing 20160628 modify for rename sim1 sim2 start @{ */
        if(OptConfig.SUNVOV_CUSTOM_C7301_XLL_FWVGA){
            String s = getString(R.string.sim_messagetone_title, (phoneId + 1), " ");
            s = s.replace("SIM1","Master Sim");
            s = s.replace("SIM2","Slave Sim");
            mMessagetonePreference[phoneId].setTitle(s);
        }else{
        ...
        if (TelephonyManager.from(mContext).hasIccCard(phoneId)) {
            ...
            for (int i = 0; i < subInfoRecords.size(); i++) {
                if (subInfoRecords.get(i).getSimSlotIndex() == phoneId) {
                	...
                	/*wangxing 20160628 modify for rename sim1 sim2 start @{ */
			        if(OptConfig.SUNVOV_CUSTOM_C7301_XLL_FWVGA){
			            String ringtoneStr = getString(R.string.sim_ringtone_title,
	                            (subInfoRecords.get(i).getSimSlotIndex() + 1),
	                            subInfoRecords.get(i).getDisplayName());
	                    String messagetoneStr = getString(R.string.sim_messagetone_title,
	                            (subInfoRecords.get(i).getSimSlotIndex() + 1),
	                            subInfoRecords.get(i).getDisplayName());        
			            ringtoneStr = ringtoneStr.replace("SIM1","Master Sim");
			            ringtoneStr = ringtoneStr.replace("SIM2","Slave Sim");
			            messagetoneStr = ringtoneStr.replace("SIM1","Master Sim");
			            messagetoneStr = ringtoneStr.replace("SIM2","Slave Sim");
			            mRingtonePreference[phoneId].setTitle(ringtoneStr);
			            mMessagetonePreference[phoneId].setTitle(messagetoneStr);
			        }else{
    ...
    public static void getDefaultUri(Context context) {
        if(mDefaultRingtoneUri == null) {
             mDefaultRingtoneUri = new Uri[mPhoneCount];
        }
        for (int i = 0; i < mPhoneCount; i++) {
            String ringerUriString = Settings.System.getString(context.getContentResolver(),
                Settings.System.DEFAULT_RINGTONE + i);
            mDefaultRingtoneUri[i] = (ringerUriString != null ? Uri.parse(ringerUriString) : null);
            Log.d(TAG, "mDefaultRingtoneUri[" + i + "]=" + (mDefaultRingtoneUri[i] !=null ? mDefaultRingtoneUri[i].toString() : mDefaultRingtoneUri[i]));
        }
        ...
        if (mDefaultMessageUri == null) {
            mDefaultMessageUri = new Uri[mPhoneCount];
            for (int i = 0; i < mPhoneCount; i++) {
                String messageString = Settings.System.getString(context.getContentResolver(),
                        Settings.System.DEFAULT_MESSAGE + i);
                mDefaultMessageUri[i] = (messageString != null ? Uri.parse(messageString) : null);
                
                //qiuyaobo,20161102,begin               
                if(mDefaultMessageUri[i] != null)
                //qiuyaobo,20161102,end
                Log.d(TAG, "mDefaultMessageUri[i]="+mDefaultMessageUri[i].toString());
            }
        }
    }
    ...
    private void updateRingtoneName(int type, int index, Preference preference, int msgId) {
        ...
        if (mIsSelected) {
            if (type == RingtoneManager.TYPE_RINGTONE) {
                ringtoneUri = RingtoneManager.getActualDefaultRingtoneUri(context, type, index);
                if (ringtoneUri != null) {
                ...
                if(messagetone == null){
                    ...
                    //qiuyaobo,20161102,begin
                    //Uri defaultUri = mDefaultMessageUri[index];
                    Uri defaultUri = null;
                    if(mDefaultMessageUri != null)
                    		defaultUri = mDefaultMessageUri[index];
                    //qiuyaobo,20161102,end		
                    if(defaultUri != null){
                    ...
                    try {
                        ...
                        if(mDefaultMessageUri != null)
                        //qiuyaobo,20161102,end
                        ringtoneUri = mDefaultMessageUri[index];
        ...
        try {
            if (type == RingtoneManager.TYPE_RINGTONE && mDefaultRingtoneUri != null && mDefaultRingtoneUri[index] != null) {
                defCursor = context.getContentResolver().query(mDefaultRingtoneUri[index],
                        new String[] {
            ...
            case RingtoneManager.TYPE_RINGTONE:
                ...
                if (AudioProfileProvider.ringtoneStr[index] != null) {
                    values.put(AudioProfileProvider.ringtoneStr[index],
                            mDefaultRingtoneUri[index] == null ? null : mDefaultRingtoneUri[index].toString());
                    if (mIsSelected) {
                        Log.d(TAG, "updateRingtoneUri----Ringtone----setActualDefaultRingtoneUri");
                        Log.d(TAG, "updateRingtoneUri----Ringtone----mDefaultRingtoneUri = "
                                + mDefaultRingtoneUri[index]);
                        RingtoneManager.setActualDefaultRingtoneUri(mContext,
                                RingtoneManager.TYPE_RINGTONE, mDefaultRingtoneUri[index], index);
                    }
                }
                break;
            ...
            case RINGTONETYPE_MESSAGE:
                ...
                if (AudioProfileProvider.mMessagetoneStr[index] != null) {
                    /*values.put(AudioProfileProvider.mMessagetoneStr[index],
                            mDefaultNotificationUri == null ? null : mDefaultNotificationUri.toString());*/
                    values.put(AudioProfileProvider.mMessagetoneStr[index],
                            mDefaultMessageUri[index] == null ? null : mDefaultMessageUri[index].toString());
                    if (mIsSelected) {
    ...
    private void showRenameDialog(final String originName) {
        ...
        builder.setMessage(R.string.rename_profile_msg);
