...
public class CachingPipeline implements PipelineInterface {
    ...
    public synchronized void render(RenderingRequest request) {
        ...
        synchronized (CachingPipeline.class) {
            /* SPRD:bug 518839 If mFilterShowActivity is null or isFinishing, it should do nothing. @{*/
            if (MasterImage.getImage().getActivity() == null
                    || MasterImage.getImage().getActivity().isFinishing()) {
                return;
            }
            /* @} */
            if (getRenderScriptContext() == null) {
            ...
            if (request.getType() == RenderingRequest.FULL_RENDERING
                ...
                if (request.getType() == RenderingRequest.ICON_RENDERING) {
                    ...
                    // SPRD: add to fix bug 502322,NullPointerException
