...
public final class ImageLoader {
    ...
    public static String getLocalPathFromUri(Context context, Uri uri) {
        /* SPRD: fix bug 372870,CursorIndexOutOfBoundsException @{ */
        Cursor cursor = null;
        String path = null;
        try{
        ...
        } catch (Exception e) {// SPRD: fix bug 507832,NullPointerException for bad uri
            Log.d(LOGTAG, "exception in query uri :" + uri, e);
        } finally {
    ...
    public static Bitmap loadBitmap(Context context, Uri uri, BitmapFactory.Options o) {
        ...
        try {
            is = context.getContentResolver().openInputStream(uri);
            /* SPRD: wbmp bitmap cannot be resized by BitmapFactory,
             * so we need resize here to avoid OOM. @{
             */
            Bitmap result = BitmapFactory.decodeStream(is, null, o);
            if (result != null && null == result.getConfig()) {
                result = LargeImageProcessingUtils.resizeBitmapByScale(result,
                        1.0f / o.inSampleSize, true);
            }
            return result;
        } catch (OutOfMemoryError e) {
            Log.e(LOGTAG, "Memory it too low, load bitmap failed." + e);
            System.gc();
            /* @} */
        } catch (FileNotFoundException e) {
            Log.e(LOGTAG, "FileNotFoundException for " + uri, e);
        ...
        if (destination != null) {
            ...
            if (bounds.width() > thresholdWidth) {
                ...
                while (w > thresholdWidth) {
                    sampleSize *= 2;
                    // SPRD: Modify 20160105 for bug520952, "java.lang.ArithmeticException: divide by zero"
                    // thrown caused by unknown reason @{
                    if (sampleSize < 1) {
                        Log.w("Gallery2/ImageLoader", "getScaleOneImageForPreset() -> sampleSize < 1, reset it to 1 and break loop.");
                        sampleSize = 1;
                        break;
                    }
                    // @}
    ...
    public static boolean queryLightCycle360(Context context) {
        InputStream is = null;
        try {
            // SPRD: Modify 20150601 Spreadst of bug438127, NullPointerException thrown if getUri() return null
