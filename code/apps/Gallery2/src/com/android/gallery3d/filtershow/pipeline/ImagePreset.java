...
public class ImagePreset {
    ...
    public ImagePreset(ImagePreset source) {
        // SPRD: fix bug 492765, when ProcessingService is not started doing some operate may cause the ImagePreset is null
        if (source != null) {
    ...
    public void updateFilterRepresentations(Collection<FilterRepresentation> reps) {
        for (FilterRepresentation r : reps) {
            // SPRD: fix bug 523979,java.lang.NullPointerException,FilterRepresentation maybe null
            if(r == null) continue;
    ...
    public boolean same(ImagePreset preset) {
        ...
        if (mDoApplyFilters && preset.mDoApplyFilters) {
            for (int i = 0; i < preset.mFilters.size(); i++) {
                ...
                // SPRD:ADD for bug 507013 check equal status when same check return true.
                if (!a.same(b) || !a.equals(b)) {
                    return false;
                }
            }
        }
