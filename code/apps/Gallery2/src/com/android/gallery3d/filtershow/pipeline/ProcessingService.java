...
public class ProcessingService extends Service {
    ...
    public void setFiltershowActivity(FilterShowActivity filtershowActivity) {
        // SPRD: fix bug 387278 can't be saved when two process edit one picture in the same time
        if (mFiltershowActivity != null)  mFiltershowActivity.finish();
    ...
    */
    public static Intent getSaveIntent(Context context, File destination,
            Uri selectedImageUri, Uri sourceImageUri, boolean doFlatten, int quality,
            float sizeFactor, boolean needsExit) {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        mNeedsAlive = true;
        // SPRD: bug 539591,saving image,kill gallery.ImagePreset is null. if null,should not continue to save
        if (intent != null && intent.getBooleanExtra(SAVING, false)
                && (MasterImage.getImage().getSavedPreset() != null)) {
            // we save using an intent to keep the service around after the
            // activity has been destroyed,if Saved Preset is not null
            ...
            */
            ImagePreset preset = MasterImage.getImage().getSavedPreset();
            MasterImage.getImage().setSavedPreset(null);
            // @}
            mNeedsAlive = false;
            mSaving = true;
    ...
    public void updateProgress(int max, int current) {
        ...
        mNotifyMgr.notify(mNotificationId, mBuilder.build());
        /*
         * SPRD: Modify 20150424 by Spreadst for bug429996, do not finishActivity
         * here it will cause exit after export @{
         */
        // /* SPRD: fix bug 388295,edit on the original picture @{ */
        // if (current == SaveImage.MAX_PROCESSING_STEPS && mFiltershowActivity != null) {
        //     mFiltershowActivity.finishActivity();
        // }
        /* @} */
    }
