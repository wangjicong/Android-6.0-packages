...
public class FilterEnvironment {
    ...
    public Bitmap applyRepresentation(FilterRepresentation representation, Bitmap bitmap) {
        ...
        filter.setEnvironment(this);
        Bitmap ret = bitmap;
        // SPRD: Modify 2016011 for bug522566, catch exception except RSRuntimeException here to avoid crash @{
        try {
            ret = filter.apply(bitmap, mScaleFactor, mQuality);
        } catch (Exception e) {
            Log.e(LOGTAG, "applyRepresentation error:");
            e.printStackTrace();
        }
        // @}
        if (bitmap != ret) {
