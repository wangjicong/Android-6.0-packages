...
public class MenuExecutor {
    ...
    public void onMenuClicked(int action, ProgressListener listener,
        ...
        startAction(action, title, message, listener, waitOnStop, showDialog);
    }
    ...
    public void startAction(int action, int title,int message, ProgressListener listener,
            boolean waitOnStop, boolean showDialog) {
        ...
        if (showDialog && ids != null) {
            ...
            mDialog = createProgressDialog(activity, title, message, ids.size(), !mSelectionManager.getIsAlbumSet());
            mDialog.show();
        } else {
            mDialog = null;
        }
    ...
    public static String getMimeType(int type) {
        ...
        /* SPRD: add to support play gif @{ */
            ...
            case MediaObject.MEDIA_TYPE_IMAGE:
                // SPRD: fix bug 384777 Support for multiple types of pictures while sharing
            case MediaObject.MEDIA_TYPE_GIF | MediaObject.MEDIA_TYPE_IMAGE:
            case MediaObject.MEDIA_TYPE_GIF | MediaObject.MEDIA_TYPE_IMAGE_WBMP:
            case MediaObject.MEDIA_TYPE_IMAGE | MediaObject.MEDIA_TYPE_IMAGE_WBMP:
            case MediaObject.MEDIA_TYPE_GIF | MediaObject.MEDIA_TYPE_IMAGE_WBMP
    ...
    private class MediaOperation implements Job<Void> {
        ...
        public Void run(JobContext jc) {
            ...
            boolean isDeleteOperation = (R.id.action_delete == mOperation) ? true : false;
            try {
                onProgressStart(mListener);
                if (R.id.action_delete != mOperation || mSelectionManager.getIsAlbumSet()
