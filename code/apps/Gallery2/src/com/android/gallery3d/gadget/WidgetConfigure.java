...
public class WidgetConfigure extends Activity {
    ...
    protected void onCreate(Bundle savedState) {
        ...
        /* SPRD: bug 511928 Gallery permissions error when use gallery widget @{ */
        checkPermissions();
        if (!mHasCriticalPermissions) {
            Toast.makeText(this, R.string.gallery_premission_error, Toast.LENGTH_LONG)
                    .show();
            finish();
            return;
        }
    ...
    // SPRD: fix bug 381847,override wrong method
    ...
    private void updateWidgetAndFinish(WidgetDatabaseHelper.Entry entry) {
        ...
        checkPermissions();
        if(!mHasCriticalPermissions){
            // Gallery permissions error , return.
            return;
        }
        /* @} */
    ...
    private void setPhotoWidget(Intent data) {
        ...
        try {
            ...
            // SPRD: fix bug 381847,crash when select "activities are not preserved"
            if (helper.getEntry(mAppWidgetId) != null)
    ...
    private void setChoosenAlbum(Intent data) {
        ...
        try {
            ...
            if (mediaSet instanceof LocalAlbum) {
                /* SPRD: Fix Bug 336435, try to scan the whole sdcard to get relative path @{ */
    ...
    /* SPRD: bug 511928 , add check gallery permissions @{ */
