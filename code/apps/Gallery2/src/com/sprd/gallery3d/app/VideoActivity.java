...
public final class VideoActivity extends AbstractGalleryActivity implements OnCancelListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!GalleryUtils.checkStoragePermissions(this) ||!GalleryUtils.checkReadPhonePermissions(this)) {
            ...
            /**SPRD:Bug520939 can't add video attachment in the email after check storage permission  @{*/
            if (Intent.isAccessUriMode(getIntent().getFlags())) {
                startIntent.setFlags(getIntent().getFlags());
            }
            if (getIntent().getAction() != null) {
                startIntent.setAction(getIntent().getAction());
            }
            if (getIntent().getType() != null) {
                startIntent.setType(getIntent().getType());
            }
            ...
            /**@}*/
            startIntent.putExtra(PermissionsActivity.UI_START_BY, PermissionsActivity.START_FROM_VIDEO);
    ...
    public void startDefaultPage() {
        ...
        data.putString(AlbumSetPage.KEY_MEDIA_PATH, getDataManager().getTopSetPath(DataManager.INCLUDE_VIDEO));
