...
public class AutoDumpModemAssertReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (SystemProperties.getBoolean("persist.sys.modemassertdump", false)) {
            if (!checkSDCard()) {
                saveToInternal(MODEM_ASSERT_PATH, MODEM_ASSERT_NAME);
            } else {
    ...
    public void saveToInternal(String preferenceName, String name) {
        Log.d(TAG, "auto dump modem assert to Internal storage");

        if (getInternalFreeSize() <= 2) {
            Log.d(TAG, "Internal storage size is too small");
            Toast.makeText(mContext, R.string.no_space_internal,
                    Toast.LENGTH_SHORT).show();
            return;
        }

        File powerInfo = new File(Environment.getDataDirectory()
                + preferenceName);
        Log.d(TAG,"powerInfo is "+powerInfo);
        File powerDir = new File(Environment.getInternalStoragePath()
        ...
        try {
            ...
            Log.d(TAG, "sucess dump file to Internal storage");
        } catch (IOException e) {
    ...
    public void saveToSd(String preferenceName, String name) {
        ...
        File powerInfo = new File(Environment.getDataDirectory() + preferenceName);
