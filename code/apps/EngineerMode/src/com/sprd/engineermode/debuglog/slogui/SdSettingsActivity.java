                            ...
                            public void onClick(DialogInterface dialog,
                                ...
                                if (inputServer.getText().toString() != null
                                        && inputServer.getText().toString()
                                                .length() != 0) {
                                    value = Integer.valueOf(inputServer
                                            .getText().toString());
                                    if (value > SET_SD_FREESPACE || value < mSigleLogSize) {
                                        Toast.makeText(
                                                SdSettingsActivity.this,
                                                "Please input between " + mSigleLogSize + "~"
                                                        + SET_SD_FREESPACE,
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case SET_LOG_SIZE:
                    ...
                    int SdValue = 0;
                    int DataValue = 0;
                    if (mTotalSdLogSize == 0 || mTotalSdLogSize > 1024)
                    {
                        SdValue = 1024;
                    } else {
                        SdValue = mTotalSdLogSize;
                    }
                    if (mTotalDataLogSize > 1024) {
                        DataValue = 1024;
                    } else {
                        DataValue = mTotalDataLogSize;
                    }
                    int comMax = (SdValue < DataValue) ? SdValue : DataValue;
                    if (comMax < 1)
                        comMax = 1;
                    final int inputMax = comMax;
                    if ("".equals(setLogSize) || Integer.parseInt(setLogSize) < SET_MIN_LOGSIZE
                            || Integer.parseInt(setLogSize) > inputMax) {
                        mUiThread.post(new Runnable() {
                            @Override
                            public void run() {
                                Toast.makeText(
                                        SdSettingsActivity.this,
                                        "Input is wrong, input must be between" + SET_MIN_LOGSIZE
                                                + "~" + inputMax, Toast.LENGTH_SHORT).show();
                            }
                        });
                ...
                case SET_DATA_SIZE:
                    ...
                    if ("".equals(setDataMaxSize)
                            || Integer.parseInt(setDataMaxSize) < mSigleLogSize
                            || Integer.parseInt(setDataMaxSize) > SET_MAX_DATA_LOGSIZE) {
                        mUiThread.post(new Runnable() {
                            @Override
                            public void run() {
                                Toast.makeText(
                                        SdSettingsActivity.this,
                                        "Input is wrong, input must be " + mSigleLogSize + "~"
                                                + SET_MAX_DATA_LOGSIZE, Toast.LENGTH_SHORT).show();
                            }
                        });
                    } else {
