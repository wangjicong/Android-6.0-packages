...
public class SendPowerPrefActivity extends Activity {
    ...
    private boolean isSupportTD = SystemProperties.get(
            "persist.radio.ssda.mode").equals("tdd-csfb")
            || (SystemProperties.get("persist.radio.ssda.mode").equals(
                    "csfb") & TelephonyManager.getRadioFeatures(0) == RadioFeatures.TD_LTE_AND_TD_AND_GSM_CSFB);
    private boolean isSupportWCD = SystemProperties.get(
            "persist.radio.ssda.mode").equals("fdd-csfb")
            || SystemProperties.get("persist.radio.ssda.mode").equals(
                    "csfb");

    private final static String ORI_WCDMA_BAND = "ORI_WCDMA_BAND";
    private static final int[] mWBandMap = {1, 2, 5, 8};
    private static final int[] mWBandET = { R.id.et_wband1, R.id.et_wband2,
            R.id.et_wband5, R.id.et_wband8 };
    private static final int[] mWBandOK = { R.id.ok_wband1, R.id.ok_wband2,
        R.id.ok_wband5, R.id.ok_wband8 };
    private static final int[] mWBandClear = { R.id.clear_wband1, R.id.clear_wband2,
        R.id.clear_wband5, R.id.clear_wband8 };
    private static final int[] mWBandText = { R.id.text_wband1, R.id.text_wband2,
        ...
        buttonOk.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                if(mGsm850.getText().toString().equals("")){
                    Toast.makeText(SendPowerPrefActivity.this, "please input data", Toast.LENGTH_SHORT)
                    .show();
                }else{
        ...
        if (isSupportWCD) {
            ...
            if (mWCDMASupportBand == -1) {
                for (int i = 0; i < mWBandMap.length; i++) {
                    String resp = IATUtils.sendATCmd(engconstents.ENG_AT_W_LOCKED_BAND+mWBandMap[i], "atchannel0");
