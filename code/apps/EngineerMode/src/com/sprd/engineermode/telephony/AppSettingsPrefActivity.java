...
public class AppSettingsPrefActivity extends PreferenceActivity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mUAsetting.setOnPreferenceClickListener(new OnPreferenceClickListener() {
            ...
            public boolean onPreferenceClick(Preference preference) {
                ...
                if (mUserId != 0) {
                    Toast.makeText(AppSettingsPrefActivity.this, R.string.not_support_visitor_or_user_mode, Toast.LENGTH_SHORT).show();
                    return false;
                }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent intent) {
        ...
        Log.d(TAG,"resultCode: "+ resultCode + "requestCode: "+requestCode +"RESULT_OK:" +RESULT_OK);
        if (resultCode == RESULT_OK) {
            switch (requestCode) {
                case REQUEST_UA_SETTINGS:
                    if (intent == null) return;
                    String customUaString = intent.getStringExtra(Settings.System.CUSTOM_USER_AGENT_STRING);
                    if (!TextUtils.isEmpty(customUaString)) {
                        Settings.System.putString(getContentResolver(),
                                Settings.System.CUSTOM_USER_AGENT_STRING, customUaString);
                    }
                    int uaChoice = intent.getIntExtra(Settings.System.USER_AGENT_CHOICE, -1);
                    if (uaChoice != -1) {
                        Settings.System.putInt(getContentResolver(),
                                Settings.System.USER_AGENT_CHOICE, uaChoice);
                    }
                    String otherUaString = intent.getStringExtra(Settings.System.OTHER_USER_AGENT_STRING);
                    if (!TextUtils.isEmpty(otherUaString)) {
                        Settings.System.putString(getContentResolver(),
                                Settings.System.OTHER_USER_AGENT_STRING, otherUaString);
                    }
                break;
            }
        } else {
