                ...
                if (isThreadFinished) {
                    ...
                    new Thread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            try {
                                Cursor mCursor = getActivity().getContentResolver().query(conv.messageListUri, UIProvider.MESSAGE_PROJECTION, null, null, null);
                                MessageCursor messageCursor = new MessageCursor(new ObjectCursor<ConversationMessage>(mCursor, ConversationMessage.FACTORY));
                                messageCursor.moveToFirst();
                                ConversationMessage message = messageCursor.getMessage();
                                ComposeActivity.editDraft(getActivity(), mAccount, message);
                            } catch (CursorIndexOutOfBoundsException e) {
                                LogUtils.e(LOG_TAG,"appear CursorIndexOutOfBoundsException when open draft" + e.toString());
                            }
