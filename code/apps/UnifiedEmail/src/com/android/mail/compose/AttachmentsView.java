...
public class AttachmentsView extends LinearLayout {
    ...
    public Attachment generateLocalAttachment(Uri contentUri) throws AttachmentFailureException {
            ...
            if (metadataCursor != null) {
                try {
                    if (metadataCursor.moveToNext()) {
                        ...
                        /* SPRD: Modify for bug498033 {@ */
                        long lSize = metadataCursor.getLong(1);
                        attachment.size = longConvertToInt(lSize);
                        /* @} */
                    }
                } finally {
                    metadataCursor.close();
                }
            /* SPRD: Modify for bug534179 {@ */
            } else if (contentUri != null && contentUri.toString().startsWith("content://mms/part/")) {
                try {
        ...
        if (DrmXposedUtils.getInstance().isDrmFile(attachment.getName(), contentType)
    ...
    /* SPRD: Modify for bug498033 {@ */
    private static int longConvertToInt(long lSize){
        int size;
