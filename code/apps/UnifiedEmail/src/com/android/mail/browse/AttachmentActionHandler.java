...
public class AttachmentActionHandler {
    ...
    public void showDownloadConfirmDialog(int dialogType, DownloadConfirmInterface cb, int destination) {
        final DownloadConfirmDialogFragment frag = DownloadConfirmDialogFragment.newInstance(mAttachment, dialogType, destination);
        frag.setCallback(cb);
        ...
        if (dialog != null && dialog.isShowingDialogForAttachment(mAttachment)) {
            ...
            if (loaderResult && mAttachment.isDownloadFinishedOrFailed()) {
                mHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        /* SPRD: Modify for bug497730,goto DeskClock from statusbar while downloading attachment in Email,sometimes Email crash.{@ */
                        /* SPRD: Modify for bug521781 */
            ...
            /*
             * Modify by SPRD Bug:499631 Downloading attachment, make
             * the lock screen, and then unlock. the display is extracting,
             * actually successful extracted. 2015.07.03 Start
             */
            if (dialog != null && !dialog.isAttachmentExistInDB()) {
                mHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        if (mContext != null && ((Activity)mContext).isResumed()) {
            ...
            /*
             * Modify by Bug:499631 Downloading attachment, make
             * the lock screen, and then unlock. the display is extracting,
             * actually successful extracted. 2015.07.03 End
             */
            mView.updateProgress(showProgress);
        }
