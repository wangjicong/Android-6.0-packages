...
public class FolderPreferences extends VersionedPrefs {
    ...
    public String getActualNotificationRingtoneUri() {
        ...
        if(ringtoneUri.equals(Settings.System.DEFAULT_NOTIFICATION_URI.toString())){
            if (hasDeleted(getContext(), Uri.parse(ringtoneUri))) {
                return Settings.System.getString(getContext().getContentResolver(), Settings.System.DEFAULT_NOTIFICATION);
            }
        }
        return ringtoneUri;
    }
    ...
    public static boolean hasDeleted(Context context, Uri uri) {
        MediaPlayer mLocalPlayer;
        if (uri == null) {
            return true;
        }
        mLocalPlayer = new MediaPlayer();
        try {
            mLocalPlayer.setDataSource(context, uri);
