...
public class InCallVideoCallCallbackNotifier {
    ...
    public void reponseAudioToVideo(Call call) {
        for (SessionModificationListener listener : mSessionModificationListeners) {
            listener.onReponseAudioToVideo(call);
        }
    }
    /* @} */
     ...
     * Listener interface for any class that wants to be notified of Camera events.
     */
    private final Set<CameraEventListener> mCameraEventListeners = Collections.newSetFromMap(
            new ConcurrentHashMap<CameraEventListener, Boolean>(8, 0.9f, 1));
    ...
    public void removeCameraEventListener(CameraEventListener listener) {
           if (listener != null) {
               mCameraEventListeners.remove(listener);
            }
    }
    public void cameraSwitchStateChanged(boolean isSwithing) {
           for (CameraEventListener listener : mCameraEventListeners) {
               listener.onCameraSwitchStateChanged(isSwithing);
            }
    }
    /** SPRD: VIDEO CALL FEATURE. */
