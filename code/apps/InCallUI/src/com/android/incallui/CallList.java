...
public class CallList {
    ...
    public void removeCallUpdateListener(String callId, CallUpdateListener listener) {
        ...
        if (listeners != null) {
            ...
            /* SPRD: Fix OOM issue, see bug#543883. @{ */
            if (listeners.isEmpty()) {
                mCallUpdateListenerMap.remove(callId);
            }
            /* @} */
        }
    }
    ...
    public Call getFirstCall() {
        ...
        // SPRD BUG492882 resume InCallActivity while call state is on hold,return call is null
        if ( result == null ) {
    ...
    /* SPRD: Add for Volte @{ */
    public int getforgroundCalNumber(){
        int number = 0;
        for (Call call : mCallById.values()) {
            if (isforgroundCall(call)) {
                number++;
            }
        }
        return number;
    ...
    public boolean hasValidGroupCall() {
        Call multiCall = null;
        for (Call call : mCallById.values()) {
            if(call.getChildCallIds().size() > 0 && call.getState() != Call.State.INVALID
                    &call.getState() != Call.State.IDLE
                    && call.getState() != Call.State.DISCONNECTED){
                multiCall = call;
                return true;
            }
    ...
    public boolean hasIncomingVideoCall() {
        for (Call call : mCallById.values()) {
    ...
    public Call getActiveConferenceCallWithState() {
        Call retval = null;
        int position = 0;
        for (Call call : mCallById.values()) {
            if (call.getGroupState() == Call.State.ACTIVE) {
                if (position >= 0) {
                    retval = call;
                    break;
                } else {
