...
public class Call {
    ...
    public long getConnectRealTimeMillis() {
        return mTelecommCall.getDetails().getConnectRealTimeMillis();
    }
    /* @} */
    /*SPRD: add for VoLTE{@*/
    ...
    /* @} */
    /*SPRD: add for bug532301 bug535400{@*/
    private void updateWithMessage() {
        if(isConferenceCall()||isGenericConferenceCall()){
            if(mHandler.hasMessages(EVENT_UPDATE)){
                mHandler.removeMessages(EVENT_UPDATE);
            } else {
                update();
            }
            mHandler.sendEmptyMessageDelayed(EVENT_UPDATE, 100);
        } else {
            update();
        }
    }
