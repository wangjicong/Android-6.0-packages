...
public class VideoCallPresenter extends Presenter<VideoCallPresenter.VideoCallUi> implements
    ...
    private Runnable mAutoFullscreenRunnable =  new Runnable() {
        ...
        public void run() {
            if (mAutoFullScreenPending) {
                ...
                /*SPRD: Modify for bug#555024 @{ */
                if (InCallPresenter.getInstance().getActivity() != null) {
                    mIsDialpadShown = InCallPresenter.getInstance().getActivity().isDialpadVisible();
                }
                if (!mIsDialpadShown) {
    ...
    private Call mBackgroundCall;//SPRD:Add for bug531347

    private boolean mIsDialpadShown = false;//SPRD:Add for bug555024
    /**
    ...
    public void onUiReady(VideoCallUi ui) {
        ...
        /* SPRD: modify for bug 497250 @ { */
        onStateChange(InCallState.NO_CALLS, InCallPresenter.getInstance().getInCallState(),
    ...
    public void onSurfaceClick(int surfaceId) {
        /* SPRD: modify click on the screen is invalid for bug 539168 and bug555024@{ */
        if (InCallPresenter.getInstance().getActivity() != null) {
            mIsDialpadShown = InCallPresenter.getInstance().getActivity().isDialpadVisible();
        }
        if ((mPrimaryCall != null && (!mPrimaryCall.isVideoCall(mContext))) || mIsDialpadShown) {
            return;
        }
        /* @} */
    ...
    public void onStateChange(InCallPresenter.InCallState oldState,
        ...
        Call backGroundCall = callList.getBackgroundCall();//SPRD:Add for bug531347
        ...
        Log.d(this, "onStateChange mBackgroundCall = " + mBackgroundCall);
        Log.d(this, "onStateChange backGroundCall = " + backGroundCall);
        /* SPRD:Add for bug531347 and bug541206@{ */
        if (backGroundCall != null && mBackgroundCall != backGroundCall) {
            mBackgroundCall = backGroundCall;
        }
        if ((!primaryChanged || (primary != null && mPrimaryCall == null)) && backGroundCall == null
                && mBackgroundCall != backGroundCall) {
            if (primary != null) {
                if (primary.isVideoCall(mContext)) {
                    Log.d(this, "onStateChange updatePreviewPos");
                    getUi().updatePreviewPos();
                }
            }
        }
        /* @ } */
...
//            cleanupSurfaces();
    ...
    private void enterVideoMode(Call call) {
        ...
        /* SPRD: SPRD:set mEnterVideoModeFlag is true for fix bug 493469 @{ */

        ui.setmEnterVideoModeFlag(true);
        /* @} */
    ...
    private void updateAudioMode(boolean enableSpeaker) {
        ...
        if (!isAudioRouteEnabled(currentAudioMode,
            ...
            || ((currentAudioMode == AudioModeProvider.AUDIO_MODE_INVALID
               || currentAudioMode == CallAudioState.ROUTE_EARPIECE) && enableSpeaker)) {
        /* @}*/
            sPrevVideoAudioMode = currentAudioMode;

            Log.d(this, "Routing audio to speaker");
            /* SPRD: SPRD:judge whether or not EnterVideoMode for fix bug 493469 @{ */
            if (getUi().ismEnterVideoModeFlag()) {
                telecomAdapter.setAudioRoute(sPrevVideoAudioMode);
            } else {
    ...
    private void exitVideoMode() {
        ...
        /* SPRD: SPRD:set mEnterVideoModeFlag is false for fix bug 493469 @{ */
        VideoCallUi ui = getUi();
        ui.setmEnterVideoModeFlag(false);
        /* @} */
        showVideoUi(VideoProfile.STATE_AUDIO_ONLY, Call.State.ACTIVE);
        enableCamera(mVideoCall, false);
        InCallPresenter.getInstance().setFullScreen(false);
        /* SPRD: add for bug494455 @{ */
        updateAudioMode(false);
        /* @} */
    ...
    private void showVideoUi(int videoState, int callState) {
        ...
        if (VideoProfile.isBidirectional(videoState)) {
            ui.showVideoViews(true, !isPaused /*&& isCallActive*/);// SPRD: fix bug 531194
            ...
            ui.showVideoViews(false, !isPaused /*&& isCallActive*/);// SPRD: fix bug 531194
    ...
    private void setPreviewSize(int orientation, float aspectRatio) {
        ...
        if (mContext != null
                && mContext.checkSelfPermission(READ_PHONE_STATE) == PackageManager.PERMISSION_GRANTED
    ...
    public interface VideoCallUi extends Ui {
        ...
        /* SPRD: SPRD:set the interface for fix bug 493469 @{ */
        boolean ismEnterVideoModeFlag();

        void setmEnterVideoModeFlag(boolean mIsEnterVideoMode);
        /* @} */
        void updatePreviewPos();//SPRD:Add for bug531347
    }
}
