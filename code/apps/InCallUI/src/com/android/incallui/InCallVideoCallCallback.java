...
public class InCallVideoCallCallback extends VideoCall.Callback {
    ...
    public void onSessionModifyResponseReceived(int status, VideoProfile requestedProfile,
        ...
        if (status == VideoProvider.SESSION_MODIFY_REQUEST_INVALID){
            InCallVideoCallCallbackNotifier.getInstance().reponseAudioToVideo(mCall);
        /* @} */
        } else if (status != VideoProvider.SESSION_MODIFY_REQUEST_SUCCESS) {
    ...
    public void onCameraCapabilitiesChanged(CameraCapabilities cameraCapabilities) {
        ...
        if (cameraCapabilities.getWidth() == 0
                && cameraCapabilities.getHeight() == 0) {
                InCallVideoCallCallbackNotifier.getInstance()
                    .cameraSwitchStateChanged(cameraCapabilities.isCameraSwitching());
            return;
        }
        /** @} */
