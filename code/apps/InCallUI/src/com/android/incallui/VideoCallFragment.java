...
public class VideoCallFragment extends BaseFragment<VideoCallPresenter,
    ...
    // SPRD: fix bug 495983
    private double sVideoRatio = 11.0/9.0;
    private boolean mIsSetTranslation = false;

    /* SPRD: SPRD:set the flag of EnterVideoMode for fix bug 493469 @{ */
    private boolean mEnterVideoModeFlag;
    private static final String SAVE_ENTERVIDEOMODE_FLAG = "isEnterVideoMode";
    /* @} */

    /* SPRD: fix bug 498528 @{ */
    private SharedPreferences mPrefs;
    private final String ENTER_VIDEOMODE_NAME = "enter_videomode_name";
    private final String KEY_ENTER_VIDEOMODE = "key_enter_videomode";
    /* @} */
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        /* SPRD: fix bug 498528 @{ */
        if(getActivity() != null){
            mPrefs = getActivity().getSharedPreferences(ENTER_VIDEOMODE_NAME, getActivity().MODE_PRIVATE);
            mEnterVideoModeFlag = mPrefs.getBoolean(KEY_ENTER_VIDEOMODE, false);
            Log.d(this, "onCreate: mEnterVideoModeFlag = " + mEnterVideoModeFlag);
            /* SPRD: fix bug 532464 @{ */
            if (getActivity().checkSelfPermission(READ_PHONE_STATE) == PackageManager.PERMISSION_GRANTED) {
    ...
    public void onResume(){
        super.onResume();
        updateUiWhenCallCardLayoutFinish();
    }
    /* @} */
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        final View view;
        /* SPRD: add for bug524634 and bug532464 @{ */
        if (getActivity() != null
                && getActivity().checkSelfPermission(READ_PHONE_STATE) == PackageManager.PERMISSION_GRANTED
                && ImsManager.isVolteEnabledByPlatform(getActivity())) {
            view = inflater.inflate(R.layout.video_call_fragment_volte,
                    container, false);
        } else {
            view = inflater.inflate(R.layout.video_call_fragment, container,
                    false);
        }
        /* @} */
        return view;
    ...
    public void onViewCreated(View view, Bundle savedInstanceState) {
        ...
        if (getActivity() != null
                && getActivity().checkSelfPermission(READ_PHONE_STATE) == PackageManager.PERMISSION_GRANTED
                && ImsManager.isVolteEnabledByPlatform(getActivity())) {
            mVideoViewsStub = (ViewStub) view
                    .findViewById(R.id.videoCallViewsStubVolte);
        } else {/* @} */
            mVideoViewsStub = (ViewStub) view
                    .findViewById(R.id.videoCallViewsStub);
        }
        ...
        // SPRD:fix for bug523253
        if (sVideoSurfacesInUse && mEnterVideoModeFlag) {
    ...
    public void setPreviewSize(int width, int height) {
        ...
        if (sPreviewSurface != null) {
            ...
            // the height.
            //qiuyaobo, for SPCSS00373712, 20170421, begin
            //SPRD: bug618764 disable mirror
            //Matrix transform = new Matrix();
            //transform.setScale(-1, 1, width/2, 0);
            //preview.setTransform(transform);
            //qiuyaobo, for SPCSS00373712, 20170421, end
        }
    }
        ...
        if (mVideoViews != null) {
            ...
            observer.addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
                ...
                public void onGlobalLayout() {
                    ...
                    float displayVideoTranslationY = InCallPresenter.getInstance().getSecondaryCallInfo();
                    Log.d(this,"onGlobalLayout->displayVideoTranslationY: " + displayVideoTranslationY);
                    if (displayVideoTranslationY > 0
                            && CallList.getInstance().getBackgroundCall() != null) {
                        adjustDisplaySurface();
                    }
                    ...
                    if (observer.isAlive() && (CallList.getInstance().getBackgroundCall() == null || displayVideoTranslationY > 0 && CallList.getInstance().getBackgroundCall() != null)) {
    ...
    /* SPRD: fix bug 495983 @{ */
    public void adjustDisplaySurface() {
        if (mVideoViews == null) {
            Log.w(this, "adjustDisplaySurface->mVideoViews is null!");
            return;
        }
        TextureView displayView = (TextureView) mVideoViews
                .findViewById(R.id.incomingVideo);
        View previewView = mVideoViews
                .findViewById(R.id.previewVideoContainer);
        if (displayView == null) {
            Log.w(this, "adjustDisplaySurface->displayView is null!");
            return;
        }
        ...
        /* SPRD:bug499792 @{ */
        float spaceBesideCallCard = 0;
        if (InCallPresenter.getInstance().getCallCardFragmentVisible()) {
            spaceBesideCallCard = InCallPresenter.getInstance().getSpaceBesideCallCard();
        }/* @} */

        if (mIsLandscape) {
            if (spaceBesideCallCard != 0) {
                params.width = (int) spaceBesideCallCard;
                params.height = (int) ((double) params.width / sVideoRatio);
                if (params.height > size.y) {
                    params.height = size.y;
                    params.width = (int) ((double) size.y * sVideoRatio);
                }
            } else {
                params.width = (int) ((double) size.y * sVideoRatio);
                params.height = size.y;
                if (params.width > size.x) {
                    params.width = size.x;
                    params.height = (int) ((double) size.x / sVideoRatio);
                }
            }
        } else {
            params.width = size.x;
        ...
        float displayVideoTranslationY = 0;
        if (!mIsLandscape && spaceBesideCallCard > 0) {
            displayVideoTranslationY = spaceBesideCallCard - params.height;//size.y/2 + params.height/2 - spaceBesideCallCard;
            if(displayVideoTranslationY > 0 && !mIsSetTranslation){
                displayView.setTranslationY(-displayVideoTranslationY);
                if (previewView != null) {
                    previewView.setTranslationY(-displayVideoTranslationY);
                } else {
                    Log.w(this, "adjustDisplaySurface->previewView is null!");
                }
                mIsSetTranslation = true;
            }
        }
        /* SPRD:Add for bug531347@{ */
        if (!mIsLandscape && spaceBesideCallCard > 0) {
            boolean mIsVideoMode;
            mIsVideoMode = getPresenter().isVideoMode();
            Log.d(this, "adjustDisplaySurface->mIsVideoMode:" + mIsVideoMode
                    + " BackgroundCall:" + CallList.getInstance().getBackgroundCall()
                    + " getUi():" + getUi());
            if (mIsVideoMode
                    && CallList.getInstance().getBackgroundCall() != null
                    && getUi() != null) {
                displayVideoTranslationY = InCallPresenter.getInstance().getSecondaryCallInfo();
                /* SPRD:Add for bug545709@{ */
                if (previewView != null) {
                    previewView.setTranslationY(-displayVideoTranslationY);
                } else {
                    Log.w(this, "adjustDisplaySurface->previewView is null!");
        ...
        /* @ } */
        Log.i(this, "adjustDisplaySurface->height:" + params.height + " width:"
                + params.width + " displayVideoTranslationY:"
                + displayVideoTranslationY + " spaceBesideCallCard:"
                + spaceBesideCallCard);
    }

    private void updateUiWhenCallCardLayoutFinish(){
        final View view = InCallPresenter.getInstance().getCallCardView();
        Log.i(this, "updateUiWhenCallCardLayoutFinish->view:"+view);
        if(view == null){
            return;
        }
        ...
        observer.addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener(){
            ...
            public void onGlobalLayout() {
                ...
                Log.i(this, "addOnGlobalLayoutListener->spaceBesideCallCard:"
                      + spaceBesideCallCard);
                if (spaceBesideCallCard == 0) {
                    return;
                }
                TextureView displayVideo = (TextureView) view
                        .findViewById(R.id.incomingVideo);
                TextureView previewVideo = (TextureView) view
                        .findViewById(R.id.previewVideo);
                Log.i(this, "updateUiWhenCallCardLayoutFinish->displayVideo:"
                        + displayVideo + " previewVideo:" + previewVideo);
                adjustDisplaySurface();
                // Remove the listener so we don't continually re-layout.
                ViewTreeObserver observer = view.getViewTreeObserver();
                if (observer.isAlive() && spaceBesideCallCard > 0) {
                    observer.removeOnGlobalLayoutListener(this);
                }
            }
        });
    ...
    /* @} */
    /* SPRD: SPRD:set the flag of EnterVideoMode for fix bug 493469 @{ */
    public boolean ismEnterVideoModeFlag() {
        return mEnterVideoModeFlag;
    }

    public void setmEnterVideoModeFlag(boolean mEnterVideoModeFlag) {
        this.mEnterVideoModeFlag = mEnterVideoModeFlag;
    }
    /* @} */
    /* SPRD: fix bug 498528 @{ */
    @Override
    public void onDestroy() {
        super.onDestroy();
        Log.d(this, "onDestroy: mEnterVideoModeFlag = " + mEnterVideoModeFlag);
        if( mPrefs!= null){
            SharedPreferences.Editor editor = mPrefs.edit();
            editor.putBoolean(KEY_ENTER_VIDEOMODE, mEnterVideoModeFlag);
            editor.apply();
        }
    }
    /* @} */
    /* SPRD: Add for bug531347 @{ */
    @Override
    public void updatePreviewPos() {
            if (mPreviewVideoContainer != null) {
                Log.d(this, "updatePreviewPos->setTranslationY 0,0f");
                mPreviewVideoContainer.setTranslationY(0.0f);
            }
    }
    /* @} */
}
