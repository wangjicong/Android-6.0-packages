...
public class CallerInfoUtils {
    ...
    private static LruCache<String, Boolean> mCache = new LruCache(10); //SPRD: add for bug532829
    ...
    public static boolean isVoiceMailNumber(Context context, Call call) {
        /** SPRD: add for bug534732 {@ */
        if (call.getNumber() == null) {
            return false;
        }
        /** @} */
        /** SPRD: add for bug532829 {@ */
        Boolean isVoicemailNumber = null;
        PhoneAccountHandle accountHandle = call.getTelecommCall().getDetails().getAccountHandle();
        String key = accountHandle != null ? accountHandle.getId() + call.getNumber().toString() : call.getNumber().toString();
        Log.d(TAG, "mCache size = " + mCache.size() + mCache.toString());
        if (mCache.size() > 0) isVoicemailNumber = mCache.get(key);
        if (isVoicemailNumber != null) {
            return isVoicemailNumber;
        } else {
            final TelecomManager telecomManager =
                    (TelecomManager) context.getSystemService(Context.TELECOM_SERVICE);
            isVoicemailNumber = telecomManager.isVoiceMailNumber(accountHandle, call.getNumber().toString());
            synchronized(CallerInfoUtils.class) {
                mCache.put(key, isVoicemailNumber);
            }
            return isVoicemailNumber;
        }
/*         TelecomManager telecomManager =
    ...
    /** SPRD: add for bug532829 {@ */
    public static void refreshVoiceMailCache() {
        Log.d(TAG, "refreshVoiceMailCache");
        synchronized(CallerInfoUtils.class) {
            mCache.evictAll();
        }
    }
