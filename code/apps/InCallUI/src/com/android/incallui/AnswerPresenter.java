...
public class AnswerPresenter extends Presenter<AnswerPresenter.AnswerUi>
    ...
    public void onUiShowing(boolean showing) {
        ...
        if (showing) {
            ...
            if (mCallId != null && calls.getIncomingCall() == null /*SPRD: add for bug507995 */) {
    ...
    public void onAnswer(int mpcMode, int videoState, Context context) {
        ...
        if (mCall.getSessionModificationState()
            ...
            TelecomAdapter.getInstance().answerCall(mCall.getId(), videoState, mpcMode);
        }
    }
    ...
    private void configureAnswerTargetsForSms(Call call, List<String> textMsgs) {
        ...
        if (VideoProfile.isBidirectional((call.getVideoState())) && um.isSystemUser()) {
            if (withSms) {
...
//------------------------------------- SPRD --------------------------------
     ...
     * Update incoming call UI when supp service failed
     */
    public void onSuppServiceFailed() {
        ...
        if (mCallId != null && mCallId.equals(call.getId())) {
            callList.removeCallUpdateListener(mCallId, this);
            processIncomingCall(call);
        }
    }
    /** @} */
