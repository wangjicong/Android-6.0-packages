...
public class CallCardFragment extends BaseFragment<CallCardPresenter, CallCardPresenter.CallCardUi>
    ...
    public  String  pcallname;
    public  String  poperatorname;
    public  String  pphonenumber;
    public String  pelapsedtime;
    public String  pdurationDescription;
...
//start - CallerIdSearch
            if(com.android.internal.telephony.CallerIDGeocodeHelper.isSupported()){
...
//end - CallerIdSearch
    ...
    public void setPrimaryLabel(String label) {
        if (!TextUtils.isEmpty(label)) {
            ...
            poperatorname= mNumberLabel.getText().toString();//qiuyaobo,20160716,SUNVOV_USE_HALL_LEATHER_FUNCTION
        } else {
...
//start - CallerIdSearch
        //Log.i(this, "@@setPrimary, nameIsNumber:"+nameIsNumber +",number:"+number +",name:"+name);
        if (com.android.internal.telephony.CallerIDGeocodeHelper.isSupported() && nameIsNumber) {
            Context c0 = getView().getContext();
            String location0 = com.android.internal.telephony.CallerIDGeocodeHelper.getGeoDescription(c0, name);
            if(!TextUtils.isEmpty(location0)){
                mCallNumberAndLabel.setVisibility(View.VISIBLE);
                setPrimaryLabel("");
                setPrimaryPhoneNumber(location0);
                mPhoneNumber.setEllipsize(TextUtils.TruncateAt.valueOf("END"));
            }else{
                mCallNumberAndLabel.setVisibility(View.GONE);
            }
        }
...
/* Sunvov:jiazhenl 20150609 add for call auto record start @{ */
		setPrimaryAutoRecordInfo(name, nameIsNumber, number);
/* Sunvov:jiazhenl 20150609 add for call auto record end @} */
        ...
        if (OptConfig.SUNVOV_USE_HALL_LEATHER_FUNCTION){
            ...
            if(lidOpen){
                InCallActivity mactivity = (InCallActivity) getActivity();
                mactivity.updateInCallUI(lidOpen);
            }
        }    
        //qiuyaobo,20160716,end    
    }
    ...
    public void setPrimaryCallElapsedTime(boolean show, long duration) {
        if (show) {
            ...
            //qiuyaobo,20160716,SUNVOV_USE_HALL_LEATHER_FUNCTION,begin        
            pelapsedtime = callTimeElapsed;
            pdurationDescription = durationDescription;	
            //qiuyaobo,20160716,SUNVOV_USE_HALL_LEATHER_FUNCTION,end                     
        } else {
                ...
                if ((isAccount || isWifi || isConference) && hasSuggestedLabel) {
                   ...
                   /* Sunvov:jiazhenl 20150609 add for call auto record start @{ */
                    ...
                    if(phoneId == PrimaryCardId){
                        ((InCallActivity) getActivity()).getRecorderHelper().mCallCardSlot = "SIM1";
                    }else{
                        ((InCallActivity) getActivity()).getRecorderHelper().mCallCardSlot = "SIM2";
                    }
                    /* Sunvov:jiazhenl 20150609 add for call auto record end @} */
    ...
    public String getSlotInfo(TelephonyManager tm, int subId) {
        ...
        /*wangxing 20160810 modify for rename sim1 sim2 start @{ */
	    if(OptConfig.SUNVOV_CUSTOM_C7301_XLL_FWVGA){
	        card_slot = card_slot.replace("Main","Master");
	        card_slot = card_slot.replace("Vice","Slave");
	    }
        /*wangxing 20160810 modify for rename sim1 sim2 end @} */ 
    ...
    /* Sunvov:jiazhenl 20150609 add for call auto record start @{ */
    public void setPrimaryAutoRecordInfo(String name, boolean nameIsNumber, String number) {
    	// Set the name
        if (TextUtils.isEmpty(name)) {
			((InCallActivity) getActivity()).getRecorderHelper().mPhoneName = null;
        } else {
			((InCallActivity) getActivity()).getRecorderHelper().mPhoneName = 
			(nameIsNumber ? PhoneNumberUtils.ttsSpanAsPhoneNumber(name).toString(): name);
        }

        // Set the number
        if (TextUtils.isEmpty(number)) {
			((InCallActivity)getActivity()).getRecorderHelper().mPhoneNumber = null;
        } else {
			((InCallActivity) getActivity()).getRecorderHelper().mPhoneNumber = PhoneNumberUtils.ttsSpanAsPhoneNumber(number).toString();
        }
    }
/* Sunvov:jiazhenl 20150609 add for call auto record end @} */
}
