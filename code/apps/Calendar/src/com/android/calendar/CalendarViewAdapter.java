...
public class CalendarViewAdapter extends BaseAdapter {
        ...
        if (mShowDate) {
            ...
            switch (mCurrentMainView) {
                case ViewType.DAY:
                    ...
                    /* SPRD: Modify for bug473571, add lunar info @{ */
                    if (Utils.mLunarFlag) {
                        lunaryear.setText(updateLunaryear());
                    }
                    /* @} */
                    break;
                case ViewType.WEEK:
    ...
    private String buildDayOfWeek() {
        ...
        */
        /* SPRD: Modify for bug473571, add lunar info @{ */
        if (Utils.mLunarFlag) {
            LunarCalendar lunarInfo = new LunarCalendar(mContext);
            LunarCalendarConvertUtil.parseLunarCalendar(t.year, t.month, t.monthDay, lunarInfo);
            ...
            if (detailInfo != null) {
                dayString.append(detailInfo[1] + detailInfo[2]);// add month
                                                                // +
                                                                // day
                ...
                if (detailInfo[4].length() != 0) {
                    // add festival
                    dayString.append("," + detailInfo[4]);
                }
            ...
            String dayOfWeek = DateUtils.formatDateRange(mContext, mFormatter, mMilliTime,
                    mMilliTime,
                    DateUtils.FORMAT_SHOW_WEEKDAY, mTimeZone).toString();
            dayString.append("  " + dayOfWeek);
    ...
    private String buildMonthYearDate() {
        ...
        mMilliTime = Utils.limitMinDate(mTimeZone, mMilliTime);
    ...
    private String buildWeekDate() {
        ...
        /* SPRD: Modify for bug392734/514346, update week date @{ */
        //long weekEndTime = weekStartTime + DateUtils.WEEK_IN_MILLIS - DateUtils.DAY_IN_MILLIS;
        long weekEndTime = weekStartTime + DateUtils.WEEK_IN_MILLIS - DateUtils.DAY_IN_MILLIS + 1;
        /* @} */

        // If week start and end is in 2 different months, use short months names
        Time t1 = new Time(mTimeZone);

        // SPRD: bug390628/521691/517443, "Week" not show properly
        t1.set(weekEndTime);
    ...
    /* SPRD: Modify for bug473571, add lunar info @{ */
    public String updateLunaryear() {
        Time time = new Time(mTimeZone);
        time.set(mMilliTime);
        time.normalize(true);
        LunarCalendar lunarInfo = new LunarCalendar(mContext);
        // SPRD: add for bug537273, Exceed the limit date, Calendar crash
        if (time.year < 1970 || time.year > 2037) {
            return null;
        }
        LunarCalendarConvertUtil
