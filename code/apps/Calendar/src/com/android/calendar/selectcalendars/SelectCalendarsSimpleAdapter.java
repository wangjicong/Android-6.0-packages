...
public class SelectCalendarsSimpleAdapter extends BaseAdapter implements ListAdapter,
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        String account = mData[position].ownerAccount;
        name = Utils.replaceStrForLocalCalendar(mInflater.getContext(), name);
        account = Utils.replaceStrForLocalCalendar(mInflater.getContext(), account);
        /* @} */
        ...
        if (syncCheckBox != null) {
            ...
            /* SPRD: Add  20151229 of bug 474232 add for local account @{ */
            if (!TextUtils.isEmpty(account)
                    && !account.equals(name)
                    && !account.endsWith("calendar.google.com")) {
                int secondaryColor;
                if (selected) {
