...
private QueryHandler mHandler;
    ...
    private static final String MIME_TYPE_VCAL = "text/x-vcalendar";
    private static final String SHARE_URI = "content://com.android.calendar/events/as_multi_vcal/";
    /* @} */

    //SPRD for bug455353, edit a action, if it has Attendees, toast should be "Updates will be sent".
    private int mAttendeesSize;
    ...
    private class QueryHandler extends AsyncQueryService {
        ...
        protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
            ...
            case TOKEN_QUERY_DUPLICATE_CALENDARS:
                ...
                /* SPRD: Modify for bug448834/474232. Translation for local accounts @{ */
                if (calendarName.equals("Local Calendar")) {
                    calendarName = (String) mActivity.getResources().getString(
                            R.string.local_account);
                }
                /* @}*/
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        // SPRD Modify for bug451704/500118, move this to onResume,in order to make sure photo change,not sync
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        /* SPRD: Modify 20150916 of bug474826 vcalendar issue @{ */
    ...
    private void share() {
        ...
        String uriStr = SHARE_URI + this.mEventId;
        Uri uri = Uri.parse(uriStr);
        Uri osUri = writeToFile(uri);
        Intent intent = new Intent();
        intent.setAction(Intent.ACTION_SEND);
        intent.setType(MIME_TYPE_VCAL);
        intent.putExtra(Intent.EXTRA_STREAM, osUri);
        intent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
        try {
            Intent chooserIntent = Intent.createChooser(intent,
                    getText(R.string.send_calendar));
            startActivity(chooserIntent);
        } catch (android.content.ActivityNotFoundException ex) {
            Toast.makeText(getActivity(),
                    R.string.no_way_to_share_calendar,
                    Toast.LENGTH_SHORT).show();
        }
    }
    /* @} */

    /* SPRD: add  474826 bluetooth can not receive the vcs file in calendar @{ */
    private Uri writeToFile(Uri uri) {
        OutputStream os = null;
        InputStream is = null;
        File folder = new File("/sdcard");
        File file = new File(folder, this.mTitle.getText().toString() + ".vcs");
        Uri osUri = Uri.fromFile(file);

        ContentResolver mContentResolver = mContext.getContentResolver();
        try {
            is = mContentResolver.openInputStream(uri);
            os = mContentResolver.openOutputStream(osUri);
    ...
    public void onStop() {
        ...
        if (!mEventDeletionStarted && act != null && !act.isChangingConfigurations()) {
            ...
            if (saveReminders() || responseSaved || eventColorSaved) {
                /* SPRD for bug455353, edit a action, if it has Attendees, toast should be "Updates will be sent". @{ */
                mAttendeesSize = mAcceptedAttendees.size() + mDeclinedAttendees.size() +
                        mTentativeAttendees.size() + mNoResponseAttendees.size();
                if(mAttendeesSize > 0){
                    Toast.makeText(getActivity(), R.string.saving_event_with_guest, Toast.LENGTH_SHORT).show();
                }else{
    ...
    private void updateEvent(View view) {
        ...
        if (eventName != null) {
            ...
            //Add for bug524609, identify information content for before and after the url of the character
            Linkify.addLinksSprd(mTitle, Linkify.ALL);
        }
        ...
        if (location == null || location.trim().length() == 0) {
            ...
            if (textView != null) {
                ...
                try {
                    ...
                    //Add for bug524609, identify information content for before and after the url of the character
                    Linkify.addLinksSprd(textView, Linkify.ALL);
                ...
                textView.setKeyListener(new KeyListener() {
                    ...
                    public boolean onKeyDown(View view, Editable text,
                        ...
                        if (keyCode == event.KEYCODE_DPAD_CENTER
                            ...
                            // if there has no map application then not to trigger the linker
                            return !Utils.isMapApplicationAvailable(getActivity());
                        }
                        return false;
                    }
    ...
    public void onResume() {
        ...
        // SPRD Modify for bug451704/500118, make sure photo change sync
        mHandler.startQuery(TOKEN_QUERY_EVENT, null, mUri, EVENT_PROJECTION,
              null, null, null);
    }
    ...
    /* SPRD: bug442745, bug491495 @{
     * this function copy from CalendarEventModel
     * Sort and uniquify mReminderMinutes.
     *
     * @return true (for convenience of caller)
     **/
    private boolean normalizeReminders() {
