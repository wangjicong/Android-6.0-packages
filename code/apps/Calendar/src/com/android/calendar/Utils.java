...
/* SPRD: bug256328, 491329 check is there any map application available. @{ */
...
public class Utils {
    ...
    /* SPRD: bug408840 limit date between 1970/01/01 and 2037/12/31 @{ */
    public static final int YEAR_MAX = 2037;

    // MAX_DATE_JULIAN_DAY is the julian day of 2037-12-31
    public static final int MAX_DATE_JULIAN_DAY = 2465424;
    // MIN_DATE_JULIAN_DAY is the julian day of 1970-01-01
    public static final int MIN_DATE_JULIAN_DAY = 2440588;
    // scroll from left to right, week view max base day is 2037-12-27
    public static final int WEEK_VIEW_MAX_BASE_DAY_JULIAN = 2465420;
    // scroll from right to left, week view min base day is 1969-12-28
    public static final int WEEK_VIEW_MIN_BASE_DAY_JULIAN = 2440584;
    // scroll from left to right, day view max base day is 2037-12-31
    public static final int DAY_VIEW_MAX_BASE_DAY_JULIAN = MAX_DATE_JULIAN_DAY;
    // scroll from right to left, day view min base day is 1970-01-01
    public static final int DAY_VIEW_MIN_BASE_DAY_JULIAN = MIN_DATE_JULIAN_DAY;
    /* SPRD: bug408840 2015-03-03 @} */

    // SPRD for bug442148/454220/463755/492933. Date beyond 2037-12-31
    public static final int MONTH_MAX = 11;
    public static final int DAY_MAX = 31;
    public static final long END_DAY_MAX = 2145888000000L; //max millis for 2037-12-31
    //SPRD: Add for bug452427/454220, Date super bound, cannot create event
    public static final long END_DAY_MAX_ALLDAY = 2145916800000L; //max millis for 2037-12-31 allday

    // SPRD for bug458035/501401, add DAY_IN_MILLIS
    ...
    public static final String ACTION_CLOSE_EMPTY_DIALOGS = "com.android.calendar.alerts.DismissAlarmsReceiver";

    public static final String APPWIDGET_DATA_TYPE = "vnd.android.data/update";
// SPRD: for bug 441656 , change the start day of week, the time can't be located to the current time
    public static final String WEEK_START_DAY_CHANGE = "week.start.day.change";
    ...
    **/
    ...
    /* SPRD: Add for bug473445/504778, alarm notification RGB light indication function. @{ */
    public static final int LED_ARGB = Color.BLUE;
    public static final int LED_ON_MS = 1500;
    public static final int LED_OFF_MS = 2500;
    /* @} */

    /* SPRD: Add for bug450678, set day high light. @{ */
    public static boolean mDrawSetDay = false;
    /* @} */

    public static final String LOCAL_CALENDAR_ACCOUNT_NAME = "Local Calendar";
    ...
    public static String getDisplayedDatetime(long startMillis, long endMillis, long currentMillis,
        ...
        if (allDay) {
            ...
            /* SPRD: for bug517909, New an allDay event of 1970.1.1，view the event from DayView, display error @{ */
            if (endMillis - startMillis > Utils.DAY_IN_MILLIS) {
                startMillis = endMillis - Utils.DAY_IN_MILLIS;
            }
            /* @} */
    ...
    public static String replaceStrForLocalCalendar(Context context,String name) {
        if (context != null && name != null) {
            if (name.equals(LOCAL_CALENDAR_ACCOUNT_NAME)) {
                name = context.getResources().getString(R.string.local_account);
            }
        }
        return name;
    }
    ...
    public static boolean isIntentAvailable(Context contex, Intent intent) {
        final PackageManager mgr = contex.getPackageManager();
    ...
    /* SPRD: bug442319, bug491329, not map appliation installed cause crash by monkey @{ */
    public static boolean isMapApplicationAvailable(Context ctx) {
        Uri geoUri = Uri.parse("geo:0,0?q=beijing");
        Intent geoIntent = new Intent(Intent.ACTION_VIEW, geoUri);
        return isIntentAvailable(ctx, geoIntent);
    }
    /* @} */

    /**
     * SPRD: bug408840 limit date between 1970/01/01 and 2037/12/31 @{
     * function setJulianDay(...), and getJulianDay(...) are come from
     * android.text.format.Time but fix the bug of calc julian day if
     * milliseconds is negative.
     */
    public static long setJulianDay(Time time, int julianDay) {
        ...
        // the day.
        long millis = (julianDay - Time.EPOCH_JULIAN_DAY)
    ...
    public static int getJulianDay(long millis, long gmtoff) {
        ...
        long julianDay = (long) java.lang.Math.floor((millis
                + offsetMillis) * 1.0 / DateUtils.DAY_IN_MILLIS);
        return (int) julianDay + Time.EPOCH_JULIAN_DAY;
    }
    public static void showDateOutOfRangeToast(Context ctx) {
        if (ctx != null) {
            Toast.makeText(ctx, R.string.select_date_out_range,
                Toast.LENGTH_SHORT).show();
        }
    }
    public static boolean isTimeInLimitation(Time day) {
        if (day != null
            && day.year >= YEAR_MIN
            && day.year <= YEAR_MAX) {
            return true;
    ...
    /*SPRD: bug454655 when date < 1970.1.1. should limit date to 1970.1.1 @{*/
    public static long limitMinDate(String timeZone, long oldMillis) {
        long newMillis;
        Time t = new Time(timeZone);
        int goToJulianDay = t.getJulianDay(oldMillis, t.gmtoff);
        if (goToJulianDay < MIN_DATE_JULIAN_DAY) {
            goToJulianDay = MIN_DATE_JULIAN_DAY;
            t.setJulianDay(goToJulianDay);
            newMillis = t.normalize(true);
        } else {
            newMillis = oldMillis;
        }
        return newMillis;
    ...
    /* SPRD: bug516908, request runtime permissions @{ */
    ...
    public static boolean mLunarFlag;
