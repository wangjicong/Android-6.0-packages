...
public class MonthListView extends ListView {
    ...
    private void doFling(float velocityY) {
        ...
        // SPRD:Add for bug417035,Month view slide error.
        int scrollToDay = getJulianDay(timeInMillis, mTempTime.gmtoff,monthsToJump)
                + ((monthsToJump > 0) ? 6 : 0);
    ...
    /* SPRD: Add for bug417035,Month view slide error.{@ */
    public int getJulianDay(long millis, long gmtoff, int monthsToJump) {
        if (millis == -1 && monthsToJump > 0) {
            return Utils.MAX_DATE_JULIAN_DAY;
        }
