...
public class SimpleWeekView extends View {
    ...
    protected static int MINI_LUNAR_NUMBER_TEXT_SIZE = 10;
    protected String[] mLunarDayNumbers;
    ...
    public SimpleWeekView(Context context) {
        ...
        if (mScale == 0) {
            ...
            if (mScale != 1) {
                ...
                /* SPRD: Modify for bug473571, add lunar info @{
                 * we only use MINI_LUNAR_NUMBER_TEXT_SIZE when lunar is
                 * enabled, so we can just set value to it now.
                 */
                MINI_LUNAR_NUMBER_TEXT_SIZE *= mScale;
                /* @} */
            }
        }
    ...
    public void setWeekParams(HashMap<String, Integer> params, String tz) {
        ...
        /* SPRD: Modify for bug473571, add lunar info @{ */
        if (Utils.mLunarFlag) {
            mLunarDayNumbers = new String[mNumCells];
            mLunarDayNumbersIsFest= new boolean[mNumCells];
        }
        /* @} */
        ...
        if (time.weekDay != mWeekStart) {
            ...
            if (diff < 0 && (mWeekStart != Time.MONDAY)) {
                diff += 7;
            }
            time.monthDay -= diff;
            time.normalize(true);
        ...
        for (; i < mNumCells; i++) {
            ...
            if (Utils.mLunarFlag) {
                ...
                LunarCalendarConvertUtil.parseLunarCalendar(time.year,
                        time.month, monthDay, mLunarCalendar);
                mLunarDayNumbers[i] = mLunarCalendar.getLunarDayInfo();
