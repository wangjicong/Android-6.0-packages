...
public class MonthWeekEventsView extends SimpleWeekView {
    ...
    public void setWeekParams(HashMap<String, Integer> params, String tz) {
        ...
        if (Utils.mLunarFlag) {
            if (mOrientation == Configuration.ORIENTATION_PORTRAIT) {
                mDNATimePaint.setStrokeWidth(DNA_WIDTH / 2);
            }
        }
        /* @} */
    }
    ...
    protected void drawWeekNums(Canvas canvas) {
        ...
        for (; i < numCount; i++) {
            ...
            /* SPRD: Modify for bug473571, add lunar info @{ */
            if (Utils.mLunarFlag) {
                int color1 = mMonthNumPaint.getColor();
                // lunar info is longer, so move more further
                lunarX = x + 2;
                mMonthNumPaint.setTextSize(MINI_LUNAR_NUMBER_TEXT_SIZE);
                String lunarStr = mLunarDayNumbers[i];
                boolean isFest = mLunarDayNumbersIsFest[i];
                if (isFest == true) {
                    mMonthNumPaint.setColor(Color.parseColor("#71C671"));
                }
                if (lunarStr.contains("/")) {
                    int count =  lunarStr.split("/").length;
                    ArrayList<String> strArray  = new ArrayList<String>();
                    int j = 0;
                    for(j = 0; j < count; j++){
                        strArray.add(lunarStr.split("/")[j].toString());
                    }
                    if (mOrientation == Configuration.ORIENTATION_LANDSCAPE) {
                        canvas.drawText(lunarStr, lunarX, lunarY,
                                mMonthNumPaint);
                    } else {
                        for(j = 0; j < count; j++){
                            canvas.drawText(strArray.get(j), lunarX, lunarY + j*20,
                                    mMonthNumPaint);
                        }
                    }
                } else {
                ...
                mMonthNumPaint.setColor(color1);
            }
            /* @} */
            if (isBold) {
    ...
    public Time getDayFromLocation(float x) {
        ...
        /* SPRD: bug412014 month view, date change to 1970-1-1 when we click
         * 1969-12-31 @{ */
        //if (mWeek == 0) {
        //    // This week is weird...
