...
public class CalendarController {
    ...
    public static final int MAX_CALENDAR_WEEK = 3549; // weeks between 1/1/1970 and 1/1/2038
    /* @} */
    ...
    public void sendEvent(Object sender, final EventInfo event) {
        ...
        /* SPRD: bug474714 can not select 8:00 am when current timezone @{
         * is +8(Beijing)
         */
//        if (event.selectedTime != null && event.selectedTime.toMillis(false) != 0) {
        ...
        /* SPRD for bug458035/501401，if date is 1970-1-1 and isAllDay is true, don't set event.startTime = mTime. @} */
        boolean useSelectedTime = false;
        if (event != null && event.startTime != null && event.endTime != null) {
            useSelectedTime = event.isAllDay() && event.startTime.toMillis(false) == 0
                    && event.endTime.toMillis(false) == Utils.DAY_IN_MILLIS;
        }
        /* @} */

        // Fix up start time if not specified
        if (!useSelectedTime && startMillis == 0) {
