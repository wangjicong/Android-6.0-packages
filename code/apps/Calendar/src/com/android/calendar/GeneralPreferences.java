...
public class GeneralPreferences extends PreferenceFragment implements
    ...
    public void onResume() {
        ...
        if (ringToneUri != null){
            ...
            if (editor != null) {
                ...
                String ringtoneDisplayString = getRingtoneTitleFromUri(getActivity(), ringToneUri);
                mRingtone.setSummary(ringtoneDisplayString == null ? "" : ringtoneDisplayString);
            }
        }
        super.onResume();
    ...
    private void showTimezoneDialog() {
        final Activity activity = getActivity();
        /* SPRD: Bug 518836 java.lang.IllegalStateException: Can not perform this action after onSaveInstanceState @{ */
    ...
    public String getRingtoneTitleFromUri(Context context, String uri) {
        ...
        /* SPRD:ADD Bug397766/466734/472865/514347, invalid ringing reminder. @{*/
        //SPRD for bug528419 Fail to ring when the song is deleted
        if (ring != null && Utils.hasDeleted(context, Uri.parse(uri))) {
            final Activity activity = getActivity();
            Toast.makeText(activity, getResources().getString(R.string.ringtone_default_message)
            ...
            Utils.setRingTonePreference(activity, GeneralPreferences.DEFAULT_RINGTONE);
            ring = RingtoneManager.getRingtone(getActivity(), Uri.parse(GeneralPreferences.DEFAULT_RINGTONE));

            // Set the ringToneUri to the backup-able shared pref only so that
            // the Ringtone dialog will open up with the correct value.
            final Editor editor = getPreferenceScreen().getEditor();
            editor.putString(GeneralPreferences.KEY_ALERTS_RINGTONE,
                    GeneralPreferences.DEFAULT_RINGTONE).apply();
            /* @} */
        }
        /* @} */
