...
public class AlertService extends Service {
    ...
    void processMessage(Message msg) {
        ...
        /*
         * SPRD: add for bug462566 , action can not be null. @{
         */
        if (null == action) {
    ...
    static boolean updateAlertNotification(Context context) {
        ...
        try {
            ...
            Log.d(TAG, "AlertService RuntimeException");
        }
        ...
        if (alertCursor != null && alertCursor.getCount() == 0) {
            ...
            context.sendBroadcast(closeEmptyDialogIntent);
        }
        /* @} */
    ...
    public static boolean generateAlerts(Context context, NotificationMgr nm,
        ...
        if (numLowPriority > 0) {
            ...
            addNotificationOptions(notification, true, expiredDigestTitle,
                    ...
                    true); /* Show the LED for the expired events. */
    ...
    private static void addNotificationOptions(NotificationWrapper nw, boolean quietUpdate,
        ...
        Log.d(TAG, "addNotificationOptions, showLights: " + showLights);
        if (showLights) {
            notification.flags |= Notification.FLAG_SHOW_LIGHTS;
            //notification.defaults |= Notification.DEFAULT_LIGHTS;
            /* SPRD: Add for bug473445/504778, alarm notification RGB light indication function. @{ */
            notification.ledARGB = Utils.LED_ARGB;
            notification.ledOnMS = Utils.LED_ON_MS;
            notification.ledOffMS = Utils.LED_OFF_MS;
            /* @} */
        }
        ...
        private String getRingtoneAndSilence() {
            if (ringtone == null) {
                if (quietUpdate) {
                    ...
                    // SPRD: add bug412870,The ringtone set to none,still have sound.
                    if (TextUtils.isEmpty(ringtone)) {
                        return EMPTY_RINGTONE;
                    }
                    /* SPRD:ADD Bug399324/510244, SD card removed ringtones, received the notice did not
                     * play the default ringtones;
                     * Add for bug528419 Fail to ring when the song is deleted. @{
                     */
                     if (Utils.hasDeleted(context, Uri.parse(ringtone))) {
                         /* SPRD: add for bug532711, java.lang.NullPointerException, use Uri.parse on a null string. @} */
                         if (TextUtils.isEmpty(GeneralPreferences.DEFAULT_RINGTONE)) {
                             return EMPTY_RINGTONE;
                         }
                         /* @} */
                        if (Utils.hasDeleted(context, Uri.parse(GeneralPreferences.DEFAULT_RINGTONE))) {
                            String notifiUriString = Settings.System.getString(context.getContentResolver(),
                            ...
                            RingtoneManager.setActualDefaultRingtoneUri(context,
                                    RingtoneManager.TYPE_NOTIFICATION, Uri.parse(notifiUriString));
                        }
                         Utils.setRingTonePreference(context, GeneralPreferences.DEFAULT_RINGTONE);
                         ringtone = Utils.getRingTonePreference(context);
                     }
                     /* @} */
                }
