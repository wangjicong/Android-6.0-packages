...
import com.sprd.calendar.SprdTextWatcher;
...
public class EditEventView implements View.OnClickListener, DialogInterface.OnCancelListener,
    ...
    /* SPRD:fix bug 474822 ,show Toast when email format error @{ */
    private static final Pattern EMAIL_ADDRESS_PATTERN = Pattern.compile("[^\\s@]+@([^\\s@\\.]+\\.)+[a-zA-z][a-zA-Z][a-zA-Z]*");
    /* @} */
    /* Bug #442566/474232 Add for Local Calendar bugs @{ */
    private static final String LOCAL_NAME_PREFERENCES="local_shareference";
    /* SPRD for bug 443435/474232,if account is local account,let mAttendeesGroup dismiss @{ */
    private static final int LOCAL_ACCOUNT_POSITION = 0;
    /* @} */
    ...
    // SPRD: Add for bug455971,switch screen,Availability status not save.
    ...
    private int mModification = EditEventHelper.MODIFY_UNINITIALIZED;
     ...
     /* SPRD: bug433456   2015-05-5 New event title can not be too long@@{ */
    private static final int MESSAGE_SUBJECT_TEXT_MAX_LENGTH = 50;
    private static final int MESSAGE_LOCATION_Text_MAX_LENGTH = 50;
    private static final int MESSAGE_CONTENT_TEXT_MAX_LENGTH = 300;
    ...
    private Time mOldTIme   = new Time();
    private boolean isRepeatDone = false;
    ...
    private class TimeListener implements OnTimeSetListener {
        ...
        public void onTimeSet(RadialPickerLayout view, int hourOfDay, int minute) {
            ...
            /* SPRD: Add for bug442148/454220/463755/492933. Date beyond 2037-12-31 @} */
            if (endMillis >= Utils.END_DAY_MAX || endMillis == -1) {
                endTime.year = Utils.YEAR_MAX;
                endTime.month = Utils.MONTH_MAX;
                endTime.monthDay = Utils.DAY_MAX;
                endMillis = endTime.normalize(true);
            }
            /* @} */
            /* SPRD: Add for bug505589, to ensure that startMillis is less than endMillis @} */
            /* SPRD: Add for bug513282, the internal and external display is not consistent for EndTime. @} */
            if (startMillis > endMillis) {
                endMillis = startMillis;
                mEndTime.hour = mStartTime.hour;
                mEndTime.minute = mStartTime.minute;
            }
            /* @} */
    ...
    private class TimeClickListener implements View.OnClickListener {
        ...
        public void onClick(View v) {
            ...
            /* SPRD for Bug529720 com.android.calendar happens JavaCrash,log:java.lang.IllegalStateException @ }*/
    ...
    private void populateRepeats() {
        ...
        if (!TextUtils.isEmpty(mRrule)) {
            ...
            if (enabled && Time.compare(mStartTime, mOldTIme) != 0 && !isRepeatDone) {
    ...
    public static class CalendarsAdapter extends ResourceCursorAdapter {
        ...
        public void bindView(View view, Context context, Cursor cursor) {
            ...
            if (name != null) {
                ...
                /* SPRD: modified  20150914 of bug 474232, add local account @{ */
                if (displayName.equals("Local Calendar")) {
                    displayName = context.getResources().getString(R.string.local_account);
                }
                name.setText(displayName);

                TextView accountName = (TextView) view.findViewById(R.id.account_name);
                if (accountName != null) {
                    String ownerName = cursor.getString(ownerColumn);
                    if (ownerName.equals("Local Calendar")) {
                        ownerName = context.getResources().getString(R.string.local_account);
                    }
                    accountName.setText(ownerName);
    ...
    public void showRecurrencePickerDialog(){
        /* SPRD for Bug531074 com.android.calendar happens JavaCrash,log:java.lang.IllegalStateException @ }*/
        if (mActivity == null || !mActivity.isResumed()) {
            return;
        }
        /* @ }*/
    ...
    private boolean fillModelFromUI() {
        ...
        /*
         * SPRD: Add for bug452427/454220, Date super bound, cannot create
         * event @{
         */
        if (mModel.mAllDay) {
            if (mModel.mStart > Utils.END_DAY_MAX_ALLDAY || mModel.mEnd > Utils.END_DAY_MAX_ALLDAY || mModel.mEnd == -1) {
                Toast.makeText(mActivity,
                        mActivity.getResources().getString(R.string.not_save_event),
                        Toast.LENGTH_SHORT).show();
                return false;
            }
        } else {
            if (mModel.mStart >= Utils.END_DAY_MAX || mModel.mEnd >= Utils.END_DAY_MAX || mModel.mEnd == -1) {
                Toast.makeText(mActivity,
                        mActivity.getResources().getString(R.string.not_save_event),
                        Toast.LENGTH_SHORT).show();
                return false;
    ...
    //Bug #455971 switch screen,Availability status not save.
    public EditEventView(Activity activity, View view, EditDoneRunnable done,
            boolean timeSelectedWasStartTime, boolean dateSelectedWasStartDate, boolean isAvailabilityExplicitlySet) {
        mActivity = activity;
        mView = view;
        mDone = done;
        ...
        // SPRD: Add for bug455971,switch screen,Availability status not save.
        mAvailabilityExplicitlySet = isAvailabilityExplicitlySet;
        ...
        /* SPRD: bug433456/498118, New event title can not be too long @{ */
        SprdTextWatcher titleTextWatcher = new SprdTextWatcher(mActivity, mTitleTextView,
                MESSAGE_SUBJECT_TEXT_MAX_LENGTH);
        mTitleTextView.addTextChangedListener(titleTextWatcher);
        SprdTextWatcher locationTextWatcher = new SprdTextWatcher(mActivity,
                mLocationTextView,
                MESSAGE_LOCATION_Text_MAX_LENGTH);
        mLocationTextView.addTextChangedListener(locationTextWatcher);
        SprdTextWatcher descriptionTextWatcher = new SprdTextWatcher(mActivity,
                mDescriptionTextView,
                MESSAGE_CONTENT_TEXT_MAX_LENGTH);
        mDescriptionTextView.addTextChangedListener(descriptionTextWatcher);
        /* @} */
    }
    ...
    public void setModel(CalendarEventModel model) {
        mModel = model;
        /* Bug #442566/474232 Add for Local Calendar bugs @{ */
        SharedPreferences  localPreferences = mActivity.getSharedPreferences(LOCAL_NAME_PREFERENCES,Activity.MODE_PRIVATE);
        SharedPreferences.Editor editor = localPreferences.edit();
        if(mModel != null){
            editor.putString("mUri", model.mUri != null ? model.mUri : null);
            editor.putString("mCalendarDisplayName", model.mCalendarDisplayName);
            editor.putString("mOwnerAccount", model.mOwnerAccount );
            editor.commit();
        }
        /* @} */
        ...
        String uri = localPreferences.getString("mUri", null);
        if (model.mUri != null || !TextUtils.isEmpty(uri)) {
            ...
            /* Bug #442566/474232 Add for Local Calendar bugs @{ */
            String name = localPreferences.getString("mCalendarDisplayName", null);
            String displayName = TextUtils.isEmpty(model.mCalendarDisplayName) ? name : model.mCalendarDisplayName;
            if (displayName.equals("Local Calendar")) {
                displayName = mActivity.getResources().getString(R.string.local_account);
                /* SPRD for bug 443435/453152/474232,if account is local account,let mAttendeesGroup dismiss @{ */
                if(mAttendeesList != null){
            ...
            String ownerName = localPreferences.getString("mOwnerAccount", null);
            if (tv != null) {
                /* Bug #442566/474232 Add for Local Calendar bugs @{ */
                String ownerAccount = TextUtils.isEmpty(model.mOwnerAccount) ? ownerName : model.mOwnerAccount;
                if (ownerAccount.equals("Local Calendar")) {
                    ownerAccount = mActivity.getResources().getString(R.string.local_account);
                }
                tv.setText(ownerAccount);
            }
        } else {
    ...
    protected void setAllDayViewsVisibility(boolean isChecked) {
        if (isChecked) {
            ...
            if (mEndTime.hour == 0 && mEndTime.minute == 0) {
                ...
                long endMillis = mEndTime.normalize(true);
                if (endMillis >= Utils.END_DAY_MAX || endMillis == -1) {
                    mEndTime.year = Utils.YEAR_MAX;
                    mEndTime.month = Utils.MONTH_MAX;
                    mEndTime.monthDay = Utils.DAY_MAX;
                    endMillis = mEndTime.normalize(true);
                }
                setDate(mEndDateButton, endMillis);
                setTime(mEndTimeButton, endMillis);
    ...
    public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
        ...
        /* SPRD for bug 443435/453152/455407/457832/463506/474232,if account is local account,let mAttendeesGroup dismiss @{ */
        String accountName = c.getString(EditEventHelper.CALENDARS_INDEX_ACCOUNT_NAME);
        if (accountName.equals("Local Calendar")) {
            if (mAttendeesList != null) {
                mAttendeesList.setText("");
            }
            if (mEtAttendeesList != null) {
                mEtAttendeesList.setText("");
            }
            mAttendeesGroup.setVisibility(View.GONE);
        } else {
        ...
        mModel.mCalendarAccountName = accountName;
    ...
    public boolean checkEmailAddress() {
        boolean result = true;
        /* SPRD: Modify 20150916 of bug474716, tip read contact. @{ */
        String attendee;
        if (mAttendeesList != null) {
            attendee = mAttendeesList.getText().toString().trim();
        } else {
            attendee = mEtAttendeesList.getText().toString().trim();
        }
        if ("".equals(attendee)) {
            result = true;
        } else {
            Rfc822Token[] tokens = Rfc822Tokenizer.tokenize(attendee);
            for (int i = 0; i < tokens.length; i++) {
                if (!EMAIL_ADDRESS_PATTERN.matcher(tokens[i].getAddress()).matches()) {
                    result = false;
                }
            }
        }
    ...
    /*SPRD for bug446277 when EditEventFragment destroyed,  EditEventView.mRrule should to be null@{*/
    public void setRruleNull(){
        if(mRrule != null){
            mRrule = null;
        }
    }
