            ...
            if (newEvent || !TextUtils.equals(originalAttendeesString, attendees)) {
                ...
                if (!newEvent) {
                    ...
                    for (String originalEmail : originalAttendees.keySet()) {
                        /* SPRD for bug500731/425916, Edit event once more, do not send invitation @} */
                        if (newAttendees.get(originalEmail) == null) {
                            removedAttendees.add(originalEmail);
                        } else {
                            String newName = newAttendees.get(originalEmail).mName;
                            String originaName = originalAttendees.get(originalEmail).mName;
                            if (newAttendees.containsKey(originalEmail) && TextUtils.equals(newName, originaName)) {
                            /* @} */
    ...
    ContentValues getContentValuesFromModel(CalendarEventModel model) {
        ...
        if (model.isEventColorInitialized()) {
            if (model.getEventColor() == model.getCalendarColor()) {
                ...
                /* SPRD for bug448986 if model.getEventColorKey() == -1,put values NO_EVENT_COLOR @{*/
                if(model.getEventColorKey() == -1){
                    values.put(Events.EVENT_COLOR_KEY, NO_EVENT_COLOR);
                }else{
