...
public class EditEventFragment extends Fragment implements EventHandler, OnColorSelectedListener {
    ...
    private static final String AVAILABILITY_EXPLICITLY_SET = "AvailabilityExplicitlySet";
    ...
    // SPRD: Add for bug455971,switch screen,Availability status not save.
    private boolean mIsAvailabilityExplicitlySet = false;
                ...
                case TOKEN_ATTENDEES:
                    try {
                        while (cursor.moveToNext()) {
                            ...
                            if (email != null) {
                                /* SPRD: for bug395387/500731, distinguish relationship, attendee or organizer @} */
                                if (mModel.mOwnerAccount != null &&
                                        mModel.mOwnerAccount.equalsIgnoreCase(email) && relationship == Attendees.RELATIONSHIP_ORGANIZER) {
                                    int attendeeId =
    ...
    private void startQuery() {
        ...
        /* SPRD for bug443486 remove  if (mBegin <= 0){} @{*/
        //        if (mBegin <= 0) {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if(savedInstanceState != null){
            mIsAvailabilityExplicitlySet = savedInstanceState.getBoolean(AVAILABILITY_EXPLICITLY_SET,false);
        }
        /* @} */
//        mContext.requestWindowFeature(Window.FEATURE_INDETERMINATE_PROGRESS);
        ...
        // SPRD: Modify 20150226 of bug408526, tip read contact. And bug455971,switch screen,Availability status not save.
        mView = new EditEventView(mActivity, view, mOnDone, mTimeSelectedWasStartTime,
                mDateSelectedWasStartDate, mIsAvailabilityExplicitlySet);
    ...
    private boolean onActionBarItemSelected(int itemId) {
        if (itemId == R.id.action_done) {
            ...
            if (!mView.checkEmailAddress()) {
                Toast.makeText(getActivity(), R.string.email_format_err, Toast.LENGTH_SHORT).show();
                return true;
            }
            /* @} */
    ...
    boolean isEmptyNewEvent() {
        ...
        */
         //SPRD for 381480: if allDay,set the originalStart and originalEnd timezone to UTC
        if(mModel.mAllDay){
            mModel.mOriginalStart = setToUTC(mModel.mOriginalStart);
            mModel.mOriginalEnd = setToUTC(mModel.mOriginalEnd);
        }
    ...
    //SPRD for 381480: add to set timezone to UTC
    private long setToUTC(long time){
        Time tempTime = new Time();
        tempTime.set(time);
        tempTime.hour = 0;
        tempTime.minute = 0;
        tempTime.second = 0;
        tempTime.timezone = Time.TIMEZONE_UTC;
        time = tempTime.normalize(true);
        return time;
    }

    /*
     * SPRD: for bug452074/468914/513480, override onResume method, set onClickListener of
     * mRruleButton to default@{
     */
    @Override
    public void onResume() {
        super.onResume();
        if (mView != null) {
           if(mView.mRruleButton != null){
               mView.mRruleButton.setOnClickListener(mView);
            }else if(mView.mStartTimeButton != null){
                mView.mStartTimeButton.setOnClickListener(mView);
            }else if(mView.mEndTimeButton != null){
                mView.mEndTimeButton.setOnClickListener(mView);
            }
        }
    }
    ...
    @Override
    public void onPause() {
        //SPRD: Add 20150115 of bug394510,disable
        ...
        super.onPause();
        // SPRD for bug449438/501157, we should setMonthRepeatByDayOfWeekStrsNull in onPause()
        EventRecurrenceFormatter.setMonthRepeatByDayOfWeekStrsNull();
        /*
         * SPRD: for bug452074/468914/513480, workaround, set onClickListener of mRruleButton
         * null to avoid click on nonexistent activity@{
         */
        if(mView != null){
            if(mView.mRruleButton != null){
                mView.mRruleButton.setOnClickListener(null);
            }else if(mView.mStartTimeButton != null){
                mView.mStartTimeButton.setOnClickListener(null);
            }else if(mView.mEndTimeButton != null){
                mView.mEndTimeButton.setOnClickListener(null);
            }
        }
    }
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        // SPRD: Add for bug455971,switch screen,Availability status not save.
        outState.putBoolean(AVAILABILITY_EXPLICITLY_SET, mView.mAvailabilityExplicitlySet);
    }
    ...
    /* SPRD: Add 20150115 of bug394510,after click home button,screen switch,dialog dismiss@{ */
    public void onBack() {
        Activity act = getActivity();
        if (mSaveOnDetach && act != null && !mIsReadOnly && !act.isChangingConfigurations()
                && mView.prepareForSave()) {
