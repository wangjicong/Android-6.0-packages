...
public class DayView extends View implements View.OnCreateContextMenuListener,
    ...
    */
    ...
    // SPRD: Add for bug382427/521691, dynamically calculates the distance.
    ...
    private void init(Context context) {
        ...
        // SPRD: Add for bug382427/521691, dynamically calculates the distance.
        mAmPmLeftMargin = mHoursWidth - HOURS_RIGHT_MARGIN;
    ...
    public void setSelected(Time time, boolean ignoreTime, boolean animateToday) {
        ...
        /**
         * SPRD: bug408840 limit date between 1970/01/01 and 2037/12/31 ,week
         * view, date selection error when date between @{ 1969-12-28 and
         * 1970-1-3
         */
        // setSelectedDay(Time.getJulianDay(millis, mBaseDate.gmtoff));
        setSelectedDay(Utils.getJulianDay(millis, mBaseDate.gmtoff));
        /* @} */
    ...
    private void recalc() {
        ...
        /**
         * SPRD: bug408840 limit date between 1970/01/01 and 2037/12/31 ,week
         * view, date selection error when date between 1969-12-28 and 1970-1-3
         * @{
         */
        // mFirstJulianDay = Time.getJulianDay(start, mBaseDate.gmtoff);
        mFirstJulianDay = Utils.getJulianDay(start, mBaseDate.gmtoff);
        /* @} */
...
//                break;
    ...
    private View switchViews(boolean forward, float xOffSet, float width, float velocity) {
        ...
        start.normalize(true);
        adjustNewSelectedDateTimeWithinLimitRange(start);
        mController.setTime(start.toMillis(true /* ignoreDst */));
    ...
    private void drawSelectedRect(Rect r, Canvas canvas, Paint p) {
        ...
        if (mSelectionMode != SELECTION_HIDDEN && !mSelectionAllday) {
            ...
            if (mNumDays > 1) {
                ...
                /*SUN:jicong.wang add for bug 53781 start {@*/
                p.setAntiAlias(true);
                //p.setTypeface(Typeface.defaultFromStyle(Typeface.BOLD));
                /*SUN:jicong.wang add for bug 53781 end @}*/
...
//    }
    ...
    private void doSingleTapUp(MotionEvent ev) {
        ...
        if (!validPosition) {
            if (y < DAY_HEADER_HEIGHT) {
                ...
                /**
                 * SPRD: bug408840 limit date between 1970/01/01 and 2037/12/31
                 * ,week view, date selection error when date between 1969-12-28
                 * and 1970-1-3
                 * @{
                 */
                // selectedTime.setJulianDay(mSelectionDay);
                Utils.setJulianDay(selectedTime, mSelectionDay);
                /* @} */
        ...
        if (pressedSelected && mSavedClickedEvent == null) {
            ...
            Utils.setJulianDay(startTime, mSelectionDay);
            /* @} */
    ...
    private void doScroll(MotionEvent e1, MotionEvent e2, float deltaX, float deltaY) {
        ...
        if (mTouchMode == TOUCH_MODE_DOWN) {
            ...
            /*
             * SPRD: bug408840 limit date between 1970/01/01 and
             * 2037/12/31 @{ we keep vertical scrolling if not in date limited
             * range
             */
            if (absDistanceX > absDistanceY && !isOutOfLimitedDateRangeWhenMove(e1, e2)) {
...
//        }
    ...
    class CalendarGestureListener extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onSingleTapUp(MotionEvent ev) {
            ...
            /* SPRD: bug408840 limit date between 1970/01/01 and 2037/12/31 @{ */
            if (isOutOfLimitedDateRangeWhenDown(ev)) {
                Utils.showDateOutOfRangeToast(mContext);
                return true;
            }
            /* SPRD: bug408840 2015-03-03 @} */
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            ...
            /* SPRD: bug408840 limit date between 1970/01/01 and 2037/12/31  @{ */
            if (isOutOfLimitedDateRangeWhenMove(e1, e2)) {
                return true;
            }
            /* SPRD: bug408840 2015-03-03 @} */
            if (mTouchStartedInAlldayArea) {
    ...
    /*
     * SPRD: bug408840 limit date between 1970/01/01 and 2037/12/31
     * @{ if newSelected is not between 1970-1-1 and 2037-12-31 than we change
     * it to 1970-1-1 or 2037-12-31
     */
    private void adjustNewSelectedDateTimeWithinLimitRange(Time newSelected) {
        boolean needAdjust = false;
        int origHour = newSelected.hour;
        int origMinute = newSelected.minute;
        int origSeconds = newSelected.second;

        int julianDay = Utils.getJulianDay(newSelected.toMillis(false /* useisDst */),
                newSelected.gmtoff);
        if (julianDay < Utils.MIN_DATE_JULIAN_DAY) {
            // julian day of 1970-01-01 if less than 2440588
            julianDay = Utils.MIN_DATE_JULIAN_DAY;
            needAdjust = true;
        } else if (julianDay > Utils.MAX_DATE_JULIAN_DAY) {
            // julian day of 2037-12-31 if larger than 2465424
            julianDay = Utils.MAX_DATE_JULIAN_DAY;
            needAdjust = true;
        }

        if (needAdjust) {
            Utils.setJulianDay(newSelected, julianDay);
            // restore hour, minute, seconds, because setJulianDay will erase
            // these fields
            newSelected.hour = origHour;
            newSelected.minute = origMinute;
            newSelected.second = origSeconds;
        }
    }

    /**
     * this function can only be called when mNumDays == 7, Week View, do not
     * call it when current is Day View
     */
    private int getJulianDayOnPoint(int x, int y) {
        if (x < mHoursWidth) {
            x = mHoursWidth;
        }
        int day = (x - mHoursWidth) / (mCellWidth + DAY_GAP);
    ...
    private boolean isOutOfLimitedDateRangeWhenMove(MotionEvent e1, MotionEvent e2) {
        int baseDayJulian = Utils.getJulianDay(mBaseDate.toMillis(false),
                mBaseDate.gmtoff);
        // Week View
        if (mNumDays == 7) {
            // SPRD: add for different firDayOfWeek julianDay changes
            baseDayJulian = updateByFirstDayOfWeek(baseDayJulian);
            if (e2.getX() - e1.getX() < 0) {
                // Scroll from right to left, aka, change to larger date, the
                // mBaseDate in
                // Week View always be 2037-12-27, it julian day is 2465420
                if (baseDayJulian >= Utils.WEEK_VIEW_MAX_BASE_DAY_JULIAN) {
                    return true;
                }
            } else if (e2.getX() - e1.getX() > 0) {
                // Scroll from left to right, aka, change to smaller date, the
                // mBaseDate in
                // Week View always be 1969-12-28
                if (baseDayJulian <= Utils.WEEK_VIEW_MIN_BASE_DAY_JULIAN) {
                    return true;
                }
            }
        } else if (mNumDays == 1) {
            ...
            // Scroll from right to left, aka, change to larger date, the
            // mBaseDate in
            // Day View will always be 2037-12-31
            if (e2.getX() - e1.getX() < 0) {
                if (baseDayJulian >= Utils.DAY_VIEW_MAX_BASE_DAY_JULIAN) {
                    return true;
                }
            } else if (e2.getX() - e1.getX() > 0) {
                // Scroll from left to right, aka, change to smaller date, the
                // mBaseDate in
                // Day View will always be 1970-01-01
                if (baseDayJulian <= Utils.DAY_VIEW_MIN_BASE_DAY_JULIAN) {
                    return true;
                }
            }
        }
     ...
     * call it when current is Day View
     */
    private boolean isOutOfLimitedDateRangeWhenDown(MotionEvent ev) {
        if (mNumDays != 7) {
            // not in Week View, we just return false here
            return false;
        }
        ...
        int julianDay = getJulianDayOnPoint(x, y);
        if (julianDay > Utils.MAX_DATE_JULIAN_DAY
                || julianDay < Utils.MIN_DATE_JULIAN_DAY) {
            return true;
        }
        return false;
    ...
    /* SPRD: bug408840 2015-03-03 @} */
    /* SPRD: add for different firDayOfWeek julianDay changes @{ */
    public int updateByFirstDayOfWeek(int julianDay) {
        if (mFirstDayOfWeek == Time.SATURDAY) {
            julianDay ++;
