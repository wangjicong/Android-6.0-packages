...
public class LanguageMgr extends ArrayList<LanguageItemData> {
    ...
    private boolean loadFromDb(int sub_id, boolean unexpected) {
        ...
        // remove ALL data
        if (!unexpected) {
            Log.d("ran1", "Activity is closed nomal.");
            clear();
        } else {
            Log.d("ran1", "Activity is closed unexpected.");
            return true;
        }
        ...
        log("--Lang-----LoadFromDb--query with subId, the cursor'length is:" + cursor.getCount()
                +" Cursor is:"+cursor.toString());
        ...
        try {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (mNoRecordBySub) {
                    ...
                    dbHelper.getWritableDatabase().insert(LangMapTableDefine.LANG_MAP_TABLE_NAME,
                            null, cv);
                } else {
                    dbHelper.getWritableDatabase().update(
                            LangMapTableDefine.LANG_MAP_TABLE_NAME, cv,
