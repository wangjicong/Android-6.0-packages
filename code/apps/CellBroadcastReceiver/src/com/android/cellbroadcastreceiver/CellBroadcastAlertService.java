...
public class CellBroadcastAlertService extends Service {
    ...
    private boolean isMessageEnabledByUser(CellBroadcastMessage message) {
        ...
        if (message.getServiceCategory() == 50) {
            ...
            // Bug#287157 @{ allow the message of 50 channel to show
    ...
    private Uri getSoundUri(CellBroadcastMessage message){
        ...
        if (Utils.DEPEND_ON_SLOT) {
            ...
            Cursor cursor = getContentResolver().query(Utils.mCommonSettingUri,
                    select_column,
                    CommonSettingTableDefine.SUBID + "=" + subId, null,
                    null);
            Log.d(TAG, "---getRingtoneUri---subId="+subId+" and cursor.getcount="+cursor.getCount());
            if (cursor == null || cursor.getCount() == 0) {
                return Uri.parse(Utils.DEFAULT_RINGTONE);
            }
            cursor.moveToFirst();
            int index = cursor.getColumnIndex(CommonSettingTableDefine.RING_URL);
            String ringtoneUri = cursor.getString(index);
            Log.d(TAG, "ringtoneName is:"+ringtoneUri);
            return Uri.parse(ringtoneUri);
        } else {
            int channelId = message.getServiceCategory();
            String[] select_soundUri = { ChannelTableDefine.SOURND_URI, ChannelTableDefine.NOTIFICATION};
            Cursor cursor = getContentResolver().query(Utils.mChannelUri,
                    select_soundUri,
                    ChannelTableDefine.SUB_ID + "=" + subId + " and "+ChannelTableDefine.CHANNEL_ID+"="+channelId,
                    null, null);
            if (cursor == null || cursor.getCount() == 0) {
                return Uri.parse(Utils.DEFAULT_RINGTONE);
            }
            cursor.moveToFirst();
            String ringtoneUri = cursor.getString(cursor.getColumnIndex(ChannelTableDefine.SOURND_URI));
            mCanNotify = cursor.getInt(cursor.getColumnIndex(ChannelTableDefine.NOTIFICATION))==1;

            Log.d(TAG, "ringtoneUri is:"+ringtoneUri);
            if (ringtoneUri != null) {
                return Uri.parse(ringtoneUri);
            } else {
                return null;
