...
public class CellBroadcastConfigService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        if (ACTION_ENABLE_CHANNELS.equals(intent.getAction())) {
            ...
            if (!Utils.queryEnabledCheckBox(CellBroadcastConfigService.this, subId)) {
                Log.d(TAG, "configService--Enable CheckBox is false.");
                return;
            }
            ...
            try {
                ...
                //add for bug 501030 switch the ETWS and CMAS
                openETWSandCMAS(subId, enableEtwsTestAlerts,
                        enableCmasExtremeAlerts ||enableCmasSevereAlerts || enableCmasAmberAlerts || enableCmasTestAlerts);
    ...
    //add for Bug 501030
    private boolean openETWSandCMAS(int subId, boolean enableEtwsTestAlerts, boolean enableCmas){
        boolean success = false;
        try {
            ISms iccISms = ISms.Stub.asInterface(ServiceManager.getService("isms"));
            if (iccISms != null) {
                success = iccISms.enablePWSMessageForSubscriber(subId,
                        enableEtwsTestAlerts?1:0, enableEtwsTestAlerts?1:0, enableEtwsTestAlerts?1:0,
                        enableCmas?1:0);
            }
