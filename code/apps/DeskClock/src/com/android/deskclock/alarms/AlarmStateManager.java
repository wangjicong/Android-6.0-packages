...
import com.sprd.deskclock.alarms.AlarmShutDownManager;
...
public final class AlarmStateManager extends BroadcastReceiver {
    ...
    /* SPRD: Bug 523973 add a clock, reboot the devices, the clock is missed @{
     * @orig
     * public static final int ALARM_FIRE_BUFFER = 15;
     */
    public static final int ALARM_FIRE_BUFFER = 59;
    /* @} */

    // SPRD bug 500092 when alarm ring, press volume button, alarm will not off or pause
    private static AlarmInstance mForVolumeInstance = null;

    /* SPRD: Bug 515840 when playing video, alarm will not ring after arrival @{ */
    private static AlarmInstance mForLiveInstance = null;
    private static String MOVIE_ONPAUSE = "com.android.gallery3d.app.MovieActivity.onPause";
    /* @} */
    ...
    public static void updateNextAlarm(Context context) {
        ...
        /* SPRD: bug 474360 add poweronoff alarm @{ */
        AlarmShutDownManager.createOrUpdateShutDownFile(context, nextAlarm);
        /* @} */
    }
    ...
    private static void updateParentAlarm(Context context, AlarmInstance instance) {
        ...
        if (!alarm.daysOfWeek.isRepeating()) {
            ...
            /* SPRD: bug 474360 add poweronoff alarm @{ */
            AlarmShutDownManager.createOrUpdateShutDownFile(context, instance);
            /* @} */
    ...
    private static void cancelScheduledInstanceStateChange(Context ctx, AlarmInstance instance) {
        ...
        /* SPRD: bug 474360 add poweronoff alarm @{ */
        AlarmShutDownManager.deleteShutDownFile();
        /* @} */
    }
    ...
    public static void setFiredState(Context context, AlarmInstance instance) {
         ...
         */
        Calendar timeout;
        if (AplicationSupportChange.SUPPORT_ALARM_ALERT_LONG) {
            timeout = instance.getTimeout(context,instance);
        } else {
            timeout = instance.getTimeout(context);
        }
        /* @} */
        if (timeout != null) {
    ...
    public static void setPreDismissState(Context context, AlarmInstance instance) {
        ...
        // SPRD: Bug 543517 when dismiss the alarm from notification, the indicator icon is still in the statusbar
        updateNextAlarm(context);
    }
    ...
    public static void registerInstance(Context context, AlarmInstance instance,
         ...
         */
        Calendar timeoutTime;
        if (AplicationSupportChange.SUPPORT_ALARM_ALERT_LONG) {
            timeoutTime = instance.getTimeout(context,instance);
        } else {
            timeoutTime = instance.getTimeout(context);
        }
        /* @} */
    ...
    private void handleIntent(Context context, Intent intent) {
        ...
        if (CHANGE_STATE_ACTION.equals(action)) {
            ...
            /* SPRD: Bug 527529 modify the date to tomorrow, the alarm will fire and can not be close @{ */
            final Calendar alarmTime = instance.getAlarmTime();
            final Calendar currentTime = getCurrentTime();
            Calendar alarmBuffer = Calendar.getInstance();
            alarmBuffer.setTime(alarmTime.getTime());
            ...
            if (intentId != globalId || (AlarmInstance.FIRED_STATE == alarmState && currentTime.after(alarmBuffer))) {
            ...
            /* SPRD: Bug 515840 when playing video, alarm will not ring after arrival @{ */
            if (AlarmStreamMediaPlugin.getInstance(context).alarmStreamMediaInteract(alarmState)) {
                Vibrator vibrator = (Vibrator) context.getSystemService(Service.VIBRATOR_SERVICE);
                vibrator.vibrate(3000);
                mForLiveInstance = instance;
                return;
            }
            /* @} */
            ...
            /* SPRD bug 500092 when alarm ring, press volume button, alarm will not off or pause @{ */
            if (AlarmInstance.FIRED_STATE == alarmState) {
                mForVolumeInstance = instance;
            }
            /* @} */
            ...
            setDismissState(context, instance);
        /* SPRD bug 500092 when alarm ring, press volume button, alarm will not off or pause @{ */
        } else if (intent.getAction().equals("com.sprd.deskclock.volume_behavior")) {
            if (mForVolumeInstance == null) {
                LogUtils.e("mForVolumeInstance = null ");
                return;
            }
            String mVolumeBehavior = PreferenceManager
                    .getDefaultSharedPreferences(context).getString(
                            SettingsActivity.KEY_VOLUME_BEHAVIOR,
                            SettingsActivity.DEFAULT_VOLUME_BEHAVIOR);
            LogUtils.d("mVolumeBehavior broadcast:" + mVolumeBehavior);
            switch (mVolumeBehavior) {
            case SettingsActivity.VOLUME_BEHAVIOR_SNOOZE:
                setSnoozeState(context, mForVolumeInstance, true);
                break;
            case SettingsActivity.VOLUME_BEHAVIOR_DISMISS:
                setDismissState(context, mForVolumeInstance);
                break;
            default:
                break;
            }
            mForVolumeInstance = null;
        /* @} */

        /* SPRD: Bug 515840 when playing video, alarm will not ring after arrival @{ */
        } else if (MOVIE_ONPAUSE.equals(intent.getAction())) {
            if (mForLiveInstance == null) {
                Log.e("AlarmStateManager","handleIntent()---mForLiveInstance = null");
                return;
            }
            setFiredState(context, mForLiveInstance);
            mForLiveInstance = null;
        }
        /* @} */
    }
    ...
    private static class AlarmManagerStateChangeScheduler implements StateChangeScheduler {
        ...
        public void scheduleInstanceStateChange(Context context, Calendar time,
            ...
            if (Utils.isKitKatOrLater()) {
                ...
                /* SPRD: bug 474360 add poweronoff alarm @{ */
                AlarmShutDownManager.createOrUpdateShutDownFile(context, instance, am, newState, true);
                /* @} */
            } else {
                am.set(AlarmManager.RTC_WAKEUP, timeInMillis, pendingIntent);
                /* SPRD: bug 474360 add poweronoff alarm @{ */
                AlarmShutDownManager.createOrUpdateShutDownFile(context, instance, am, newState, false);
                /* @} */
            }
        }
        ...
        public void cancelScheduledInstanceStateChange(Context context, AlarmInstance instance) {
            ...
            /* SPRD: Bug 520813 set two alarms, dismiss the first, shutdown the device, the second will not fire as a poweroff alarm @{ */
            updatePowerOffAlarm(context, instance);
            /* @} */
        }
    }

    public static void updatePowerOffAlarm(Context context, AlarmInstance instance) {
        final ContentResolver cr = context.getContentResolver();
        final String activeAlarmQuery = AlarmInstance.ALARM_STATE + "<" + AlarmInstance.FIRED_STATE;
        final List<AlarmInstance> alarmInstances = AlarmInstance.getInstances(cr, activeAlarmQuery);

        for (AlarmInstance alarmInstance : alarmInstances) {
            if (alarmInstance!= null && alarmInstance.getAlarmTime().compareTo(instance.getAlarmTime()) != 0) {
                final AlarmManager am = (AlarmManager) context.getSystemService(Context.ALARM_SERVICE);
                AlarmShutDownManager.createOrUpdateShutDownFile(context, alarmInstance);
                AlarmShutDownManager.createOrUpdateShutDownFile(context, alarmInstance, am, alarmInstance.mAlarmState, Utils.isKitKatOrLater());
            }
        }
    }
}
