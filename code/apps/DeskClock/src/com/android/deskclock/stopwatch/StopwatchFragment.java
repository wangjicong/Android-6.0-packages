...
public class StopwatchFragment extends DeskClockFragment
    ...
    public static boolean mShowContinue = false;
    ...
    private void toggleStopwatchState() {
        ...
        switch (mState) {
            case Stopwatches.STOPWATCH_RUNNING:
                ...
                releaseWakeLock();
                /* SPRD : bug 473433 add stopwatch stop function @{ */
                if (AplicationSupportChange.SUPPORT_STOPWATCH_CMCC) {
                    mShowContinue = true;
                }
                /* @} */
                break;
            ...
            case Stopwatches.STOPWATCH_STOPPED:
                // do start
                /* SPRD: bug 473433 add stopwatch stop function @{ */
                StopwatchService.toApps = false;
                LogUtils.v("STOPWATCHFRAGMENT.END LAPS-LIST DISAPPEARING");
                if (!mShowContinue
                       && AplicationSupportChange.SUPPORT_STOPWATCH_CMCC) {
                    doReset();
                    intent.putExtra("reset_and_start", true);
                }
                /* @} */
                doStart(time);
    ...
    /* SPRD: bug 473433 add stopwatch stop function @{ */
        if (AplicationSupportChange.SUPPORT_STOPWATCH_CMCC
            ...
            Context context = getActivity().getApplicationContext();
            Intent intent= new Intent(context, StopwatchService.class);
            intent.putExtra(STOP_BUTTON_CLICK, true);
            intent.putExtra(Stopwatches.MESSAGE_TIME, time);
            intent.putExtra(Stopwatches.SHOW_NOTIF, false);
            long curTime = Utils.getTimeNow();
            mAccumulatedTime += (curTime - mStartTime);
    ...
    public void setLeftRightButtonAppearance() {
        ...
        switch (mState) {
            ...
            case Stopwatches.STOPWATCH_RUNNING:
                ...
                /* SPRD: bug 473433 add stopwatch stop function @{ */
                if (AplicationSupportChange.SUPPORT_STOPWATCH_CMCC) {
                    mRightButton.setImageResource(R.drawable.ic_stop);
                    mRightButton
                        .setContentDescription(getString(R.string.sw_stop_button));
