...
public class ClockFragment extends DeskClockFragment implements OnSharedPreferenceChangeListener {
    ...
    public static final String DELETE_CITY ="android.intent.action.DELETECITY";
    public static final String REORDE_CITY ="android.intent.action.REORDECITY";
    /* @} */
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (menuButton != null) {
            ...
            /* SPRD: bug 474598 WorldClock add multi-select delete and drag sorting function @{ */
            if (AplicationSupportChange.SUPPORT_WORLD_CLOCK_RECORDER
                    || AplicationSupportChange.SUPPORT_WORLD_CLOCK_DELETE) {
    ...
    public void setupFakeOverflowMenuButton(View menuButton) {
        fakeOverflow = new PopupMenu(menuButton.getContext(), menuButton) {
            @Override
            public void show() {
                Menu selfMenu = getMenu();
                onPrepareOptionsMenu(selfMenu);
                getActivity().onPrepareOptionsMenu(selfMenu);
                super.show();
            }
        };
        if (AplicationSupportChange.SUPPORT_WORLD_CLOCK_DELETE
                || AplicationSupportChange.SUPPORT_WORLD_CLOCK_RECORDER) {
            fakeOverflow.inflate(R.menu.desk_clock_menu_sprd);
        } else {
            ...
            public boolean onMenuItemClick(MenuItem item) {
                switch (item.getItemId()) {
                    ...
                    case R.id.menu_item_delete_city:
                        startDeleteCity();
                        return true;
                    case R.id.menu_item_reorder_city:
                        startWCReorderActivity();
                        return true;
                    default:
                        break;
                }
    ...
    public void onPrepareOptionsMenu(Menu menu) {
        MenuItem deleteCity = menu.findItem(R.id.menu_item_delete_city);
        MenuItem reorderCity = menu.findItem(R.id.menu_item_reorder_city);
        if(AplicationSupportChange.SUPPORT_WORLD_CLOCK_DELETE && deleteCity != null){
            if (mAdapter.getCitiesListCount() > 0) {
                if (mAdapter.getCitiesListCount() == 1 && mAdapter.hasHomeCity()) {
        ...
        if(AplicationSupportChange.SUPPORT_WORLD_CLOCK_RECORDER && reorderCity != null){
            if (mAdapter.getCitiesListCount() > 1) {
                reorderCity.setVisible(true);
            } else {
                reorderCity.setVisible(false);
            }
        } else {
            if(reorderCity != null){
                reorderCity.setVisible(false);
    ...
    public void startDeleteCity(){
        Intent intent = new Intent(getActivity(), DeleteCity.class);
        intent.setAction(ClockFragment.DELETE_CITY);
        startActivity(intent);
    }

    public void startWCReorderActivity(){
        Intent intent = new Intent(getActivity(), WCReorderActivity.class);
        intent.setAction(ClockFragment.REORDE_CITY);
        startActivity(intent);
    }
    /* @} */
}
