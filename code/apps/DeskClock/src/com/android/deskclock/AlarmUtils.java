...
public class AlarmUtils {
    ...
    public static void showTimeEditDialog(FragmentManager manager, final Alarm alarm,
        ...
        /* SPRD Bug 493440 IllegalStateException, can not perform this action after onSaveInstanceState @{
         * @orig
        ft.commit();
         */
    ...
    public static void showTimeEditDialog(Fragment fragment, final Alarm alarm) {
        ...
        /* SPRD: Bug 520385,513332,509968,522260 AlarmUtils.showTimeEditDialog, java.lang.IllegalStateException @{ */
        if (timePickerFragment != null && !timePickerFragment.isAdded()) {
            timePickerFragment.showAllowingStateLoss(manager, FRAG_TAG_TIME_PICKER);
        }
        /* @} */
    }

    /* SPRD: bug 473507 add auto silences for each alarm @{ */
    public static void showAutoSilenceDialog(Alarm alarm, Fragment fragment) {
        final FragmentManager manager = fragment.getFragmentManager();
        final FragmentTransaction ft = manager.beginTransaction();
        final Fragment prev = manager.findFragmentByTag(FRAG_TAG_SLIENCE_DIALOG);
        if (prev != null) {
            ft.remove(prev);
        }
        ft.commitAllowingStateLoss();
        AlarmSlienceDialog slienceDialogFragment = AlarmSlienceDialog.newInstance(alarm);
        slienceDialogFragment.setTargetFragment(fragment, 0);
        if (slienceDialogFragment != null && !slienceDialogFragment.isAdded()) {
            slienceDialogFragment.showAllowingStateLoss(manager, FRAG_TAG_SLIENCE_DIALOG);
        }
    }
    /* @} */
