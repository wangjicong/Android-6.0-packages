...
public final class DaysOfWeek {
    ...
    private static int mBitSetFotTemp;
    ...
    /* SPRD: bug 473670 new feature add repeat preference for each alarm @{ */
    public void setDaysOfWeekForTemp(boolean value, int... daysOfWeek) {
        for (int day : daysOfWeek) {
            setBitForTemp(convertDayToBitIndex(day), value);
        }
    }

    private void setBitForTemp(int bitIndex, boolean set) {
        if (set) {
            mBitSetFotTemp |= (1 << bitIndex);
        } else {
            mBitSetFotTemp &= ~(1 << bitIndex);
        }
    }

    public int getBitSetForTemp() {
        return mBitSetFotTemp;
    }

    public void setBitSetForTemp(int bitSet) {
        mBitSetFotTemp = bitSet;
    }

    public HashSet<Integer> getSetDaysForTemp() {
        final HashSet<Integer> result = new HashSet<Integer>();
        for (int bitIndex = 0; bitIndex < DAYS_IN_A_WEEK; bitIndex++) {
            if (isBitEnabledForTemp(bitIndex)) {
                result.add(convertBitIndexToDay(bitIndex));
            }
        }
        return result;
    ...
    private boolean isBitEnabledForTemp(int bitIndex) {
        return ((mBitSetFotTemp & (1 << bitIndex)) > 0);
    }
    /* @} */
}
