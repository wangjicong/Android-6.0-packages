...
public class Cities {
    ...
    public static  HashMap<String, CityObj> readCitiesFromSharedPrefs(SharedPreferences prefs) {
         ...
         */
        HashMap<String, CityObj> c = new LinkedHashMap<String, CityObj>();
        /* @} */
        if (size > 0) {
    ...
    /* SPRD: bug 474598 WorldClock add multi-select delete and drag sorting function @{ */
    public static void saveCitiesToSharedPrefs(SharedPreferences prefs, ArrayList<CityObj> cities) {
        SharedPreferences.Editor editor = prefs.edit();
        int len = cities.size();
        editor.putInt(NUMBER_OF_CITIES, len);
        for (int i = 0; i < len; i++) {
            CityObj c = cities.get(i);
            c.saveCityToSharedPrefs(editor, i);
        }
        editor.apply();
    }

    public static ArrayList<CityObj> getCitiesListFromPrefs(SharedPreferences prefs) {
        int size = prefs.getInt(NUMBER_OF_CITIES, -1);
        ArrayList<CityObj> citiesList = new ArrayList<CityObj>();
        if (size > 0) {
            for (int i = 0; i < size; i++) {
                CityObj city = new CityObj(prefs, i);
