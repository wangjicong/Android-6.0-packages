...
public class WorldClockAdapter extends BaseAdapter {
    ...
    public void loadData(Context context) {
        ...
        if (AplicationSupportChange.SUPPORT_WORLD_CLOCK_RECORDER) {
            mCitiesList = Cities.getCitiesListFromPrefs(prefs).toArray();
        } else {
            mCitiesList = Cities.readCitiesFromSharedPrefs(prefs).values().toArray();
            sortList();
        }
    ...
    private void updateView(View clock, CityObj cityObj) {
        ...
        String worldName = Utils.getCityName(cityObj, cityInDb);
        int index = worldName.indexOf("/");
        if (index < 0) {
            name.setText(worldName);
        } else {
            name.setText(worldName.substring(0, index));
        }
        /* @} */
