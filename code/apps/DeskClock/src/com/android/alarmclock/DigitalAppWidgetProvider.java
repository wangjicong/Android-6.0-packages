...
import com.sprd.android.config.OptConfig;//SUN:jicong.wang 
...
public class DigitalAppWidgetProvider extends AppWidgetProvider {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (ACTION_ON_QUARTER_HOUR.equals(action)
            ...
            if (appWidgetManager != null) {
                ...
                for (int appWidgetId : appWidgetIds) {
                    ...
                    // SPRD: Bug 519250 add am/pm for widget
                    WidgetUtils.setTimeFormat(context, widget, (int)context.getResources().getDimension(R.dimen.widget_label_font_size), R.id.the_clock);
		...
		/*hsj add 20160310 @{ */
        else if (action.equals("android.intent.action.DATEFORMAT_SET")){
            ...
            if (appWidgetManager != null) {
                ...
                for (int appWidgetId : appWidgetIds) {
                    RemoteViews widget = new RemoteViews(context.getPackageName(),
                            R.layout.digital_appwidget);
                    refreshDateFormat(context, widget);
                    appWidgetManager.partiallyUpdateAppWidget(appWidgetId, widget);
                }
            }
        }
    ...
    private void updateClock(
        ...
        /*SUN:jicong.wang add for date pattern start {@ */     
        if (OptConfig.SUN_SUBCUSTOM_C7367_HWD_FWVGA_R6_WINDS){
            final CharSequence dateFormat_sunvov =  "EEEE, dd MMMM, yyyy";
            widget.setCharSequence(R.id.date, "setFormat12Hour", dateFormat_sunvov);
            widget.setCharSequence(R.id.date, "setFormat24Hour", dateFormat_sunvov);
            widget.setBoolean(R.id.date, "setAllCaps", false);
        } else {
            widget.setCharSequence(R.id.date, "setFormat12Hour", dateFormat);
            widget.setCharSequence(R.id.date, "setFormat24Hour", dateFormat);
        }
        /*SUN:jicong.wang add for date pattern end @}*/        
	...
	/*hsj add 20160310 @{ */
    protected void refreshDateFormat(Context context, RemoteViews widget) {
        final CharSequence dateFormat = DateFormat.getBestDateTimePattern(Locale.getDefault(),
                                            context.getString(R.string.abbrev_wday_month_day_no_year));
