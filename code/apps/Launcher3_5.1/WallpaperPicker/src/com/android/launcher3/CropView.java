...
public class CropView extends TiledImageView implements OnScaleGestureListener {
    ...
    public RectF getCrop() {
        ...
        // SPRD: bug383081 2014-12-19 Bugfix wrong crop rect cause crash.
        return adjustCropRect(new RectF(cropLeft, cropTop, cropRight, cropBottom));
    }
    ...
    /* SPRD: bug383081 2014-12-19 Bugfix wrong crop rect cause crash. @{ */
    // limit the crop rect within the size of source picture, or it will cause
    // decode exception when try to decode region beyond the size of source.
    private RectF adjustCropRect(RectF cropRect) {

        Point sourceSize = getSourceDimensions();
        ...
        if (cropRect.left >= sourceWidth) {
            cropRect.left = sourceWidth - 0.01f;
        }

        // right->(0, width]
        if (cropRect.right <= 0) {
            cropRect.right = 0.01f;
        }
        if (cropRect.right > sourceWidth) {
            cropRect.right = sourceWidth;
        }
        ...
        if (cropRect.top >= sourceHeight) {
            cropRect.top = sourceHeight - 0.01f;
        }

        // bottom->(0, height]
        if (cropRect.bottom <= 0) {
            cropRect.bottom = 0.01f;
        }
        if (cropRect.bottom > sourceHeight) {
            cropRect.bottom = sourceHeight;
        }
