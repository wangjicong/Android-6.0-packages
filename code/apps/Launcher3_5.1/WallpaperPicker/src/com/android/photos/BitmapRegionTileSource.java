...
class DumbBitmapRegionDecoder implements SimpleBitmapRegionDecoder {
    ...
    // cache object to avoid create new instance frequently.
    BitmapFactory.Options mDecodeOption = new BitmapFactory.Options();
    private static final String TAG = "DumbBitmapRegionDecoder";
    ...
    private DumbBitmapRegionDecoder(Context context, Uri uri, int width, int height, int finalSampleSize) {
        mUri = uri;
        mContext = context;
        mWidth = width;
        mHeight = height;
        mDecodeOption.inSampleSize = finalSampleSize;
    }

    private DumbBitmapRegionDecoder(String path, int width, int height, int finalSampleSize) {
        mFilePath = path;
        mWidth = width;
        mHeight = height;
        mDecodeOption.inSampleSize = finalSampleSize;
    ...
    public static DumbBitmapRegionDecoder newInstance(String pathName) {
        // Hold a bitmap will cause OOM when the picture is is very large.
        BitmapFactory.Options opts = Utils.getPropertyBitmapFactoryOptions(
                null, null, pathName, null, 0, null, 1);
        // SPRD: bug381427 2014-12-15 Bugfix avoid carsh when picture has wrong size.
        if (opts != null && opts.outWidth > 0 && opts.outHeight > 0) {
            return new DumbBitmapRegionDecoder(pathName, opts.outWidth, opts.outHeight, opts.inSampleSize);
        }
    ...
    public static DumbBitmapRegionDecoder newInstance(InputStream is) {
        // Hold a very big bitmap in memory will cause OOM. We will regenerate the
        // bitmap every time if the picture be handled refer to a uri.
        // Note: pictures refer to resource id not affected for we assume nobody
        // will compile a very big picture to the resource.
    ...
    public static DumbBitmapRegionDecoder newInstance(Context context, Uri uri) {
        BitmapFactory.Options opts = Utils.getPropertyBitmapFactoryOptions(
                context, uri, null, null, 0, null, 1);
        // SPRD: bug381427 2014-12-15 Bugfix avoid carsh when picture has wrong size.
        if (opts != null && opts.outWidth > 0 && opts.outHeight > 0) {
            return new DumbBitmapRegionDecoder(context, uri, opts.outWidth,
    ...
    public Bitmap decodeRegion(Rect wantRegion, BitmapFactory.Options options) {
        ...
        Bitmap origin = decodeOriginBitmap();
        if (origin == null) {
            mTempCanvas.restore();
        ...
        mTempCanvas.drawBitmap(origin, -wantRegion.left, -wantRegion.top, mTempPaint);
        origin.recycle();
    ...
    /**
     * Note: this function always try to decode the origin bitmap in-memory. We
     * only handle picture that refer to a uri or a file path, because we assume
     * these two types of pictures will hold a very big bitmap. I don't think
     * any one will compile a very big picture into the resources.
     *
     * @return Bitmap the in-memory bitmap or null.
     */
    private Bitmap decodeOriginBitmap() {
        ...
        if (mUri != null && mContext != null) {
            ...
            try {
                ...
                Log.w(TAG, "file not found, uri: " + mUri);
            }
            if (is != null) {
                is = new BufferedInputStream(is);
                result = BitmapFactory.decodeStream(is, null, mDecodeOption);
                Utils.closeSilently(is);
            }
        } else if (mFilePath != null) {
            result = BitmapFactory.decodeFile(mFilePath, mDecodeOption);
        }
        return result;
    }
}
...
public class BitmapRegionTileSource implements TiledImageRenderer.TileSource {
    ...
    public static abstract class BitmapSource {
        ...
        // SPRD: bug375727 2014-12-05 Bugfix avoid OOM when handle in-memory decode.
        protected boolean mIsInMemoryDecoder;
        ...
        /* SPRD: bug375727 2014-12-05 Bugfix avoid OOM when handle in-memory decode. @{ */
        public boolean isInMemoryDecoderInternally() {
            return mIsInMemoryDecoder;
        }
        /* SPRD: bug375727 2014-12-05 Bugfix avoid OOM when handle in-memory decode. @} */
    }
    ...
    public static class UriBitmapSource extends BitmapSource {
        ...
        public SimpleBitmapRegionDecoder loadBitmapRegionDecoder() {
            try {
                ...
                if (regionDecoder == null) {
                    ...
                    Utils.closeSilently(is);
                    */
                    regionDecoder = DumbBitmapRegionDecoder.newInstance(mContext, mUri);
                    mIsInMemoryDecoder = true;
                    /* SPRD: bug375727 2014-12-05 Bugfix avoid OOM when handle in-memory decode. @} */
                }
        ...
        public Bitmap loadPreviewBitmap(BitmapFactory.Options options) {
            /* SPRD: bug375727 2014-12-05 Bugfix avoid OOM when handle in-memory decode. @{ */
            // only apply the following logic when use in-memory decoder.
            if (isInMemoryDecoderInternally()) {
                BitmapFactory.Options opts = Utils.getPropertyBitmapFactoryOptions(
                        mContext, mUri, null, null, 0, null, 1);
                // SPRD: bug381427 2014-12-15 Bugfix avoid carsh when picture has wrong size.
                if (opts != null && opts.outWidth > 0 && opts.outHeight > 0) {
    ...
    public BitmapRegionTileSource(Context context, BitmapSource source) {
        ...
        if (mDecoder != null) {
            ...
            if (previewSize != 0) {
                ...
                /* SPRD: bug381427 2014-12-15 Bugfix avoid carsh when picture has wrong size. @{ */
                if (preview == null) {
                    Log.w(TAG, "preview is null, just return.");
                    return;
                }
                /* SPRD: bug381427 2014-12-15 Bugfix avoid carsh when picture has wrong size. @} */
