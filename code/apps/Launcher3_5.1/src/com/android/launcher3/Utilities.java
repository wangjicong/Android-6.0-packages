...
public final class Utilities {
    ...
    static Bitmap createIconBitmap(String packageName, String resourceName, IconCache cache,
        ...
        try {
            ...
            if (resources != null) {
                ...
                if(false/*OptConfig.SUNVOV_MENU_ADD_BG*/){
                }else{
    ...
    public static Bitmap createIconBitmap(Drawable icon, Context context) {
        synchronized (sCanvas) { // we share the statics :-(
            ...
            if(true/*OptConfig.SUNVOV_C7359_C5D_JIVI*/){
                int resize = 4;
                icon.setBounds(left+resize, top+resize, left+width-resize, top+height-resize);
            }else{
    ...
    /* Kalyy add for menu bg */
    public static Bitmap createIconBitmapWithBg(Drawable icon, Context context) {
        synchronized (sCanvas) { // we share the statics :-(
            if (sIconWidth == -1) {
                initStatics(context);
            }
            ...
            Drawable icon_bg = context.getResources().getDrawable(R.drawable.ic_menu_background);
            icon_bg.setBounds(0, 0, sIconWidth, sIconHeight);
            icon_bg.draw(canvas);
            final int left = (textureWidth-width) / 2;
            final int top = (textureHeight-height) / 2;
            ...
            sOldBounds.set(icon.getBounds());
            sIconMargin = 10;
            icon.setBounds(left+sIconMargin, top+sIconMargin, left+width-sIconMargin, top+height-sIconMargin);
            icon.draw(canvas);
            icon.setBounds(sOldBounds);
            canvas.setBitmap(null);
    ...
    public static Bitmap createIconBitmap(Drawable icon, Context context,int icon_resize) {
        synchronized (sCanvas) { // we share the statics :-(
            if (sIconWidth == -1) {
                initStatics(context);
            }
            ...
            sOldBounds.set(icon.getBounds());
            //if(OptConfig.SUNVOV_C7359_C5D_JIVI){
                icon_resize += 2;
            //}
            icon.setBounds(left+icon_resize, top+icon_resize, left+width-icon_resize, top+height-icon_resize);
            icon.draw(canvas);
            icon.setBounds(sOldBounds);
            canvas.setBitmap(null);
