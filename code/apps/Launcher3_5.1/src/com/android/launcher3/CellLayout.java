...
public class CellLayout extends ViewGroup {
    ...
    void visualizeDropLocation(View v, Bitmap dragOutline, int originX, int originY, int cellX,
        ...
        //SPRD:fix bug 458727,java.lang.NullPointerException
        ...
        for (int y = 0; y < countY - (spanY - 1); y++) {
            ...
            for (int x = 0; x < countX - (spanX - 1); x++) {
                ...
                boolean directionMatches = direction[0] == curDirection[0] &&
                        direction[1] == curDirection[1]; //bug409934 coverity problem.
...
out:            for (int i = x; i < x + spanX - 1 && x < xCount; i++) {
    ...
    public void registerDragEnforcerInstance(DragController dragController) {
        if (dragController != null && mDragEnforcer != null) {
            // first unregister the mDragEnforcer instance if it exists.
            unregisterDragEnforcerInstance(dragController);
            dragController.addDragListener(mDragEnforcer);
        }
    }

    public void unregisterDragEnforcerInstance(DragController dragController) {
        if (dragController != null) {
            dragController.removeDragListener(mDragEnforcer);
        }
    }
    /* SPRD: bug397326 2015-01-22 Bugfix DragEnforcer instance leak. @} */
}
