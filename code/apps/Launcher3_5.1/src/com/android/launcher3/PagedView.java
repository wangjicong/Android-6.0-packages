...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener,
    ...
    /* SPRD: bug371310 2014-11-19 Feature circular sliding. @{ */
    protected boolean mIsSupportCircular;
    protected static final int OVER_FIRST_PAGE_INDEX = -2; // for -1 is occupied by INVALID_PAGE;
    private static final int OVER_FIRST_PAGE_SCROLL_INDEX = 0;
    private static final int OVER_LAST_PAGE_SCROLL_INDEX = 1;
    ...
    protected void init() {
        ...
        // SPRD: bug371310 2014-11-19 Feature circular sliding.
        mIsSupportCircular = LauncherAppState.getInstance().getCircularSlidingStatus();
    }
    ...
    protected boolean computeScrollHelper() {
        if (mScroller.computeScrollOffset()) {
            ...
            /* SPRD: bug371310 2014-11-19 Feature circular sliding. @{ */
            // SPRD: bug376770 2014-12-04 Bugfix avoid NPE.
            if (!mIsSupportCircular || (mNextPage != OVER_FIRST_PAGE_INDEX && mNextPage != getPageCount())
                    || (mPageScrolls == null)) {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        int overPageIndex = OVER_FIRST_PAGE_SCROLL_INDEX;
        if (isRtl) {
            // calculate over last page
            overPageIndex = OVER_LAST_PAGE_SCROLL_INDEX;
        }
        ...
        if (isRtl) {
            ...
            overPageIndex = OVER_FIRST_PAGE_SCROLL_INDEX;
        }
        // here we assume that one page occupied width is equals to view port's width.
    ...
    protected void screenScrolled(int screenCenter) {
        ...
        // SPRD: bug371310 2014-11-19 Feature circular sliding.
        isInOverscroll = isInOverscroll && !mIsSupportCircular;
    ...
    public int getScrollForPage(int index) {
        ...
        if (mIsSupportCircular) {
            if (index == OVER_FIRST_PAGE_INDEX) {
                return mPageScrollsForCircleSlide[OVER_FIRST_PAGE_SCROLL_INDEX];
            } else if (index == getChildCount()){
                return mPageScrollsForCircleSlide[OVER_LAST_PAGE_SCROLL_INDEX];
            }
        }
        /* SPRD: bug371310 2014-11-19 Feature circular sliding. @} */
            ...
            if (mTouchState == TOUCH_STATE_SCROLLING) {
                ...
                if (!mFreeScroll) {
                    ...
                    if (((isSignificantMove && !isDeltaXLeft && !isFling) ||
                        ...
                        if (finalPage == -1) {
                            finalPage = OVER_FIRST_PAGE_INDEX;
                        }
    ...
    protected void snapToPageWithVelocity(int whichPage, int velocity) {
        ...
        if (!mIsSupportCircular) {
            ...
            whichPage = validateNewPageWithSupportCircularSliding(whichPage);
        }
        /* SPRD: bug371310 2014-11-19 Feature circular sliding. @} */
    ...
    private void drawCircularPage(Canvas canvas) {
        final boolean isRtl = isLayoutRtl();
        boolean isXBeforeFirstPage = isRtl ? (mOverScrollX > mMaxScrollX) : (mOverScrollX < 0);
        boolean isXAfterLastPage = isRtl ? (mOverScrollX < 0) : (mOverScrollX > mMaxScrollX);
        if (isXBeforeFirstPage || isXAfterLastPage) {
            ...
            int offset = (isRtl ? - childCount : childCount) * (width + mPageSpacing);
            if (isXBeforeFirstPage) {
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
        if (LauncherAppState.getCircularSlidingStatusPreferenceKey().equals(key)) {
            mIsSupportCircular = LauncherAppState.getInstance().getCircularSlidingStatus();
        }
    }

    private void registerOnSharedPreferenceChangeListener() {
        SharedPreferences sp = LauncherAppState.getInstance().getContext().getSharedPreferences(
    ...
    protected int validateNewPageWithSupportCircularSliding(int newPage) {
        int validatedPage = newPage;
        if (mIsSupportCircular) {
            // whichPage can be OVER_FIRST_PAGE_INDEX or [0, count]
            if (!(validatedPage == OVER_FIRST_PAGE_INDEX)) {
