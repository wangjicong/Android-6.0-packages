...
public class LauncherModel extends BroadcastReceiver
    ...
    // SPRD: bug374163 2014-11-26 Feature dynamic change switch.
    static boolean DEBUG_LOADERS = false;
    ...
    public interface Callbacks {
        ...
        public void updateAllApplications(ArrayList<AppInfo> apps);
    }
            ...
            if(c.moveToFirst()){
                ...
                if(c.moveToFirst()){
                    ...
                    do{
                        ...
                        Intent intent_Temp = Intent.parseUri(intentDescription, 0);
                        ComponentName cn = intent.getComponent();
                        ComponentName cn_temp = intent_Temp.getComponent();
                        if (cn.getPackageName() != null && cn.getClassName()!= null) {
                            if (cn.getPackageName().equals(cn_temp.getPackageName()) && cn.getClassName() .equals(cn_temp.getClassName())) {
                                isExist = true;
                                break;
                            }
                        }else if(cn.getPackageName() != null){
                             if (cn.getPackageName().equals(cn_temp.getPackageName())) {
                                 isExist = true;
                                 break;
                             }
                        }
    ...
    public void onPackagesAvailable(String[] packageNames, UserHandleCompat user,
            boolean replacing) {
        if (!replacing) {
            /* SPRD: bug470785, Bugfix, icon will be disabled when enable/disable usb storage multi-times. @{ */
            PackageUpdatedTask packageUpdatedTask = new PackageUpdatedTask(
                    PackageUpdatedTask.OP_ADD, packageNames, user);
            packageUpdatedTask.setPackageAvailableOperation(true);
            enqueuePackageUpdated(packageUpdatedTask);
            /* SPRD: bug470785, Bugfix, icon will be disabled when enable/disable usb storage multi-times. @} */
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        }else if(action.equals("launcher3.updateStkName.action")){//Kalyy 20160727
            //Log.d("Kalyy","STK 013");
            forceReload();
        }
    }
    ...
    private class LoaderTask implements Runnable {
        ...
        private boolean loadWorkspace() {
            ...
            synchronized (sBgLock) {
                ...
                if (itemsToRemove.size() > 0) {
                    ...
                    /* SPRD: bug377990 2014-12-06 Bugfix release ContentProviderClient after done. @{ */
                    if (client != null) {
                        client.release();
                    }
                    /* SPRD: bug377990 2014-12-06 Bugfix release ContentProviderClient after done. @} */
        ...
        private void bindWorkspaceItems(final Callbacks oldCallbacks,
            ...
            for (int i = 0; i < N; i++) {
                ...
                if (postOnMainThread) {
                    synchronized (deferredBindRunnables) { //bug409934 coverity problem.
                        deferredBindRunnables.add(r);
                    }
                } else {
                    runOnMainThread(r, MAIN_THREAD_BINDING_RUNNABLE);
    ...
    private class PackageUpdatedTask implements Runnable {
        ...
        // SPRD: bug470785, Bugfix, icon will be disabled when enable/disable usb storage multi-times.
        private boolean mIsPackageAvailableOperation = false;
        ...
        /* SPRD: bug470785, Bugfix, icon will be disabled when enable/disable usb storage multi-times. @{ */
        public void setPackageAvailableOperation(boolean yes) {
            mIsPackageAvailableOperation = yes;
        }
        /* SPRD: bug470785, Bugfix, icon will be disabled when enable/disable usb storage multi-times. @} */
                        ...
                        if (info instanceof ShortcutInfo && mUser.equals(info.user)) {
                            ...
                            if (cn != null && packageSet.contains(cn.getPackageName())) {
                                ...
                                if ((si.isDisabled & ShortcutInfo.FLAG_DISABLED_NOT_AVAILABLE) != 0
                                        || mIsPackageAvailableOperation) {
                                    // Since package was just updated, the target must be available now.
                                    // We need to update isDisabled until resume, make it take effect in the last runnable.
                                    si.updateStackSize = si.updateStackSize + 1;
                                    si.delayValue = si.isDisabled & ~ShortcutInfo.FLAG_DISABLED_NOT_AVAILABLE;
    ...
    /* SPRD: Feature 461130,Add the sort method to all application pages @{*/
    public class LauncherTimesComparator implements Comparator<AppInfo> {
        List<UsageStats> mUsageList;

        public LauncherTimesComparator() {
            Calendar cal = Calendar.getInstance();
            cal.add(Calendar.DAY_OF_YEAR, -5);
            UsageStatsManager usageStatsManager = (UsageStatsManager) mApp.getContext().getSystemService(
                    Context.USAGE_STATS_SERVICE);
            mUsageList = usageStatsManager.queryUsageStats(UsageStatsManager.INTERVAL_BEST, cal.getTimeInMillis(),
                    System.currentTimeMillis());
        }

        @Override
        public int compare(AppInfo aStats, AppInfo bStats) {
            String aName = aStats.componentName.getPackageName();
            String bName = bStats.componentName.getPackageName();

            try {
                UsageStats aUsage = getUsage(aName, mUsageList);
                UsageStats bUsage = getUsage(bName, mUsageList);
                if (aUsage != null && bUsage != null) {
                    if (aUsage.getPackageName() != null && bUsage.getPackageName() != null) {
                        Log.d(TAG, "get aUsage pkgName: " + aUsage.getPackageName() + ",aUsage.mLaunchCount is: "
                                + aUsage.mLaunchCount);
                        Log.d(TAG, "get bUsage pkgName: " + bUsage.getPackageName() + ",bUsage.mLaunchCount is: "
                                + bUsage.mLaunchCount);
                    }
                    if ((aUsage.mLaunchCount > bUsage.mLaunchCount)
                            || ((aUsage.mLaunchCount == bUsage.mLaunchCount)
                            && (aUsage.mTotalTimeInForeground > bUsage.mTotalTimeInForeground))) {
                        return -1;
                    } else if ((aUsage.mLaunchCount < bUsage.mLaunchCount)
                            || ((aUsage.mLaunchCount == bUsage.mLaunchCount)
                            && (aUsage.mTotalTimeInForeground < bUsage.mTotalTimeInForeground))) {
                        return 1;
                    } else {
                ...
                } else if (aUsage != null && bUsage == null) {
                    return -1;
                } else if (aUsage == null && bUsage != null) {
                    return 1;
                }
    ...
    public UsageStats getUsage(String pkgName, List<UsageStats> usageList) {
        UsageStats pkgUsage = null;
        if (usageList != null) {
            int size = usageList.size();
            for (int i = 0; i < size; i++) {
                String name = usageList.get(i).getPackageName();
                if (pkgName != null && pkgName.equals(name)) {
                    pkgUsage = usageList.get(i);
                    return pkgUsage;
                }
            }
        }
    ...
    public void sortApps(ArrayList<AppInfo> apps) {
        SharedPreferences prefs = mApp.getContext().getSharedPreferences(LauncherAppState.getSharedPreferencesKey(),
                Context.MODE_PRIVATE);
        int sortType = prefs.getInt(Launcher.SORT_TYPE_KEY, 0);
        sortApps(apps, sortType);
    }

    public void sortApps(ArrayList<AppInfo> apps, int sortType) {
        if (sortType == 0 || sortType == 3) {
            Collections.sort(apps, getAppNameComparator());
        } else if (sortType == 1) {
            Collections.sort(apps, APP_INSTALL_TIME_COMPARATOR);
        } else if (sortType == 2) {
            Collections.sort(apps, new LauncherTimesComparator());
        }
    }

    public void updateSortedApps(final int type) {
        final ArrayList<AppInfo> data = mBgAllAppsList.data;
        final Callbacks oldCallbacks = mCallbacks.get();
        runOnWorkerThread(new Runnable() {
            @Override
            public void run() {
                SharedPreferences.Editor editor = mApp.getContext()
                        .getSharedPreferences(LauncherAppState.getSharedPreferencesKey(), Context.MODE_PRIVATE).edit();
                editor.putInt(Launcher.SORT_TYPE_KEY, type).commit();
                sortApps(data, type);
                mHandler.post(new Runnable() {
                    public void run() {
                        final long bindTime = SystemClock.uptimeMillis();
                        ...
                        if (callbacks != null) {
                            callbacks.updateAllApplications(data);
                        } else {
                            Log.i(TAG, "not update apps: no Launcher activity");
                        }
                    }
                });
            }
    ...
    /* SPRD: bug377992 2014-12-06 Feature dynamic change switch. @{ */
    public static void changeDebugLoadersState(boolean enable) {
        DEBUG_LOADERS = enable;
        Log.d(TAG, "dynamically set DEBUG_LOADERS to " + DEBUG_LOADERS);
    }
    /* SPRD: bug377992 2014-12-06 Feature dynamic change switch. @} */
