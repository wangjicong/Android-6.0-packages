...
public class FmFavoriteActivity extends Activity {
     ...
     * @{
     */
    private MenuItem mMenuItemScanFromCurrent = null;
    /**
     * @}
     */
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.fm_station_list_refresh:
                 ...
                 *
                 */
                if (!FmUtils.hasLocationPermission(mContext)) {
                    mOptionSelectedId = R.id.fm_station_list_refresh;
                    String[] permissionsToRequest = new String[2];
                    permissionsToRequest[0] = Manifest.permission.ACCESS_COARSE_LOCATION;
                    permissionsToRequest[1] = Manifest.permission.ACCESS_FINE_LOCATION;
                ...
                if (null != mService) {
                    ...
                    /**
                     * SPRD: bug511231 Both emptyView and mSearchTips displayed when no station searched.
                     * @{
                     */
                ...
                break;
            /**
             * SPRD: bug474750, scan from current freq. bug511231
             *
             * @{
             */
            case R.id.scan_from_current:
                if (!FmUtils.hasLocationPermission(mContext)) {
                    mOptionSelectedId = R.id.scan_from_current;
                    String[] permissionsToRequest = new String[2];
                    permissionsToRequest[0] = Manifest.permission.ACCESS_COARSE_LOCATION;
                    permissionsToRequest[1] = Manifest.permission.ACCESS_FINE_LOCATION;
                    requestPermissions(permissionsToRequest, FmUtils.FM_PERMISSIONS_REQUEST_CODE);
                ...
                onOptionsItemSelectedScan(true /* fromCurrent */);
                break;
            /**
             * @}
             */
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
         ...
         * @{
         */
        mMenuItemScanFromCurrent = menu.findItem(R.id.scan_from_current);
        /**
         * @}
         */
    ...
    private void resetMenuTitleColor(boolean enabled) {
         ...
         * @{
         */
        SpannableString sfc = new SpannableString(mMenuItemScanFromCurrent.getTitle());
        sfc.setSpan(new ForegroundColorSpan(titleColor), 0, sfc.length(), 0);
        mMenuItemScanFromCurrent.setTitle(sfc);
        /**
         * @}
         */
    }
    ...
    private void refreshMenuItem(boolean enabled) {
         ...
         * @{
         */
        if (mMenuRefresh != null && mMenuItemScanFromCurrent != null) {
            mMenuRefresh.setEnabled(enabled);
            mMenuItemScanFromCurrent.setEnabled(enabled);
    ...
    private final ServiceConnection mServiceConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName className, IBinder service) {
            ...
            if (FmUtils.isFirstEnterStationList(mContext) || (0 == mMyAdapter.getCount())) {
                ...
                mSearchProgress.setIndeterminate(true);
                mMyAdapter.swipResult(null);
                //SPRD: bug535714, FM search station gain when change orientation.
        ...
        switch (requestCode) {
            case FmUtils.FM_PERMISSIONS_REQUEST_CODE: {
                ...
                if (resultsAllGranted) {
                    if(mOptionSelectedId == R.id.fm_station_list_refresh) {
                        onOptionsItemSelectedScan(false);
                    } else {
                        onOptionsItemSelectedScan(true);
                    }
                } else {
    ...
    private void onOptionsItemSelectedScan(boolean fromCurrent){
        if (null != mService) {
            ...
            if (fromCurrent) {
                mService.startScanAsyncFromCurrent(mService.getFrequency());
            } else {
                mService.startScanAsync();
            }
        }
