 ...
 * @}
 */
import android.provider.Settings.System;//Kalyy Bug49819
    ...
    */
    ...
    private class FmServiceBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            } else if (Intent.ACTION_AIRPLANE_MODE_CHANGED.equals(action)) {//Kalyy Bug49819
                // switch antenna should not impact audio focus status
                boolean isAirplane = isAirplaneMode();
                switchAirplaneAsync();
    ...
    private synchronized void startRender() {
        ...
        try {
            ...
            Log.e(TAG, "mAudioRecord or mAudioTrack stop error : " + e.getMessage());
        } finally {
            mAudioRecord = null;
            mAudioTrack = null;
        }
    ...
    private int setRds(boolean on) {
        ...
        if (isRdsSupported()) {
             ...
             * @{
             */
            ret = mFmManager.setRdsMode(on, false);
            /**
             * @}
             */
        }
    ...
    public int setMute(boolean mute) {
        ...
        if (mute) {
            ...
            int volume = mAudioManager.getStreamVolume(AudioManager.STREAM_FM);
            setVolume(volume);
        }
        /**
         * @}
    ...
    //Kalyy Bug49819
    public void switchAirplaneAsync() {
        final int bundleSize = 1;
        Bundle bundle = new Bundle(bundleSize);
        mFmServiceHandler.removeMessages(FmListener.MSGID_SWITCH_AIRPLANE);
        Message msg = mFmServiceHandler.obtainMessage(FmListener.MSGID_SWITCH_AIRPLANE);
		msg.setData(bundle);
        mFmServiceHandler.sendMessage(msg);
    }
    //Kalyy Bug49819
    ...
    private void registerFmBroadcastReceiver() {
        ...
        filter.addAction(Intent.ACTION_AIRPLANE_MODE_CHANGED);//Kalyy Bug49819
        /**
         * SPRD: bug492835, FM audio route change.
         * @{
         */
    ...
    private void showPlayingNotification() {
        ...
        /**
         * SPRD: bug513353, When the automatic station search, clear FM, enter FM again, all channels have noise.
         * @{
         */
        if(mIsScanning) {
            Notification n = new Notification.Builder(mContext).build();
            //n.flags &= ~Notification.FLAG_NO_CLEAR;
            Log.d(TAG, "startForeground showNotification but no icon");
            startForeground(1, n);
            return;
        }
        /**
    ...
    public boolean requestAudioFocus() {
         ...
         */
        int audioFocus = mAudioManager.requestAudioFocus(mAudioFocusChangeListener,
                AudioManager.STREAM_FM, AudioManager.AUDIOFOCUS_GAIN);
        /**
         * @}
         */
        ...
        if (null != stations) {
            ...
            if (mIsDistanceExceed) {
                ...
                for (int j = 0; j < searchedListSize; j++) {
                    ...
                    /**
                     * SPRD: bug474728, new feature, preset station. check the station is presetStation or not. bug490888
                     *
                     * Original Android code:
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case FmListener.MSGID_SWITCH_AIRPLANE:
                    bundle = msg.getData();
                    bundle.putInt(FmListener.CALLBACK_FLAG,
                        FmListener.MSGID_SWITCH_AIRPLANE);
                    notifyActivityStateChanged(bundle);
                    break;
                //Kalyy Bug49819
    ...
    private void handlePowerUp(Bundle bundle) {
        ...
        float curFrequency = bundle.getFloat(FM_FREQUENCY);
            ...
            if(isAirplaneMode()){
                bundle = new Bundle(2);
                bundle.putInt(FmListener.CALLBACK_FLAG, FmListener.MSGID_SWITCH_AIRPLANE);
                notifyActivityStateChanged(bundle);
                return;
            }
            //Kalyy Bug49819
