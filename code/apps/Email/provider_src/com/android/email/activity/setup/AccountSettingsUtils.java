...
public class AccountSettingsUtils {
    ...
    public static void commitSettings(Context context, Account account) {
        if (!account.isSaved()) {
            ...
            final SharedPreferences statePrefs = context.getSharedPreferences(
                    QuickResponsesCursorAdapter.QUICK_RESPONSE_PREF_STATE,
    ...
    public static ContentValues getAccountContentValues(Account account) {
        ...
        if(OptConfig.SUN_SUBCUSTOM_S7358_HWD_MITO_QHD){  
              cv.put(AccountColumns.SIGNATURE, "Sent from my MITO A69");
        //qiuyaobo,20160816,end
        }else if(OptConfig.SUN_SUBCUSTOM_C7356_TYC_S417_WVGA){  
              cv.put(AccountColumns.SIGNATURE, "Sent from RED-X Smartphone");
        }else if(OptConfig.SUN_SUBCUSTOM_C7367_HWD_FWVGA_R8_SINGTECH){
              cv.put(AccountColumns.SIGNATURE, "H8");
        }else if(OptConfig.SUN_SUBCUSTOM_C7367_HWD_QHD_R9_SINGTECH){
              cv.put(AccountColumns.SIGNATURE, "S2 PLUS");
        //qiuyaobo,20170424,begin      
        }else if(OptConfig.SUN_SUBCUSTOM_C7367_HWD_FWVGA_R3_SINGTECH){
              cv.put(AccountColumns.SIGNATURE, "S2"); 
        //qiuyaobo,20170424,end                   
        }else{ 
   ...
   /**
    ...
    /* SPRD:bug471289 use XML file customization for server address {@ */
