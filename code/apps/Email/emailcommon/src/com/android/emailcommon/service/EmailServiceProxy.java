...
public class EmailServiceProxy extends ServiceProxy implements IEmailService {
    ...
    public Bundle validate(final HostAuthCompat hostAuthCom) throws RemoteException {
        setTask(new ProxyTask() {
            @Override
            public void run() throws RemoteException{
                /* SPRD: bug495943, resolve IllegalException @{ */
                try {
                    LogUtils.i(TAG, "EmailServiceProxy: validate() hostAuthCom = " + hostAuthCom);
                    mReturn = mService.validate(hostAuthCom);
                } catch (IllegalArgumentException e) {
                    android.util.Log.i(TAG, "EmailServiceProxy: validate() exception: ");
                    e.printStackTrace();
                    mReturn = null;
                }
                /* @} */
    ...
    public Bundle autoDiscover(final String userName, final String password)
        ...
        setTask(new ProxyTask() {
            @Override
            public void run() throws RemoteException{
                // SPRD:MODIFY bug495943 add try exception for this
                try {
                    mReturn = mService.autoDiscover(userName, password);
                } catch (Exception e) {
                    LogUtils.w(TAG, "autoDiscover find an exception " + e.getMessage());
                }
            }
        }, "autoDiscover");
        waitForCompletion();
