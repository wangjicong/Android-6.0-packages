...
package com.sprd.music.lrc.widget;
...
import com.sprd.music.lrc.text.method.MovementMethod;
...
public class TextView extends View implements ViewTreeObserver.OnPreDrawListener {
    ...
    // How can we get this from the XML instead of hardcoding it here?
    ...
    /**
     * This flag is set if the TextView tries to display an error before it is
     * attached to the window (so its position is still unknown). It causes the
     * error to be shown later, when onAttachedToWindow() is called.
     */
    ...
    public TextView(Context context,
         ...
         * almost every TextView has one and it greatly simplifies the logic to
         * be able to parse the appearance first and then let specific tags for
         * this View override it.
         */
        ...
        /*
         * Views are not normally focusable unless specified to be. However,
         * TextViews that have input or movement methods *are* focusable by
         * default.
         */
     ...
     * Drawables must already have had {@link Drawable#setBounds} called.
     *
    ...
    public void setTextColorQuiet(int color) {
        mTextColor = ColorStateList.valueOf(color);
    ...
    public void setGravity(int gravity) {
        ...
        if ((gravity & Gravity.HORIZONTAL_GRAVITY_MASK) != (mGravity & Gravity.HORIZONTAL_GRAVITY_MASK)) {
            newLayout = true;
        }
    ...
    private void updateTextColors() {
        boolean inval = false;
        /*
         * SPRD: fix bug 274986 Coverity CID 70720 Reference is checked against
         * null but then dereferenced anyway @{
         */
        int color;
    ...
    public Parcelable onSaveInstanceState() {
        ...
        if (save) {
            ...
            if (mText instanceof Spanned) {
                /*
                 * Calling setText() strips off any ChangeWatchers; strip them
                 * now to avoid leaking references. But do it to a copy so that
                 * if there are any further changes to the text of this view, it
    ...
    private void setText(CharSequence text, BufferType type,
        ...
        if (text instanceof Spannable) {
            ...
            if (mMovement != null) {
                ...
                /*
                 * Initializing the movement method will have set the selection,
                 * so reset mSelectionMoved to keep that from interfering with
                 * the normal on-focus selection-setting.
                 */
    ...
    public final void setText(char[] text, int start, int len) {
        ...
        /*
         * We must do the before-notification here ourselves because if the old
         * text is a CharWrapper we destroy it before calling into the normal
         * path.
         */
        if (mText != null) {
            oldlen = mText.length();
    ...
    private static class CharWrapper
        ...
        public void drawTextRun(Canvas c, int start, int end, int contextStart, int contextEnd, float x,
                float y, boolean isRtl, Paint p) {
        };
        ...
        public int getTextRunCursor(int contextStart, int contextEnd,
                int flags, int offset, int cursorOpt, Paint p) {
            // TODO Auto-generated method stub
            return 0;
        }
    }
     ...
     * participate in determining the size of the view. This method is
     * deprecated. Use {link #setHint(int, String)} or {link
     * #setHint(CharSequence, String)} instead.
     *
    ...
    /**
     * Sets the text to be displayed when the text of the TextView is empty,
     * from a resource. This method is deprecated. Use {link #setHint(int,
     * String)} or {link #setHint(CharSequence, String)} instead.
     *
     * @attr ref android.R.styleable#TextView_hint
     */
    @android.view.RemotableViewMethod
     ...
     * {@link EditorInfo#IME_ACTION_NEXT EditorInfo.IME_ACTION_NEXT} or
     * {@link EditorInfo#IME_ACTION_DONE EditorInfo.IME_ACTION_DONE}.
     * <p>
    ...
    private void chooseSize(PopupWindow pop, CharSequence text, TextView tv) {
        ...
        /*
         * Figure out how big the text would be if we laid it out to the full
         * width of this view minus the border.
         */
    ...
    /**
     * Sets the list of input filters on the specified Editable, and includes
     * mInput in the list if it is an InputFilter.
     */
    ...
    private void invalidateCursorPath() {
        if (mHighlightPathBogus) {
            ...
            synchronized (sTempRect) {
                /*
                 * The reason for this concern about the thickness of the cursor
                 * and doing the floor/ceil on the coordinates is that some
                 * EditTexts (notably textfields in the Browser) have
                 * anti-aliased text where not all the characters are
                 * necessarily at integer-multiple locations. This should make
                 * sure the entire cursor gets invalidated instead of sometimes
                 * missing half a pixel.
                 */
    ...
    public boolean onPreDraw() {
        ...
        if (mMovement != null) {
            ...
            /*
             * TODO: This should really only keep the end in view if it already
             * was before the text changed. I'm not sure of a good way to tell
             * from here if it was.
             */
            if (curs < 0 &&
    ...
    public void invalidateDrawable(Drawable drawable) {
        if (verifyDrawable(drawable)) {
            ...
            // IMPORTANT: The coordinates below are based on the coordinates
            // computed
            // for each compound drawable in onDraw(). Make sure to update each
            // section
            // accordingly.
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (dr != null) {
            /*
             * Compound, not extended, because the icon is not clipped if the
             * text height is smaller.
             */
        ...
        /*
         * Comment out until we decide what to do about animations boolean
         * isLinearTextOn = false; if (currentTransformation != null) {
         * isLinearTextOn = mTextPaint.isLinearTextOn(); Matrix m =
         * currentTransformation.getMatrix(); if (!m.isIdentity()) { //
        ...
        /*
         * Comment out until we decide what to do about animations if
    ...
    private int doKeyDown(int keyCode, KeyEvent event, KeyEvent otherEvent) {
        ...
        if (mInput != null) {
            /*
             * Keep track of what the error was before doing the input so that
             * if an input filter changed the error, we leave that error
             * showing. Otherwise, we take down whatever error was showing when
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        ...
        switch (keyCode) {
            case KeyEvent.KEYCODE_DPAD_CENTER:
                 ...
                 * which will invoke it. If there isn't a click listener, try to
                 * show the soft input method. (It will also call
            ...
            case KeyEvent.KEYCODE_ENTER:
                ...
                if ((event.getFlags() & KeyEvent.FLAG_EDITOR_ACTION) != 0
                    ...
                    /*
                     * If there is a click listener, just call through to super,
                     * which will invoke it. If there isn't a click listener,
                     * try to advance focus, but still call through to super,
                     * which will reset the pressed state and longpress state.
                     * (It will also call performClick(), but that won't do
        ...
        if (ims != null) {
            ...
            if (contentChanged || ims.mSelectionModeChanged) {
                ...
                if (req != null) {
                    ...
                    if (imm != null) {
                        if (DEBUG_EXTRACT) {
                            Log.v(TAG, "Retrieving extracted start="
                        ...
                        if (extractTextInternal(req, ims.mChangedStart, ims.mChangedEnd,
                            ...
                            if (DEBUG_EXTRACT) {
                                Log.v(TAG, "Reporting extracted start="
     ...
     * operations through a call to link {@link #beginBatchEdit()}.
     */
     ...
     * operations through a call to link {@link #endBatchEdit}.
     */
    ...
    /**
     * Make a new Layout based on the already-measured size of the view, on the
     * assumption that it was measured correctly at some point.
     */
    ...
    /**
     * The width passed in is now the desired layout width, not the full view
     * width with padding. {@hide}
     */
    protected void makeNewLayout(int w, int hintWidth,
        ...
        if (mEllipsize == TextUtils.TruncateAt.MARQUEE) {
            if (!compressText(ellipsisWidth)) {
                ...
                // If the size of the view does not depend on the size of the
                // text, try to
                // start the marquee immediately
                if (height != LayoutParams.WRAP_CONTENT && height != LayoutParams.FILL_PARENT) {
                    startMarquee();
                } else {
                    // Defer the start of the marquee until we know our width
                    // (see setFrame())
    ...
    private boolean compressText(float width) {
        // Only compress the text if it hasn't been compressed by the previous
        // pass
    ...
    /**
     * Check whether a change to the existing text layout requires a new view
     * layout.
     */
    ...
    /**
     * Check whether entirely new text requires a new view layout or merely a
     * new text layout.
     */
    private void checkForRelayout() {
        ...
        if ((mLayoutParams.width != LayoutParams.WRAP_CONTENT ||
             ...
             * changing (unless we do the requestLayout(), in which case it will
             * happen at measure).
             */
            makeNewLayout(want, hintWant, UNKNOWN_BORING, UNKNOWN_BORING,
    ...
    private boolean bringTextIntoView() {
        ...
        if (a == Layout.Alignment.ALIGN_CENTER) {
            /*
             * Keep centered if possible, or, if it is too wide to fit, keep
             * leading edge in view.
             */
    ...
    public boolean bringPointIntoView(int offset) {
        ...
        if (grav < 0) {
            ...
            if (right - left <= hspace) {
                ...
                /*
                 * If we are near the right edge, keep the right edge at the
                 * edge of the view.
                 */
                hs = right - hspace;
            } else if (x < left + hslack) {
                /*
                 * If we are near the left edge, keep the left edge at the edge
                 * of the view.
                 */
                hs = left;
            } else if (left > hs) {
     ...
     * the text toa single line. Use <code>null</code> to turn off ellipsizing.
     *
     ...
     *            modified.
     * @param before The offset of the former end of the range of the text that
     *            was modified. If text was simply inserted, this will be the
     *            same as <code>start</code>. If text was replaced with new text
     *            or deleted, the length of the old text was
     *            <code>before-start</code>.
     * @param after The offset of the end of the range of the text that was
     *            modified. If text was simply deleted, this will be the same as
     *            <code>start</code>. If text was replaced with new text or
     *            inserted, the length of the new text is
        ...
        if (what instanceof ParcelableSpan) {
            ...
            if (ims != null && ims.mExtracting != null) {
                if (ims.mBatchEditNesting != 0) {
                    ...
                    if (DEBUG_EXTRACT) {
                        Log.v(TAG, "Span change outside of batch: "
                                + oldStart + "-" + oldEnd + ","
    ...
    private class ChangeWatcher
        ...
        public void beforeTextChanged(CharSequence buffer, int start,
                int before, int after) {
            if (DEBUG_EXTRACT) {
                Log.v(TAG, "beforeTextChanged start=" + start
        ...
        public void onTextChanged(CharSequence buffer, int start,
                int before, int after) {
            if (DEBUG_EXTRACT) {
                Log.v(TAG, "onTextChanged start=" + start
                        + " before=" + before + " after=" + after + ": " + buffer);
            }
        ...
        public void afterTextChanged(Editable buffer) {
            if (DEBUG_EXTRACT) {
                Log.v(TAG, "afterTextChanged: " + buffer);
            }
        ...
        public void onSpanChanged(Spannable buf,
            ...
            if (DEBUG_EXTRACT) {
                Log.v(TAG, "onSpanChanged s=" + s + " e=" + e
        ...
        public void onSpanRemoved(Spannable buf, Object what, int s, int e) {
            if (DEBUG_EXTRACT) {
                Log.v(TAG, "onSpanRemoved s=" + s + " e=" + e
                        + " what=" + what + ": " + buf);
            }
        ...
        if (focused) {
            ...
            if (!mFrozenWithFocus || (selStart < 0 || selEnd < 0)) {
                ...
                if (selMoved && selStart >= 0 && selEnd >= 0) {
                    /*
                     * Someone intentionally set the selection, so let them do
                     * whatever it is that they wanted to do instead of the
                     * default on-focus behavior. We reset the selection here
                     * instead of just skipping the onTakeFocus() call because
                     * some movement methods do something other than just
                     * setting the selection in theirs and we still need to go
                     * through that path.
                     */
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        /*
         * Don't handle the release after a long press, because it will move the
        ...
        if ((mMovement != null || onCheckIsTextEditor()) && mText instanceof Spannable
                && mLayout != null) {
            ...
            if (mText instanceof Editable && onCheckIsTextEditor()) {
                if (action == MotionEvent.ACTION_UP && isFocused() && !mScrolled) {
                    ...
                    // causes the IME to be displayed, we don't want the
                    // selection
                    ...
                    if (newSelStart != oldSelStart || newSelEnd != oldSelEnd) {
                        // Check if the selection bounds does not exceed the
                        // length.
                        // check the posible race condition which could happen
                        // in monkey.
    ...
    /**
     * Returns a word to add to the dictionary from the context menu, or null if
     * there is no cursor or no word at the cursor.
     */
    private String getWordForDictionary() {
        /*
         * Quick return if the input type is one where adding words to the
         * dictionary doesn't make any sense.
         */
    ...
    protected void onCreateContextMenu(ContextMenu menu) {
        ...
        if (!isFocused()) {
            if (isFocusable() && mInput != null) {
                if (canCopy()) {
                    ...
                    int name = com.android.internal.R.string.copy;
        ...
        if (canSelectText()) {
            if (MetaKeyKeyListener.getMetaState(mText, MetaKeyKeyListener.META_SELECTING) != 0) {
                ...
                menu.add(0, ID_START_SELECTING_TEXT, 0,
                        com.android.internal.R.string.selectTextMode).
                        setOnMenuItemClickListener(handler);
                added = true;
            }
        }
        ...
        if (canCut()) {
            /*
             * int name; if (selection) { name =
             * com.android.internal.R.string.cut; } else { name =
             * com.android.internal.R.string.cut; }
             */
            int name = com.android.internal.R.string.cut;
     ...
     * Currently this will be one of: {@link android.R.id#selectAll},
     * {@link android.R.id#startSelectingText},
     * {@link android.R.id#stopSelectingText}, {@link android.R.id#cut},
     * {@link android.R.id#copy}, {@link android.R.id#paste},
     * {@link android.R.id#copyUrl}, or {@link android.R.id#switchInputMethod}.
     *
     * @return boolean
     */
    public boolean onTextContextMenuItem(int id) {
        ...
        switch (id) {
            ...
            case ID_ADD_TO_DICTIONARY:
                ...
                if (word != null) {
                    Intent i = new Intent(MusicIntent.SETTING_USER_DICTIONARY_INSERT);
