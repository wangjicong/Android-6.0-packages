...
public class RawContactEditorView extends BaseRawContactEditorView {
    ...
    public void setState(RawContactDelta state, AccountType type, ViewIdGenerator vig,
        ...
        /**
         * SPRD: edit sim-card contact, bug421665
         *
         * @{
         */
        ...
        if (SimAccountType.ACCOUNT_TYPE.equals(accoutTypeFromValues)
                || USimAccountType.ACCOUNT_TYPE.equals(accoutTypeFromValues)) {
            char slot = accountName.charAt(accountName.length() - 1);
            int slotId = -1;
            if (Character.isDigit(slot)) {
                slotId = Integer.parseInt(String.valueOf(slot)) - 1;
            }
            if (slotId >= 0) {
                ...
                if (subInfos != null) {
                    for (SubscriptionInfo subInfo : subInfos) {
                        if (subInfo.getSimSlotIndex() == slotId) {
                            simName = subInfo.getDisplayName().toString();
                            break;
                        }
                    }
                }
                if (simName != null) {
                    accountName = accountName + " " + simName;
                }
            }
        }
        /**
        ...
        /**
         * SPRD: bug425262, rest kind.typeOverallMax according to (account.name,account.type) instead of
         * account.type original
         *
         * @{
         */
        AccountWithDataSet account = null;
        if (!isProfile) {
            accountName = state.getAccountName();
        ...
        for (DataKind kind : type.getSortedDataKinds()) {
            ...
            if (StructuredName.CONTENT_ITEM_TYPE.equals(mimeType)) {
               ...
               /**
                * SPRD:Sim account do not support PhoneticName.
                * Orig:
                *
                ...
                */
                ...
                /**
                 * SPRD: Bug433241 More gap between name and phone text field when edit SIM contacts.
                 */
                if (account != null
                        && (account.type.equals("sprd.com.android.account.sim") || account.type
