...
public class CompactRawContactsEditorView extends LinearLayout implements View.OnClickListener {
    ...
    private void addAccountInfo(RawContactDeltaList rawContactDeltas, boolean hasNewContact,
        ...
        /**
         * SPRD: Bug 530854 The dut fails to display the account information after
         * creating the contact of the newly added account.
         * @{ Android original code:
        if (defaultAccountWithDataSet == null) {
         */
        if (defaultAccountWithDataSet == null && rawContactDeltas.isEmpty()) {
          /*
           * @}
           */
        ...
        /**
         * SPRD:No need select account when from compact edit fragment,set the accountName,accountType.
         * @{
         */
        if (accountType != null && defaultAccountRawContactDelta.getAccountName() != null) {
            setAccountName(defaultAccountRawContactDelta.getAccountName());
    ...
    private void addEditorViews(RawContactDeltaList rawContactDeltas) {
        for (RawContactDelta rawContactDelta : rawContactDeltas) {
            ...
            for (DataKind dataKind : accountType.getSortedDataKinds()) {
                ...
                /**
                 * SPRD: bug425262, rest kind.typeOverallMax according to (account.name,account.type)
                 * instead of account.type original
                 *
                 * @{
                 */
                AccountWithDataSet account = null;
                String accName = rawContactDelta.getAccountName();
                String accType = rawContactDelta.getAccountType();
                if (accName != null) {
                    account = new AccountWithDataSet(accName, accType, null);
                } else {
                    Log.e(TAG, "accountName is null");
                }
                if (account != null) {
                    int typeOverallMax = AccountRestrictionUtils.get(getContext())
                            .getTypeOverallMax(
                                    account, mimeType);
                    if (typeOverallMax != 0) {
                        dataKind.typeOverallMax = typeOverallMax;
                    }
                }
                /**
                 * @}
    ...
    private StructuredNameEditorView inflateStructuredNameEditorView(ViewGroup viewGroup,
        ...
        /**
         * SPRD:Bug501081 If this is a sim contact, we should not listen its modification.
         * Original Android code:
        if (nameEditorListener != null) {
         * @{
         */
        boolean isSimContact = (rawContactDelta.getAccountName() != null) ?
                rawContactDelta.getAccountName().toLowerCase(Locale.US).contains("sim") : false;
        if (nameEditorListener != null && !isSimContact) {
        /**
         * @}
         */
    ...
    private void setAccountName(String accountName){
        ...
        vlog("Current accountName " + mAccountName);
    }
    ...
    private void setAccountType(String accountType){
        ...
        vlog("Current accountType " + mAccountType);
    }
