...
import android.telephony.TelephonyManager;
/*
 * Bug 538431 The DUT fails to select the default ring tone while the
 * picked ring tone is unavailable.
 * @{
 */
...
/**
 * SPRD:Bug474795 Copy feature and quick activity menu of SIM contacts.
 *
 * Original Android code:
 * @{
public class QuickContactActivity extends ContactsActivity {
 */
public class QuickContactActivity extends ContactsActivity implements SelectAccountDialogFragment.Listener, SensorEventListener {
 /**
  * @}
  */
    ...
    private boolean mHasPhonetic = false;
    /**
     *@}
     */
    ...
    /**
     * SPRD:Bug515875 The feature Sensor Dialer.
     * @{
     */
    private SensorManager mSensorManager;
    private Sensor mSigMotion;
    private boolean mEasyDialPreference;
    /**
     * @}
     */
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        /**
         * SPRD:Bug425719 Limit the contact edit operation when batch operations.
        ...
        /**
         * SPRD:Bug525160 Show video icon only when VoLte is enabled.
         * @{
         */
        mLtePhoneStateListener = new PhoneStateListener() {
            @Override
            public void onVoLteServiceStateChanged(VoLteServiceState serviceState) {
                boolean enableState = mEnableButton;
                mEnableButton = (serviceState.getSrvccState() == VoLteServiceState.IMS_REG_STATE_REGISTERED);
                Log.d(TAG, "onVoLteServiceStateChanged: " + serviceState + " , mEnableButton: " + mEnableButton);
                if (enableState != mEnableButton && mContactData != null) {
                    Log.d(TAG, "volte enable state change and refresh data.");
                    bindContactData(mContactData);
                }
            }
        };
        /**
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_CODE_CONTACT_EDITOR_ACTIVITY &&
            ...
            finish();
          /**
           * SPRD: Bug542863 The contact screen does not update after adding call log
           * to a contact and pressing the back key.
           ...
           */
            processIntent(data);
        /**
         * SPRD:Bug474795: Add for ringtone pick.
         *
         * @{
         */
        } else if (requestCode == REQUEST_CODE_PICK_RINGTONE) {
    ...
    protected void onResume() {
        ...
        /**
         * SPRD:Bug530887 When turn off screen, restart the loader.
         * @{
         */
        mContactLoader = (ContactLoader) getLoaderManager().restartLoader(
                              LOADER_CONTACT_ID, null, mLoaderContactCallbacks);
         ...
         */
        mEasyDialPreference =
                Settings.Global.getInt(getContentResolver(), Settings.Global.EASY_DIAL, 0) != 0;
        if (mEasyDialPreference) {
            mSensorManager = (SensorManager)getSystemService(Context.SENSOR_SERVICE);
            mSigMotion = mSensorManager.getDefaultSensor(Sensor.TYPE_SPRDHUB_HAND_UP);
            if (mSensorManager != null && mSigMotion != null) {
    ...
    public void onPause() {
         ...
         * @{
         */
        if (mEasyDialPreference && mSensorManager != null && mSigMotion != null) {
    ...
    private static Entry dataItemToEntry(DataItem dataItem, DataItem secondDataItem,
        ...
        if (dataItem instanceof ImDataItem) {
            ...
            // SPRD:Bug533917 add contactData null judgement to avoid monkey test JavaCrash
            if (contactData != null) {
...
+                   thirdIcon = res.getDrawable(R.drawable.ic_videocam);
     ...
     /**
      * SPRD:bug449720, show GroupMembership tile in QuickContacts
      * @{
      */
            ...
            long groupId = groupDataItem.getGroupRowId();
            if(contactData != null){
                for (GroupMetaData groupMetaData : contactData.getGroupMetaData()) {
                     if(groupId == groupMetaData.getGroupId()) {
                       if(!groupMetaData.isDefaultGroup() && !groupMetaData.isFavorites()) {
                          groupTitle = groupMetaData.getTitle();
                        }
                        break;
                     }
                }
    ...
    private List<Entry> contactInteractionsToEntries(List<ContactInteraction> interactions) {
        ...
        for (ContactInteraction interaction : interactions) {
            ...
            entries.add(new Entry(/* id = */ -1,
                     ...
                     * @{
                     */
                    interaction.getIcon(this, mEnableButton),
                    /**
                     * @}
                     */
                     ...
                     * @{
                     */
                    interaction.getIntent(this, mEnableButton),
                    /**
                     * @}
                     */
                    /* alternateIcon = */ null,
    ...
    private boolean isContactEditable() {
        return mContactData != null && !mContactData.isDirectoryEntry() && (!mContactData.isReadOnly());
    }

    public boolean isContactOptionsDeleteChangeEnabled() {
        return mContactData != null && !mContactData.isDirectoryEntry()
                && PhoneCapabilityTester.isPhone(mContext)&& (!mContactData.isReadOnly());
    }
    /**
    ...
    private void deleteContact() {
        ...
        if (mContactData.isUserProfile()) {
            Drawable d = UserIcons.getDefaultUserIcon(UserHandle.USER_NULL, false);
            mUserManager.setUserName(UserHandle.myUserId(),
                    getResources().getString(com.android.internal.R.string.owner_name));
            mUserManager.setUserIcon(UserHandle.myUserId(), UserIcons.convertToBitmap(d));
        }
        /**
         * @}
         */
    ...
    private void shareContact() {
        ...
        try {
            mHasIntentLaunched = true;
            /**
             * SPRD bug534705 The shareContact interface is diferent between contacts list interface
             * and contact detail interface
             * @orig:
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
       ...
       /**
        * SPRD:
        * Bug421397: add RingTone 、sendToVoicemail menu.
        * @ orig:
        ...
        * @}
        */
        /* SPRD: Bug 474744 Porting Contacts blacklist plugin.@ { */
        BlacklistUtils.getInstance().addBlackItem(this,menu,mContactData);
        /* @} */
        return true;
    }
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        if (mContactData != null) {
            ...
            /* SPRD:
             * Bug 421397 Add Ringtone and sendToVoicemail menuItem in QuickContactActivity.
             * @{
             */
            boolean mMenuEnabled = (!ContactsApplication.sApplication.isBatchOperation())
              ...
              */
            /* SPRD: Bug 474744 Porting Contacts blacklist plugin.@ { */
            BlacklistUtils.getInstance().blackOrNot(this, menu, mContactData);
            /* @} */
            ContactDisplayUtils.configureStarredMenuItem(starredMenuItem,
                    mContactData.isDirectoryEntry(), mContactData.isUserProfile(),
            ...
            ArrayList<AccountWithDataSet> allAccounts = (ArrayList) AccountTypeManager.getInstance(mContext).getAccounts(true);
            if (!mContactData.isUserProfile() && allAccounts != null && allAccounts.size() > 1) {
                canCopy = true;
                /* this is not a stored contact */
                if(mContactData.getContactId() == -1 && mContactData.getNameRawContactId() == -1
                        || (mContactData.getContactId() != -1 && mContactData.getNameRawContactId() != -1 && mContactData.getDirectoryId() > 0)){
                    canCopy = false;
                }
            }
            copyMenu.setVisible(canCopy);
             ...
             *  @{
             **/
            AccountWithDataSet accountType = mContactData.getAccount();
            if ((accountType != null) && (SimAccountType.ACCOUNT_TYPE.equals(accountType.type) || USimAccountType.ACCOUNT_TYPE
                            .equals(accountType.type) || mContactData.isUserProfile())) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.menu_edit:
                if (DirectoryContactUtil.isDirectoryContact(mContactData)) {
                    ...
                    if (mContactData.getDirectoryExportSupport() ==
                         ...
                         */
                        intent.putExtra(Intents.Insert.EXTRA_ACCOUNT,
                                new AccountWithDataSet(mContactData.getDirectoryAccountName(),
            ...
            case R.id.menu_help:
                ...
                return true;
            /**
             * SPRD: Bug 421397: add RingTone 、sendToVoicemail menu.
             * @{
             */
            ...
            case R.id.menu_copy: {
                if (mContactData != null)
                    onCopyRequested(mContactData.getLookupKey());
                return true;
            }
            /**
             * @}
    ...
    /**
     * SPRD:
     * Bug 421397: add RingTone 、sendToVoicemail menu.
     * Bug 538431 The DUT fails to select the default ring tone while the
     * picked ring tone is unavailable.
     * @{
     */
     ...
     */
    /**
     * SPRD:Bug377698: Add for RingTone 、sendToVoicemail 、delete menu visiable.
     * @{
     */
    ...
    private void onCopyRequested(final String lookupKey) {
        if (mIsSaveInstance) {
            return;
        }
        ...
        SelectAccountDialogFragment.show(getFragmentManager(),
                R.string.copy_to, AccountListFilter.ACCOUNTS_CONTACT_WRITABLE, args);
    }
    ...
    private void confirmImport(final AccountWithDataSet dstAccount, final String[] ids) {
        if (dstAccount != null
            ...
            intent.setComponent(new ComponentName(QuickContactActivity.this,
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        Log.d("SensorDialer", "onSensorChanged: " + event.values[0]);
        if(mTelephonyManager != null &&
                mTelephonyManager.getCallState() == TelephonyManager.CALL_STATE_IDLE) {
            try {
                List<Entry> firstEntryList = mCachedCp2DataCardModel.contactCardEntries.get(0);
                Intent intent = firstEntryList.get(0).getIntent();
                if(intent.getAction().equals(Intent.ACTION_CALL)) {
                    startActivity(intent);
                }
            } catch (Exception e) {
                // TODO: handle exception
                Log.d("SensorDialer", "Fail to make a phone call, Exception: "+e);
            }
        }
    }
