...
public class SmartDialNameMatcher {
    ...
    private static final String TAG = SmartDialNameMatcher.class.getSimpleName();
    ...
    /**
     * SPRD: add for bug505212 @{
     * Strips a phone number that start with '+44'
     *
    ...
    public static String stripNumber(String number) {
        ...
        if (!TextUtils.isEmpty(number)
                && (number.length() > 3) && number.startsWith("+44")) {
            strippedNumber = number.substring(3);
        } else {
            strippedNumber = number;
        }
    ...
    public SmartDialMatchPosition matchesNumber(String phoneNumber, String query, boolean useNanp) {
        ...
        if (TextUtils.isEmpty(phoneNumber)) {
            Log.w(TAG, "matchesNumber: phoneNumber is empty");
            return null;
        }
        /* @} */
    ...
    boolean matchesCombination(String displayName, String query,
        ...
        StringBuilder builder = new StringBuilder();
        /* SPRD: add for bug509939 to avoid NullPointException @{ */
        if (displayName == null) {
            return false;
        }
        /* @} */
