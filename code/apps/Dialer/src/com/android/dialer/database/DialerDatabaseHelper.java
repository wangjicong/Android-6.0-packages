...
public class DialerDatabaseHelper extends SQLiteOpenHelper {
    ...
    public static interface PhoneQuery {
       ...
       static final String[] PROJECTION = new String[] {
        ...
        public static final String[] PROJECTION_DIALER = new String[]{
                Phone._ID,                           // 0
                ...
                Phone.CONTACT_ID,                    // 4
                Phone.LOOKUP_KEY,                    // 5
                Contacts.DISPLAY_ACCOUNT_TYPE,       // 6
                Contacts.DISPLAY_ACCOUNT_NAME,       // 7
        };
        /* @} */
    ...
    public void updateSmartDialDatabase() {
        /* SPRD: add for bug 522251 @{ */
        //final SQLiteDatabase db = getWritableDatabase();
        SQLiteDatabase db = null;
        try {
            db = getWritableDatabase();
        } catch (SQLiteCantOpenDatabaseException e) {
            Log.v(TAG, "updateSmartDialDatabase" + e.toString());
            return;
        }
        /* @} */
    ...
    public ArrayList<ContactNumber>  getLooseMatches(String query,
        ...
        Log.d(TAG, "getLooseMatches --->> query = " + query);
        final Cursor cursor;
        if (!TextUtils.isEmpty(query)) {
            Uri.Builder builder = Uri.parse(CallLog.Calls.CONTENT_URI + "/contacts").buildUpon();
            Uri uri = builder.appendPath(query).build();
            cursor = mContext.getContentResolver().query(uri, PhoneQuery.PROJECTION_DIALER, null,
...
//                if (nameMatches || numberMatches) {
                    ...
                    result.add(new ContactNumber(id, dataID, displayName, phoneNumber, lookupKey,
                            photoId, /** SPRD: DIALER SEARCH FEATURE */accountType, accountName));
                    counter++;
                    if (DEBUG) {
...
//                }
    ...
    public static class DialMatchInfo extends ContactNumber {
        ...
        public DialMatchInfo(long id, long dataID, String displayName, String phoneNumber,
                String lookupKey, long photoId, String accountType, String accountName,
                boolean isCallLog, long callLogDate, int callLogType, String cachedLookupUri,
                String SYNC1) {
            super(id, dataID, displayName, phoneNumber, lookupKey, photoId, accountType, accountName);
        ...
        public int hashCode() {
            return Objects.hashCode(id, dataId, displayName, phoneNumber, lookupKey, photoId,
                    itemType, callsDate, callsType, callsCachedLookupUri, SYNC1);
        }
        ...
        public boolean equals(Object object) {
            if (object instanceof DialMatchInfo && super.equals(object)) {
                ...
                return Objects.equal(this.itemType, that.itemType);
            }
    ...
    public ArrayList<DialMatchInfo> getLooseMatchesForDialMatchInfo(String query,
                                                                    SmartDialNameMatcher nameMatcher) {
        final Cursor cursor;
        ...
        if (!TextUtils.isEmpty(query)) {
            ...
            Builder builder = Uri.parse(CallLog.Calls.CONTENT_URI + "/callabledialmatch").buildUpon();
            Uri uri = builder.appendPath(query).build();
            Log.d("PerformanceTest", new SimpleDateFormat("yyyyMMdd-HH:mm:ss:SSS") .format(new Date()) + " DialerDatabaseHelper getLooseMatchesForDialMatchInfo before get cursor");
            cursor = mContext.getContentResolver().query(uri, PhoneQuery.DIALMATCH_PROJECTION,
                    null, null, PhoneQuery.ITEM_TYPE);
            Log.d("PerformanceTest", new SimpleDateFormat("yyyyMMdd-HH:mm:ss:SSS") .format(new Date()) + " DialerDatabaseHelper getLooseMatchesForDialMatchInfo after get cursor");
        } else {
            cursor = null;
        ...
        try {
            while ((cursor.moveToNext())) {
                ...
                /* SPRD: add for fdn function @{ */
                //final String displayName = cursor.getString(1);
                final long photoId = cursor.getLong(2);
                final String phoneNumber = cursor.getString(3);
                final String displayName = FdnNumberHelper.getInstance(mContext).
                        queryFdnCacheForAllSubs(mContext, phoneNumber, cursor.getString(1));
                /* @} */
                final long contactId = cursor.getLong(4);
                final String lookupKey = cursor.getString(5);
                final long item_type = cursor.getLong(6);
                final long calls_date = cursor.getLong(7);
                final int calls_type = cursor.getInt(8);
                final String accountType = cursor.getString(9);
                final String accountName = cursor.getString(10);
                final String lookupUri = cursor.getString(11);
                final String SYNC1 = cursor.getString(12);

                if (item_type == DialMatchInfo.TYPE_CALLLOG) {
                    result.add(new DialMatchInfo(contactId, dataId, displayName, phoneNumber,
                            lookupKey, photoId, accountType, accountName, true, calls_date,
                            calls_type, lookupUri, SYNC1));
                } else {
                    result.add(new DialMatchInfo(contactId, dataId, displayName, phoneNumber,
                            lookupKey, photoId, accountType, accountName, false, 0, 0, null, null));
                }
            }
        } finally {
            cursor.close();
