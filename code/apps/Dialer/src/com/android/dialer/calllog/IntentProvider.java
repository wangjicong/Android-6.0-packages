...
public abstract class IntentProvider {
    ...
    /**
     * SPRD: for add to blacklist
     * Retrieves the add to blacklist intent provider for an entry in the call log.
     *
     * @param name
     * @param number
     * @return The add to blacklist intent provider.
     */
    public static IntentProvider getAddToBlackListIntentProvider(
            final CharSequence name, final String number) {
        return new IntentProvider() {
            @Override
            public Intent getIntent(Context context) {
                Intent intent = new Intent("com.sprd.firewall.ui.BlackCallsListAddActivity.action");
                intent.putExtra("Click_BlackCalls_Number", number);
                if (name != null) {
                    intent.putExtra("Click_BlackCalls_Name", name.toString());
                }
                return intent;
            }
        };
