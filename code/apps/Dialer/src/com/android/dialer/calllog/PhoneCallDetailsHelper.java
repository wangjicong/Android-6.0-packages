...
public class PhoneCallDetailsHelper {
    ...
    public void setPhoneCallDetails(PhoneCallDetailsViews views, PhoneCallDetails details) {
        ...
        if (accountLabel != null) {
            ...
            /* SPRD: Add identification for sim card  @{ */
            ...
            if (iccId != null) {
                for (int i = 0; i < phoneCount; i++) {
                    ...
                    SubscriptionInfo subscriptionInfo = DialerUtils
                            .getActiveSubscriptionInfo(mContext, i, false);
                    if (subscriptionInfo != null && iccId.equals(subscriptionInfo.getIccId())) {
                        simIdentification = mResources.getString(R.string.sim) + (i + 1);
                    }
                }
            }
		    ...
		    if(OptConfig.SUNVOV_CUSTOM_C7301_XLL_FWVGA){
		        accountLabel = accountLabel.replace("SIM1","Master Sim");
		        accountLabel = accountLabel.replace("SIM2","Slave Sim");
		    }
        ...
        if (TextUtils.isEmpty(details.name)) {
            /* SPRD: modify for emergency call @{*/
            nameText = EmergencyNumberHelper.getInstance(
                    mContext).getEmergencyNumber(
                    mContext, displayNumber.toString(), details.number.toString());
            /* @} */
        ...
        TelephonyManager telephonyManager = (TelephonyManager) mContext.getSystemService(Context.TELEPHONY_SERVICE);
        if (((details.features & Calls.FEATURES_VOLTE) == Calls.FEATURES_VOLTE)
                && telephonyManager.isVolteCallEnabled()) {
