...
public class CallLogAdapter extends GroupingListAdapter
    ...
    public CallLogAdapter(
        ...
        /* SPRD: callLog show fdn name @{ */
        mTelephonyManager = (TelephonyManager) mContext.getSystemService(
                Context.TELEPHONY_SERVICE);
        int phoneCount = mTelephonyManager.getPhoneCount();
        for (int i = 0; i < phoneCount; i++) {
            SubscriptionInfo subscriptionInfo =
                    DialerUtils.getActiveSubscriptionInfo(mContext, i, false);
            if (subscriptionInfo != null) {
                int subId = subscriptionInfo.getSubscriptionId();
                if (mTelephonyManager.getIccFdnEnabled(subId)) {
                    FdnNumberHelper.getInstance(mContext).queryFdnList(
                            subId, mContext);
                }
            }
        }
        ...
        if (accountHandle != null && accountHandle.getId() != null) {
            for (int i = 0; i < phoneCount; i++) {
                ...
                if (subscriptionInfo != null) {
                    ...
                    String name = FdnNumberHelper.getInstance(mContext).queryFdnCache(
                            number, subId, info.name);
                    if (mTelephonyManager.getIccFdnEnabled(subId) && name != null) {
                        details.name = name;
                    }
                }
            }
