...
public class CallDetailHistoryAdapter extends BaseAdapter {
    ...
    // SPRD: Should not show duration time in call detail for cmcc case feature.
    private final boolean mShowDuration = DialerPlugin.getInstance()
            .callLogDetailShowDuration();
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        ImageView volteCallIcon = (ImageView) result.findViewById(R.id.volte_call_icon);

        int callType = details.callTypes[0];
        boolean isVideoCall = (details.features & Calls.FEATURES_VIDEO) == Calls.FEATURES_VIDEO
                /* SPRD: modify for vt porting bug479433 @{ */
                //&& CallUtil.isVideoEnabled(mContext);
                && TelephonyManager.isSupportVT();
                /* @} */

        /* SPRD: modify for volte @{*/
        boolean volteCall = (details.features & Calls.FEATURES_VOLTE) == Calls.FEATURES_VOLTE;
        TelephonyManager telephonyManager = (TelephonyManager) mContext.getSystemService(Context.TELEPHONY_SERVICE);
        if (volteCall
                && ((CallDetailActivity)mContext).isVolte) {
        ...
        // SPRD: add for bug502046
        callTypeIconView.forceLayout();
        return result;
    }
