...
public class AddBookmarkPage extends Activity
    ...
    private long mOriginalCurrentFolder;
        ...
        if (v == mFolderNamer) {
            if (v.getText().length() > 0) {
                if (actionId == EditorInfo.IME_NULL) {
                    ...
                    /* Fix Bug:493212 In cross screen mode no response to clicking the "complete" 2015.11.04 */
                    completeOrCancelFolderNaming(false);
                }
            }
    ...
    public void onClick(View v) {
        if (v == mButton) {
            ...
            /**
             *modify for bookmarks override feature
             *
            ...
            /*Bug535446: set the EditText cursor to the end,start*/
            String newFolder = getResources().getString(R.string.new_folder);
            mFolderNamer.setSelection(newFolder.length());
            /*Bug535446: set the EditText cursor to the end,end*/
             ...
             *@{
             */
            long folder = mEditingFolder ? 1 : 0;
            createHandler();
            Message msg = Message.obtain(mHandler, BOOKMARK_DELETED);
            BookmarkUtils.displayRemoveBookmarkDialog(id,
                    mTitle.getText().toString(), this, msg,folder);
            /*@}*/
        }
    }
    ...
    private void completeOrCancelFolderNaming(boolean cancel) {
        if (!cancel && !TextUtils.isEmpty(mFolderNamer.getText())) {
            ...
            /**
             *modify for update bookmark in folder,check duplicate folder
             *Original Android code:
             * descendInto(name, id);
             *@{
             */
            if (id == -2) {
                // duplicate warning
                Toast.makeText(this, R.string.edit_folder_exist,Toast.LENGTH_SHORT).show();
                return;
            } else {
                // insert successfully
                descendInto(name, id);
    ...
    private long addFolderToCurrent(String name) {
         ...
         *@{
         */
        String where = BrowserContract.Bookmarks.IS_FOLDER + "=1 AND "
        ...
        try {
            cursor = getContentResolver().query(
                    ...
                    new String[] { BrowserContract.Bookmarks._ID }, where,
                    new String[] { name, Long.toString(currentFolder) }, null);
        } catch (SQLiteException e) {
            Log.e(LOGTAG, "addFolderToCurrent: " + e);
        }
        if (cursor != null ) {
    ...
    private LoaderCallbacks<EditBookmarkInfo> mEditInfoLoaderCallbacks =
        ...
        public void onLoadFinished(Loader<EditBookmarkInfo> loader,
            ...
            /**
             * Add for override bookmark
             *@{
             */
            Log.i("AddBookmarkPage", "onLoadFinished");
            if (isPause){
                return;
            }
            /*@}*/
            ...
            if (info.id != -1) {
                ...
                if (info.title != null) {
                    if (isPause) {
                        ...
                        mTitle.setSelection(info.title.length());
                    }
                }
                /*@}*/
                ...
                mOriginalCurrentFolder = mCurrentFolder;
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        ...
        switch (id) {
            ...
            case LOADER_ID_FOLDER_CONTENTS:
                ...
                return new BookmarkLoader(this,
    ...
    /*add for bug497069 to refresh Adapter when rotate the screen,begin*/
    class AccountArrayAdapter extends ArrayAdapter<BookmarkAccount> {
        private boolean needRefresh;

        public AccountArrayAdapter(Context context, int resource) {
    ...
    public void onConfigurationChanged(Configuration config) {
        mFolderAdapter.setNeedRefresh();
        mAccountAdapter.setNeedRefresh();
        super.onConfigurationChanged(config);
    }
    /*add for bug497069 to refresh Adapter when rotate the screen,end*/
    ...
    protected void onCreate(Bundle icicle) {
        ...
        if(null == mMap){
            Log.d(LOGTAG,"mMap is null,return");
            finish();
            return;
        }
        ...
        if (mMap != null) {
            /**
             * modify save bookmark to folder
            ...
            /*@}*/
                 ...
                 */
                if (mEditingFolder) {
                    mFakeTitle.setText(R.string.new_folder);
                    findViewById(R.id.row_address).setVisibility(View.GONE);
                }
                /*@}*/
         ...
         *@{
         */
        mTitle.setFilters(new InputFilter[]{ new InputFilter.LengthFilter(2000)});
        mTitle.setText(title);
        if (title != null) {
        ...
        mAccountAdapter = new AccountArrayAdapter(this,
    ...
    /**
     * Modify for upate and delete bookmark in task
     * Orignal android code:
     ...
     *@{
     */
    private class UpdateBookmarkTask extends AsyncTask<ContentValues, Void, Boolean> {
        Context mContext;
        Long mId;
        Long mOriginalId;

        public UpdateBookmarkTask(Context context, long id, long original_id) {
            mContext = context.getApplicationContext();
            mId = id;
            mOriginalId = original_id;
        }
        ...
        protected Boolean doInBackground(ContentValues... params) {
            ...
            Uri uri = ContentUris.withAppendedId(BookmarkUtils.getBookmarksUri(mContext), mId);
            Log.d(LOGTAG,"AddBookmarkPage UpdateBookmarkTask uri= " + uri);
            long updateId = -1;
            try {
                updateId = mContext.getContentResolver().update(
                        uri,
                        params[0], null, null);
                if(mOriginalId != -1 && mOriginalId != mId) {
                    uri = ContentUris.withAppendedId(BookmarkUtils.getBookmarksUri(mContext), mOriginalId);
                    Log.d(LOGTAG,"AddBookmarkPage delete original uri= " + uri);
                    mContext.getContentResolver().delete(uri, null, null);
                }
            } catch (SQLiteException e) {
                Log.e(LOGTAG, "UpdateBookmarkTask: " + e);
            }
        ...
        protected void onPostExecute(Boolean result) {
            Message msg = Message.obtain(mHandler, UPDATE_BOOKMARK);
            if(result) {
                msg.arg1 = 1;
            } else {
                msg.arg1 = -1;
                ...
                public void handleMessage(Message msg) {
                    switch (msg.what) {
                        ...
                        case UPDATE_BOOKMARK:
                            if (1 == msg.arg1) {
                                Toast.makeText(AddBookmarkPage.this, R.string.update_bookmark_success,
                                        Toast.LENGTH_LONG).show();
                            } else {
                                Toast.makeText(AddBookmarkPage.this, R.string.update_bookmark_fail,
                                        Toast.LENGTH_LONG).show();
                            }
                            break;
                        case SAVE_FOLDER:
                            if (1 == msg.arg1) {
                                Toast.makeText(AddBookmarkPage.this, R.string.save_folder_success,
                                        Toast.LENGTH_LONG).show();
                            } else {
                                Toast.makeText(AddBookmarkPage.this, R.string.save_folder_fail,
                                        Toast.LENGTH_LONG).show();
                            }
                            break;
                        /*@}*/
    ...
    boolean save() {
        ...
        Log.d(LOGTAG,"AddBookmarkPage.save() mEditingExisting= " + mEditingExisting);
        if (mEditingExisting) {
            Long id = mMap.getLong(BrowserContract.Bookmarks._ID);
            ContentValues values = new ContentValues();
            ...
            if (values.size() > 0) {
                 ...
                 */
                Log.d(LOGTAG,"AddBookmarkPage.save() UpdateBookmarkTask() values= " + values);
                Long original_id = mMap.getLong("original_id", -1);
                new UpdateBookmarkTask(getApplicationContext(), id, original_id).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR ,values);
                /*@}*/
            }
            setResult(RESULT_OK);
            ...
            if (mEditingFolder) {
                ...
                new Thread() {
                    public void run() {
                        long id = addFolderToCurrent(tmpTitle);
                        // id != -1 means saved successfully
                        mHandler.obtainMessage(SAVE_FOLDER, (id != -1) ? 1 : 0, 0).sendToTarget();
                    }
                }.start();
                /*@}*/
            } else {
    ...
    static class AccountsLoader extends CursorLoader {
        ...
        public Cursor loadInBackground() {
            ...
            try {
                ...
                Log.e(LOGTAG,"loadInBackground :"+e);
                if (cursor != null) {
                    cursor.close();
                    cursor = null;
                }
    ...
    static class BookmarkLoader extends CursorLoader {

        public BookmarkLoader(Context context, Uri uri, String[] projection, String selection,
    ...
    static class EditBookmarkInfoLoader extends AsyncTaskLoader<EditBookmarkInfo> {
        ...
        public EditBookmarkInfo loadInBackground() {
            ...
            try {
                ...
                Log.e("AddBookmarkPage", "EditBookmarkInfoLoader: " + e);
            } finally {
                if (c != null) {
                    c.close();
                }
    ...
    /**
     *Add for bookmark:save,delete,update
     * @{
     */
    public static boolean HAS_RECENT_FOLDER = false;
    private static final int SAVE_FOLDER = 103;
    private boolean isPause = false;
    private String mOriginalFolderName;
    private static final int UPDATE_BOOKMARK = 104;//add for update bookmark
    private long exist_id = -1;
    ...
    private void onSaveWithConfirm() {
        ...
        final boolean mEditingFolder_f = mEditingFolder;
        if ((title_f != null) && (title_f.length() == 0)){
            if(mEditingFolder){
                mTitle.setError(getResources().getText(R.string.folder_needs_title));
            }else{
                mTitle.setError(getResources().getText(R.string.bookmark_needs_title));
            }
            return;
        }
        if (!mEditingFolder_f && (url_f != null) && (url_f.length() == 0)){
            mAddress.setError(getResources().getText(R.string.bookmark_needs_url));
            return;
        }
        new AsyncTask<Void, Void, Cursor>() {
            ...
            protected void onPreExecute() {
                ...
                super.onPreExecute();
                Log.i("AddBookmarkPage", "onSaveWithConfirm -> onPreExecute() start");
                CharSequence message = getApplicationContext().getText(R.string.processing_please_wait);
                mProgressDialog = ProgressDialog.show(AddBookmarkPage.this, null, message,
                        true, true, null);
                mProgressDialog.setCanceledOnTouchOutside(false);
                Log.i("AddBookmarkPage", "onSaveWithConfirm -> onPreExecute() end");
            }

            protected Cursor doInBackground(Void... unused) {
                Log.i("AddBookmarkPage", "onSaveWithConfirm -> doInBackground() start");
                Cursor cursor = null;
                if(mEditingFolder_f){
                    try {
                        // query folders
                        cursor = cr.query(BrowserContract.Bookmarks.CONTENT_URI,
                                new String[] { "_id" },
                                "title = ? AND parent = ? AND folder = 1", new String[] {
                                        title_f, Long.toString(mCurrentFolder) }, null);
                    } catch (SQLiteException e) {
                        Log.e(LOGTAG, "onSaveWithConfirm: " + e);
                    }
                } else {
                    String queryUrl = null;
                    try {
                        if (Bookmarks.urlHasAcceptableScheme(url_f)) {
                            queryUrl = url_f;
                        }
                        else{
                            WebAddress address;
                            try {
                                address = new WebAddress(url_f);
                            } catch (ParseException e) {
                                throw new URISyntaxException("", "");
                            }
                            if (address.getHost().length() == 0) {
                        ...
                        Log.e("AddBookmarkPage","Exception when WebAddress url :"+e);
                        return null;
                    }
                    try {
                        // query bookmarks
                        cursor = cr.query(BrowserContract.Bookmarks.CONTENT_URI,
                                new String[] { "_id" },
                                "(title = ? OR url = ?) AND parent = ? AND folder = 0", new String[] {
                                        title_f, queryUrl, Long.toString(mCurrentFolder) }, null);
                    } catch (SQLiteException e) {
                        Log.e(LOGTAG, "onSaveWithConfirm: " + e);
                    }
                }
                Log.i("AddBookmarkPage", "onSaveWithConfirm -> doInBackground() end");
                return cursor;
            }

            protected void onPostExecute(Cursor cursor){
                Log.i("AddBookmarkPage", "onSaveWithConfirm -> onPostExecute() start mProgressDialog = " + mProgressDialog);
                if (mProgressDialog != null){
                    if (mProgressDialog.isShowing()){
                        Log.i("AddBookmarkPage", "onSaveWithConfirm -> onPostExecute() -> mProgressDialog.dismiss()");
                            mProgressDialog.dismiss();
                    }
                    else{
                        return;
                ...
                if (mEditingFolder) {
                    if(cursor.getCount() ==0){
                        Log.i("AddBookmarkPage", "onSaveWithConfirm -> onPostExecute(), new folder");
                        saveConfirmed();
                        cursor.close();
                        return;
                    }

                    if ((mOriginalFolderName != null)&&(mOriginalFolderName.equals(title_f))&&(mOriginalCurrentFolder == mCurrentFolder)){
                        Log.i("AddBookmarkPage", "onSaveWithConfirm -> onPostExecute(), title not changed");
                        if(mFakeTitle == null){
                            cursor.close();
                            return;
                        }
                        if (mFakeTitle.getText().toString().equalsIgnoreCase(getResources().
                            getString(R.string.edit_folder))){
                            Log.i("AddBookmarkPage", "onSaveWithConfirm -> onPostExecute(), edit_folder, finish");
                            finish();
                         }else{
                            showFolderExistToast();
                         }
                    }else{
                        showFolderExistToast();
                    }
                    ...
                    if(count == 0){
                        cursor.close();
                        Log.i("AddBookmarkPage", "onSaveWithConfirm -> onPostExecute(), new bookmark");
                        saveConfirmed();
                        return;
                    }
                    /**
                     * Add for override bookmark feature
                     *@{
                     */
                    int exist_count = 0;
                    long id;
                    while(cursor.moveToNext()) {
                        id = cursor.getLong(0);
                        if ((mEditingExisting && id != mMap.getLong("_id")) || !mEditingExisting) {
                            exist_id = id;
                            exist_count ++;
                        }
                    }
                    cursor.close();
                    Log.i("AddBookmarkPage", "onSaveWithConfirm -> onPostExecute(), exist_count = " + exist_count);
                    if (exist_count == 0) {
                        saveConfirmed();
                    } else if (exist_count == 1) {
                        showDialogOverRidebookmark();
                    } else {
                        Log.i("AddBookmarkPage", "onSaveWithConfirm -> onPostExecute(), exist_count > 1, can not override, show fail");
                        showDialogFailed();
                    }
                    /*Add for override bookmark feature @}*/
                }
                Log.i("AddBookmarkPage", "onSaveWithConfirm -> onPostExecute() end");
            }
        }.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
    }

    private void showFolderExistToast(){
        Log.i("AddBookmarkPage", "showFolderExistToast");
        Toast.makeText(this, R.string.edit_folder_exist, Toast.LENGTH_SHORT).show();
    }
    /**
     * Add for override bookmark feature
    ...
    private void showDialogFailed(){
        try {
            ...
            .setTitle(getString(R.string.failed_edit_bookmark_title))
            .setMessage(getString(R.string.failed_edit_bookmark))
            ...
            Log.e("AddBookmarkPage","Exception when show failed dialog :"+e.getMessage());
        }
    }

    AlertDialog mAlertDialog = null;
    private void showDialogOverRidebookmark(){
        try {
            mAlertDialog = new AlertDialog.Builder(this)
            .setTitle(getString(R.string.save_to_bookmarks))
            .setMessage(getString(R.string.override_bookmark))
            .setNegativeButton(android.R.string.cancel, null)
            .setPositiveButton(android.R.string.ok,
                    new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int which) {
                            mEditingExisting = true;
                            Long id = mMap.getLong(BrowserContract.Bookmarks._ID, -1);
                            if(id != -1) {
                                mMap.putLong("original_id", id);
                            }
                            Log.e(LOGTAG,"showDialogOverRidebookmark, original_id = " + id + ", exist_id = " + exist_id);
                            mMap.putLong(BrowserContract.Bookmarks._ID, exist_id);
            ...
            // TODO: handle exception
            Log.e("AddBookmarkPage","Exception when show overideBookmark dialog :"+e.getMessage());
        }
    }
    /*Add for override bookmark feature @}*/
