...
public class BrowserHistoryPage extends Fragment
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        ...
        switch (id) {
            case LOADER_HISTORY: {
                ...
                HistoryLoader loader = new HistoryLoader(getActivity(), combinedBuilder.build(),
            ...
            case LOADER_MOST_VISITED: {
                Uri uri = combinedBuilder
                        ...
                        .build();
                String where = Combined.VISITS + " > 0";
                HistoryLoader loader = new HistoryLoader(getActivity(), uri,
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        /**
         *  SPRD:feature 522253 clear history menu
         *@{
         */
        if (item.getItemId() == R.id.clear_today_history_menu_id) {
            promptToClearTodayHistory();
            return true;
        }
        /**
         *@}
    ...
    public void onCreateContextMenu(ContextMenu menu, View v, ContextMenuInfo menuInfo) {
        ...
        if (mDisableNewWindow) {
            ...
            menu.findItem(R.id.new_window_context_menu_id)
                    .setTitle(
                            BrowserSettings.getInstance().openInBackground() ? R.string.contextmenu_openlink_newwindow_background
        ...
        switch (item.getItemId()) {
            ...
            case R.id.save_to_bookmarks_menu_id:
                if (historyItem.isBookmark()) {
                    ...
                    Util.saveBookmark(activity, title, url);
                }
                return true;
            ...
            case R.id.delete_context_menu_id:
                Util.deleteFromHistory(activity.getContentResolver(), url);
                return true;
    ...
    static class HistoryLoader extends CursorLoader {

        public HistoryLoader(Context context, Uri uri, String[] projection, String selection,
        ...
        public Cursor loadInBackground() {
            ...
            try {
                ...
                Log.e("BrowserHistoryPage","loadInBackground :"+e);
                if (cursor != null) {
                    cursor.close();
                    cursor = null;
                }
    ...
    /**
     *SPRD:feature 522253 of menu add history menu:@{
     */
    private MenuItem menuAll,menuToday;
    @Override
    public void onPrepareOptionsMenu(Menu menu) {
        if (menu != null) {
            menuAll = menu.findItem(R.id.clear_history_menu_id);
            if(menuAll != null){
                menuAll.setVisible(Util.canClearHistory(getActivity().getContentResolver()));
            }
            //ChinaMobile spec added for clear today's history
            menuToday = menu.findItem(R.id.clear_today_history_menu_id);
            if(menuToday != null){
                menuToday.setVisible(canClearTodayHistory());
            }
        }
    }

    public boolean canClearTodayHistory(){
        ...
        String query = "(bookmark = 0 OR visits > 0) AND date > "+begin;
        Cursor cursor = null;
        boolean ret = false;
        try{
            cursor = getActivity().getContentResolver().query(Browser.BOOKMARKS_URI,
            ...
            Log.e("BrowserHistoryPage", "canClearTodayHistory", e);
        } finally {
            if (cursor != null) {
                cursor.close();
            }
    ...
    // SPRD:feature 522253 of menu add history menu:add create today history dialog
    void promptToClearTodayHistory() {
        AlertDialog.Builder builder = new AlertDialog.Builder(getActivity())
                .setMessage(R.string.pref_privacy_clear_today_history_dlg)
                ...
                .setPositiveButton(R.string.ok, new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        if (which == DialogInterface.BUTTON_POSITIVE) {
                            ...
                            c.set(Calendar.SECOND, 0);
                            c.set(Calendar.MILLISECOND, 0);
                            long begin = c.getTimeInMillis()+1;
                            Util.deleteHistoryTimeFrame(getActivity().getContentResolver(), begin, -1);
                        }
                    }
                });
        final Dialog dialog = builder.create();
