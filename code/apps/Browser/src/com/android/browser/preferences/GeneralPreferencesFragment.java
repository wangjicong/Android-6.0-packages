...
public class GeneralPreferencesFragment extends PreferenceFragment
    ...
    static final String BROWSER_HOME_URL = "browser:home";
    /*@}*/
    ...
    public void onCreate(Bundle savedInstanceState) {
         ...
         *@{
         */
        ArrayList<String> cmccValue = new ArrayList<String>();
        ArrayList<String> cmccChoice = new ArrayList<String>();
        cmccChoice.addAll(Arrays.asList(mChoices));
        cmccValue.addAll(Arrays.asList(mValues));
        if (!Util.SUPPORT_NAVIGATION_TAB) {
            for (int i = 0; i < cmccValue.size(); i++) {
                if (BROWSER_HOME.equals(cmccValue.get(i))) {
                      cmccChoice.remove(i);
                      cmccValue.remove(i);
                      break;
                 }
        ...
        String[] value = new String[cmccValue.size()];
        cmccValue.toArray(value);
        String[] choice = new String[cmccChoice.size()];
        cmccChoice.toArray(choice);

        pref.setEntries(choice);
        pref.setEntryValues(value);
        /*@}*/
        ...
        /*remove most visited web in mainpage setting begin*/
        ArrayList<String> arrayValue = new ArrayList<String>();
        ArrayList<String> arrayChoice = new ArrayList<String>();
        arrayChoice.addAll(Arrays.asList(mChoices));
        arrayValue.addAll(Arrays.asList(mValues));
        for (int i = 0; i < arrayValue.size(); i++) {
            if (MOST_VISITED.equals(mValues[i])) {
                arrayChoice.remove(i);
                arrayValue.remove(i);
                break;
            }
        }
        String[] strValue = new String[arrayValue.size()];
        arrayValue.toArray(strValue);
        String[] strChoice = new String[arrayChoice.size()];
        arrayChoice.toArray(strChoice);

        pref.setEntries(strChoice);
        pref.setEntryValues(strValue);
        /*remove most visited web in mainpage setting end*/
    ...
    public boolean onPreferenceChange(Preference pref, Object objValue) {
        ...
        if (pref.getKey().equals(PREF_HOMEPAGE_PICKER)) {
            ...
            if(BROWSER_HOME.equals(objValue)){
                settings.setHomePage(BROWSER_HOME_URL);
            }
            /*@}*/
    ...
    String getHomepageValue() {
         ...
         *@{
         */
        if(BROWSER_HOME_URL.equals(homepage)){
            return BROWSER_HOME;
        }
        /*@}*/
