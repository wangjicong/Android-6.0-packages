...
public class UrlHandler {
    ...
    boolean shouldOverrideUrlLoading(Tab tab, WebView view, String url) {
        ...
        if (url!=null) {
            ...
            if (url.startsWith("mailto:")||url.startsWith("tel:")) {
                ...
                specialIntent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_NEW_TASK);
                mActivity.startActivity(specialIntent);
                return true;
            }
        }
    ...
    boolean startActivityForUrl(Tab tab, String url) {
      ...
      Matcher m = UrlUtils.ACCEPTED_URI_SCHEMA.matcher(url);
      if (Util.isMatches(m) && !isSpecializedHandlerAvailable(intent)) {
          return false;
      }
    ...
    boolean parseWtai(String url){
        ...
        if (url.startsWith(SCHEME_WTAI_SD)) {
            String number = url.substring(SCHEME_WTAI_SD.length());
            Intent intent = new Intent(Intent.ACTION_CALL,Uri.parse(WebView.SCHEME_TEL+number));
            if (mActivity.checkSelfPermission(Manifest.permission.CALL_PHONE)
                    != PackageManager.PERMISSION_GRANTED) {
                Log.i("UrlHandler", "no call phone permission, Controller.mPermissionObj = " + Controller.mPermissionObj);
                if (Controller.mPermissionObj == null) {
                    Controller.mPermissionObj = intent;
                    Log.i("UrlHandler", "requestPermissions call phone permission");
                    mActivity.requestPermissions(
                            new String[]{Manifest.permission.CALL_PHONE},
                            Controller.PERMISSIONS_REQUEST_CALL_PHONE);
                }
            } else {
                mActivity.startActivity(intent);
                mController.closeEmptyTab();
        ...
        if (url.startsWith(SCHEME_WTAI_AP)) {
            String contacts = url.toString().substring(
                    SCHEME_WTAI_AP.length());
            int index = contacts.indexOf(";");
            String name = null;
            String numbile = null;
            if (index > 0) {
                name = contacts.substring(index + 1, contacts.length());
                numbile = contacts.substring(0, index);
            } else {
                name = contacts;
                numbile = contacts;
            }
            Bundle extras = new Bundle();
            extras.putString(Insert.NAME, name);
            extras.putInt(Insert.PHONE_TYPE, Phone.TYPE_MOBILE);
            extras.putString(Insert.PHONE, numbile);
