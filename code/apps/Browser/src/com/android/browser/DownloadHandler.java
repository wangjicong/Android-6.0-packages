...
public class DownloadHandler {
    ...
    /*package */ static void onDownloadStartNoStream(Activity activity,
         ...
         *@{
         */
        request.setNotificationClass(OpenDownloadReceiver.class.getCanonicalName());
        /*@}*/
        if (mimetype == null) {
    ...
    public static void onDownloadStart(Controller controller,Activity activity, String url,
        String userAgent, String contentDisposition, String mimetype, boolean privateBrowsing, final boolean closeOnExit) {
        ...
        if(!StorageUtils.isExternalStorageMounted() && !StorageUtils.isInternalStorageMounted()){
            ...
            Toast.makeText(activity, R.string.download_failure, Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        if (DataUri.isDataUri(url)) {
            if(Util.USE_SYETEM_STORGE_PATH){
                // controller.onDownloadStartNotChoosePath(url, userAgent, contentDisposition, mimetype, privateBrowsing,closeOnExit)//Bug #478329 add storage location
           }else{
                ...
                if (!myName.getPackageName().equals(info.activityInfo.packageName) || !myName.getClassName().equals(info.activityInfo.name)) {
                    ...
                    .setNeutralButton(R.string.mdownload,
                        new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog, int which) {
                                    if(Util.USE_SYETEM_STORGE_PATH){
                                        mycontroller.onDownloadStartNotChoosePath(myurl, myuserAgent,mycontentDisposition, mytype, myprivateBrowsing,closeOnExit);//Bug #478329 add storage location
                                        }else{
    ...
    static void onDownloadStartNoStream(Controller controller,Activity activity,
        ...
        if (!BrowserPlugInDrm.getInstance().canGetDrmPath())
            return;
/*
        String status = null;
        String path = null;
...
*/
        ...
        /*@}*/
        Uri pathUri = null;
        pathUri = BrowserPlugInDrm.getInstance().getDrmPath(url, mimetype, filename);
        if  (pathUri != null) {
