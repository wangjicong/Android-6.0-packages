...
public class UploadHandler {
    ...
    void openFileChooser(ValueCallback<Uri[]> callback, FileChooserParams fileChooserParams) {
        Log.d("UploadHandler", "openFileChooser, mUploadMessage = " + mUploadMessage);
        ...
        //modify for permission
        int numPermissionsToRequest = 0;
        boolean requestReadStoragePermission = false;
        boolean requestCameraPermission = false;
        if (mController.getActivity().checkSelfPermission(Manifest.permission.CAMERA)
                != PackageManager.PERMISSION_GRANTED) {
            requestCameraPermission = true;
            numPermissionsToRequest ++;

        }
        if (mController.getActivity().checkSelfPermission(Manifest.permission.READ_EXTERNAL_STORAGE)
                != PackageManager.PERMISSION_GRANTED) {
        ...
        if (requestCameraPermission) {
            Log.i("UploadHandler", "no camera permission");
            permissionsToRequest[permissionsRequestIndex] = Manifest.permission.CAMERA;
            permissionsRequestIndex++;
        }
        if (requestReadStoragePermission) {
            Log.i("UploadHandler", "no read external storage permission");
            permissionsToRequest[permissionsRequestIndex] = Manifest.permission.READ_EXTERNAL_STORAGE;
            permissionsRequestIndex++;
        }

        if (permissionsRequestIndex > 0) {
            Log.i("UploadHandler", "no open file permission, Controller.mPermissionObj = " + Controller.mPermissionObj);
            if (Controller.mPermissionObj == null) {
                Controller.mPermissionObj = intent;
                Log.i("UploadHandler", "requestPermissions open file permission");
                mController.getActivity().requestPermissions(
                        permissionsToRequest,
                        Controller.PERMISSIONS_REQUEST_OPENFILE);
            }
        } else {
    ...
    /* modify for camera permission
     * Orignal android code:private void startActivity(Intent intent) {
     */
