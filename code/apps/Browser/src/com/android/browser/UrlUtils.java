...
public class UrlUtils {
    ...
    static final Pattern ACCEPTED_URI_SCHEMA = Pattern.compile(
            ...
            "(?:http|https|file|rtsp|tel):\\/\\/" +  //modify for rtsp feature
    ...
    public static String smartUrlFilter(String url, boolean canBeSearch) {
        ...
        if (Util.isMatches(matcher)) {
            ...
            if (hasSpace && Util.isMatches(Patterns.WEB_URL.matcher(inUrl))) {
                inUrl = inUrl.replace(" ", "%20");
            }
            return inUrl;
        }
        if (!hasSpace) {
            if (Util.isMatches(Patterns.WEB_URL.matcher(inUrl))) {
