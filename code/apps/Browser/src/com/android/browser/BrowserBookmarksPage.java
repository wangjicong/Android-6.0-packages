...
public class BrowserBookmarksPage extends Fragment implements View.OnCreateContextMenuListener,
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
        if (loader.getId() == LOADER_ACCOUNTS) {
           ...
           /* Add for Bug:527526 Start the bookmarks loader 2016.01.22 start */
            if (cursor != null && cursor.getCount() == 0) {
                BrowserBookmarksAdapter adapter = new BrowserBookmarksAdapter(
                        getActivity());
                mBookmarkAdapters.put(LOADER_BOOKMARKS, adapter);
                Bundle args = new Bundle();
                args.putString(ACCOUNT_NAME, null);
                args.putString(ACCOUNT_TYPE, null);
                mGrid.addAccount(null, adapter, true);
                lm.restartLoader(LOADER_BOOKMARKS, args, this);
            }
            /* Add for Bug:527526 Start the bookmarks loader 2016.01.22 end */
        ...
        if (isFolder) {
            ...
            if(mDisableNewWindow){
                for (int index = 0; index < menu.size(); index++) {
                    MenuItem child = menu.getItem(index);
                    if (child != null) {
                        if (child.getItemId() == (R.id.new_window_context_menu_id)
                                && child.getGroupId() == (R.id.FOLDER_CONTEXT_MENU)) {
            ...
            if (mDisableNewWindow) {
                ...
                /* Fix Bug:535434 The title displayed is incorrect. 2016.02.22 start */
                menu.findItem(R.id.new_window_context_menu_id)
                        .setTitle(
                                BrowserSettings.getInstance()
                                        .openInBackground() ? R.string.contextmenu_openlink_newwindow_background
                                        : R.string.contextmenu_openlink_newwindow);
                /* Fix Bug:535434 The title displayed is incorrect. 2016.02.22 end */
            }
        }
    ...
    /* SPRD: Modify for Bug:496755, can not open new tab with bookmarks in sub folders @{ */
    class OpenAllInTabsTask extends AsyncTask<Void, Void, ArrayList<String>> {
        long mFolderId;
        private Context mContext;
        private Uri mBookmarksUri;
        private ContentResolver mContentResolver;
        private ArrayList<String> mBookmarksUrls;

        public OpenAllInTabsTask(long id) {
            mFolderId = id;
            mContext = getActivity();
            mContentResolver = mContext.getContentResolver();
            mBookmarksUri = BookmarkUtils.getBookmarksUri(mContext);
            mBookmarksUrls = new ArrayList<String>();
        }

        @Override
        protected ArrayList<String> doInBackground(Void... params) {
            getBookmarksUrl(mFolderId);
            return mBookmarksUrls;
        }

        @Override
        protected void onPostExecute(ArrayList<String> results) {
            String[] urls = (String[]) results.toArray(new String[0]);
            if (results.size()<=0) {
                Toast.makeText(mContext, R.string.empty_bookmark_folder, Toast.LENGTH_LONG).show();
                return;
            }
            mCallbacks.onOpenInNewWindow(urls);
        }

        private void getBookmarksUrl(long id) {
            Cursor c = null;
            try {
                c = mContentResolver.query(mBookmarksUri, null,
                ...
                while (c.moveToNext()) {
                    if (c.getInt(c
                            .getColumnIndex(BrowserContract.Bookmarks.IS_FOLDER)) == 1) {
                        //get bookmarks url from sub folders
                        getBookmarksUrl(c.getInt(c
                                .getColumnIndex(BrowserContract.Bookmarks._ID)));
                    } else {
                        String url = c.getString(c
                                .getColumnIndex(BrowserContract.Bookmarks.URL));
                        mBookmarksUrls.add(url);
                    }
                }
            } catch (Exception e) {
                Log.e(LOGTAG, "getBookmarksUrl error", e);
            } finally {
                if(c != null) {
                    c.close();
                }
    ...
    private void editBookmark(BrowserBookmarksAdapter adapter, int position) {
        ...
        if (data != null) {
            ...
            item.putParcelable(BrowserContract.Bookmarks.FAVICON,
                    Util.compressImage(BitmapFactory.decodeByteArray(data, 0, data.length)));
    ...
    private void displayRemoveBookmarkDialog(BrowserBookmarksAdapter adapter,
        ...
        /**
         * modify update bookmark in folder
         * Orignal android code:
         * BookmarkUtils.displayRemoveBookmarkDialog(id, title, context, null);
         *@{
        */
        long folder = cursor.getLong(BookmarksLoader.COLUMN_INDEX_IS_FOLDER);
        BookmarkUtils.displayRemoveBookmarkDialog(id, title, context, null,folder);
        /*@}*/
    }
    ...
    /* Fix Bug:496830 Folders are identified as bookmarks 2015.11.11 start */
    private static class LookupBookmarkCount extends AsyncTask<Long, Void, Integer> {
        Context mContext;
        BookmarkItem mHeader;
        private int mBookmarksCount = 0;
        private Uri mBookmarksUri;
        private ContentResolver mContentResolver;
        ...
        private  void getBookmarkCount(long id) {
            Cursor c = null;
            try {
                c = mContentResolver.query(mBookmarksUri, null,
                ...
                Log.e(LOGTAG, "getBookmarkCount: " + e);
            }
            if (null == c) {
                Log.i(LOGTAG, "The cursor of getBookmarksCount is null");
                return;
            }
            while (c.moveToNext()) {
                if (c.getInt(c.getColumnIndex(BrowserContract.Bookmarks.IS_FOLDER)) == 1) {
                    getBookmarkCount(c.getInt(c
                            .getColumnIndex(BrowserContract.Bookmarks._ID)));
                } else {
                    mBookmarksCount++;
                }
        ...
        protected Integer doInBackground(Long... params) {
            ...
            getBookmarkCount(params[0]);
            return mBookmarksCount;
        }

        @Override
        protected void onPostExecute(Integer result) {
            mHeader.setUrl(mContext.getString(R.string.contextheader_folder_bookmarkcount,result));
        }
    }
    /* Fix Bug:496830 Folders are identified as bookmarks 2015.11.11 end */
    ...
    /**
     *SPRD:feature 522253 of menu add bookmark menu@{
     */
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        ...
        inflater.inflate(R.menu.bookmarks_add, menu);
    }

    @Override
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == R.id.new_bookmark_menu_id) {
            // add bookmark
            addNewPage();
            return true;
        }
        if (item.getItemId() == R.id.new_folder_menu_id) {
            // add new folder
            addFolder();
            return true;
        }
    ...
    private void addNewPage() {
        ...
        bundle.putString(BrowserContract.Bookmarks.URL, "");
    ...
    private void addFolder() {
        Bundle bundle = new Bundle();
        bundle.putLong(BookmarksLoader.EXTRA_ADD_BOOKMARK_IN_FOLDER_ID,
                mFolderId);
        bundle.putString(BookmarksLoader.EXTRA_ADD_BOOKMARK_IN_FOLDER_NAME,
                mFolderName);
        Intent intent = new Intent(getActivity(), AddBookmarkPage.class);
        intent.putExtra(BrowserContract.Bookmarks.TITLE, getActivity()
                .getString(R.string.new_folder));
        intent.putExtra(AddBookmarkPage.EXTRA_IS_FOLDER, true);
        intent.putExtras(bundle);
        startActivity(intent);
    }
    /**
