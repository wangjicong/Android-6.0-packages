...
public class SmilPlayer implements Runnable {
    ...
    private synchronized boolean isBeginOfSlide(TimelineEntry entry) {
        return (TimelineEntry.ACTION_BEGIN == entry.getAction())
                    && (entry.getElement() instanceof SmilParElementImpl
                            /** SPRD: add condition, the begin or end slide is SmilDocumentImpl @{*/
                            || entry.getElement() instanceof SmilDocumentImpl);
                            /** @}*/
    }
    ...
    public synchronized void run() { //changed by 4645 2014-12-23 coverity
        if (isStoppedState()) {
            return;
        }
        if (LOCAL_LOGV) {
    ...
    /**
     * SPRD bitmap recycle crash.
     * for bug 4.1-216608,4.3-215559 @{
     */
    public synchronized void release() { //changed by 4645 2014-12-23 coverity
        mMediaTimeUpdatedEvent = null;
        if(mAllEntries != null){
            mAllEntries.clear();
        }
        mAllEntries = null;
    ...
    /** SPRD: Do not deal with first slide,for bug226731 @{ */
