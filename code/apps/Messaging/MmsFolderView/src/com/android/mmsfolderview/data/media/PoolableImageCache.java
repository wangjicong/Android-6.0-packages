...
/**
 * A media cache that holds image resources, which doubles as a bitmap pool that
 * allows the consumer to optionally decode image resources using unused bitmaps
 * stored in the cache.
 */
public class PoolableImageCache extends MediaCache<ImageResource> {
    ...
    /**
     * Creates a new BitmapFactory.Options for using the self-contained bitmap
     * pool.
     */
    ...
    /**
     * A bitmap pool representation built on top of the image cache. It treats
     * the image resources stored in the image cache as a self-contained bitmap
     * pool and is able to create or reclaim bitmap resource as needed.
     */
    public class ReusableImageResourcePool {
        ...
        /**
         * Number of reuse failures to skip before reporting. For debugging
         * purposes, change to a lower number for more frequent reporting.
         */
        ...
        /**
         * A sparse array from bitmap size to a list of image cache entries that
         * match the given size. This map is used to quickly retrieve a usable
         * bitmap to be reused by an incoming ImageRequest. We need to ensure
         * that this sparse array always contains only elements currently in the
         * image cache with no other consumer.
         */
        ...
        /**
         * Load an input stream into a bitmap. Uses a bitmap from the pool if
         * possible to reduce memory turnover.
         * 
         ...
         * @throws IOException
         */
        public Bitmap decodeSampledBitmapFromInputStream(@NonNull
        final InputStream inputStream, @NonNull
        final BitmapFactory.Options optionsTmp, final int width, final int height)
                throws IOException {
            if (width <= 0 || height <= 0) {
                // This is an invalid / corrupted image of zero size.
                Log.d(TAG, "PoolableImageCache: Decoding bitmap with " + "invalid size");
            ...
            try {
                ...
                // BitmapFactory couldn't decode the file, try again without an
                // inputBufferBitmap.
                ...
                Log.e(TAG, "Oom decoding inputStream");
        ...
        /**
         * Turn encoded bytes into a bitmap. Uses a bitmap from the pool if
         * possible to reduce memory turnover.
         * 
         * @param bytes Encoded bytes to draw on the bitmap. Cannot be null.
         * @param optionsTmp The bitmap will set here and the input should be
         *            generated from getBitmapOptionsForPool(). Cannot be null.
         * @param width The width of the bitmap.
         * @param height The height of the bitmap.
         * @return A Bitmap with the encoded bytes drawn in it.
         * @throws IOException
         */
        public Bitmap decodeByteArray(@NonNull
        final byte[] bytes, @NonNull
        final BitmapFactory.Options optionsTmp, final int width, final int height)
            ...
            try {
                ...
                Log.d(TAG, "Oom decoding inputStream");
                MemoryCacheManager.get().reclaimMemory();
            }
            return b;
        }
        ...
        /**
         * Called when a new image resource is added to the cache. We add the
         * resource to the pool so it's properly keyed into the pool structure.
         */
        ...
        /**
         * Called when an image resource is evicted from the cache. Bitmap
         * pool's entries are strictly tied to their presence in the image
         * cache. Once an image is evicted from the cache, it should be removed
         * from the pool.
         */
        ...
        /**
         * Try to get a reusable bitmap from the pool with the given width and
         * height. As a result of this call, the caller will assume ownership of
         * the returned bitmap.
         */
            ...
            synchronized (PoolableImageCache.this) {
                ...
                if (poolKey != INVALID_POOL_KEY) {
                    ...
                    if (images != null && images.size() > 0) {
                        // Try to reuse the first available bitmap from the pool
                        // list. We start from
                        // the least recently added cache entry of the given
                        // size.
                        ...
                        for (int i = 0; i < images.size(); i++) {
                            ...
                            if (image.getRefCount() == 1) {
                                image.acquireLock();
                                if (image.getRefCount() == 1) {
                                    // The image is only used by the cache, so
                                    // it's reusable.
                                    imageToUse = images.remove(i);
                                    break;
                                } else {
                                    // Logically, this shouldn't happen, because
                                    // as soon as the
                                    // cache is the only user of this resource,
                                    // it will not be
                                    // used by anyone else until the next cache
                                    // access, but we
                                    // currently hold on to the cache lock. But
                                    // technically
                                    // future changes may violate this
                                    // assumption, so warn about
                                    // this.
                                    Log.d(TAG, "Image refCount changed "
                        ...
                        try {
                            ...
                            // Only reuse the bitmap if the last time we use was
                            // greater than 5s.
                            // This allows the cache a chance to reuse instead
                            // of always taking the
                            // oldest.
                            ...
                            // Try to reuse the bitmap from the image resource.
                            // This will transfer
                            // ownership of the bitmap object to the caller of
                            // this method.
                            ...
                            // We are either done with the reuse operation, or
                            // decided not to use
        ...
        /**
         * Try to locate and return a reusable bitmap from the pool, or create a
         * new bitmap.
         * 
         * @param width desired bitmap width
         * @param height desired bitmap height
         * @return the created or reused mutable bitmap that has its background
         *         cleared to {@value Color#TRANSPARENT}
         */
         ...
         * @return the created or reused mutable bitmap with the requested
         *         background color
         */
        public Bitmap createOrReuseBitmap(final int width, final int height,
            ...
            try {
                ...
                retBitmap = (poolBitmap != null) ? poolBitmap : Bitmap.createBitmap(width, height,
                        Bitmap.Config.ARGB_8888);
                retBitmap.eraseColor(backgroundColor);
            } catch (final OutOfMemoryError e) {
                Log.d(TAG, "PoolableImageCache:try to createOrReuseBitmap");
                MemoryCacheManager.get().reclaimMemory();
            }
            return retBitmap;
        }
        ...
        /**
         * @return The pool key for the provided image dimensions or 0 if either
         *         width or height is greater than the max supported image
         *         dimension.
         */
        ...
        /**
         * Called when bitmap reuse fails. Conditionally report the failure with
         * statistics.
         */
        private void onFailedToReuse() {
            mFailedBitmapReuseCount++;
            if (mFailedBitmapReuseCount % FAILED_REPORTING_FREQUENCY == 0) {
                Log.d(TAG, "Pooled bitmap consistently not being reused. Failure count = "
