...
import com.android.mmsfolderview.data.binding.Binding;
import com.android.mmsfolderview.data.binding.BindingBase;
import com.android.mmsfolderview.data.media.BindableMediaRequest;
import com.android.mmsfolderview.data.media.GifImageResource;
import com.android.mmsfolderview.data.media.ImageRequestDescriptor;
import com.android.mmsfolderview.data.media.ImageResource;
import com.android.mmsfolderview.data.media.MediaRequest;
import com.android.mmsfolderview.data.media.MediaResourceManager;
import com.android.mmsfolderview.data.media.MediaResourceManager.MediaResourceLoadListener;
import com.android.mmsfolderview.data.media.PlaceholderInsetDrawable;
import com.android.mmsfolderview.util.IntentUiUtils;
import com.android.mmsfolderview.util.ThreadUtil;
import com.google.common.annotations.VisibleForTesting;
import java.util.HashSet;

/**
 * An ImageView used to asynchronously request an image from
 * MediaResourceManager and render it.
 */
public class AsyncImageView extends ImageView implements MediaResourceLoadListener<ImageResource> {
    ...
    // 100ms delay before disposing the image in case the AsyncImageView is
    // re-added to the UI
    private static final int DISPOSE_IMAGE_DELAY = 100;

    // AsyncImageView has a 1-1 binding relationship with an ImageRequest
    // instance that requests
    // the image from the MediaResourceManager. Since the request is done
    // asynchronously, we
    // want to make sure the image view is always bound to the latest image
    // request that it
    // issues, so that when the image is loaded, the ImageRequest (which extends
    // BindableData)
    // will be able to figure out whether the binding is still valid and whether
    // the loaded image
    // should be delivered to the AsyncImageView via onMediaResourceLoaded()
    // callback.
    @VisibleForTesting
    ...
    /**
     * True if we want the image to reveal (scale) when it loads. When set to
     * true, this will take precedence over {@link #mFadeIn}
     */
    private final boolean mReveal;

    // The corner radius for drawing rounded corners around bitmap. The default
    // value is zero
    // (no rounded corners)
    ...
    // A placeholder drawable that takes the spot of the image when it's
    // loading. The default
    ...
    private final Runnable mDisposeRunnable = new Runnable() {
        ...
        public void run() {
            if (mImageRequestBinding.isBound()) {
                mDetachedRequestDescriptor = (ImageRequestDescriptor) mImageRequestBinding
                        .getData().getDescriptor();
            }
            unbindView();
    ...
    /**
     * The main entrypoint for AsyncImageView to load image resource given an
     * ImageRequestDescriptor
     * 
     * @param descriptor the request descriptor, or null if no image should be
     *            displayed
     */
    public void setImageResourceId(@Nullable
    final ImageRequestDescriptor descriptor) {
        ...
        if (mImageRequestBinding.isBound()) {
            if (TextUtils.equals(mImageRequestBinding.getData().getKey(), requestKey)) {
                // Don't re-request the bitmap if the new request is for the
                // same resource.
                return;
            }
            unbindView();
        ...
        if (!TextUtils.isEmpty(requestKey)) {
            ...
            final BindableMediaRequest<ImageResource> imageRequest = descriptor
                    .buildAsyncMediaRequest(getContext(), this);
            requestImage(imageRequest);
        }
    }

    /**
     * Sets a delay loader that centrally manages image request delay loading
     * logic.
     */
    ...
    public void resumeLoading() {
        if ((mDelayLoader != null) && mImageRequestBinding.isBound()) {
            MediaResourceManager.get().requestMediaResourceAsync(mImageRequestBinding.getData());
        }
    }

    /**
     * Setup the placeholder drawable if: 1. There's an image to be loaded AND
     * 2. We are given a placeholder drawable AND 3. The descriptor provided us
     * with source width and height.
     */
    private void maybeSetupPlaceholderDrawable(final ImageRequestDescriptor descriptor) {
        if (!TextUtils.isEmpty(descriptor.getKey()) && mPlaceholderDrawable != null) {
            if (descriptor.sourceWidth != SortMsgDataCollector.UNSPECIFIED_SIZE
                    && descriptor.sourceHeight != SortMsgDataCollector.UNSPECIFIED_SIZE) {
                // Set a transparent inset drawable to the foreground so it will
                // mimick the final
                // size of the image, and use the background to show the actual
                // placeholder
                // drawable.
                setImageDrawable(PlaceholderInsetDrawable.fromDrawable(new ColorDrawable(
                        Color.TRANSPARENT), descriptor.sourceWidth, descriptor.sourceHeight));
            }
    ...
    protected void setImage(final ImageResource resource, final boolean isCached) {
        // Switch reference to the new ImageResource. Make sure we release the
        // current
        // resource and addRef() on the new resource so that the underlying
        // bitmaps don't
        ...
        // The drawable may require work to get if its a static object so try to
        // only make this call
        // once.
        ...
        if (drawable != null) {
            ...
            if (getVisibility() == VISIBLE) {
                if (mReveal) {
                    ...
                    IntentUiUtils.revealOrHideViewWithAnimation(getContext(), this, VISIBLE, null);
            ...
            if (mImageResource instanceof GifImageResource) {
                Log.d(TAG, "setImage size unknown -- it's a GIF");
            } else {
                Log.d(TAG, "setImage size: " + mImageResource.getMediaSize() + " width: "
                        + mImageResource.getBitmap().getWidth() + " heigh: "
                        + mImageResource.getBitmap().getHeight());
            }
        }
        invalidate();
    }
    ...
    /**
     * Resets transient view states (eg. alpha, animations) before
     * rebinding/reusing the view.
     */
    ...
    protected void onAttachedToWindow() {
        ...
        // If it was recently removed, then cancel disposing, we're still using
        // it.
        ThreadUtil.getMainThreadHandler().removeCallbacks(mDisposeRunnable);

        // When the image view gets detached and immediately re-attached, any
        // fade-in animation
        // will be terminated, leaving the view in a semi-transparent state.
        ...
        // Check whether we are in a simple reuse scenario: detached from
        // window, and reattached
        // later without rebinding. This may be done by containers such as the
        // RecyclerView to
        // reuse the views. In this case, we would like to rebind the original
        // image request.
    ...
    protected void onDetachedFromWindow() {
        ...
        // Dispose the bitmap, but if an AysncImageView is removed from the
        // window, then quickly
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // The base implementation does not honor the minimum sizes. We try to
        // to honor it here.

        final int measuredWidth = getMeasuredWidth();
        final int measuredHeight = getMeasuredHeight();
        if (measuredWidth >= getMinimumWidth() || measuredHeight >= getMinimumHeight()) {
            // We are ok if either of the minimum sizes is honored. Note that
            // satisfying both the
            // sizes may not be possible, depending on the aspect ratio of the
            // image and whether
            // a maximum size has been specified. This implementation only tries
            // to handle the case
        ...
        if (!getAdjustViewBounds()) {
            // The base implementation is reasonable in this case. If the view
            // bounds cannot be
        ...
        final int minimumWidth = resolveSize(getMinimumWidth(), getMaxWidth(), widthMeasureSpec);
        final int minimumHeight = resolveSize(getMinimumHeight(), getMaxHeight(), heightMeasureSpec);
        final float aspectRatio = measuredWidth / (float) measuredHeight;
        if (aspectRatio == 0) {
            // If the image is (close to) infinitely high, there is not much we
            // can do.
            return;
        }

        if (width < minimumWidth) {
            height = resolveSize((int) (minimumWidth / aspectRatio), getMaxHeight(),
                    heightMeasureSpec);
            width = (int) (height * aspectRatio);
        }

        if (height < minimumHeight) {
            width = resolveSize((int) (minimumHeight * aspectRatio), getMaxWidth(),
                    widthMeasureSpec);
    ...
    /**
     * As a performance optimization, the consumer of the AsyncImageView may opt
     * to delay loading the image when it's busy doing other things (such as
     * when a list view is scrolling). In order to do this, the consumer can
     * create a new AsyncImageViewDelayLoader instance to be shared among all
     * relevant AsyncImageViews (through setDelayLoader() method), and call
     * onStartDelayLoading() and onStopDelayLoading() to start and stop delay
    ...
    public static class AsyncImageViewDelayLoader {
        ...
        public void onDelayLoading() {
            // Don't need to explicitly tell the AsyncImageView to stop loading
            // since
