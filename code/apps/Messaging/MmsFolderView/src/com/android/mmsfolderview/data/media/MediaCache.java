...
/**
 * A modified LruCache that is able to hold RefCountedMediaResource instances.
 * It releases ref on the entries as they are evicted from the cache, and it
 * uses the media resource size in kilobytes, instead of the entry count, as the
 * size of the cache. This class is used by the MediaResourceManager class to
 * maintain a number of caches for holding different types of
 * {@link RefCountedMediaResource}
 */
public class MediaCache<T extends RefCountedMediaResource> extends LruCache<String, T> {
    ...
    /**
     * Gets a media resource from this cache. Must use this method to get
     * resource instead of get() to ensure addRef() on the resource.
     */
    public synchronized T fetchResourceFromCache(final String key) {
        final T ret = get(key);
        if (ret != null) {
            Log.d(TAG, "cache hit in mediaCache @ " + getName() + ", total cache hit = "
                    + hitCount() + ", total cache miss = " + missCount());
            ret.addRef();
        } else {
            Log.d(TAG, "cache miss in mediaCache @ " + getName() + ", total cache hit = "
                    + hitCount() + ", total cache miss = " + missCount());
        }
        return ret;
    }
    ...
    /**
     * Add a media resource to this cache. Must use this method to add resource
     * instead of put() to ensure addRef() on the resource.
     */
