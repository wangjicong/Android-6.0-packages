...
public class WakeLockHelper {
    ...
    public void acquire(final Context context, final Intent intent, final int opcode) {
        synchronized (mLock) {
            if (mWakeLock == null) {
                if (DEBUG) {
                    Log.d(TAG, "initializing wakelock");
                }
        ...
        if (DEBUG) {
            Log.d(TAG, "acquiring " + mWakeLockId + " for opcode " + opcode);
        }
        mWakeLock.acquire();
    ...
    public boolean ensure(final Intent intent, final int opcode) {
        final boolean respectWakeLock = (mMyPid == intent.getIntExtra(EXTRA_CALLING_PID, -1));
        if (DEBUG) {
            Log.d(TAG, "WakeLockHelper.ensure Intent " + intent + " "
        ...
        if (respectWakeLock) {
            ...
            if (!isHeld) {
                Log.d(TAG, "WakeLockHelper.ensure called " + intent + " " + intent.getAction()
    ...
    public void release(final Intent intent, final int opcode) {
        ...
        if (respectWakeLock) {
            try {
                ...
                Log.d(TAG, "KeepAliveService.onHandleIntent exit crash " + intent + " "
