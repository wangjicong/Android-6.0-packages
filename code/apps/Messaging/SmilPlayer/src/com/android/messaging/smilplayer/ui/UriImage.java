...
import com.android.messaging.smilplayer.exif.ExifInterface;
import com.android.messaging.smilplayer.model.ImageModel;
...
public class UriImage {
    ...
    private void initFromFile(Context context, Uri uri) {
        ...
        /* SPRD:Bug 375488 Add for drm @{ */
        DrmModelUtils.getInstance(context).setImageType(this, uri, extension);
        /* @} */
    ...
    private void initFromContentUri(Context context, Uri uri) {
        ...
        /*
         * SPRD:Bug#283182 uri, null, null, null, null); 
         *  * orig :Cursor c = SqliteWrapper.query(context, resolver,
                            uri, null, null, null, null);
         * @{
         */
        Cursor c = null;
        /** SPRD: add for Bug#393796 @{ */
        try {
            if (isEmailUri(uri)) {
            ...
            Log.e(TAG, "initFromContentUri: reading uri: " + uri + ", occur Security Exception!", e);
        /** @} */
        }
        /* @} */
        ...
        if (c == null) {
            //bug 451329 begin
            Log.i(TAG,"Query on " + uri + " returns null result.");
            return;
            /*throw new IllegalArgumentException(
        ...
        try {
            ...
            if (ImageModel.isMmsUri(uri)) {
                ...
                if (mContentType != null && mContentType.contains("image")) {
                    try {
                        mOrientation = c.getInt(c.getColumnIndexOrThrow(Images.Media.ORIENTATION));
                    } catch (Exception e) {
                        Log.e(TAG, "Get mOrientation failed", e);
                    }
                }
                ...
                if (nameIndex != -1) {
                    ...
                    if (!TextUtils.isEmpty(mSrc)) {
                        ...
                        mSrc = mSrc.replace(' ', '_');
                        // SPRD: gif
                        // correct mimetype which may be wrong passed from media database.
                        if (mSrc.endsWith(".gif")){
                            mContentType = ContentType.IMAGE_GIF;
                        }
                        /* SPRD: Bug 375488 Add for drm */
                        DrmModelUtils.getInstance(context).setImageTypeBySrc(
                                this, mSrc);
                        /* @} */
                    } else {
                        mSrc = null;
    ...
    private void decodeBoundsInfo() {
        ...
        if(resolver == null){
            Log.e(TAG,"decodeBoundsInfo: the failure to get contentResolver!");
            return;
        }
        if ( mUri == null ) {
            Log.e(TAG,"decodeBoundsInfo: mUri is null.");
            return;
        }
        try {
            if (isEmailUri(mUri) ) {
                ...
                try {
                    fileDescriptor = resolver.openFileDescriptor(mUri, "r");
                    input = new ParcelFileDescriptor.AutoCloseInputStream(fileDescriptor);
                } catch (FileNotFoundException e) {
                    fileDescriptor = null;
                    Log.e(TAG,"decodeBoundsInfo:occur FileNotFoundException!");
                } catch (SecurityException e) {
                    fileDescriptor = null;
                    Log.e(TAG,"decodeBoundsInfo:occur Security Exception!");
                } catch (IOException ex) {
                    fileDescriptor = null;
                    Log.e(TAG, "decodeBoundsInfo:occur IOException!");
                }
            } else {
                input = resolver.openInputStream(mUri);
            }
    ...
    public static byte[] getResizedImageData(int width, int height,
        ...
        /*
         * SPRD:Bug#272578,delete this oraginal method because not used any more.
         * @orig int orientation = getOrientation(context, uri);
         */
        mOrientation = getOrientation(context, uri);//add by bug 428250
        ...
        Log.i(TAG,"getResizedImageData mOrientation:"+mOrientation);//add by bug 428250
        ...
        try {
            ...
            int quality = 95/*MessageUtils.IMAGE_COMPRESSION_QUALITY*/;
            Bitmap b = null;
            ...
            do {
                try {
                    ...
                    if (jpgFileSize > byteLimit) {
                        ...
                        if (quality < 50/*MessageUtils.MINIMUM_IMAGE_COMPRESSION_QUALITY*/) {
                            quality = 50/*MessageUtils.MINIMUM_IMAGE_COMPRESSION_QUALITY*/;
                        }
            ...
            /* SPRD: The attached picture is rotated 90 degrees Mms already has this function.*/
    ...
    private static Bitmap rotateAndScale(Bitmap b, int degrees, int dstWidth, int dstHeight,
            boolean filter) {
        ...
        if (b != null) {
            ...
            m.setScale(sx, sy);
            if (degrees != 0) {
                m.setRotate(degrees, (float) dstWidth / 2, (float) dstHeight / 2);
            }

            try {
                b2 = Bitmap.createBitmap(b, 0, 0, width, height, m, filter);
                b.recycle();
                mOrientation = 0;
            } catch (OutOfMemoryError ex) {
                // We have no memory to rotate. Return the original bitmap.
