...
public class TempFileProvider extends ContentProvider {
    ...
    /**
     * SPRD: Add this for double-T card. @{
     * Internal storage cache directory the temporal media file is stored in.
     */
    public static final String INTERNAL_STORAGE_CACHE_DIRECTORY = "Android/data/com.android.mms/";
    /** @} */
    ...
    public static String getScrapPath(Context context, String fileName) {
        /* SPRD: use internal  @{*/
         return Environment.getInternalStoragePath() + "/" + fileName;
        /* @} */
    }
    ...
    public static Uri renameScrapFile(String fileExtension, String uniqueIdentifier,
        ...
        // SPRD: modify for new file delete
        newTempFile.delete();
    ...
    public static Uri renameScrapPictureFile(String fileExtension, String uniqueIdentifier,
            Context context) {
        String filePath = getScrapPath(context);
        // There's only a single scrap file, but there can be several slides. We rename
        // the scrap file to a new scrap file with the slide number as part of the filename.
        ...
        Uri uri = Uri.fromFile(newTempFile);
        // Remove the old captured picture's thumbnail from the cache
        SmilPlayerApp.getThumbnailManager().removeThumbnail(uri);

        //for rotate the Picture
        //MessageUtils.rotatePicture(newTempFile.toString());
        return Uri.fromFile(newTempFile);
    }
}
