...
public abstract class CustomHeaderPagerListViewHolder extends BasePagerViewHolder
    ...
    /* Modify by SPRD for Bug:519832 2016.01.18 Start */
    // Reconstruct this method
...
//    }
    ...
    private Set<View> mInvisibleChildren = new HashSet<>();
    /**
    ...
    public void toggleVisibilityForPendingTransition(final boolean show, final View epicenterView) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (childView != epicenterView) {
                childView.setVisibility(show ? View.VISIBLE : View.INVISIBLE);
                if(!show) {
                    mInvisibleChildren.add(childView);
                } else {
                    mInvisibleChildren.remove(childView);
                }
            }
        }
        if(show) {
            for(View inVisibleChild: mInvisibleChildren) {
                if(inVisibleChild != null) {
                    inVisibleChild.setVisibility(View.VISIBLE);
                }
            }
            mInvisibleChildren.clear();
        }
    }
    /* Modify by SPRD for Bug:519832 2016.01.18 End */
