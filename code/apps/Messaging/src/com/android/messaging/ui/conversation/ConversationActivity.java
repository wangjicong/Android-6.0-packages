...
public class ConversationActivity extends BugleActionBarActivity
    ...
    //bug 496495 begin
    private static ConversationActivity mLastActivity = null;
    ...
    public void onBackPressed() {
        ...
        if ((conversationFragment != null) && (!conversationFragment.isVisible() || conversationFragment.onBackPressed())) {
            System.out.println("enter onBackPressed()");
            return;
        }
        if (mInstanceStateSaved){
            LogUtil.d(LogUtil.BUGLE_TAG, "ConversationActivity onBackPressed ");
            return;
        }
        super.onBackPressed();
    ...
    private void updateUiState(final boolean animate) {
        ...
        if (needConversationFragment) {
            ...
            final boolean isForwardingMms = intent.getBooleanExtra(UIIntents.UI_INTENT_EXTRA_IS_FORWARDING_MMS, false);
            final String forwardedMmsUri = intent.getStringExtra(UIIntents.UI_INTENT_EXTRA_FORWARDED_MMS_URI);
            ...
            if (!needContactPickerFragment) {
                ...
                /* Add by SPRD for bug 563344 Start */
                intent.removeExtra(UIIntents.UI_INTENT_EXTRA_IS_FORWARDING_MMS);
                intent.removeExtra(UIIntents.UI_INTENT_EXTRA_FORWARDED_MMS_URI);
                /* Add by SPRD for bug 563344 End */
            }
            conversationFragment.setHost(this);
            conversationFragment.setConversationInfo(this, conversationId, draftData);
            /* Add by SPRD for bug 563344 Start */
            conversationFragment.setIsForwardingMms(isForwardingMms);
            conversationFragment.setForwardedMmsUri(forwardedMmsUri);
            /* Add by SPRD for bug 563344 End */
