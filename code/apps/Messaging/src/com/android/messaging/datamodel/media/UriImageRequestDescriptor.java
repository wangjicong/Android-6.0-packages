...
public class UriImageRequestDescriptor extends ImageRequestDescriptor {
    ...
    public MediaRequest<ImageResource> buildSyncMediaRequest(final Context context) {
        ImageRequest<UriImageRequestDescriptor> imageRequest = null;
        if (uri == null || UriUtil.isLocalUri(uri)) {
            imageRequest = new UriImageRequest<UriImageRequestDescriptor>(context, this);
        } else {
            imageRequest = new NetworkUriImageRequest<UriImageRequestDescriptor>(context, this);
        }
        if (isDrm){
           try{
               imageRequest.setDrmType(true);
               imageRequest.setDrmContentType(mDrmContentType);
               if (uri != null){
                   imageRequest.setDrmDataPath(MessagingDrmSession.get().getPath(uri));
                   imageRequest.setDrmPreviewInPhotoViewActivity(mDrmPreviewInPhotoViewActivity);
	            imageRequest.setDrmDecryptHandle(mDecryptHandle);
               }
           }catch(Exception ex){
           }
        }
