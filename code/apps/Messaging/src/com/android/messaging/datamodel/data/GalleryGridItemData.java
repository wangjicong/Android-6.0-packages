...
public class GalleryGridItemData {
    ...
    public void bind(final Cursor cursor, final int desiredWidth, final int desiredHeight) {
        ...
        if (mIsDocumentPickerItem) {
            ...
            boolean isJpg = ContentType.IMAGE_JPEG.equals(mContentType)|| ContentType.IMAGE_JPG.equals(mContentType);
            mImageData = new FileImageRequestDescriptor(
                    ...
                    sourceWidth,
                    sourceHeight,
                    isJpg?false:true /* canUseThumbnail */,
                    true /* allowCompression */,
                    true /* isStatic */);

            if (MessagingDrmSession.get().drmCanHandle(dataPath, null)){
                 mImageData.setupDrm(true, ContentType.APP_DRM_CONTENT, dataPath);
                 mContentType = ContentType.APP_DRM_CONTENT;
            }
        }
    }
