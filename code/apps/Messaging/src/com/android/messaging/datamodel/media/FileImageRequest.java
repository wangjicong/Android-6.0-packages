...
public class FileImageRequest extends UriImageRequest {
    ...
    private Bitmap getGenericBitmap(){
             //if (MessagingDrmSession.get().getDrmFileRightsStatus(mPath, mDrmContentType) == true){
             //    return MessagingDrmSession.get().decodeDrmThumbnail(mPath, null, 0, 0);
             //}
             Bitmap bitmap = null;
             int resourceId = MessagingDrmSession.get().getIconImage(mPath, mDrmContentType);
...
/*
    ...
    protected InputStream getInputStreamForResource() throws FileNotFoundException {
        if (isDrm && mPath != null){
	       Log.d(TAG, "getInputStreamForResource is drm path "+mPath);
		if (MessagingDrmSession.get().getDrmFileRightsStatus(mDrmDataPath, mDrmContentType) == true){
                 return MessagingDrmSession.get().decodeGifStream(mPath);
              }
	       Bitmap bitmap = getGenericBitmap();
...
*/
    ...
    protected Bitmap loadBitmapInternal()
            throws IOException {
        Log.d(TAG, " path is "+mPath+" isDrm "+isDrm);
        if (isDrm){
             //if (MessagingDrmSession.get().getDrmFileRightsStatus(mPath, mDrmContentType) == true){
             if (mDrmPreviewInPhotoViewActivity){
                 mNotBitmapNeedCache = false;
                 return MessagingDrmSession.get().decodeDrmThumbnail(mPath, null, 0, 0, mDecryptHandle);
             }
             return getGenericBitmap();
        }
