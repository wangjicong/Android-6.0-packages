    ...
    */
    ...
    private static  String[] getConversationIdCondition(final String[]  conversationIds, final int nLimit ){
           ...
           for(   nIndex =0; nIndex <nRetLen -1; ){
               ...
               LogUtil.i(TAG,"jessica add getConversationIdCondition szRet[nIndex] = " + szRet[nIndex]);
               nIndex++;
               nStart = nStart + MAX_LIMIT;
           }
           nRom =  nLen % MAX_LIMIT;
           LogUtil.i(TAG,"jessica add getConversationIdCondition nRom = " + nRom);
           if( nRom != 0 ){
                 szRet[nIndex]= getConversationId(conversationIds, nRom, nStart);
                 LogUtil.i(TAG,"jessica add getConversationIdCondition nRom != 0 szRet[nIndex] = " + szRet[nIndex]);
           }
           if(nRetLen >= 1 && nRom == 0){
               szRet[nIndex]= getConversationId(conversationIds, MAX_LIMIT, nStart);
           }
    ...
    public static boolean deleteMultiConversation(final DatabaseWrapper dbWrapper, String[] conversationIds, final long cutoffTimestamp, long smax, long mmax) {
        ...
        boolean conversationDeleted = false;
        boolean conversationMessagesDeleted = false;
        LogUtil.i(TAG,"jessica add enter deleteMultiConversation ");
        ...
        long lmax = getConversationIdMax();
        ...
        try {
            if(szConversationIdCondition == null ){
                szRet =  dbWrapper.delete( DatabaseHelper.MESSAGES_TABLE,  "conversation_id <= ?" ,   szMax);
                 ...
                 for( String szCondition  : szConversationIdCondition){
                     if( !TextUtils.isEmpty(szCondition)){
                         ...
                         if(smax == 0 && mmax == 0){
                              ...
                              szRet = dbWrapper.delete(DatabaseHelper.MESSAGES_TABLE,  szSingleConversationIdCondition,  null);
                         ...
                         }else
                         {
                             ...
                             szRet = dbWrapper.delete(DatabaseHelper.MESSAGES_TABLE,  szSingleConversationIdCondition,  szMax);
                             count = dbWrapper.queryNumEntries(DatabaseHelper.MESSAGES_TABLE,
                                     szSingleConversationIdCondition, szMax);
                         }
                        LogUtil.i(TAG,"jessica enter deleteMultiConversation szSingleConversationIdCondition = " + szSingleConversationIdCondition);
                                conversationMessagesDeleted = (count == 0);
                                LogUtil.i(TAG,"jessica add deleteMultiConversation conversationMessagesDeleted= " + conversationMessagesDeleted);
                                if (!conversationMessagesDeleted) {
                                    LogUtil.w(TAG, "BugleDatabaseOperations:"
                                            + " cannot delete all messages in a conversation"
                                            + ", after deletion: count=" + count
                                ...
                                if (conversationMessagesDeleted) {
                                   ...
                                   }else
                                   {
                                    ...
                                    LogUtil.i(TAG,"jessica add deleteMultiConversation szSingleThreadCondition = " + szSingleConversationIdCondition);
                                    conversationDeleted = (count > 0);
                                    LogUtil.i(TAG,"jessica add deleteMultiConversation count = " + count);
                               }
                     }
                 }
