...
//add for wakelock begin
import android.os.PowerManager;
import android.os.PowerManager.WakeLock;
//add for wakelock end
/**
...
public class BugleNotifications {
    ...
    // SPRD: The lock will be released after the given timeout in milliseconds.add for wake lock
    protected static final int WAKETIME = 5000;
    ...
    Assert.isNotMainThread();
        checkInitialized();
        Log.d("BugleNotifications","====zhongjihao====update=====conversationId: "+conversationId);
    ...
    private static boolean shouldNotify(String conversationId) {
        ...
        try {
            ...
            cursor = db.rawQuery("SELECT "
                    ...
                    /* Modify by SPRD for Bug:525508 2016.01.19 Start */
//                  + ConversationColumns.SMS_THREAD_ID + "=" + conversationId,
                  + ConversationColumns._ID + "=" + conversationId,
                  /* Modify by SPRD for Bug:525508 2016.01.19 End */
                    null);

            if (cursor.moveToFirst()) {
                Log.d("BugleNotifications",
                        "======zhongjihao====shouldNotify======notification_enabled: "
                                + cursor.getInt(0));
                Assert.isTrue(cursor.getCount() == 1);
                isNotify = cursor.getInt(0) == 1 ? true : false;
            }
        } finally {
            if (cursor != null) {
                cursor.close();
    ...
    public static boolean shouldVibrate(final NotificationState state) {
        ...
        if (!state.getNotificationVibrate()) {
            ...
            if (MmsConfig.getKeepOrgSoundVibrate()) {
                ...
                try {
                    return Settings.System.getInt(context.getContentResolver(), "vibrate_when_message") == 0 ? false : true;
                }catch (Exception e) {
                    return false;
                }
            }
    ...
    /*SPRD: modified for bug 499870 end*/
        ...
        if (softSound) {
            ...
            return;
        /* SPRD: add for Bug 498629 begin */
        } else if (MmsConfig.getBeepOnCallStateEnabled()
        ...
        // bug 478514: Add for MmsFolderView Feature -- Begin
        final String MESSAGING_PACKAGE_NAME = "com.android.messaging";
        final String DETAILS_ACTIVITY_CLASS_NAME = "com.sprd.messaging.ui.folderview.MessageDetailsActivity";
        int isFolderView = BuglePrefs.getApplicationPrefs().getInt(
                BuglePrefsKeys.IS_FOLDER_VIEW_MODE, 0);
        if (isFolderView == 1 || isFolderView == 3) {
            final String UI_INTENT_EXTRA_CONVERSATION_ID = "conversation_id";
            final String UI_INTENT_EXTRA_MESSAGE_ID = "message_id";
            final String UI_INTENT_EXTRA_PARTICIPANT_NAME = "participant_name";
            final String UI_INTENT_EXTRA_MMS_SUBJECT = "mms_subject";
            final String UI_INTENT_EXTRA_RECEIVED_TIMESTAMP = "received_timestamp";
            final String UI_INTENT_EXTRA_MESSAGE_PROTOCOL = "message_protocol";
            final String UI_INTENT_EXTRA_READ = "read";
            final String UI_INTENT_EXTRA_SEEN = "seen";
            final String UI_INTENT_EXTRA_BODY_TEXT = "body_text";
            final String UI_INTENT_EXTRA_MESSAGE_STATUS = "message_status";
            final String UI_INTENT_EXTRA_TELEPHONY_DB_URI = "sms_message_uri";
            final String UI_INTENT_EXTRA_CONTENT_TYPE = "content_type";
            final String UI_INTENT_EXTRA_PARTICIPANT_NORMALIZED_DESTINATION = "participant_normalized_destination";
            final String UI_INTENT_EXTRA_IS_HAVED_STORE_CONTACT_NAME = "is_haved_store_contact_name";
            final String UI_INTENT_EXTRA_IS_COME_FROM_NOTIFICATION = "is_come_from_notification";
            if (state.mMsgCnt > 1) {
                destinationIntent = UIIntents.get().getPendingIntentForSortMsgListActivity(context,
                ...
                ComponentName componentName = new ComponentName(MESSAGING_PACKAGE_NAME,DETAILS_ACTIVITY_CLASS_NAME);
                intent.setComponent(componentName);
                intent.putExtra(UI_INTENT_EXTRA_CONVERSATION_ID,
                        singleMessageData.getConversationId());
                intent.putExtra(UI_INTENT_EXTRA_MESSAGE_ID, singleMessageData.getMessageId());
                intent.putExtra(UI_INTENT_EXTRA_PARTICIPANT_NAME,
                        singleMessageData.getParticipantName());
                intent.putExtra(UI_INTENT_EXTRA_MMS_SUBJECT, singleMessageData.getMmsSubject());
                intent.putExtra(UI_INTENT_EXTRA_RECEIVED_TIMESTAMP,
                        singleMessageData.getReceivedTimestamp());
                intent.putExtra(UI_INTENT_EXTRA_MESSAGE_PROTOCOL,
                        singleMessageData.getMessageProtocol());
                intent.putExtra(UI_INTENT_EXTRA_READ, singleMessageData.getIsRead());
                intent.putExtra(UI_INTENT_EXTRA_SEEN, singleMessageData.getIsSeen());
                intent.putExtra(UI_INTENT_EXTRA_BODY_TEXT, singleMessageData.getBobyText());
                intent.putExtra(UI_INTENT_EXTRA_MESSAGE_STATUS,
                        singleMessageData.getMessageStatus());
                intent.putExtra(UI_INTENT_EXTRA_TELEPHONY_DB_URI,
                        singleMessageData.getSmsMessageUri());
                intent.putExtra(UI_INTENT_EXTRA_CONTENT_TYPE, singleMessageData.getContentType());
                intent.putExtra(UI_INTENT_EXTRA_IS_HAVED_STORE_CONTACT_NAME,
                        singleMessageData.getIsHavedStoreContactName());
                intent.putExtra(UI_INTENT_EXTRA_IS_COME_FROM_NOTIFICATION, true);
                if (isFolderView == 3) {
                    intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK | Intent.FLAG_ACTIVITY_NEW_TASK);
                }
                destinationIntent = UIIntents.get().getPendingIntentForMessageDetailsActivity(
                        context, intent);
            }
        } else {
            // bug 478514: Add for MmsFolderView Feature -- End
    ...
    /*SPRD: modified for bug 499870 end*/
        ...
        if (state instanceof MultiConversationNotificationState) {
            for (final NotificationState child :
                ((MultiConversationNotificationState) state).mChildren) {
                /*SPRD: modified for bug 499870 begin*/
                processAndSend(child, true /* silent */, softSound, subId);
                /*SPRD: modified for bug 499870 end*/
                if (child.mConversationIds != null) {
...
//            .setPublicVersion(null)    // TODO: when/if we ever support different
    ...
    private static void fireOffNotification(final NotificationState notificationState,
        ...
        WakeLock wl = pm.newWakeLock(PowerManager.FULL_WAKE_LOCK
                | PowerManager.ACQUIRE_CAUSES_WAKEUP, "BugleNotifications");
        wl.acquire(WAKETIME);
        //add for wakelock end
    }
    ...
    public static void notifySmscEmpty(int subId) {
        ...
        failedIntent = new Intent(
                context,
                com.sprd.messaging.ui.appsettings.ShowSmscEditDialogActivity.class);
        Log.d("BugleNotification","[BugleNotification]====notifySmscEmpty===subId:"
                + subId);
