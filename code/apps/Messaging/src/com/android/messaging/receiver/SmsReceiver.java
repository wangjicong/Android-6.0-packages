...
public final class SmsReceiver extends BroadcastReceiver {
    ...
    public void onReceive(final Context context, final Intent intent) {
        Log.d(TAG, "=======SmsReceiver.onReceive " + intent);
    ...
    public static android.telephony.SmsMessage[] getMessagesFromIntent(Intent intent) {
        Log.d("SmsReceiver","========wap push===0==getMessagesFromIntent=====type:  "+intent.getType()+"  action: "+intent.getAction());
        final android.telephony.SmsMessage[] messages = Sms.Intents.getMessagesFromIntent(intent);

        // Check messages for validity
        // add for bug 506314 begin
        if (messages == null || (messages != null && messages.length < 1)) {
        // add for bug 506314 end
            return null;
        }
