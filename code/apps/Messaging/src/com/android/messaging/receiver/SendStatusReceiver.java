    ...
    public void onReceive(final Context context, final Intent intent) {
        ...
        if (MESSAGE_SENT_ACTION.equals(action)) {
            ...
            try {
                ...
                address = ((com.android.internal.telephony.gsm.SmsMessage)smsMessage.mWrappedSmsMessage)
                          .getRecipientAddress().address;   // Currently, this only deals with the GSM format pdu
                if (null != address) {
                    String report = context.getString(R.string.delivery_toast_body);
                    report = String.format(report, address);
                    Intent notify = new Intent(MmsUtils.NOTIFY_SHOW_MMS_SMS_REPORT_ACTION);
