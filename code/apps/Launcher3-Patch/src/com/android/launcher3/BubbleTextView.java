...
public class BubbleTextView extends TextView
    ...
    public void applyFromShortcutInfo(ItemInfo info, IconCache iconCache) {
    ...
    public void applyFromShortcutInfo(ItemInfo info, IconCache iconCache,
        ...
        //The function will create shortcut drawable in both allapplist and workspace.
        //It may set AppInfo or ShortcutInfo, so handle both types.
        if (info instanceof ShortcutInfo) {
            Bitmap b = ((ShortcutInfo)info).getIcon(iconCache);

            LauncherAppState app = LauncherAppState.getInstance();
            //b = app.getUnreadInfoManager().getBitmapWithUnreadInfo((ShortcutInfo)info, b);
            b = SprdUnreadInfoManager.getInstance().getBitmapWithUnreadInfo((ShortcutInfo)info, b);

            FastBitmapDrawable iconDrawable = mLauncher.createIconDrawable(b);
            iconDrawable.setGhostModeEnabled(((ShortcutInfo)info).isDisabled != 0);
        ...
        /*HWD asked CH play to be translated to Play store --up1705113@{*/
        if("CH Play".equals(info.title) /*&& "vi-VN".equals(SystemProperties.get("persist.sys.locale"))*/){
        	info.title = "Play store";
        }
		/*HWD asked CH play to be translated to Play store --up1705113@}*/
        setText(info.title);
        setTag(info);
        ...
        if (info instanceof ShortcutInfo) {
            if (promiseStateChanged || ((ShortcutInfo)info).isPromise()) {
    ...
    public void applyFromApplicationInfo(AppInfo info) {
		...
		/*HWD asked CH play to be translated to Play store --up1705113@{*/
        if("com.android.vending".equals(info.componentName.getPackageName()) /*&& "vi-VN".equals(SystemProperties.get("persist.sys.locale"))*/ && "CH Play".equals(info.title)){
        	info.title = "Play store";
        }
		/*HWD asked CH play to be translated to Play store --up1705113@}*/
        setText(info.title);
