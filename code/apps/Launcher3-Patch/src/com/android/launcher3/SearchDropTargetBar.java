...
public class SearchDropTargetBar extends FrameLayout implements DragController.DragListener {
    ...
    public void setQsbSearchBar(View qsb) {
        ...
        if (mQSBSearchBar != null) {
            //mHideSearchBarAnim = LauncherAnimUtils.ofFloat(mQSBSearchBar, "alpha", 1f, 0f);//Kalyy Bug49004
            //setupAnimation(mHideSearchBarAnim, mQSBSearchBar);//Kalyy Bug49004
        } else {
            // Create a no-op animation of the search bar is null
            //mHideSearchBarAnim = ValueAnimator.ofFloat(0, 0);//Kalyy Bug49004
            //mHideSearchBarAnim.setDuration(TRANSITION_DURATION);//Kalyy Bug49004
        }
    }
    ...
    public void finishAnimations() {
        ...
        //mHideSearchBarAnim.reverse();//Kalyy Bug49004
    }
    ...
    public void showSearchBar(boolean animated) {
        ...
        if (animated) {
            ...
            //mHideSearchBarAnim.cancel();//Kalyy Bug49004
            if (mQSBSearchBar != null) {
                mQSBSearchBar.setAlpha(1f);
            }
        }
    ...
    public void hideSearchBar(boolean animated) {
        if (mIsSearchBarHidden) return;
        if (animated) {
            //prepareStartAnimation(mQSBSearchBar);//Kalyy Bug49004
            //mHideSearchBarAnim.start();//Kalyy Bug49004
            if (mQSBSearchBar != null) {
    ...
    public void foreHideSearchBar(boolean animated) { //yanghua@20170428 Forced to hide SearchBar
        if (animated) {
            prepareStartAnimation(mQSBSearchBar);
    ...
    /*SUN:jicong.wang add for qsb start {@*/
    public void hideAnimSearchBar() {
        mHideSearchBarAnim = LauncherAnimUtils.ofFloat(mQSBSearchBar, "alpha", 1f, 0f);
        setupAnimation(mHideSearchBarAnim, mQSBSearchBar);
        prepareStartAnimation(mQSBSearchBar);
        mHideSearchBarAnim.start();
    }
    /*SUN:jicong.wang add for qsb end @}*/
    /**
     * Shows the drop target bar.
     */
    public void showDeleteTarget() {
        // Animate out the QSB search bar, and animate in the drop target bar
        /*SUN:jicong.wang modify for bug 52694 start {@*/
        mDeleteDropTarget.setText(R.string.delete_target_label);
        /*SUN:jicong.wang add for bug 52694 end @}*/
        prepareStartAnimation(mDropTargetBar);
