...
public class Launcher extends Activity
	...
	/*SUN:jicong.wang add for search enable {@ */
    private boolean mDisablAllAppQsb;    
	/*SUN:jicong.wang add for search enable @}*/
    ...
    /* SPRD: 508092 check CALL_PHONE permission @{ */
    private static final int PERMISSION_REQUESTCODE_CALL_PHONE = 1;
    //SPRD:519064 Rotating screen,unable to dial the phone
    private static View mClickedView = null;
    /* @} */
    ...
    // SPRD: bug501394 Bugfix launcher will crash when add widget if launcher recreated.
    ...
    // SPRD:523052 There is a empty screen when roating the screen
    ...
    private void handleActivityResult(
            ...
            if (appWidgetId < 0 || resultCode == RESULT_CANCELED) {
                ...
                if (!workspaceLocked) {
                    if (mPendingAddInfo.container == LauncherSettings.Favorites.CONTAINER_DESKTOP) {
                        ...
                        if (mPendingAddInfo != null) {
                            Log.d(TAG, "==== 1-screenId = "+mPendingAddInfo.screenId
                                    +"  ,container = "+mPendingAddInfo.container);
                        }
                    }
                    ...
                    final Runnable onComplete = new Runnable() {
                        @Override
                        public void run() {
                            /*SPRD: bug501394 ,launcher will crash in the first time add widget.@{*/
                            if( mWorkspace == null ){
                                mWaitingConfigForWidget = true;
                                setWorkspaceLoading(false);
                                Toast.makeText(getApplicationContext(), R.string.gadget_error_text, Toast.LENGTH_SHORT).show();
                                return;
                            }
                    ...
                    if (sPendingAddItem != null) {
                        Log.d(TAG, "====  2-screenid = "+sPendingAddItem.screenId
                                +"   ,container = "+sPendingAddItem.container);
                    }
                }
            }
            //SPRD:523052 There is a empty screen when roating the screen
            mHasConfigWidget = false;
            return;
        }
        ...
        if (requestCode == REQUEST_RECONFIGURE_APPWIDGET) {
            if (resultCode == RESULT_OK) {
                ...
                if (workspaceLocked) {
                    sPendingAddItem = args;
                    if (sPendingAddItem != null) {
                        Log.d(TAG, "====  3-screenid = "+sPendingAddItem.screenId
                                +"   ,container = "+sPendingAddItem.container);
                    }
                } else {
                    completeAdd(args);
        ...
        if (resultCode == RESULT_OK && mPendingAddInfo.container != ItemInfo.NO_ID) {
            ...
            if (isWorkspaceLocked()) {
                sPendingAddItem = args;
                if (sPendingAddItem != null) {
                    Log.d(TAG, "====  4-screenid = "+sPendingAddItem.screenId
                            +"   ,container = "+sPendingAddItem.container);
                }
            } else {
                completeAdd(args);
        ...
        switch(requestCode) {
            case PERMISSION_REQUESTCODE_CALL_PHONE:
                if (grantResults.length != 1 || grantResults[0] != PackageManager.PERMISSION_GRANTED) {
                    Toast.makeText(Launcher.this, R.string.callphone_permission_missed_hint, Toast.LENGTH_SHORT).show();
                    //SPRD:519064 Rotating screen,unable to dial the phone
                    mClickedView = null;
                } else {
                    if (mClickedView != null) {
                        startAppShortcutOrInfoActivity(mClickedView);
                        //SPRD:519064 Rotating screen,unable to dial the phone
                        mClickedView = null;
                    }
                }
    ...
    protected void onResume() {
        ...
        /* SPRD: 514202 control to invisible/visible page indicatior @{ */
        if (mPageIndicators != null) {
            if (isAllAppsVisible()) {
                mPageIndicators.setVisibility(View.INVISIBLE);
                //SPRD: bug 520608 After changing the language,The allapp interface display was not normal
            ...
            //SPRD: bug 526734 After changing the language,The allapp interface display was not normal
    ...
    protected boolean hasSettings() {
        if (mLauncherCallbacks != null && mLauncherCallbacks.hasSettings()) {   // Modify: For support Launcher3-Patch Portable START
    ...
    /* wangxing add 20160709 start @{ */
    public boolean onKeyUp(int keyCode, KeyEvent event) {
    	if(OptConfig.SUNVOV_CUSTOM_C7301_XLL_FWVGA||OptConfig.SUN_C7359_C5D_FWVGA_CHERRY){//wangxing add 20160711
    ...
    private void restoreState(Bundle savedState) {
        ...
        //SPRD:523052 There is a empty screen when roating the screen
        mHasConfigWidget = savedState.getBoolean(HASCONFIGWIDGET);
        ...
        Log.d(TAG, "==== 6-screenid = "+pendingAddScreen+"  ,container = "+pendingAddContainer);
    ...
    private void setupViews() {
		...
		/*SUN:jicong.wang add for search enable {@*/
        mDisablAllAppQsb = getResources().getBoolean(R.bool.disable_allapp_search_bar);
        Log.d(TAG,"mDisablAllAppQsb is "+mDisablAllAppQsb);
		/*SUN:jicong.wang add for search enable @}*/
    ...
    public View createShortcut(ViewGroup parent, ItemInfo info) {
        ...
        LauncherUtils.getInstance().createCustomeShortCut(info, favorite, mIconCache);
    ...
    protected void onSaveInstanceState(Bundle outState) {
        ...
        if (mPendingAddInfo != null) {
            Log.d(TAG, "====  5-screenid = "+mPendingAddInfo.screenId
                    +"   ,container = "+mPendingAddInfo.container);
        } else {
            Log.d(TAG, "==== onSaveInstanceState mPendingAddInfo is null");
        }
    ...
    private void processShortcutFromDrop(ComponentName componentName, long container, long screenId,
        ...
        mPendingAddInfo.dropPos = null;
        if (mPendingAddInfo != null) {
            Log.d(TAG, "==== 7-screenId = "+mPendingAddInfo.screenId
                    +"  ,container = "+mPendingAddInfo.container);
        }
    ...
    private void addAppWidgetFromDrop(PendingAddWidgetInfo info, long container, long screenId,
        ...
        if (mPendingAddInfo != null) {
            Log.d(TAG, "==== 8-screenId = "+mPendingAddInfo.screenId
                    +"  ,container = "+mPendingAddInfo.container);
        }
    ...
    public FolderIcon addFolder(CellLayout layout, long container,
                    final long screenId, int cellX, int cellY, FolderInfo info) {
        return LauncherUtils.getInstance().addCustomeFolder(layout, container, screenId, cellX, cellY,
                info, mWorkspace, mIconCache, isWorkspaceLocked(), Launcher.this,sFolders);
    }
    ...
    public void onBackPressed() {
        ...
        if (isAppsViewVisible()) {
            ...
            //SPRD: bug 520608 After changing the language,The allapp interface display was not normal
            showWorkspaceSearchAndHotseat();
            LauncherUtils.getInstance().closeFolderInAllApp(this, mAppsView);
    ...
    public void onClickPendingWidget(final PendingAppWidgetHostView v) {
        ...
        if (v.isReadyForClickSetup()) {
            ...
            if (appWidgetInfo != null) {
                ...
                if (mPendingAddInfo != null) {
                    Log.d(TAG, "==== 9-screenId = "+mPendingAddInfo.screenId
                            +"  ,container = "+mPendingAddInfo.container);
                }
    ...
    protected void onClickAllAppsButton(View v) {
        ...
        if (!isAppsViewVisible()) {
            /*SUN:jicong.wang modify for qsb start {@ */
            if(mQsb!=null && !mDisablAllAppQsb){
                mSearchDropTargetBar.hideAnimSearchBar();
            }
            /*SUN:jicong.wang modify for qsb end @}*/
    ...
    @Thunk void startAppShortcutOrInfoActivity(View v) {
        ...
        if (Intent.ACTION_CALL.equals(intent.getAction())
            ...
            mClickedView = v;
            requestPermissions(new String[] {CALL_PHONE}, PERMISSION_REQUESTCODE_CALL_PHONE);
            return;
        } else {
            mClickedView = null;
        }
    ...
    /** Modify: For support Launcher3-Patch Portable START ++++ **/
    //@Thunk  boolean startActivitySafely(View v, Intent intent, Object tag) {
    public  boolean startActivitySafely(View v, Intent intent, Object tag) {
    /** Modify: For support Launcher3-Patch Portable END ++++ **/
    ...
    public void growAndFadeOutFolderIcon(FolderIcon fi) {
        if (fi == null) return;
    ...
    public void openFolder(FolderIcon folderIcon) {
        ...
        LauncherUtils.getInstance().showCling(this, info);
        info.opened = true;

        // While the folder is open, the position of the icon cannot change.
        if (LauncherUtils.getInstance().isInstanceofLayoutParams(folderIcon)) {
        ...
        //folderIcon.setFolderEnable(info.isEnable());
        // Very strange that open folder keep INVISIBLE, here we do something to avoid this.
    ...
    void showOverviewMode(boolean animated) {
        ...
        mState = State.WORKSPACE;
        /*SUN:jicong.wang add for transtion start {@*/
        TextView widgetsButton = (TextView)findViewById(R.id.widget_button);
        widgetsButton.setText(getString(R.string.widget_button_text));
        
        TextView wallpaperButton = (TextView)findViewById(R.id.wallpaper_button);
        wallpaperButton.setText(getString(R.string.wallpaper_button_text));
        
        TextView settingsButton = (TextView)findViewById(R.id.settings_button);
        settingsButton.setText(getString(R.string.settings_button_text));        
        /*SUN:jicong.wang add for transtion end @}*/
    }
    ...
    public View getOrCreateQsbBar() {
        ...
        /*SUN:jicong remove qsb bar start {@*/
        if (getResources().getBoolean(R.bool.disable_qsb_bar)){
             return null;
        }
        /*SUN:jicong.wang remove qsb bar end @}*/
    ...
    public void bindAppWidget(final LauncherAppWidgetInfo item) {
        ...
        /*SPRD: bug501394 ,launcher will crash in the first time add widget. @{*/
        /*SPRD: bug508420 ,prevent deleting screen information from database. @{*/
        /*if(mWaitingConfigForWidget){
            setWorkspaceLoading(false);
            mWorkspace.removeExtraEmptyScreen(false, false);
        }*/
        /* @} */
    ...
    public void finishBindingItems() {
        /* SPRD:bug 508437 Launcher3 crash @{*/
        if (mWorkspace == null) {
            return;
        }
        /* @} */
        ...
        if (sPendingAddItem != null) {
            ...
            Log.d(TAG, "==== sPendingAddItem = null");
        }
    ...
    public void bindSearchablesChanged() {
        ...
        if(isAppsViewVisible()) { //yanghua@20170428 Forced to hide SearchBar in mainmenu
    ...
    public void bindAllApplications2Folder(final ArrayList<AppInfo> apps, final ArrayList<ItemInfo> items) {
        LauncherUtils.getInstance().bindAllApplications2Folder(apps, items, mAppsView, mLauncherCallbacks);
    }
     ...
     */
    public boolean shouldShowIntroScreen() {
    ...
    public void showFirstVolteRunClings() {
        // The two first run cling paths are mutually exclusive, if the launcher is preinstalled
        // on the device, then we always show the first run cling experience (or if there is no
        // launcher2). Otherwise, we prompt the user upon started for migration
        LauncherClingsVolte launcherClingsVolte = new LauncherClingsVolte(this);
    ...
    /*SPRD: bug 520608 After changing the language,The allapp interface display was not normal @{ */
    public void showWorkspaceAndHotseat(){
        showWorkspaceSearchAndHotseat();
    }
    /* @} */
