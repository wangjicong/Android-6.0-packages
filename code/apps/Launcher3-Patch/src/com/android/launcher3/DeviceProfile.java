...
public class DeviceProfile {
    ...
    float mCircularSlidingLayoutTopMargin;
    float mCircularSlidingLayoutLeftMargin;
    /* SPRD: bug371310 2014-11-19 Feature circular sliding. @} */
    ...
    public DeviceProfile(Context context, InvariantDeviceProfile inv,
		...
		/* SPRD: bug371310 2014-11-19 Feature circular sliding. @{ */
		 mCircularSlidingLayoutTopMargin = res.getInteger(R.integer.config_circular_sliding_layout_top_margin) / 100f;
		 mCircularSlidingLayoutLeftMargin = res.getInteger(R.integer.config_circular_sliding_layout_left_margin) / 100f;
		 /* SPRD: bug371310 2014-11-19 Feature circular sliding. @} */

        
        if(OptConfig.SUNVOV_CUSTOM_C7301_XLL_FWVGA || OptConfig.SUNVOV_CUSTOM_C7368_BP_MITO_FWVGA){
        	  allAppsIconTextSizePx = Utilities.pxFromDp(inv.iconTextSize, dm) - 3; 
            allAppsIconSizePx = Utilities.pxFromDp(inv.iconSize, dm) - 10;
        //qiuyaobo,20170317,begin    
				}else if(OptConfig.SUN_CUSTOM_C7367_QHD_R9){   
        	  allAppsIconTextSizePx = Utilities.pxFromDp(inv.iconTextSize, dm) + 3;
            allAppsIconSizePx = Utilities.pxFromDp(inv.iconSize, dm) - 10;
        //qiuyaobo,20170317,end    					         
        }else{
    ...
    private void updateIconSize(float scale, int drawablePadding, Resources res,
                                DisplayMetrics dm) {
        if(OptConfig.SUNVOV_CUSTOM_C7301_XLL_FWVGA || OptConfig.SUNVOV_CUSTOM_C7368_BP_MITO_FWVGA){                    	
            iconSizePx = (int) (Utilities.pxFromDp(inv.iconSize, dm) * scale) - 10;
            iconTextSizePx = (int) (Utilities.pxFromSp(inv.iconTextSize, dm) * scale) - 3; 
        //qiuyaobo,20170317,begin    
        }else if(OptConfig.SUN_CUSTOM_C7367_QHD_R9){
            iconSizePx = (int) (Utilities.pxFromDp(inv.iconSize, dm) * scale) - 10;
            iconTextSizePx = (int) (Utilities.pxFromSp(inv.iconTextSize, dm) * scale)+ 3;  
        ...
        /* Kalyy 160820 add start @{ */
        cellHeightPx=cellHeightPx+8+(iconTextSizePx-19)*3;
        /* Kalyy 160820 add end @} */
    ...
    public void layout(Launcher launcher) {
        ...
        if (pageIndicator != null) {
            /* SPRD: fix bug 500490,Show the page indicators when we have vertical search/hotseat @{*/
...
//                pageIndicator.setVisibility(View.GONE);
        ...
        if (overviewMode != null) {
         ...
         /* SPRD: bug371310 2014-11-19 Feature circular sliding. @{ */
            ViewGroup circularSlidingPanel = launcher.getCircularSlidingPanel();
            if (circularSlidingPanel != null) {
                lp = (FrameLayout.LayoutParams) circularSlidingPanel.getLayoutParams();
                int horizontalMargin = (int)(availableWidthPx * mCircularSlidingLayoutLeftMargin);
                lp.topMargin = (int)(availableHeightPx * mCircularSlidingLayoutTopMargin);
                if (isLayoutRtl) {
                    lp.rightMargin = horizontalMargin;
                } else {
                    lp.leftMargin = horizontalMargin;
                }
                circularSlidingPanel.setLayoutParams(lp);
            }
        /* SPRD: bug371310 2014-11-19 Feature circular sliding. @} */
