...
public class UCamDataProvider extends ContentProvider {
    ...
    public boolean onCreate() {
        ...
        if(!com.ucamera.ugallery.integration.Build.isTelecomCloud() && "QRD-8625DSDS".equals(Build.MODEL)){
            String internalRootDir = Util.getInternalStorageDir();
            if(internalRootDir != null){
                rootDir = new File(internalRootDir);
            }
        }

        String fileName = (new StringBuilder("Android")).append(File.separatorChar).append("data").append(File.separatorChar)
                .append(getContext().getPackageName()).append(File.separatorChar).append("cache").append(File.separatorChar).toString();
        ...
        try{
            ...
            Method m = cls.getMethod("getFatVolumeId", String.class);
            volumeID = (Integer) m.invoke(path);
        }catch(Exception e){
            Log.e(TAG, "Exception : "+e);
            e.printStackTrace();
            volumeID = fileName.hashCode();
        }
        String dbName = "thumb-" + Integer.toHexString(volumeID) + ".db";
        Log.d(TAG, "onCreate(): the path is " + path + ", and the volumeID is " + volumeID);

        mOpenHelper = new DatabaseHelper(getContext(), path, dbName);
        return true;
    }
