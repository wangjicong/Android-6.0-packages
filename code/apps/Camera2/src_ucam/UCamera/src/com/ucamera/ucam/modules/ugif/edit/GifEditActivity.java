...
import com.ucamera.uphoto.EffectShowedTipDlg;
    ...
    private AdapterView.OnItemClickListener mMyGalleryClickedListener = new AdapterView.OnItemClickListener() {
        public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
            ...
            if (mAllImageList[position] != -1) {
                if (mCounter <= 2) {
                    Toast.makeText(mContext, getResources().getString(R.string.gif_compose_edit_tips),
...
//                    }
    ...
    private void saveBitmap2Gif(final String saveTag, final boolean finish) {
        ...
        if (directory == null) {
            ...
            BackGroundWorkTask.processTask(this,
                    getString(R.string.ugif_edit_text_waiting), new Runnable() {
                        public void run() {
                            if (mSaveBtn.isEnabled()
                                    || (TAG_SHARE_GIF.equals(saveTag) && (false == mSaveCurrentGif || mCurrentGifUri == null))) {
                                ...
                                for (int i = 0; i < length; i++) {
                                    if (resultBitmaps[list[i]] != null
                                            && !resultBitmaps[list[i]]
                                                    .isRecycled())
                                        mAnimatedGifEncoder
                                                .addFrame(resultBitmaps[list[i]]); // fix
                                                                                   // bug
                                                                                   // 5734
                                }
                                mAnimatedGifEncoder.finish();
                                mSaveCurrentGif = insertDB(
                                        context.getContentResolver(), title,
            ...
            /*
             * FIX BUG: 6100 BUG COMMENT: remove last thumbnail of file dir after save image in
