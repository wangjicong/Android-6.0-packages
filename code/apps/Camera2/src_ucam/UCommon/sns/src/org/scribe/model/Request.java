...
//        conn.setChunkedStreamingMode(1024);
    ...
    /*
     * Caller is responible to close this ByteArrayOutputStream.
     */
    protected ByteArrayOutputStream createStreamBodyContent() {
        return null;
    }

    private boolean addStreamBody(HttpURLConnection conn) throws IOException {
        ByteArrayOutputStream baos = createStreamBodyContent();
        if (baos != null) {
            try {
                conn.setRequestProperty(CONTENT_LENGTH, String.valueOf(baos.size()));
                if (conn.getRequestProperty(CONTENT_TYPE) == null) {
                    conn.setRequestProperty(CONTENT_TYPE, DEFAULT_CONTENT_TYPE);
                }
                conn.setDoOutput(true);
    ...
    /**
     * Add body payload. This method is used when the HTTP body is not a
     * form-url-encoded string, but another thing. Like for example XML. Note:
     * The contents are not part of the OAuth signature
     *
