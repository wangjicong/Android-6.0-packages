...
public class CameraAppUI implements ModeListView.ModeSwitchListener,
    ...
    public static interface BottomPanel {
        ...
        public static interface Listener {
            ...
            /**
             * SPRD: fix bug 474674 Called when the "puzzle" button is pressed.
             */
     ...
     */
    private final static int DOWN_SAMPLE_RATE_FOR_SCREENSHOT = 1;
            ...
            if (ev.getActionMasked() == MotionEvent.ACTION_MOVE) {
                if (Math.abs(deltaX) > mSlop || Math.abs(deltaY) > mSlop) {
                    ...
                    if (deltaX >= Math.abs(deltaY)) {
                        ...
                        if (mController != null
                                && mController.getCurrentModuleIndex() != mController
                                        .getAndroidContext()
                                        .getResources()
...
//                showFilmstrip();
//            } else if ((-Math.abs(velocityX) > velocityY) && nowOrientation == 270) {
//                Log.i(TAG, "nowOrientation=" + nowOrientation + "-showFilmstrip");
//                ((Activity) mController)
//                        .setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_FULL_SENSOR);
//                showFilmstrip();
...
//        }
    ...
    public CameraAppUI(AppController controller, MainActivityLayout appRootView,
        ...
        mRoundedThumbnailView.setCallback(new RoundedThumbnailView.Callback() {
            ...
            public void onHitStateFinished() {
                //SPRD:fix bug535473 the orientation is wrong when close Filmstripview in landscape
                hidePanels();
    ...
    public void hideModeCover() {
        if (mHideCoverRunnable != null) {
            ...
            mAppRootView.postDelayed(mHideCoverRunnable , 150);
    ...
    public void prepareModuleUI() {
        ...
        // SPRD Bug:519334 Refactor Rotation UI of Camera.
        mController.getOrientationManager().addOnOrientationChangeListener(mModeOptionsOverlay);
    ...
    public void onPreviewStarted() {
        ...
        if (!isInIntentReview() && !isInFreezeReview()) {// SPRD:Fix bug 400555 && bug391138
            enableModeOptions();
        }
    }
    ...
    public void onSurfaceTextureUpdated() {
        Log.i(TAG, "onSurfaceTextureUpdated() mModeCoverState = " + mModeCoverState);
        if (mModeCoverState == COVER_WILL_HIDE_AT_NEXT_TEXTURE_UPDATE) {
            Log.i(TAG, "hiding cover via onSurfaceTextureUpdated");
            CameraPerformanceTracker.onEvent(CameraPerformanceTracker.FIRST_PREVIEW_FRAME);
            hideModeCover();
            if(!isInIntentReview() && !isInFreezeReview()){//SPRD:Fix bug 414865
                setSwipeEnabled(true);
            }
        }
    }
    ...
    public void setIndicatorBottomBarWrapperVisible(boolean visible) {
         ...
         */
        mStickyBottomCaptureLayout.setVisibility(visible && showPanel ? View.VISIBLE : View.INVISIBLE);
    }
    ...
    private void applyModuleSpecs(final HardwareSpec hardwareSpec,
        ...
        if (skipScopeCheck
            ...
            if (mController.getCameraProvider().getNumberOfCameras() > 1 &&
                ...
                if (bottomBarSpec.enableCamera) {
                    ...
                    buttonManager.initializeButton(ButtonManager.BUTTON_CAMERA,
                            bottomBarSpec.cameraCallback,
                            getDisableCameraButtonCallback(hdrButtonId));
            ...
            if (bottomBarSpec.hideHdr || mIsCaptureIntent) {
                ...
                if (mController.getCameraProvider().getCharacteristics(
                        mController.getCameraProvider().getCurrentCameraId().getLegacyValue()).isFacingFront()) {
        ...
        if (bottomBarSpec.enableExposureCompensation
            ...
            // SPRD: WHEN CAMERA IS BACK, HDR MUTEX WITH EXPOSURE,
            if (mController.getCameraProvider().getCharacteristics(mController.getCameraProvider().getCurrentCameraId().getLegacyValue())
                    .isFacingBack() && Keys.isHdrOn(mController.getSettingsManager())) {
    ...
    public void resetIndicatorIcon() {
        ...
        mController.getSettingsManager().addListener(this);
        mController.getButtonManager().setListener(mIndicatorIconController);
        mController.getSettingsManager().addListener(mController.getButtonManager());
        mController.getSettingsManager().addListener(mIndicatorIconController);
    }
    /* @} */
    ...
    /* SPRD:fix bug518054 ModeListView is appear when begin to capture using volume key */
    ...
    public void onOrientationChanged(OrientationManager orientationManager,
        ...
        setOrientation(mCameraRootView, orientation, true);
        nowOrientation = orientation;

        // SPRD Bug WideAngle Rotate
        if (previewStatusListener != null && previewStatusListener instanceof WideAnglePanoramaUI) {
            ((WideAnglePanoramaUI) previewStatusListener).adjustUI(orientation);
        }
    }
    ...
    public void showPanels() {
        if (mStickyBottomCaptureLayout != null) {
            ...
            showPanel = true;
            if (mPreviewStatusListener != null && mPreviewStatusListener instanceof PhotoUI) {
                ((PhotoUI) mPreviewStatusListener).showPanels();
            }
        }
    }
    ...
    public void hidePanels() {
        if (mStickyBottomCaptureLayout != null) {
            ...
            showPanel = false;
            if (mPreviewStatusListener != null && mPreviewStatusListener instanceof PhotoUI) {
                ((PhotoUI) mPreviewStatusListener).hidePanels();
            }
        }
    }
    /* @} */
    ...
    public int getBottomHeight() {
        ...
        int previewRectBottom =  (int) mCaptureLayoutHelper.getUncoveredPreviewRect().bottom;
        int bottomHeight = windowHeight - previewRectBottom ;
        Resources res = mController.getAndroidContext().getResources();
        // CID 123736 : DLS: Dead local store (FB.DLS_DEAD_LOCAL_STORE)
        // int maxHeight = res.getDimensionPixelSize(R.dimen.bottom_bar_height_max);
        int optimalHeight = res.getDimensionPixelSize(R.dimen.bottom_bar_height_optimal);
