...
class FilmstripBottomPanel implements CameraAppUI.BottomPanel {
    ...
    private ImageButton mPuzzleButton;//SPRD:fix bug 474674
    ...
    private static final Log.Tag TAG = new Log.Tag("FilmstripBottomPanel");//SPRD:fix bug 474674
    ...
    public FilmstripBottomPanel(AppController controller, ViewGroup bottomControlsLayout) {
        ...
        if (UCamUtill.isUcamPuzzleEnable()) {
            Log.i(TAG, "isUcamPuzzleEnnable="+UCamUtill.isUcamPuzzleEnable());
            setupPuzzleButton();
        }
        /* @}*/
    }
    ...
    /* SPRD: fix bug 474674 Add Feature About Ucam Of Puzzle @{*/
    @Override
    public void setPuzzleButtonVisibility(boolean visible) {
        Log.i(TAG, "isUcamPuzzleEnnable="+UCamUtill.isUcamPuzzleEnable());
        mPuzzleButton.setVisibility(visible ? View.VISIBLE : View.GONE);
        updateMiddleFillerLayoutVisibility();
    }
    ...
    public void setPuzzleEnabled(boolean enabled) {
        ...
        mPuzzleButton.setEnabled(enabled);
    }
    /* @}*/
    ...
    private void setupPuzzleButton() {
        mPuzzleButton = (ImageButton) mLayout.findViewById(R.id.filmstrip_bottom_control_puzzle);
        mPuzzleButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (mTinyPlanetEnabled) {
                    mController.openContextMenu(mPuzzleButton);
                } else if (mListener != null) {
                    mListener.onPuzzle();
                }
            }
        });
        mController.registerForContextMenu(mPuzzleButton);
        mPuzzleButton.setLongClickable(false);
    }
    /* @}*/
