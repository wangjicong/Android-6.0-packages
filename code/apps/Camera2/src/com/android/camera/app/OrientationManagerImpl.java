...
public class OrientationManagerImpl implements OrientationManager {
    ...
    /* SPRD:Fix bug 512594
     * bug description: the preview will auto-rotate based on how the user moves the device
     * when set the "portrait" in system @{ */
    // if the orientation is defined by user
    private boolean isCustomerDefined = false;

    // observe the setting change
    ...
    @Override
    public void resetOrientaion() {
        isCustomerDefined = false;
        if (Settings.System.getInt(mActivity.getContentResolver(),
                Settings.System.ACCELEROMETER_ROTATION, 0) == ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE) {
            mActivity
