...
public class VideoModule extends CameraModule
    ...
    private Toast mToastRecording;

    private int mTimeBetweenTimeLapseFrameCaptureMs;// SPRD: for bug 509708 add time lapse
    ...
    private final MediaSaver.OnMediaSavedListener mOnVideoSavedListener =
            new MediaSaver.OnMediaSavedListener() {
                ...
                public void onMediaSaved(Uri uri) {
                    /*SPRD:fix bug523146 Airtel, Shark L shows Camera force close@{
                    if (uri != null) {
                    */
                    ...
                    if (path.equals(currentPath)) {
                        ...
                        Toast.makeText(
                                ...
                                mActivity.getResources()
                                        .getString(R.string.sdcard_remove),
    ...
    public void onSingleTapUp(View view, int x, int y) {
        /*
         * SPRD Bug:509945,not start preview,intercept snapshot@{
    ...
    private void takeASnapshot() {
        ...
        if (!mIsVideoCaptureIntent) {
            ...
            // Set JPEG orientation. Even if screen UI is locked in portrait, camera orientation
            // should
            ...
            int jpegRotation =
                    CameraUtil
                            .getImageRotation(sensorOrientation, deviceOrientation, isFrontCamera);
    ...
    public void onShutterButtonClick() {
        /*
         * SPRD Bug:509945,not start preview,intercept recording @{
         * Original Android code:
        if (mSwitchingCamera) {
        */
    ...
    public void onShutterButtonFocus(boolean pressed) {
      ...
      //Sprd: Add for bug 529369 stop Video recording before switch camera
        Log.d(TAG,"mMediaRecorderRecording = "+mMediaRecorderRecording+",pressed"+pressed);
        ...
        /*sunvov:dlj add for 151014 start*/
          if(OptConfig.SUN_CAMERA_SCREEN){
            ...
            boolean isFullPreview = settingsManager.getBoolean(
                    SettingsManager.SCOPE_GLOBAL, Keys.KEY_VIDEO_FULL_PREVIEW,
                    mAppController.getAndroidContext().getResources().getBoolean(R.bool.default_video_full_preview));//Kalyy
            float targetRation;
            Size optimalSize=null;
            if(isFullPreview){
               DisplayMetrics dm2 = mActivity.getResources().getDisplayMetrics();
               targetRation = (float) dm2.heightPixels / (float) dm2.widthPixels;
               if(dm2.heightPixels == 800){//Kalyy Bug 46719
                   optimalSize = new Size(dm2.heightPixels,dm2.widthPixels);
               //qiuyaobo,20170523,begin    
               }else if((dm2.heightPixels == 888) && (dm2.widthPixels == 540)){   
               	   optimalSize = new Size(dm2.heightPixels,dm2.widthPixels); 
               //qiuyaobo,20170523,end	   
               }else{
                   optimalSize = CameraUtil.getOptimalPreviewSize(Size.convert(mCameraCapabilities.getSupportedPreviewSizes()),targetRation);
               }
            } else {
                targetRation = (float) 4 / 3;
                optimalSize = CameraUtil
                    .getOptimalPreviewSize(Size.convert(mCameraCapabilities.getSupportedPreviewSizes()),
                            targetRation);
            }
            mDesiredPreviewWidth = optimalSize.width();
            mDesiredPreviewHeight = optimalSize.height();
            mUI.setAspectRatio(targetRation);
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_VOLUME_DOWN:
                if (event.getRepeatCount() == 0) {
                    /**
                     * SPRD: fix bug 473602 add half-press @{
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_VOLUME_UP:
            case KeyEvent.KEYCODE_VOLUME_DOWN:
                /* SPRD:fix bug 546663 We will not handle the event because activity is in review mode@{ */
                if (isInReviewMode()) {
                    return true;
                }
                /* }@ */
    ...
    private void generateVideoFilename(int outputFileFormat) {
        ...
        /*
         * SPRD: Change storage videopath for storage path Feature
    ...
    private void saveVideo() {
        if (mVideoFileDescriptor == null) {
            ...
            // SPRD Bug:474704 Video Recording Pause.
            long duration = SystemClock.uptimeMillis() - mRecordingStartTime - mAllPauseTime;
            if (duration > 0) {
                //TimeLapse
                if (mCaptureTimeLapse) {
                    duration = duration * 1000 / (long)Math.ceil(30 * mTimeBetweenTimeLapseFrameCaptureMs);
                    Log.d(TAG, "saveVideo duration=" + duration);
                } else {

                    SettingsManager sm = mActivity.getSettingsManager();
                    int slow_motion = Integer.valueOf(sm.getString(SettingsManager.SCOPE_GLOBAL,
    ...
    private void startVideoRecording() {
        ...
        /* SPRD:fix 527653 Monkey is so fast that 2ms later after startVideoRecording
         * called, it shows the filmStrip, and then edit a photo, which will take a
         * long time, which caused ANR. setSwipeEnabled at the begin of startVideoRecording
         * to avoid ANR. @{*/
        ...
        mActivity.updateStorageSpaceAndHint(new CameraActivity.OnStorageUpdateDoneListener() {
            ...
            public void onStorageUpdateDone(long bytes) {
                if (bytes <= Storage.LOW_STORAGE_THRESHOLD_BYTES) {
                    ...
                    mUI.showRecordingUI(true, mActivity.getOrientationManager()
    ...
    private boolean stopVideoRecording() {
        ...
        */
        ...
        mAppController.getCameraAppUI().setSwipeEnabled(true);//SPRD:Fix bug391820
    ...
    private void setCameraParameters() {
        ...
        //jiazhenl 20170316 modify for Bug51132 for front flash
        //enableTorchMode(Keys.isCameraBackFacing(settingsManager, mAppController.getModuleScope()));
        if (OptConfig.SUN_FRONT_CAMERA_FLASH){
            enableTorchMode(true);
        }
        else
        {
    ...
    public void resume() {
        ...
        /* SPRD:Add for bug 496130 when recording video ,snap photo the optionbar shows @{ */
        getServices().getMediaSaver().setListener(new MediaSaverImpl.Listener() {
    ...
    private void switchCamera() {
        ...
        Log.i(TAG, "Start to switch camera. id=" + id + " mPendingSwitchCameraId=" + mPendingSwitchCameraId);
    ...
    private void enableTorchMode(boolean enable) {
        ...
        /*SUN:jicong.wang add for bug52442 start {@ */
        if(mActivity.getCameraAppUI().getFilmstripVisibility() == View.VISIBLE && enable){
            return;
        }
        /*SUN:jicong.wang add for bug52442 end @}*/
    ...
    public void onPreviewVisibilityChanged(int visibility) {
        if (mPreviewing) {
            /**
             * SPRD BUG 507795: won't open the flash when enlarge the picture in picture preview UI @{
             * Original Code
            ...
            */
            ...
            //enableTorchMode(visibility == ModuleController.VISIBILITY_VISIBLE  && mActivity.getCameraAppUI().getFilmstripVisibility() != View.VISIBLE);
            enableTorchMode(visibility != ModuleController.VISIBILITY_HIDDEN && mActivity.getCameraAppUI().getFilmstripVisibility() != View.VISIBLE);
            /* @} */
        }
    }
    ...
    public void onPauseClicked() {
        ...
        Log.d(TAG, "onPauseClicked mMediaRecorderRecording:" + mMediaRecorderRecording
                + ",mPauseRecorderRecording:" + mPauseRecorderRecording);
    ...
    public int getVideoEncodeType() {
        SettingsManager settingsManager = mActivity.getSettingsManager();
        String str_encode_type = settingsManager.getString(SettingsManager.SCOPE_GLOBAL,
    ...
    private void updateCameraShutterSound() {
        ...
        boolean isShutterOn = settingsManager.getBoolean(
                ...
                true);
        if (mCameraDevice != null) {
            mCameraDevice.enableShutterSound(isShutterOn);
        }
    }
    /* @} */
    ...
    private void updateParametersAntibanding() {
        ...
        String mAntibanding = settingsManager.getString(
                SettingsManager.SCOPE_GLOBAL, Keys.KEY_VIDEO_ANTIBANDING);
    ...
    private void updateTimeLapse() {
        SettingsManager settingsManager = mActivity.getSettingsManager();
        String frameIntervalStr = settingsManager.getString(SettingsManager.SCOPE_GLOBAL,
    ...
    private void setSlowmotionParameters() {
        SettingsManager settingsManager = mActivity.getSettingsManager();
        String slow_motion = settingsManager.getString(SettingsManager.SCOPE_GLOBAL,
                Keys.KEY_VIDEO_SLOW_MOTION);
        if (mIsVideoCaptureIntent) {
    ...
    public int getRecorderAudioSoruceBySlowMotion(boolean isVideoCaptureIntent) {
        ...
        if (settingsManager != null) {
            String sSlowMotion = settingsManager.getString(SettingsManager.SCOPE_GLOBAL,
            ...
            try {
                if ((Integer.parseInt(sSlowMotion) > 1 && !isVideoCaptureIntent)
                    ...
                    // @Reference media/audio/include/system/audio.h AUDIO_SOURCE_RECORD_NO_AUDIO = 9
                     result = 9;//MediaRecorder.AudioSource.RECORD_NO_AUDIO;
                }
