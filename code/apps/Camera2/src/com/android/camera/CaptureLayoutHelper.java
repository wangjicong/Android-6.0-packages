...
public class CaptureLayoutHelper implements CameraAppUI.NonDecorWindowSizeChangedListener,
    ...
    private int mDyLayoutChanged = 0;
	/*Hide navigation keys--up170217@}*/
    ...
    public RectF getBottomBarRect() {
        ...
        /*sunvov:dlj add  for HVGA camera adjus  SPCSS00383697*/
        if(OptConfig.SUN_HVGA_CAMERA){
        	if(mWindowWidth == 0){
            return new RectF();
		       }
		       RectF bottomBarRect = mPositionConfiguration.mBottomBarRect;
		       if(bottomBarRect.width() < mWindowWidth){
		           bottomBarRect.set(0, bottomBarRect.top, mWindowWidth, bottomBarRect.bottom);
		       }
		       return new RectF(bottomBarRect);
        }
        else{
    ...
    public void onNonDecorWindowSizeChanged(int width, int height, int rotation) {
        ...
        if(OptConfig.SUN_HIDE_NAVIGATION){
	        if(mWindowHeight != 0 && mDyLayoutChanged == 0 && (height - mWindowHeight) > 0){
	            mDyLayoutChanged = height - mWindowHeight;
	        }
    	}
		/*Hide navigation keys--up170217@}*/
	...
	/*Hide navigation keys--up170217@{*/
    public int getLayoutChanged(){
        return mDyLayoutChanged;
    }
    public void resetLayoutChanged(){ mDyLayoutChanged = 0; }
	/*Hide navigation keys--up170217@}*/
    /**
