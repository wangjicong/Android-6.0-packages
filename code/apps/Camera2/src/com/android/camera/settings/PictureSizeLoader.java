...
public class PictureSizeLoader {
    ...
    /** wangxing add 20160704 @{ */
    public List<Size> getMmiDefaultSize(List<Size> sizes, int cameraId){
    	String str;
    	if(cameraId == 0){
    	    str = mContext.getResources().getString(R.string.mmi_default_back_camera_size);
    	}else{
    	    str = mContext.getResources().getString(R.string.mmi_default_front_camera_size);
    	}
    	
    	String[] defaultSizesArray = str.split(",");
    	Set<String> defaultSizesSet = new HashSet(Lists.newArrayList(defaultSizesArray));
    	List<Size> newSizeList = new ArrayList<>();
    	for (Size size : sizes) {
    	    String sizeStr = size.getWidth() + "x" + size.getHeight();
    	    if(defaultSizesSet.contains(sizeStr)){
    ...
    private List<Size> computeSizesForCamera(CameraDeviceSelector facingSelector) {
        ...
        if (cameraId >= 0) {
            ...
            if (sizes != null) {
                ...
                if(OptConfig.SUNVOV_CUSTOM_C7301_XLL_FWVGA){//wangxing add 20160704
                	return getMmiDefaultSize(sizes, cameraId);
                }else{
                    return sizes;
                }
