...
public class CameraFilmstripDataAdapter implements LocalFilmstripDataAdapter {
    ...
    private class LoadNewPhotosTask extends AsyncTask<ContentResolver, Void, List<PhotoItem>> {
        ...
        private final Context mContext;
        private List<PhotoItem> photoData = null;

        public LoadNewPhotosTask(Context context, long lastPhotoId) {
            /* SPRD:fix bug496594 Edit and save one photo,then filmstrip does not have this edited
             * photo @{ */
            if (lastPhotoId == FilmstripItemBase.QUERY_ALL_MEDIA_ID
                    && (photoData = mPhotoItemFactory.queryAll()) != null
                    && !photoData.isEmpty()) {
                lastPhotoId = photoData.get(0).getData().getContentId() - 1;
            }
            /* }@ */
            mContext = context;
        ...
        protected void onPostExecute(List<PhotoItem> newPhotoData) {
            Log.i(TAG, "new photos query items:" + newPhotoData);
