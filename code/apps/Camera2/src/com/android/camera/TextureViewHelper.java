...
public class TextureViewHelper implements TextureView.SurfaceTextureListener,
	...
	/*Hide navigation keys--up170217@{*/
    private final float FULL_SCREEN_ASPECT = 16 / 9f;
	/*Hide navigation keys--up170217@}*/
    ...
    private boolean updateTransform() {
        ...
        /* SPRD:Fix bug 547544 java.lang.NullPointerException @{ */
        if (cameraKey == null) {
            return true;
        }
        /* @} */
        ...
        if (ApiHelper.IS_NEXUS_4 && mAppController.getCameraFeatureConfig().isUsingCaptureModule()
            ...
            matrix = getPreviewRotationalTransform(mOrientation,
				...
				/*sunvov:dlj add  for HVGA camera adjus  SPCSS00375630*/
        		if(OptConfig.SUN_HVGA_CAMERA){
		        	RectF previewRect = mCaptureLayoutHelper.getPreviewRect();
		            if(previewRect.width() < mWidth) {
		                float top = -(mWidth * mAspectRatio - mHeight) / 2;
		                float bottom = mHeight+ (mWidth * mAspectRatio - mHeight) / 2;
		                previewRect = new RectF(0f, top, mWidth * 1f, bottom);
		            }
		            ...
		            if(!OptConfig.SUN_HIDE_NAVIGATION){
			            ...
			            matrix = info.getPreviewTransform(mOrientation, new RectF(0, 0, mWidth, mHeight),
			                    mCaptureLayoutHelper.getPreviewRect());
									/*Reset the preview area--up170217@{*/
		            }else{
			            RectF previewRect = mCaptureLayoutHelper.getPreviewRect();
								int dy = mCaptureLayoutHelper.getLayoutChanged();
								float bottom = previewRect.height() + dy;
								mAspectRatio = bottom/previewRect.width();
						            if(dy != 0 && Math.abs(mAspectRatio - FULL_SCREEN_ASPECT) < 0.01){
						                previewRect.set(0, previewRect.top - dy, mWidth, previewRect.bottom);
						            }
						            Log.i(TAG, "xuehui, mAspectRatio:" + mAspectRatio);
						            matrix = info.getPreviewTransform(mOrientation, new RectF(0, 0, mWidth, mHeight),
						                    previewRect);
					}
					/*Reset the preview area--up170217@}*/
        ...
        /*Hide navigation keys--up170217@{*/
        if(OptConfig.SUN_HIDE_NAVIGATION){
        	mCaptureLayoutHelper.resetLayoutChanged();
    	}
    	/*Hide navigation keys--up170217@}*/
        return true;
    }
