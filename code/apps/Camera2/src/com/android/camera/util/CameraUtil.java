...
public class CameraUtil {
    ...
    private static boolean isWideAngleEnable = false;
    private final static String TARGET_TS_WIDEANGLE_ENABLE = "persist.sys.cam.wideangle";
    ...
    public static void showError(final Activity activity, final int dialogMsgId, final int feedbackMsgId,
        ...
        if (!activity.isFinishing()) {
            ...
            /*
             * SPRD Bug:488399 Remove Google Report. @{
     ...
     */
    private static boolean isNinetynieBurstEnabled = false;
    ...
    public static boolean mTimeStamp = false;

    public static void initialize(Context context) {
        isNinetynieBurstEnabled = android.os.SystemProperties.getBoolean(
                TARGET_TS_BURST_NINETYNINE_ENABLE, false);

        isZslBurstEnabled = android.os.SystemProperties.getBoolean(
                TARGET_TS_ZSL_BURST_ENABLE, false);

        isWideAngleEnable = android.os.SystemProperties.getBoolean(
                TARGET_TS_WIDEANGLE_ENABLE, true);

				//qiuyaobo,20161111,begin
        //isZslEnable = android.os.SystemProperties.getBoolean(
        //        TARGET_TS_ZSL_ENABLE, true);
        isZslEnable = android.os.SystemProperties.getBoolean(
                TARGET_TS_ZSL_ENABLE, context.getResources().getBoolean(R.bool.def_zsl_enable));
        //qiuyaobo,20161111,end                        

        isFrontCameraMirrorEnable = android.os.SystemProperties.getBoolean(
                TARGET_TS_FRONT_CAMERA_MIRROR_ENABLE, true);
    }
    ...
    public static boolean isNinetyNineBurstEnabled() {
        return isNinetynieBurstEnabled;
    }
    ...
    public static void initializeTimeFormat(Context context) {
        mImageFileNamer = new ImageFileNamer(
                context.getString(R.string.image_file_name_format));
    }
    ...
    public static String setTimeStampPicPath() {
        String resourcePath = StorageUtil.getInstance().getFileDir();

        if (resourcePath == null) {
            return null;
        }

        resourcePath = resourcePath.substring(0, resourcePath.lastIndexOf("Camera")-1);
        String timeStampResPath = resourcePath + subPathTimeStampRes;
        String timeStampRootResPath = resourcePath + subPathRootRes;

        if (resourcePath == null) {
            return null;
        }

        File file = new File(timeStampResPath);
        ...
        synchronized(CameraUtil.class){
            if (!file.exists()) {
                ...
                copyTimeStampResToPath(timeStampResPath);
            }
        }
        ImageProcessJni.SetResourcePath(timeStampRootResPath);
        return timeStampResPath;
    }

    public static void copyTimeStampResToPath( final String path) {
                for (int i = 0; i < 12; ++i) {
                    String strFileName = i + ".png";
                    createFileToSdcard(strFileName, path, AndroidContext.instance().get());
                }
    }

    private static void createFileToSdcard(String strFileName, String strDirectory, Context mContext) {
       ...
       try {
        ...
        /*
         * CID 123706/123705: Resource leak on an exceptional path (RESOURCE_LEAK)
         * @{ */
        InputStream in = null;
