...
import com.ucamera.ucam.modules.utils.UCamUtill;//SPRD:fix bug 474674
...
public class CameraActivity extends QuickActivity
    ...
    private static final boolean IS_LOW_RAM = ActivityManager.isLowRamDeviceStatic();
    ...
    private int mFilmStripCount;//SPRD:fix bug520618 Video is still appear if it is deleted from fileManager
        ...
        public void onServiceConnected(ComponentName arg0, IBinder arg1) {
            ...
            try {
                if (iFloatWindowController.closeFloatWindow()) {
                    Toast.makeText(CameraActivity.this, R.string.close_float_video,Toast.LENGTH_LONG).show();
                }
            } catch (Exception e) {
    ...
    private final CameraAppUI.BottomPanel.Listener mMyFilmstripBottomControlListener =
            new CameraAppUI.BottomPanel.Listener() {
                ...
                /* SPRD: fix bug 474674 Add Feature About Ucam Of Puzzle @{*/
                @Override
    ...
    private final FilmstripContentPanel.Listener mFilmstripListener =
            new FilmstripContentPanel.Listener() {
                ...
                public void onFilmstripShown() {
                    // SPRD Bug:519334, 527627 Refactor Rotation UI of Camera.
                    if (mOrientationManager == null) {
                        Log.e(TAG, "onFilmstripShown returned,"
                                + "because cameraActivity is destroyed.");
                        return;
                    }
                    mOrientationManager
    ...
    public void notifyNewMedia(final Uri uri) {
        ...
        /**
         * SPRD:fix bug 438428 getType be hold due parallel operate database @{
        ...
        */
        ...
        new AsyncTask<Void, Void, FilmstripItem>() {
            ...
            protected void onPostExecute(final FilmstripItem data) {
                ...
                // Legacy modules don't use CaptureSession, so we show the capture indicator when
                // the item was safed.
                if (mCurrentModule instanceof PhotoModule && mCurrentModule != null
                        && mCurrentModule.getBurstHasCaptureCount() < 2 && !((PhotoModule)mCurrentModule).mBurstMode ||
                        mCurrentModule instanceof VideoModule) {//SPRD:fix bug 501739 when capture in burst mode a  thumbnail appears
                    AsyncTask.THREAD_POOL_EXECUTOR.execute(new Runnable() {
                        @Override
                        public void run() {
                            final Optional<Bitmap> bitmap = data.generateThumbnail(
    ...
    private void removeItemAt(int index) {
        Log.i(TAG, "removeItemAt");
        mDataAdapter.removeAt(index);
        /*SPRD:fix bug 496611 when delete photos to the last one,
         * Camera automatically returns to preview interface  */
    ...
    public void onCreateTasks(Bundle state) {
        ...
        /*
         * SPRD: add for 565002 that resetting camera defaults from CameraSettingsActivity
         * will lead to null exception @{
         * orginal code
         *
        ...
        storageUtil.initialize(getServices());
    ...
    private int getModeIndex() {
        ...
        */
        if (!returnFromOtherActivity && (MediaStore.INTENT_ACTION_VIDEO_CAMERA.equals(intentAction)
                || MediaStore.ACTION_VIDEO_CAPTURE.equals(intentAction))) {
         ...
         * Android original code
                ...
                || MediaStore.ACTION_IMAGE_CAPTURE_SECURE.equals(intentAction)) {
        */
        } else if (!returnFromOtherActivity && (MediaStore.INTENT_ACTION_STILL_IMAGE_CAMERA.equals(intentAction)
                ||MediaStore.INTENT_ACTION_STILL_IMAGE_CAMERA_SECURE.equals(intentAction)
                || MediaStore.ACTION_IMAGE_CAPTURE_SECURE.equals(intentAction))) {
            ...
            */
            int maxIndex = getResources().getInteger(R.integer.camera_mode_max_index);

            if ((modeIndex == gcamIndex &&
                    !GcamHelper.hasGcamAsSeparateModule(mFeatureConfig)) || modeIndex < 0 || modeIndex > maxIndex) {
                modeIndex = photoIndex;
            }
            /**@}*/
        }
    ...
    public void onPauseTasks() {
        ...
        /* SPRD: fix bug 498612 If the detailDialog is shown,
         it should dismiss when the activity is onPause. @{ */
        ...
        /*
         * SPRD: CameraActivity onPause need to updateStorage ensure OnResume to
         * check storage update to updateAdapter
         */
        StorageUtil storageUtil = StorageUtil.getInstance();
    ...
    public void onResumeTasks() {
        ...
        new Thread(new Runnable() {
            public void run() {
                ...
                if (cost > 100) {
                    Log.i(TAG, "acquireProvider cost: " + cost);
                }
            }
        }).start();
        /* @} */
    ...
    private void resume() {
        ...
        if (mLocalVideosObserver.isMediaDataChangedDuringPause()
            ...
            if (!mSecureCamera) {
                ...
                if (!mFilmstripVisible || haveDeletion) {
                /*@}*/
                    mDataAdapter.requestLoad(new Callback<Void>() {
                        @Override
                        public void onCallback(Void result) {
    ...
    public void onStartTasks() {
        ...
        floatWindowIntent.setAction("android.intent.action.FloatWindowAIDLService");
    ...
    public void onBackPressed() {
        if (!mCameraAppUI.onBackPressed()) {
            ...
            */
            if (!mCurrentModule.onBackPressed() && isResumed()) {
                super.onBackPressed();
            }
            /* }@ */
        }
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        /* SPRD:Fix bug 494050
         * First come in Camera,then capture,the gallery icon in camera disappears@{ */
    ...
    /**
     * SPRD:fix bug 474690 modify for Pano
     * original code
    ...
    /**
     *SPRD: Change for New Feature scenery
     *original code
     * @{
   ...
   /**
    ...
    public void onModeSelected(int modeIndex) {
        /* SPRD: Fix bug 563781 that panorama mode acquires large memory, which results too many anr @{ */
        if (IS_LOW_RAM && Debug.isMonkey()) {
            if (modeIndex == getResources().getInteger(R.integer.camera_mode_panorama)) {
                Log.i(TAG, "not switch into panorama mode in monkey test");
                return;
            }
        }
        /* @} */

        // SPRD Bug 540471: It's still in COVER_SHOWN state before.
        if (mCurrentModeIndex == modeIndex || mPaused) {
            return;
        }
    ...
    public void onSettingsSelected() {
        ...
        intent.putExtra(CameraSettingsActivity.CAMERA_SCOPE, getCameraScope());
     ...
     */
    /**
     * SPRD:fix bug 474673 add ucam edit @{
     ...
     */
    public void launchEditor(FilmstripItem data) {
        Log.i(TAG,"launchEditor isUcamEditSupport="+UCamUtill.isUcamEditEnable());
        if (UCamUtill.isUcamEditEnable()) {
            launchUCameraEdit(data);
        } else {
    ...
    public void launchUCameraEdit(FilmstripItem data) {
        Log.i(TAG, "launchUCameraEdit ImageFileName=" + data.getData().getFilePath());
        try {
            Intent intent = new Intent();
            intent.setAction("android.intent.action.SPRD_EDITOR");
            intent.setDataAndType(data.getData().getUri(), "image/*");
            //intent.putExtra("PictureDegree", 90);
            intent.putExtra("extra_from_inner", true);
            intent.putExtra("ImageFileName", data.getData().getFilePath());
            startActivity(intent);
            overridePendingTransition(0, 0);
        } catch (Exception e) {
            String errorMsg = new StringBuilder()
                    .append("Fail start com.ucamera.ucam/com.ucamera.uphoto.ImageEditControlActivity")
                    .append("uri=").append(data.getData().getUri())
                    .append("fullname=").append(data.getData().getFilePath())
                    .toString();
            Log.e(TAG, errorMsg);
        }
    }
    ...
    private void launchPuzzle() {
        Log.i(TAG, "launchPuzzle start");
        Intent puzzleIntent = new Intent();
        /* SPRD:Fix bug 539907 Puzzle function is error. @{ */
        puzzleIntent.setClass(this, ImageGallery.class);
        puzzleIntent.putExtra("extra_image_entry", "puzzle");
        puzzleIntent.putExtra("extra_image_storage", "external");
        puzzleIntent.putExtra("Extra.Wait.For.Result", true);
        puzzleIntent.putExtra("Extra.ImageGallery.Class",
                PuzzleImagePicker.class);
        /* @} */
        puzzleIntent.putExtra("mediaTypes",1 << 0);
        try {
            startActivity(puzzleIntent);
        } catch (ActivityNotFoundException e) {
            Log.i(TAG, "Failed startActivity launchPuzzle exception");
        }
    }
    /* @}*/
    ...
    private void updateBottomControlsByData(final FilmstripItem currentData) {
        ...
        /* SPRD: fix bug 474674 Add Feature About Ucam Of Puzzle @{*/
        if (UCamUtill.isUcamPuzzleEnable()) {
            filmstripBottomPanel.setPuzzleButtonVisibility(
                    currentData.getAttributes().canPuzzle());
        }
        /* Progress bar */
    ...
    /* SPRD:updateAdapter if StorageUpdated */
    public void updateAdapterIfStorageUpdated() {
        if (!mSecureCamera) {// SPRD:Fix bug 520083
            StorageUtil storageutil = StorageUtil.getInstance();
            if (storageutil.isStorageUpdated()) {
                mDataAdapter.requestLoad(new Callback<Void>() {
                    @Override
                    public void onCallback(Void result) {
                        fillTemporarySessions();
    ...
    private String getCurrentStorage() {
         /* SUN:jicong.wang remove for bug 50746
        if (mSettingsManager != null) {
            return mSettingsManager.getString(SettingsManager.SCOPE_GLOBAL,
                        Keys.KEY_CAMERA_STORAGE_PATH);
        }
        return null;
        */
        return StorageUtil.getInstance().getCurrentStorage();//SUN:jicong.wang modify for bug 50746
    }
    /* }@ */
    ...
    private class MyBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            String storagePath = mSettingsManager.getString(SettingsManager.SCOPE_GLOBAL,
                    Keys.KEY_CAMERA_STORAGE_PATH);
            if (action.equals(Intent.ACTION_MEDIA_EJECT) && !mSecureCamera && !isCaptureIntent()
                    && MultiStorage.KEY_DEFAULT_EXTERNAL.equals(storagePath)) {
                Log.i(TAG, "T card has been removed");
                if (StorageUtil.getStoragePathState(MultiStorage.KEY_DEFAULT_INTERNAL)) {
                        ...
                        protected Void doInBackground(Void... arg) {
                            mDataAdapter.requestLoad(new Callback<Void>() {
                                ...
                                public void onCallback(Void result) {
                                    fillTemporarySessions();
                                    mFilmstripController.setDataAdapter(mDataAdapter);
                                    if (mDataAdapter.getTotalNumber() < 1 && mFilmstripVisible) {
                                        mCameraAppUI.getFilmstripContentPanel().animateHide();
                                    }
                                    updateStorageSpaceAndHint(null);
                                }
