...
public class MediaSaverImpl implements MediaSaver {
    ...
    public void addImage(final byte[] data, String title, long date, Location loc, int width,
            ...
            String mimeType) {
        /*
         * Fix bug 547144 that app is killed because of low memory
         * original code @{
         *
        if (isQueueFull()) {
        ...
        t.execute();
         */
        t.executeOnExecutor(CameraFilmstripDataAdapter.THREAD_POOL_EXECUTOR);
    }
        ...
        protected Uri doInBackground(Void... v) {
            ...
            try {
                if (CameraUtil.mTimeStamp) {
                    ...
                    Log.i(TAG, "CML_addWatermarkForJpeg start");

                    String timeStampPath = CameraUtil.setTimeStampPicPath();
                    if(timeStampPath == null){
                        return null;
                    }
                    byte[] newData = ImageProcessJni.ImageTimeStamp(data,data.length,markDate);
                    Log.i(TAG, "CML_addWatermarkForJpeg end");
                    return Storage.addImage(
                            resolver, title, date, loc, orientation, exif, newData, width, height);
                }
                return Storage.addImage(
