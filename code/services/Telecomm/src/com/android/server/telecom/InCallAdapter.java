...
class InCallAdapter extends IInCallAdapter.Stub {
    ...
    public void answerCall(String callId, int videoState) {
        ...
        answerMPCCall(callId,videoState, MultiPartCallHelper.MPC_MODE_HF);
    }
     ...
     */
    @Override
    public void answerMPCCall(String callId, int videoState, int mpcMode) {
        long token = Binder.clearCallingIdentity();
        try {
            synchronized (mLock) {
                Log.d(this, "answerCall(%s,%d,%d)", callId, videoState, mpcMode);
                if (mCallIdMapper.isValidCallId(callId)) {
                    Call call = mCallIdMapper.getCall(callId);
                    if (call != null) {
                        mCallsManager.answerCall(call, videoState, mpcMode);
                    } else {
            ...
            synchronized (mLock) {
                if (mCallIdMapper.isValidCallId(callId)) {
                    ...
                    if (call != null) {
                        ...
                        Log.w(this, "explicitCallTransfer, unknown call id: %s", callId);
                    }
                }
            }
        } finally {
