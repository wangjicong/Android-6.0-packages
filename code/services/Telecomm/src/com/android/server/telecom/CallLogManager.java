...
import com.sprd.server.telecom.IccUriUtils;
...
import com.sprd.telecom.plugin.TelecomPluginsHelper;
...
final class CallLogManager extends CallsManagerListenerBase {
    ...
    public void onCallStateChanged(Call call, int oldState, int newState) {
        ...
        if (isNewlyDisconnected &&
            ...
            /** SPRD: add for log rejected calls plugin. @{ */
            int type;
            /* if (!call.isIncoming()) {
             if (!call.isIncoming()) {
            ...
            if (call.isUnknown() && call.isOutGoing()) {
                type = Calls.OUTGOING_TYPE;
            } else {
                type = getCallLogType(call.isIncoming(), disconnectCause);
            }
            /** @} */
            logCall(call, type);
        }
    ...
    void logCall(Call call, int callLogType) {
        ...
        /*SPRD:add for VoLTE @{*/
        int volteFlag = 0;
        if (call.can(android.telecom.Connection.CAPABILITY_HIGH_DEF_AUDIO)) {
            volteFlag = 0x2;
        } else {
            volteFlag = 0;
        }
        callFeatures = callFeatures | volteFlag;
    ...
    public boolean okToLogEmergencyNumber() {
        CallLogUtils callLogUtils = CallLogUtils.getInstance(mContext);
        return callLogUtils.okToLogEmergencyNumber();
    }
    /** @} */
    ...
    private void logCall(
        ...
        // SPRD: Using plugins scheme to handle multi-situations.
        // SPRD: modify for bug532286.
        final boolean okToLogEmergencyNumber = okToLogEmergencyNumber();
                /* SPRD: Using plugins scheme to handle "log emergency calls" feature. @{
                * @orig
                * mContext.getResources().getBoolean(R.bool.allow_emergency_numbers_in_call_log); */
                //TelecomPluginsHelper.getInstance(mContext).shouldLogEmergencyCalls(mContext);
                 /* @} */
        ...
        if (isOkToLogThisCall) {
            ...
            logCallAsync(args);
            /* SPRD: Add for feature LND/SDN list. BEGIN */
            logLndAsync(args);
            /* SPRD: END */
        } else {
    ...
    /** SPRD: Add for feature LND/SDN list. BEGIN */
    public AsyncTask<AddCallArgs, Void, Void> logLndAsync(AddCallArgs args) {
    ...
    private class LogLndAsyncTask extends AsyncTask<AddCallArgs, Void, Void> {
        ...
        protected Void doInBackground(AddCallArgs... callList) {
            ...
            for (int i = 0; i < count; i++) {
                AddCallArgs c = callList[i];
                try {
                     /** SPRD: Add for bug 505352. BEGIN */
                     if(c  !=  null){
                        ContentValues contentValues = new ContentValues();
                        if (c.callerInfo.name == null) {
                            contentValues.put(IccUriUtils.NAME, c.number);
                        } else {
                            contentValues.put(IccUriUtils.NAME, c.callerInfo.name);
                        }
                        contentValues.put(IccUriUtils.NUMBER, c.number);
                        if( c.accountHandle  !=  null && c.accountHandle.getId()  !=  null){
                              SubscriptionManager subscriptionManager = SubscriptionManager.from(mContext);
                              SubscriptionInfo subscriptionInfo =   subscriptionManager.getActiveSubscriptionInfoForIccIndex(c.accountHandle.getId());
                              String subId =String.valueOf(subscriptionInfo.getSubscriptionId());
                              Uri uri = c.context.getContentResolver().insert(
                              IccUriUtils.getIccURI("lnd", subId ), contentValues);
                              Log.d(TAG, "NAME : " + c.callerInfo.name + ", NUMBER : " + c.number + ", SUBID = " +subId + ", uri : "+ uri);
                         }
                      }
                     /** SPRD: END */
                    } catch (Exception e) {
    ...
    /** SPRD: add for log rejected calls plugin @{ */
    private int getCallLogType(boolean isIncoming, int disconnectCause) {
        LogRejectedCallsUtils logRejectedCallsUtils = LogRejectedCallsUtils.getInstance(mContext);
        return logRejectedCallsUtils.getCallLogType(isIncoming, disconnectCause);
    }
    /** @} */
}
