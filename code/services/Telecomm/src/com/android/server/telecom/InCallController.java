...
public final class InCallController extends CallsManagerListenerBase {
    ...
    private ParcelableCall toParcelableCall(Call call, boolean includeVideoProvider) {
       ...
       /* boolean isDefaultSmsAccount =
        ...
        // SPRD: Update connectRealTimeMillis when call is Conference Call
        long connectRealTimeMillis = call.getConnectRealTimeMillis();
        List<Call> childCalls = call.getChildCalls();
        List<String> childCallIds = new ArrayList<>();
        if (!childCalls.isEmpty()) {
            long childConnectTimeMillis = Long.MAX_VALUE;
            // SPRD: Update connectRealTimeMillis when call is Conference Call
            long childRealConnectTimeMillis = Long.MAX_VALUE;
            for (Call child : childCalls) {
                if (child.getConnectTimeMillis() > 0) {
                    ...
                    // SPRD: Update connectRealTimeMillis when call is Conference Call
                    childRealConnectTimeMillis = Math.min(child.getConnectRealTimeMillis(),
                            childRealConnectTimeMillis);
                }
            ...
            // SPRD: Update connectRealTimeMillis when call is Conference Call
            if (childRealConnectTimeMillis != Long.MAX_VALUE) {
                connectRealTimeMillis = childRealConnectTimeMillis;
            }
        }
    ...
    public boolean isGenericConferenceCall(Call call) {
        int properties = convertConnectionToCallProperties(call.getConnectionCapabilities());
        return (properties & android.telecom.Call.Details.PROPERTY_GENERIC_CONFERENCE) != 0;
    }
    /* @} */
}
