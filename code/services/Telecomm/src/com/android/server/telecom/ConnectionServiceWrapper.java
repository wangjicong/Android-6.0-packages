...
final class ConnectionServiceWrapper extends ServiceBinder {
    ...
    void createConnection(final Call call, final CreateConnectionResponse response) {
        ...
        BindCallback callback = new BindCallback() {
            ...
            public void onSuccess() {
                ...
                /* SPRD: Fix the phone crash issue @{ */
                if (callId == null) {
                    Log.d(this, "createConnection fail because callId is null");
     ...
     * @orig void answer(Call, int)
     *
     * @see ConnectionService#answer(String,int)
     * */
    void answer(Call call, int videoState, int mpcMode) {
        final String callId = mCallIdMapper.getCallId(call);
        if (callId != null && isServiceValid("answer")) {
            try {
                logOutgoing("answer %s %d %d", callId, videoState, mpcMode);
                if (VideoProfile.isAudioOnly(videoState)) {
                    mServiceInterface.answerMPC(callId, mpcMode);
                } else {
                    mServiceInterface.answerMPCVideo(callId, videoState, mpcMode);
                }
            } catch (RemoteException e) {
            }
        }
