...
public class TelecomServiceImpl {
    ...
    private final ITelecomService.Stub mBinderImpl = new ITelecomService.Stub() {
            ...
            synchronized (mLock) {
                ...
                try {
                    ...
                    if(ImsManager.isVolteEnabledByPlatform(mContext)){
                        PhoneAccount.Builder builder = new PhoneAccount.Builder(account);
                        int capabilites = account.getCapabilities();
                        if(isVideoEnable){
                           capabilites |= PhoneAccount.CAPABILITY_VIDEO_CALLING;
                        }
                        builder.setCapabilities(capabilites);
                        mPhoneAccountRegistrar.registerPhoneAccount(builder.build());
                    }else{
         ...
         */
        @Override
        public PhoneAccountHandle getLivePhoneAccount(String callingPackage) {
            if (!canReadPhoneState(callingPackage, "getLivePhoneAccount")) {
                return null;
            }

            synchronized (mLock) {
                Call liveCall = mCallsManager.getLiveCall();
                if(liveCall != null) {
                    return liveCall.getTargetPhoneAccount();
                }
                return null;
            }
        }
    ...
    private int mVolteServiceState = VoLteServiceState.IMS_REG_STATE_NOT_EGISTERED;
    /* @} */
    ...
    public TelecomServiceImpl(
        ...
        if(ImsManager.isVolteEnabledByPlatform(context)){
            ...
            mTelephonyManager.listen(mPhoneStateListener, PhoneStateListener.LISTEN_VOLTE_STATE);
        }
        /* @} */
    }
    ...
    private final PhoneStateListener mPhoneStateListener = new PhoneStateListener() {
        ...
        public void onVoLteServiceStateChanged(VoLteServiceState voLteServiceState){
            int newState = voLteServiceState.getSrvccState();
            if (newState == voLteServiceState.IMS_REG_STATE_REGISTERED && mVolteServiceState != newState) {
                isVideoEnable = true;
            }
            mVolteServiceState = newState;
        }
    };
    /* @} */
