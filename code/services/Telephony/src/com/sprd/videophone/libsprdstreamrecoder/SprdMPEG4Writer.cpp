...
#define LOG_NDEBUG 0
#define LOG_TAG "SprdMPEG4Writer"
#include <utils/Log.h>
...
class SprdMPEG4Writer::Track {
public:
    Track(SprdMPEG4Writer *owner, const sp<MediaSource> &source, size_t trackId);
...
SprdMPEG4Writer::SprdMPEG4Writer(const char *filename)
    : mFd(-1),
      mInitCheck(NO_INIT),
...
SprdMPEG4Writer::SprdMPEG4Writer(int fd)
    : mFd(dup(fd)),
...
SprdMPEG4Writer::~SprdMPEG4Writer() {
    stop();
...
status_t SprdMPEG4Writer::Track::dump(
...
status_t SprdMPEG4Writer::addSource(const sp<MediaSource> &source) {
...
status_t SprdMPEG4Writer::startTracks(MetaData *params) {
    for (List<Track *>::iterator it = mTracks.begin();
         it != mTracks.end(); ++it) {
...
int64_t SprdMPEG4Writer::estimateMoovBoxSize(int32_t bitRate) {
...
status_t SprdMPEG4Writer::start(MetaData *param) {
    if (mInitCheck != OK) {
        return UNKNOWN_ERROR;
    }
...
bool SprdMPEG4Writer::use32BitFileOffset() const {
    return mUse32BitOffset;
}

status_t SprdMPEG4Writer::pause() {
    if (mInitCheck != OK) {
        return OK;
    }
    mPaused = true;
...
void SprdMPEG4Writer::stopWriterThread() {
 ...
 */
void SprdMPEG4Writer::writeCompositionMatrix(int degrees) {
...
status_t SprdMPEG4Writer::stop() {
    ...
    if (mUse32BitOffset) {
        ...
        size =  ((uint64_t)htonl(size & 0xffffffff) << 32) | htonl(size >> 32);// hton64(size);
        ::write(mFd, &size, 8);
    }
...
void SprdMPEG4Writer::writeMvhdBox(int64_t durationUs) {
...
void SprdMPEG4Writer::writeMoovBox(int64_t durationUs) {
    beginBox("moov");
...
void SprdMPEG4Writer::writeFtypBox(MetaData *param) {
    beginBox("ftyp");
...
void SprdMPEG4Writer::sendSessionSummary() {
...
status_t SprdMPEG4Writer::setInterleaveDuration(uint32_t durationUs) {
    mInterleaveDurationUs = durationUs;
    return OK;
}

void SprdMPEG4Writer::lock() {
    mLock.lock();
}

void SprdMPEG4Writer::unlock() {
    mLock.unlock();
}

off64_t SprdMPEG4Writer::addSample_l(MediaBuffer *buffer) {
...
off64_t SprdMPEG4Writer::addLengthPrefixedSample_l(MediaBuffer *buffer) {
    off64_t old_offset = mOffset;
...
void SprdMPEG4Writer::beginBox(const char *fourcc) {
...
void SprdMPEG4Writer::writeInt8(int8_t x) {
    write(&x, 1, 1);
}

void SprdMPEG4Writer::writeInt16(int16_t x) {
    x = htons(x);
    write(&x, 1, 2);
}

void SprdMPEG4Writer::writeInt32(int32_t x) {
    x = htonl(x);
    write(&x, 1, 4);
}

void SprdMPEG4Writer::writeInt64(int64_t x) {
    x =  ((uint64_t)htonl(x & 0xffffffff) << 32) | htonl(x >> 32); // hton64(x);
    write(&x, 1, 8);
}

void SprdMPEG4Writer::writeCString(const char *s) {
    size_t n = strlen(s);
    write(s, 1, n + 1);
}

void SprdMPEG4Writer::writeFourcc(const char *s) {
    CHECK_EQ(strlen(s), 4);
    write(s, 1, 4);
}
...
void SprdMPEG4Writer::writeLatitude(int degreex10000) {
...
void SprdMPEG4Writer::writeLongitude(int degreex10000) {
    bool isNegative = (degreex10000 < 0);
    char sign = isNegative? '-': '+';
 ...
 */
status_t SprdMPEG4Writer::setGeoData(int latitudex10000, int longitudex10000) {
...
void SprdMPEG4Writer::write(const void *data, size_t size) {
    write(data, 1, size);
}

bool SprdMPEG4Writer::isFileStreamable() const {
    return mStreamableFile;
}

bool SprdMPEG4Writer::exceedsFileSizeLimit() {
    // No limit
    if (mMaxFileSizeLimitBytes == 0) {
        return false;
    }
...
bool SprdMPEG4Writer::exceedsFileDurationLimit() {
    // No limit
    if (mMaxFileDurationLimitUs == 0) {
        return false;
    }
...
bool SprdMPEG4Writer::reachedEOS() {
    bool allDone = true;
    for (List<Track *>::iterator it = mTracks.begin();
         it != mTracks.end(); ++it) {
        if (!(*it)->reachedEOS()) {
...
void SprdMPEG4Writer::setStartTimestampUs(int64_t timeUs) {
...
int64_t SprdMPEG4Writer::getStartTimestampUs() {
    Mutex::Autolock autoLock(mLock);
    return mStartTimestampUs;
}

size_t SprdMPEG4Writer::numTracks() {
    Mutex::Autolock autolock(mLock);
    return mTracks.size();
}
...
SprdMPEG4Writer::Track::Track(
        SprdMPEG4Writer *owner, const sp<MediaSource> &source, size_t trackId)
    : mOwner(owner),
...
void SprdMPEG4Writer::Track::updateTrackSizeEstimate() {
...
void SprdMPEG4Writer::Track::addOneStscTableEntry(
...
void SprdMPEG4Writer::Track::addOneStssTableEntry(size_t sampleId) {
    mStssTableEntries.push_back(sampleId);
    ++mNumStssTableEntries;
}

void SprdMPEG4Writer::Track::addOneSttsTableEntry(
...
void SprdMPEG4Writer::Track::addOneCttsTableEntry(
        size_t sampleCount, int32_t duration) {
...
void SprdMPEG4Writer::Track::addChunkOffset(off64_t offset) {
    ++mNumStcoTableEntries;
    mChunkOffsets.push_back(offset);
}

void SprdMPEG4Writer::Track::setTimeScale() {
    ALOGV("setTimeScale");
    // Default time scale
    mTimeScale = 90000;
...
void SprdMPEG4Writer::Track::getCodecSpecificDataFromInputFormatIfPossible() {
    const char *mime;
...
void SprdMPEG4Writer::Track::initTrackingProgressStatus(MetaData *params) {
...
// static
void *SprdMPEG4Writer::ThreadWrapper(void *me) {
    ALOGV("ThreadWrapper: %p", me);
    SprdMPEG4Writer *writer = static_cast<SprdMPEG4Writer *>(me);
    writer->threadFunc();
    return NULL;
}

void SprdMPEG4Writer::bufferChunk(const Chunk& chunk) {
...
void SprdMPEG4Writer::writeChunkToFile(Chunk* chunk) {
...
void SprdMPEG4Writer::writeAllChunks() {
...
bool SprdMPEG4Writer::findChunkToWrite(Chunk *chunk) {
...
void SprdMPEG4Writer::threadFunc() {
    ALOGV("threadFunc");

    prctl(PR_SET_NAME, (unsigned long)"SprdMPEG4Writer", 0, 0, 0);
...
status_t SprdMPEG4Writer::startWriterThread() {
...
status_t SprdMPEG4Writer::Track::start(MetaData *params) {
    if (!mDone && mPaused) {
        mPaused = false;
        mResumed = true;
        return OK;
...
status_t SprdMPEG4Writer::Track::pause() {
    mPaused = true;
    mResumed = false; //wxz20120528: add the resume for pause/resume recording function.
    return OK;
}

status_t SprdMPEG4Writer::Track::stop() {
...
bool SprdMPEG4Writer::Track::reachedEOS() {
    return mReachedEOS;
}

// static
void *SprdMPEG4Writer::Track::ThreadWrapper(void *me) {
...
const uint8_t *SprdMPEG4Writer::Track::parseParamSet(
        const uint8_t *data, size_t length, int type, size_t *paramSetLen) {
...
status_t SprdMPEG4Writer::Track::copyAVCCodecSpecificData(
...
status_t SprdMPEG4Writer::Track::parseAVCCodecSpecificData(
        const uint8_t *data, size_t size) {
...
status_t SprdMPEG4Writer::Track::makeAVCCodecSpecificData(
        const uint8_t *data, size_t size) {
 ...
 */
void SprdMPEG4Writer::Track::updateDriftTime(const sp<MetaData>& meta) {
...
status_t SprdMPEG4Writer::Track::threadEntry() {
    int32_t count = 0;
    ...
    while (!mDone && (err = mSource->read(&buffer)) == OK) {
     ...
     //wxz20120528: find the I frame after resume. CR:207919/207937
     if(mResumed && !mIsAudio){
             int32_t isIFrame = false;
            buffer->meta_data()->findInt32(kKeyIsSyncFrame, &isIFrame);
         ALOGV("wxz: check the IFrame after resume. isIFrame: %d.", isIFrame);
         if(!isIFrame){
                    buffer->release();
                    buffer = NULL;
             continue;
...
bool SprdMPEG4Writer::Track::isTrackMalFormed() const {
    ...
    if (OK != checkCodecSpecificData()) {         // no codec specific data
        ALOGE("checkCodecSpecificData error");
        return true;
    }
...
void SprdMPEG4Writer::Track::sendTrackSummary(bool hasMultipleTracks) {
...
void SprdMPEG4Writer::Track::trackProgressStatus(int64_t timeUs, status_t err) {
...
void SprdMPEG4Writer::trackProgressStatus(
...
void SprdMPEG4Writer::setDriftTimeUs(int64_t driftTimeUs) {
    ALOGV("setDriftTimeUs: %lld us", driftTimeUs);
    Mutex::Autolock autolock(mLock);
    mDriftTimeUs = driftTimeUs;
}

int64_t SprdMPEG4Writer::getDriftTimeUs() {
    ALOGV("getDriftTimeUs: %lld us", mDriftTimeUs);
    Mutex::Autolock autolock(mLock);
    return mDriftTimeUs;
}

bool SprdMPEG4Writer::useNalLengthFour() {
    return mUse4ByteNalLength;
}

void SprdMPEG4Writer::Track::bufferChunk(int64_t timestampUs) {
    ALOGV("bufferChunk");
...
int64_t SprdMPEG4Writer::Track::getDurationUs() const {
    return mTrackDurationUs;
}

int64_t SprdMPEG4Writer::Track::getEstimatedTrackSizeBytes() const {
    return mEstimatedTrackSizeBytes;
}

status_t SprdMPEG4Writer::Track::checkCodecSpecificData() const {
    const char *mime;
    CHECK(mMeta->findCString(kKeyMIMEType, &mime));
    if (!strcasecmp(MEDIA_MIMETYPE_AUDIO_AAC, mime) ||
        ...
        if (!mCodecSpecificData ||
            mCodecSpecificDataSize <= 0) {
            ALOGE("Missing codec specific data mCodecSpecificData = %p,mCodecSpecificDataSize = %d ",mCodecSpecificData,mCodecSpecificDataSize);
            return ERROR_MALFORMED;
        }
    } else {
        if (mCodecSpecificData ||
...
void SprdMPEG4Writer::Track::writeTrackHeader(bool use32BitOffset) {
...
void SprdMPEG4Writer::Track::writeStblBox(bool use32BitOffset) {
...
void SprdMPEG4Writer::Track::writeVideoFourCCBox() {
    const char *mime;
...
void SprdMPEG4Writer::Track::writeAudioFourCCBox() {
    const char *mime;
    bool success = mMeta->findCString(kKeyMIMEType, &mime);
    CHECK(success);
...
void SprdMPEG4Writer::Track::writeMp4aEsdsBox() {
...
void SprdMPEG4Writer::Track::writeMp4vEsdsBox() {
    CHECK(mCodecSpecificData);
    CHECK(mCodecSpecificDataSize > 0);
    mOwner->beginBox("esds");
...
void SprdMPEG4Writer::Track::writeTkhdBox(time_t now) {
...
void SprdMPEG4Writer::Track::writeVmhdBox() {
...
void SprdMPEG4Writer::Track::writeSmhdBox() {
...
void SprdMPEG4Writer::Track::writeHdlrBox() {
...
void SprdMPEG4Writer::Track::writeMdhdBox(time_t now) {
    int64_t trakDurationUs = getDurationUs();
...
void SprdMPEG4Writer::Track::writeDamrBox() {
...
void SprdMPEG4Writer::Track::writeUrlBox() {
...
void SprdMPEG4Writer::Track::writeDrefBox() {
...
void SprdMPEG4Writer::Track::writeDinfBox() {
    mOwner->beginBox("dinf");
    writeDrefBox();
    mOwner->endBox();  // dinf
}

void SprdMPEG4Writer::Track::writeAvccBox() {
    CHECK(mCodecSpecificData);
...
void SprdMPEG4Writer::Track::writeD263Box() {
...
void SprdMPEG4Writer::Track::writePaspBox() {
...
void SprdMPEG4Writer::Track::writeSttsBox() {
...
void SprdMPEG4Writer::Track::writeCttsBox() {
...
void SprdMPEG4Writer::Track::writeStssBox() {
...
void SprdMPEG4Writer::Track::writeStszBox() {
...
void SprdMPEG4Writer::Track::writeStscBox() {
...
void SprdMPEG4Writer::Track::writeStcoBox(bool use32BitOffset) {
...
void SprdMPEG4Writer::writeUdtaBox() {
    beginBox("udta");
    writeGeoDataBox();
    endBox();
}
 ...
 * Geodata is stored according to ISO-6709 standard.
 */
void SprdMPEG4Writer::writeGeoDataBox() {
    beginBox("\xA9xyz");
    /*
