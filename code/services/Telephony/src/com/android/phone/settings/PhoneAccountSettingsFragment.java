...
public class PhoneAccountSettingsFragment extends PreferenceFragment
    ...
    private Preference mAllAccounts;
    private Preference mNoAccounts;
    private PreferenceCategory mSipSetting;
    ...
    /*[Modify] by SPRD for Bug:[493805] 2015.11.08 Start */

    /* SPRD: add for bug 521780 for sim hot plug @{ */
    private boolean[] mIsSimInsert;
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(TelephonyIntents.ACTION_SIM_STATE_CHANGED)) {
                ...
                if (IccCardConstants.INTENT_VALUE_ICC_ABSENT.equals(state)) {
                    if (mIsSimInsert[phoneId]) {
                        mIsSimInsert[phoneId] = false;
                        drawPreference();
                    }
                } else if (IccCardConstants.INTENT_VALUE_ICC_READY.equals(state)) {
                    if (!mIsSimInsert[phoneId]) {
                        mIsSimInsert[phoneId] = true;
                        drawPreference();
                    }
                }
            }
    ...
    public void onCreate(Bundle icicle) {
        ...
        mAllAccounts = getPreferenceScreen().findPreference(ALL_CALLING_ACCOUNTS_KEY);
        ...
        mSipSetting = (PreferenceCategory)getPreferenceScreen().findPreference(SIP_SETTINGS_CATEGORY_PREF_KEY);

        /* SPRD: add for bug 521780 for sim hot plug @{ */
        if (getActivity() != null) {
            TelephonyManager tm = TelephonyManager.from(getActivity());
            mIsSimInsert = new boolean[tm.getPhoneCount()];
            for (int i = 0; i < mIsSimInsert.length; i++) {
                if (tm.getSimState(i) == TelephonyManager.SIM_STATE_ABSENT) {
                    mIsSimInsert[i] = false;
                } else {
                    mIsSimInsert[i] = true;
                }
            }
    ...
    public void onResume() {
        ...
        if (SipUtil.isVoipSupported(getActivity())) {
            ...
            getPreferenceScreen().removePreference(mSipSetting);
        }
    }
    ...
    private void drawPreference() {
        if (mAccountList != null) {
           ...
           mAccountList.addPreference(mAllAccounts);
        ...
        if (shouldShowConnectionServiceList(allNonSimAccounts)) {
            ...
            if (allNonSimAccounts.isEmpty() && mAllAccounts != null) {
                mAccountList.removePreference(mAllAccounts);
            }
            /* SPRD: add by bug492949 @{ */
            if ((enabledAccounts.size() > 0 || !allNonSimAccounts.isEmpty())
                    && mNoAccounts != null) {
    ...
    /*[Modify] by SPRD for Bug:[493805] 2015.11.08 End */
    /**
    ...
    private void initAccountList(List<PhoneAccountHandle> enabledAccounts) {
        ...
        Collections.sort(accounts, new Comparator<PhoneAccount>() {
            ...
            public int compare(PhoneAccount account1, PhoneAccount account2) {
                ...
                //order by slot id
                if (retval == 0) {
                    SubscriptionManager sManager = SubscriptionManager.from(getActivity());
                    int slotIndex1 = sManager.getActiveSubscriptionInfoForIccIndex(
                            account2.getAccountHandle().getId()).getSimSlotIndex();
                    int slotIndex2 = sManager.getActiveSubscriptionInfoForIccIndex(
                            account1.getAccountHandle().getId()).getSimSlotIndex();
                    retval = slotIndex2 - slotIndex1;
                }
    ...
    private List<PhoneAccountHandle> getCallingAccounts(
        ...
        for (Iterator<PhoneAccountHandle> i = accountHandles.iterator(); i.hasNext();) {
            ...
            if (account == null) {
                ...
                i.remove();
            } else if(!includeDisabledAccounts &&
                    account.hasCapabilities(PhoneAccount.CAPABILITY_SIM_SUBSCRIPTION)) {
                boolean isSimStandby = false;
                int phoneId = -1;
                int subId = mTelephonyManager.getSubIdForPhoneAccount(account);
                if(mSubscriptionManager.isValidSubscriptionId(subId)) {
                    phoneId = mTelephonyManager.getPhoneId(subId);
                    if(mSubscriptionManager.isValidPhoneId(phoneId)) {
                        isSimStandby = mTelephonyManager.isSimStandby(phoneId);
                    }
                }
                if(!isSimStandby) {
                    i.remove();
