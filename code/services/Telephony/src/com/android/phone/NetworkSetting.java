...
public class NetworkSetting extends PreferenceActivity
    ...
    protected void onCreate(Bundle icicle) {
        ...
        configManager = (CarrierConfigManager) getSystemService(Context.CARRIER_CONFIG_SERVICE);
...
//    }
    ...
    private String formatNetworkTitle(OperatorInfo ni){
        ...
        String mvnoSpn ="";
        String simMccmnc="";
        if (configManager.getConfigForPhoneId(mPhoneId) != null) {
		
            isMvno = configManager.getConfigForPhoneId(mPhoneId)
                    .getBoolean(CarrierConfigManager.KEY_GLO_CONF_MVNO);
					
			mvnoSpn = configManager.getConfigForPhoneId(mPhoneId)
                    .getString(CarrierConfigManager.KEY_GLO_CONF_SPN);
        }
		
        simMccmnc= mTelephonyManager.getSimOperatorNumericForPhone(mPhoneId);
		
        android.util.Log.d(" sprd-- NetworkSetting: "," simMccmnc="+simMccmnc+" mccmnc="+mccmnc+" isMvno="+isMvno+" mvnoSpn="+mvnoSpn);
        if(isMvno && !TextUtils.isEmpty(mvnoSpn)){
			if(!TextUtils.isEmpty(mccmnc) && mccmnc.equals(simMccmnc)){
				operatorAlphaShort = mvnoSpn;
			}
        }
        //SPRD:Net name
