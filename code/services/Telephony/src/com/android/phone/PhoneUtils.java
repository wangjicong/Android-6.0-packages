...
import com.android.phone.plugin.USSDForOrangeUtils;
...
public class PhoneUtils {
     ...
     * @param previousAlert a previous alert used in this activity.
     */
    static AlertDialog displayMMIComplete(final Phone phone, Context context, final MmiCode mmiCode,
        ...
        /* SPRD: Add for ussd dialog for bug490252 @{ */
        USSDForOrangeUtils uSSDForOrangeUtils = USSDForOrangeUtils.getInstance();
        boolean isSupportUSSDCall = uSSDForOrangeUtils.isSupportUSSDCall();
        log("isSupportUSSDCall = " + isSupportUSSDCall);
        /* @} */
        ...
        if ((app.getPUKEntryActivity() != null) && (state == MmiCode.State.COMPLETE)) {
            ...
            /* SPRD: modify for ussd dialog for bug437974 @{ */
            if (isSupportUSSDCall) {
                TextView tv = new TextView(context);
                tv.setTextColor(Color.BLACK);
                tv.setText(text);
                uSSDForOrangeUtils.setSpan(context, tv);
                pd.setView(tv);
            } else {
                pd.setMessage(text);
            }
            ...
            if (state != MmiCode.State.PENDING) {
                ...
                if (isSupportUSSDCall) {
                    ...
                    tv.setTextColor(Color.BLACK);
                    uSSDForOrangeUtils.setSpan(context, tv);
                    sUssdDialog.setView(tv);
                } else {
                    sUssdDialog.setMessage(sUssdMsg);
                }
                /** @} */
                sUssdDialog.show();
                // SPRD: add for bug490253
                ...
                /* SPRD: add for bug531451 @{ */
                TextView textMessage = (TextView) dialogView.findViewById(R.id.ussd_message);
                if (isSupportUSSDCall) {
                ...
                /* SPRD: Add for ussd dialog for bug490252 @{ */
                uSSDForOrangeUtils.setSpan(context, textMessage);
                /* @} */
                // build the dialog
                final AlertDialog newDialog = new AlertDialog.Builder(contextThemeWrapper)
                        // SPRD: modify for bug531451
                        //.setMessage(text)
                        .setView(dialogView)
                ...
                /* SPRD: add for bug531451 @{ */
                newDialog.getWindow().setSoftInputMode(
    ...
    */
    ...
    /*SPRD: private*/public static Phone getPhoneFromIccId(String iccId) {
    ...
    /* SPRD: function FDN support. @{ */
    public static Phone getPhoneForSubscriber(int subId) {
