...
import com.sprd.phone.settings.callbarring.CallBarringOptions;;
...
public class GsmUmtsCallOptions extends PreferenceActivity {
    ...
    public static void init(PreferenceScreen prefScreen, SubscriptionInfoHelper subInfoHelper) {
        ...
        if (ImsManager.isVolteEnabledByPlatform(subInfoHelper.getPhone().getContext())) {
            callForwardingPref
                    .setIntent(subInfoHelper.getIntent(GsmUmtsAllCallForwardOptions.class));
        } else {
            callForwardingPref.setIntent(subInfoHelper.getIntent(GsmUmtsCallForwardOptions.class));
        }
        /* @} */

        /*[Add] by SPRD for Bug:[498469] 2015.11.16 End */
        Preference callBarringPref = prefScreen.findPreference(CALL_BARRING_KEY);
        callBarringPref.setIntent(subInfoHelper.getIntent(CallBarringOptions.class));
        /*[Add] by SPRD for Bug:[498469] 2015.11.16 End */
