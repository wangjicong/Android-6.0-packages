...
public class PhoneInterfaceManager extends ITelephony.Stub {
    ...
    /*SPRD: add for invokeOemRequest  by phoneId @{ */
    ...
    private static final class IccAPDUArgument {
        ...
        public IccAPDUArgument(int channel, int cla, int command,
                int p1, int p2, int p3, String data,String filePath) {
            this.channel = channel;
            this.cla = cla;
            this.command = command;
            this.p1 = p1;
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case EVENT_INVOKE_OEM_RIL_REQUEST_STRINGS_DONE:
                    ...
                    break;
                /* @} */
                case CMD_EXCHANGE_SIM_IO_UPDATE:
                    sendSimIORequest(msg, true);
                    break;
                case CMD_EXCHANGE_SIM_IO_WITHSUBID:
                    sendSimIORequest(msg, false);
                    break;
                default:
    ...
    public Bundle getCellLocation(String callingPackage) {
        ...
        if (checkIfCallerIsSelfOrForegroundUser()) {
            ...
            /*SPRD: Bug 474224 For AGPS
             *@orign
             *
            Phone phone = getPhone(mSubscriptionController.getDefaultDataSubId());
            /*@{*/
            Phone phone = getPhone(mSubscriptionController.getDefaultSubId());
            /*@}*/
            if (phone == null) {
                return null;
            }
    ...
    */
    ...
    /**
     * SPRD:add invokeOemRilRequest by phoneId. @{
     */
    @Override
    ...
    /**
     * SPRD: Add for third part application support.
     * @param type
     * @return
     */
    public boolean isSupportApplication(int type) {
        return OtherGlobals.getInstance().isSupportApplication(type);
    }
    ...
    */
    public boolean getIccFdnEnabled() {
        return getIccFdnEnabledForSubscriber(getDefaultSubscription());
    }
    ...
    public boolean isVTCall() {
        return isVTCallForSubscriber(getPreferredVoiceSubscription());
    }

    public boolean isVTCallForSubscriber(int subId) {
        boolean is_vtcall = getPhone(subId).isVideoCallPresent();
        Log.d(LOG_TAG, "isVTCall is_vtcall:" + is_vtcall);
        return is_vtcall;
    }
    /* SPRD: add by EXT @{ */

    public void setSimStandby(int phoneId, boolean onOff) {
        RadioTaskManager manager = RadioTaskManager.getDefault();
        if (manager != null) {
            manager.setSimStandby(phoneId, onOff);
        }
    }
    ...
    public void setPrimaryCard(int phoneId) {
        RadioTaskManager manager = RadioTaskManager.getDefault();
        if (manager != null) {
            manager.manualSetPrimaryCard(phoneId);
        }
    }
    ...
    public int getRemainTimes(int type) {
        return getRemainTimesForSubscriber(type, getDefaultSubscription());
    }

    public int getRemainTimesForSubscriber(int type, int subId) {
        int count = -1;
        Phone phone = getPhone(subId);
        if (phone != null) {
            count = getPhone(subId).getRemainTimes(type);
        }
        Log.d(LOG_TAG, "getRemainTimes:" + count + ", subId =" + subId);
        return count;
    }

    private static final int CMD_EXCHANGE_SIM_IO_UPDATE = 46;
    private static final int CMD_EXCHANGE_SIM_IO_WITHSUBID = 47;
    @Override
    public byte[] iccExchangeSimIOwithSubId(int fileID, int command, int p1, int p2, int p3,
        ...
        if (DBG) {
            log("iccExchangeSimIOwithSubId: " + fileID + ":" + command + " " +
                    p1 + " " + p2 + " " + p3 + ":" + filePath);
        }

        IccIoResult response =
                (IccIoResult)sendRequest(CMD_EXCHANGE_SIM_IO_WITHSUBID,
                        new IccAPDUArgument(-1, fileID, command, p1, p2, p3, filePath), subId);

        if (DBG) {
            log("iccExchangeSimIOwithSubId: response:" + response);
        }
    ...
    public  byte[] iccExchangeSimIOUpdate(int fileID, int command, int p1, int p2, int p3, String data,
        ...
        if (DBG) {
            log("iccExchangeSimIOUpdate " + fileID + ":" + command + " " +
                    +  p1 + " " + p2 + " " + p3 + ":" + filePath + ", data = "+data);
        }

        IccIoResult response =
                (IccIoResult)sendRequest(CMD_EXCHANGE_SIM_IO_UPDATE,
                        new IccAPDUArgument(-1, fileID, command, p1, p2, p3, data, filePath), Integer.valueOf(subId));

        if (DBG) {
            log("iccExchangeSimIOUpdate: response:" + response);
        }
    ...
    private void sendSimIORequest (Message msg, boolean update){
        MainThreadRequest request = (MainThreadRequest) msg.obj;
        IccAPDUArgument iccArgument = (IccAPDUArgument) request.argument;
        Phone phone = getPhone(request.subId);
        if(phone == null){
            loge("sendSimIORequest get phone by phoneId failed, use mPhone");
            phone = mPhone;
        }
        UiccCard uiccCard = UiccController.getInstance().getUiccCard(phone.getPhoneId());
        if (uiccCard == null) {
            loge("sendSimIORequest: No UICC");
            request.result = new IccIoResult(0x6F, 0, (byte[])null);
            synchronized (request) {
                request.notifyAll();
            }
        } else {
            Message onCompleted = mMainThreadHandler.obtainMessage(EVENT_EXCHANGE_SIM_IO_DONE,
                    request);
            if(!update){
                uiccCard.iccExchangeSimIO(iccArgument.cla, /* fileID */
                        iccArgument.command, iccArgument.p1, iccArgument.p2, iccArgument.p3,
                ...
                uiccCard.iccExchangeSimIOUpdate(iccArgument.cla, /* fileID */
                        iccArgument.command, iccArgument.p1, iccArgument.p2, iccArgument.p3,
                        iccArgument.data, iccArgument.filePath, onCompleted);
            }
        }
    }

    /**
     * SPRD: Add for simlock extra function @{
     * @param type the type of simlock to be checked
     * @return true if this type of simlock is turned on
     */
    public boolean getSimLockStatus(int type) {
        return mPhone.getSimLockStatus(type);
    }
    /** @} */
    ...
    public String getPnnHomeName(int subId, String callingPackage) {
        if (!canReadPhoneState(callingPackage, "getPnnHomeName")) {
            return null;
        }
    ...
    public void setInternalPreferredNetworkTypeForPhone(int phoneId, int networkType) {
        ...
        RadioTaskManager manager = RadioTaskManager.getDefault();
        if (manager != null) {
            manager.setInternalPreferredNetworkTypeForPhone(phoneId, networkType);
        }
    }
     ...
     */
    public int getInternalPreferredNetworkTypeForPhone(int phoneId, String callingPackage) {
        if (canReadPhoneState(callingPackage, "getInternalPreferredNetworkType")) {
            RadioTaskManager manager = RadioTaskManager.getDefault();
            if (manager != null) {
                return manager.getInternalPreferredNetworkTypeForPhone(phoneId);
            }
        }
        return TelephonyManager.NT_UNKNOWN;
    }
}
