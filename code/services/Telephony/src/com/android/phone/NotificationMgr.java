...
public class NotificationMgr {
    ...
    private MediaPlayer mLocalPlayer; // SPRD: add for bug546774
    ...
    private NotificationMgr(PhoneGlobals app) {
        ...
        /* SPRD: receive simLoaded broadcast for bug528478 @{ */
        IntentFilter simLoadedFilter = new IntentFilter();
        simLoadedFilter.addAction(TelephonyIntents.ACTION_SIM_STATE_CHANGED);
        Log.i(LOG_TAG,"Register simLoaded broadcast");
        mContext.registerReceiver(mSimLoadedBroadcast, simLoadedFilter);
        /* }@ */
    }
    ...
    void updateMwi(int subId, boolean visible, boolean enableNotificationSound) {
        ...
        if (visible) {
            ...
            // SPRD: add for bug528478
            Log.i(LOG_TAG, "notificationText:"+notificationText);
            ...
            if (enableNotificationSound) {
                ringtoneUri = VoicemailNotificationSettingsUtil.getRingtoneUri(/** SPRD: mPhone */phone);
            }

            /* SPRD: add for bug546774, try opening uri locally before delegating to remote player @{ */
            destroyLocalPlayer();
            mLocalPlayer = new MediaPlayer();
            try {
                mLocalPlayer.setDataSource(mContext, ringtoneUri);
            } catch (SecurityException | IOException | NullPointerException e) {
                ringtoneUri = RingtoneManager.getActualDefaultRingtoneUri(mContext,  RingtoneManager.TYPE_NOTIFICATION);
                destroyLocalPlayer();
            }
            /* @} */
    ...
    /* package */ void updateCfi(int subId, boolean visible) {
        ...
        if (visible) {
            ...
            if (isMultiSimEnabled) {
                int phoneId = mTelephonyManager.getPhoneId(subId);
     ...
     */
    public void updateCfiVisibility(boolean visible, int phoneId) {
        ...
        if (TelephonyCapabilities.supportsNetworkSelection(mPhone)) {
            ...
            if (SubscriptionManager.isValidSubscriptionId(subId)) {
                ...
                String networkSelection =
                        // SPRD : Modify for BUG 505914
                        sp.getString(PhoneBase.NETWORK_SELECTION_SHORT_KEY + subId, "");
    ...
    /**
     * SPRD: receiver simloaded broadcast for bug528478 @{
     */
    private BroadcastReceiver mSimLoadedBroadcast = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            Log.i(LOG_TAG,
                    "SIM_STATE_CHANGED,state:"
