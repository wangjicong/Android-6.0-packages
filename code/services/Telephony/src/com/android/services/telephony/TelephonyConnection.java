...
abstract class TelephonyConnection extends Connection {
    ...
    private static final int MSG_SUPP_SERVICE_FAILED = 8;  // SPRD: Porting dealing with SS notification.
            ...
            switch (msg.what) {
                ...
                case MSG_SUPP_SERVICE_FAILED:
                    ...
                    if (getConnectionService() != null) {
                        ...
                        getConnectionService().sendBroadcast(intent);
                    }
                    /* @} */
                    break;
                /* @} */
    ...
    public void onAnswer(int videoState) {
        ...
        if (isValidRingingCall() && getPhone() != null) {
            try {
                int mpcMode = getAnswerMode(); // SPRD: add for bug502037
                getPhone().acceptCall(videoState, mpcMode);
            } catch (CallStateException e) {
    ...
    protected int buildConnectionCapabilities() {
        ...
        /*SPRD: add for VoLTE{@*/
        if(mOriginalConnection != null && mOriginalConnection.isMultiparty()){
            callCapabilities |= CAPABILITY_GENERIC_CONFERENCE;
        }
        /* @} */
    ...
    void setOriginalConnection(com.android.internal.telephony.Connection originalConnection) {
        ...
        getPhone().registerForPreciseCallStateChanged(mHandler, MSG_PRECISE_CALL_STATE_CHANGED,
                null);
        getPhone().registerForHandoverStateChanged(mHandler, MSG_HANDOVER_STATE_CHANGED, null);
        ...
        updateState();
        // SPRD: bug541078 add condition mOriginalConnection != null
        if (mOriginalConnection != null) {
        ...
        if (mOriginalConnectionState != newState || force) {
            ...
            switch (newState) {
                ...
                case WAITING:
                    ...
                    break;
                case DISCONNECTED:
                    /** SPRD: kill-stop mechanism BEGIN*/
                    try {
                        ActivityManagerNative.getDefault().killStopFrontApp(
                                ActivityManager.KILL_CONT_STOPPED_APP);
                    } catch (Exception e) {
                        Log.d(this, "killStopFrontApp : KILL_CONT_STOPPED_APP");
                        e.printStackTrace();
                    }
                    /** END */
                    ...
                    /* SPRD: Call Fail Cause Feature @{ */
                    Context context = PhoneGlobals.getInstance().getApplicationContext();
                    CallFailCauseHelper callFailCauseHelper = CallFailCauseHelper.getInstance(context);

                    if (callFailCauseHelper.isSupportCallFailCause()) {
                        callFailCauseHelper.parserAttributeValue(context);
                        String callFailCause = callFailCauseHelper
                                .getAttributeValue(mOriginalConnection.getCallFailCause());
    ...
    public void onTransferCall() {
        performExplicitCallTransfer();
    }

    void performExplicitCallTransfer() {
        Log.v(this, "Transfer active call");
        if (mOriginalConnection == null) {
            Log.v(this, "Transfer active call failed--No connection");
            return;
        }
        try {
            Phone phone = mOriginalConnection.getCall().getPhone();
            ...
            Log.e(this, e, "Exception occurred while trying to transfer calls.");
        }
    }
    /** @} */
}
